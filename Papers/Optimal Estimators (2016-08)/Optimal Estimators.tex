%
\documentclass[review,nohypdvips]{siamart1116}
...
\usepackage{amssymb}%amsmath,amsthm %%% Already loaded by document class
\usepackage{csquotes}
...
\headers{Optimal Polynomial-Time Estimators}{Vadim Kosoy}

\title{Optimal Polynomial-Time Estimators: A Bayesian Notion of Approximation Algorithm}
\author{Vadim Kosoy\thanks{Independent, Petah Tikva, Israel
    (\email{vadim.kosoy@intelligence.org}). Partially supported by the Machine Intelligence Research Institute, Berkeley, CA (\url{https://intelligence.org/}).}}
...
% REQUIRED
\begin{keywords}
  complexity theory, average-case complexity, bounded rationality
\end{keywords}
...
Khot's Unique Games Conjecture implies that many optimization problems have an algorithm which produces the best approximation factor possible in polynomial-time (see e.g. \cite{Khot_2010}). Barak and Steurer \cite{Barak_2014} speculate that even if the Unique Games Conjugate is false, the existence of an algorithm that is optimal in this sense for a large class of problems is plausible, and propose the Sum-of-Squares algorithm as a candidate.
...
\begin{equation*}
\frac{
\splitfrac{(\E_{\Dist^{K} \times \Un_P^{K}}[\theta(P^{K}(x,y)-A^{K})\theta(B^{K}-P^{K}(x,y))]}
{\E[P^{K}-f \mid A^{K} \leq P^{K} \leq B^{K}])^2}}
{\alpha(K)} = \varepsilon(K)
\end{equation*}
...
\begin{multline*}
_{\chi f}^K(x,yz) - \chi_L(x) f(x) = P_L^K(x,y) P_{f \mid L}^K(x,z) -\\ \chi_L(x) P_{f \mid L}^K(x,z) + \chi_L(x) P_{f \mid L}^K(x,z) - \chi_L(x) f(x)
\end{multline*}
...
We think of $S$ as a randomized algorithm with advice which is random in itself. In particular any polynomial-time $\Gamma$-scheme can $S$ can be regarded as a polynomial-time $\MGrow$-scheme with
...
\begin{multline*}
\R_{Q_g}^K(\Chev{z_{\sigma1}, \A_{Q_f}(K),\A_{\pi}(K),\A_{P_g}(K)}) = \\ 
\R_\sigma^K(z_{\sigma1}) + \R_{Q_f}(K) + \gamma(K)\R_{\pi}(K) + \R_{P_g}(K)
\end{multline*}
...
\end{multline*}
%%% Replaced by multline version ^^^
%\begin{align*}
%&\E_{\Dist^{K} \times \Un_{P_g}^K}[(\E_{\Un_\pi^K}[(P_g \circ \pi)^K]-\E_{\Un_\pi^K}[\bar{g} \circ \pi^K])^2] \leq\\ 
%&\E_{\Dist^{K} \times \Un_{Q_f}^K \times \Un_{P_f}^K}[(Q_f ^K - P_f^K+\E_{\Un_\pi^K}[(P_g \circ \pi)^K] - \E_{\Un_\pi^K}[\bar{g} \circ \pi^K])^2] \pmod \Fall 
%\end{align*}
...
Consider ${Q: \Words \Scheme \Rats}$ bounded. Without loss of generality we can assume $\sup \Abs{Q} \leq M$ (otherwise we can replace ${Q}$ by ${\tilde{Q}:=\max(\min(Q,+M),-M)}$ and have ${\E[(\tilde{Q}-f)^2]\leq\E[(Q-f)^2]}$). Choose ${q \in \NatPoly}$ s.t. for any ${K \in \Nats^n}$ there exits ${a_Q^K \in \Bool^{l(\alpha_q(K))}}$ for which
...
Denote ${l(K):=\Floor{\log (K_{n-1} + 2)}}$. Construct ${R: \Words \Scheme \Rats}$ s.t. for any ${K \in \Nats^n}$, $w \in \Words$, ${a \in \Bool^{l(K)}}$, ${\{y_i \in \Bool^{r_\sigma(K,w)}\}_{i \in [l(K)^4]}}$ and ${\{z_i \in \Bool^{r(K)}\}_{i \in [l(K)^4]}}$
...
\begin{multline*}
\Prb_{\Un_R^K}[\varrho^K(A^K(y)) + \varrho_0^K - \varepsilon(K) - l(K)^{-1} > \varrho^K(b) + \varrho_0^K + \varepsilon(K) + l(K)^{-1}] \leq \\ 2^{-\epsilon l(K)^2} + 2^{-\epsilon l(K)^2+l(K)+1}
\end{multline*}

\[\Prb_{\Un_R^K}[\varrho^K(A^K(y)) > \varrho^K(b) + 2(\varepsilon(K) + l(K)^{-1})] \leq 2^{-\epsilon l(K)^2+l(K)+2}\]

It follows that for some ${M_0 \in \Reals^{>0}}$

\[\E_{\Un_R^K}[\varrho^K(A^K(y)] \leq \varrho^K(b) + 2(\varepsilon(K) + l(K)^{-1}) + 2^{-\epsilon l(K)^2+l(K)+2} M_0\]
...
The following propositions lead to disproving the existence of optimal polynomial-time estimators for certain distributional estimation problems in the case ${\GrowA = \Gamma_0^n}$.
...
Again, the statement can be reversed to disprove existence of ${\Fall(\Delta)}$-optimal estimators for ${\Delta_{\mathfrak{A}}}=\Gamma_0^n$.
