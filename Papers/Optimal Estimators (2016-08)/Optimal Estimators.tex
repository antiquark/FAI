...
\author{Vadim Kosoy\thanks{E-mail: \texttt{vadim.kosoy@intelligence.org}, Telephone: \texttt{+97237448629}}}
...
\begin{align*}
&\E[(P_f^K - f)S] \equiv\\ 
&\E_{\Dist^{K} \times \Un_{PS}^K}[(\frac{1}{\gamma(K)}\sum_{i \in [\gamma(K)]} P_g^K(\pi^K(x,z_i),w_i) - \frac{1}{\gamma(K)} \sum_{i \in [\gamma(K)]} \bar{g}(\pi^K(x,z_i)))S(x,P_f^K(x))] \pmod \Fall
\end{align*}
