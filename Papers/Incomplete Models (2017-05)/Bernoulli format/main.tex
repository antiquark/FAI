...
\begin{aug}
\author[A]{\fnms{Vanessa} \snm{Kosoy}\ead[label=e1]{vanessa.kosoy@intelligence.org}},
...
%% Addresses                                %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\address[A]{Independent Researcher, \printead{e1}}
...
\end{abstract} %Abstract is 151 words

\begin{keyword}
\kwd{statistical learning theory}
\kwd{online learning}
\kwd{sequence prediction}
\kwd{probability theory}
\kwd{imprecise probability}
\end{keyword}
...
\begin{multline*}
   \SV \Bd_b G_{m_0+1}\left(y_{:m_0},\BM_{:m_0+1};x\right) = \SV \Bd_b G_{m_0}\left(y_{:m_0-1},\BM_{:m_0};x\right) + \\
   \left(\V \Bd_b G_{m_0}\left(y_{:m_0},\BM_{:m_0}\right)\right)\left(\BM_{m_0},x\right) 
\end{multline*}
...
\begin{multline*}
    \SV \Bd_b G_{m_0+1}\left(y_{:m_0},\BM_{:m_0+1};x\right) = \SV G_{m_0}\left(y_{:m_0-1},\BM_{:m_0};x\right) + \\ 
    \Nr_b G_{m_0}\left(y_{:m_0},\BM_{:m_0};\BM_{m_0}\right) \cdot \left(\V G_{m_0}\left(y_{:m_0},\BM_{:m_0}\right)\right)\left(\BM_{m_0},x\right)
\end{multline*}
...
\begin{multline*}
    ...
    \geq \SVM G_{m_0}\left(y_{:m_0-1},\BM_{:m_0}\right) > -b
\end{multline*}
...
\begin{multline*}
    ...
    \left(\V \Bd_b G_{m-1}\left(y_{:m-1},\BM_{:m-1}\right)\right)\left(\BM_{m-1},x\right)
\end{multline*}
...
\begin{multline*}
    \SV \PG{S}_{n+1}\left(y,\BM\right) - \SVM \PG{S}_{n+1}\left(y,\BM\right) \leq \SVX \PG{S}_n\left(y_{:n-1},\BM_{:n}\right) - \\
    \SVM \PG{S}_n\left(y_{:n-1},\BM_{:n}\right) + 2 \N{S_n\left(y\right)}
\end{multline*}
...
\begin{multline*}
    \E_\nu\left[f\right] \geq \varphi\left(\nu\right) - \frac{1}{16} r \left(r - r_0\right) \geq \varphi\left(\mu_0\right) + \frac{11}{16} r \left(r - r_0\right)     \geq \\
    \E_{\mu_0}\left[f\right] +  \frac{5}{8} r \left(r - r_0\right) > \E_{\mu_0}\left[f\right] +  \frac{1}{2} r \left(r - r_0\right)
\end{multline*}
\end{proof}
...
\begin{multline*}
Z_3:=\{\left(y,\beta,\mu,\nu,\xi,\zeta\right) \in Y \times \Gm\left(X\right) \times \PM\left(X\right)^4 \mid \E_\nu\left[\beta\left(\mu\right)\right] - \E_\mu\left[\beta\left(\mu\right)\right] \geq \\ 
\frac{1}{2}\left(\DKR\left(\mu,\zeta\right) - \epsilon\right)_+ \DKR\left(\mu,\zeta\right)\}
\end{multline*}
...
\section{Examples of Regular Upper Bounds}
\label{sec:examples}
...
%%% This syntax is also listed on the website %%%%
