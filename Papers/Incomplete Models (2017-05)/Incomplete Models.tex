...
\begin{enumerate}[i.]
\item $N$ is closed.
%\item If $y \in \Ob^n$, $\mu \in \PMO$ and $\nu \in M$ are s.t. $y \in \Sp \PO_{n*}\nu$ and $\mu = \lim_{r \rightarrow 0}{\left(\nu \mid \left(\PO_n\right)^{-1}\left(\B_r\left(y\right)\right)\right)}$, then $\left(y,\mu\right) \in M \mid \Ob^n$. Here, $\nu \mid \left(\PO_n\right)^{-1}\left(\B_r\left(y\right)\right)$ is the conditional probability measure and the limit is understood to be in the weak topology.
\item For every $y \in \Ob^n$, the set $N_y:=\{\nu \in \PMO \mid \left(y,\nu\right) \in N\}$ is convex.
\item For every $\mu \in M$ and $\mu$-almost every $y \in Y$, $(\mu \mid \pi)(y) \in N_y$.
\end{enumerate}
%
...
Suppose that $M$ is convex and for each $n \in \Nats$, $\Ob_n$ is a finite set. Then, for each $y \in \Ob^n$
...
Consider some $I \subseteq \Nats$ and suppose that for each $n \in I$, we are given $K_n: \Ob^n \M \Ob_n$. Define $M_K$ by

\begin{equation}
M_K:=\{\mu \in \PMO \mid \forall n \in I: \PO_{n+1*}\mu = \PO_{n*}\mu \ltimes K_n\}
\end{equation}

Let $n \in \Nats$ and $y \in \Ob^n$ be s.t. for any $m < n$, if $m \in I$ then $y_m \in \Sp{K_n(y_{:m})}$. Then???

\begin{equation}
M_K \mid y = \{\mu \in \PMO \mid \Sp{\mu} \subseteq y\OO,\, \forall m \in I: m \geq n \implies \PO_{m+1*}\mu = \PO_{m*}\mu \ltimes K_m\}
\end{equation}
