...
Let $X,Y$ be Polish spaces, $\pi: X \rightarrow Y$ continuous and $M \subseteq \PM(X)$. We say that $N: Y \rightarrow 2^{\PM(X)}$ is an \emph{upper bound for $M \mid \pi$} when 
%
\begin{enumerate}[i.]
...
Given $X,Y,Z$ Polish spaces, $K: Z \M X$ and $\pi: X \rightarrow Y$ continuous, we define $\pi_* K: Z \M Y$ by $\left(\pi_* K\right)(z):=\pi_*\left(K(z)\right)$...
