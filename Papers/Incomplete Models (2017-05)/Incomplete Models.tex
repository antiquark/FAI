...
\item\label{con:def__update_incomplete__conv} For every $y \in \Ob^n$, the set $N_y:=\{\nu \in \PMO \mid \left(y,\nu\right) \in N\}$ is convex.
\item For every $\mu \in M$ and $\pi_*\mu$-almost every $y \in Y$, $(\mu \mid \pi)(y) \in N_y$.
\end{enumerate}
...
Suppose that $M$ is convex and $Y$ is a finite set (in particular, this example is applicable when the $\Ob_n$ are finite sets, $X=\OO$ and $Y=\Ob^n$). Then, there is a unique $N$ which is a \emph{minimal} upper bound for $M \mid \pi$ and we have

\begin{equation}
N_y = \overline{\{(\mu \mid \pi^{-1}(y)) \mid \mu \in M,\, \mu\left(\pi^{-1}(y)\right) > 0\}}
\end{equation}
...
Assume that for all $n \in \Nats$, $\Ob_n$ is a compact subset of $\Reals^{d_n}$ for some $d_n \in \Nats$. In particular, $\Ob^n$ is a subset of $\Reals^{\sum_{m < n} d_n}$, and we will regard it as a metric space using the Euclidean metric. For any $y \in \Ob^n$ and $r > 0$, $\B_r\left(y\right)$ will denote the open ball of radius $r$ with center at $y$. 

Fix any $M \subseteq \PMO$. Then, for any $n \in \Nats$, there is a unique $M_n \subseteq \Ob^n \times \PMO$ which is minimal among sets satisfying conditions \ref{con:def__update_incomplete__clos} and \ref{con:def__update_incomplete__conv} of Definition~\ref{def:update_incomplete} and s.t. for any $y \in \Ob^n$, $\nu \in \PMO$ and $\mu \in M$, if $y \in \Sp \PO_{n*}\mu$ and $\nu = \lim_{r \rightarrow 0}{\left(\mu \mid \left(\PO_n\right)^{-1}\left(\B_r\left(y\right)\right)\right)}$, then $\left(y,\nu\right) \in M_n$ (the limit is defined using the the weak topology). Moreover, $M_n$ is an upper bound for $M \mid \PO_n$.
...
\section{Appendix: Useful Theorems}
\label{sec:theorems}
