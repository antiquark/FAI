...
Let $X,Y$ be Polish spaces, $\pi: X \rightarrow Y$ continuous and $M \subseteq \PM(X)$. We say that $N \subseteq Y \times \PM(X)$ is an \emph{upper bound for $M \mid \pi$} when 
%
\begin{enumerate}[i.]
\item\label{con:def__update_incomplete__clos} $N$ is closed.
\item\label{con:def__update_incomplete__conv} For every $y \in \Ob^n$, the set $N(y):=\{\nu \in \PMO \mid \left(y,\nu\right) \in N\}$ is convex.
\item For every $\mu \in M$ and $\pi_*\mu$-almost every $y \in Y$, $(\mu \mid \pi)(y) \in N(y)$.
\end{enumerate}

\end{definition}
\end{samepage}

Note that given $N$ an upper bound for $M \mid \pi$ and any $N' \subseteq Y \times \PM(X)$ s.t. $N \subseteq N'$, $N'$ is also an upper bound for $M \mid \pi$.
...
Suppose that $M$ is convex and $Y$ is a finite set (in particular, this example is applicable when the $\Ob_n$ are finite sets, $X=\OO$, $Y=\Ob^n$ and $\pi=\PO_n$). Then, there is a unique $N$ which is a \emph{minimal} upper bound for $M \mid \pi$ and we have

\begin{equation}
N(y) = \overline{\{(\mu \mid \pi^{-1}(y)) \mid \mu \in M,\, \mu\left(\pi^{-1}(y)\right) > 0\}}
\end{equation}
...
For each $n \in \Nats$, define $M^K_n$ by

\begin{equation}
M^K_n = \{(y,\mu) \in \Ob^n \times \PMO \mid \Sp{\mu} \subseteq y\OO,\, \forall m \in I: m \geq n \implies \PO_{m+1*}\mu = \PO_{m*}\mu \ltimes K_m\}
\end{equation}
...
Fix any $M \subseteq \PM(X)$. Then, there is a unique $N \subseteq Y \times \PM(X)$ which is minimal among sets satisfying conditions \ref{con:def__update_incomplete__clos} and \ref{con:def__update_incomplete__conv} of Definition~\ref{def:update_incomplete} and s.t. for any $y \in Y$, $\nu \in \PM(X)$ and $\mu \in M$, if $y \in \Sp \pi_*\mu$ and $\nu = \lim_{r \rightarrow 0}{\left(\mu \mid \pi^{-1}\left(\B_r\left(y\right)\right)\right)}$, then $\left(y,\nu\right) \in N$ (the limit is defined using the the weak topology). Moreover, $N$ is an upper bound for $M \mid \pi$.
...
\begin{equation}
\label{eqn:thm_main}
\lim_{n \rightarrow 0} \DKR^n\left(F_n\left(x_{:n}\right),\, M_n(x_{:n})\right) = 0
\end{equation}
...
Consider $M \subseteq \PMO$, $\Sqn{M_n \subseteq \Ob^n \times \PMO}$ and $\epsilon > 0$. Assume $M_n$ is an upper bound for $M \mid \PO_n$. Let $S$ be a gambling strategy s.t. for each $n \in \Nats$ and $y \in \Ob^n$, $S_n\left(y\right)$ is $\left(M_n(y),\, \epsilon\right)$-savvy (relatively to $\rho_n$). Then, $S$ is $\mu^*$-prudent for any $\mu^* \in M$.
