...
%"preprint" option removes info line on the page footer
%Put target journal in the options:
...
\runtitle{Forecasting Using Incomplete Models}
%-Title

%+Author
\begin{aug}
\author{\fnms{Vadim} \snm{Kosoy}\ead[label=e1]{vadim.kosoy@intelligence.org}},
...
\begin{keyword}
\kwd{statistical learning theory}
\kwd{online learning}
\kwd{sequence prediction}
\kwd{knightian uncertainty}
\end{keyword}
...
\sloppy{For any ${y \in Y_1}$, ${\beta \in \Gm\left(X\right)}$, define ${i_y: Y_2 \rightarrow X}$ by ${i_y\left(y_2\right):=\left(y,y_2\right)}$ and ${\beta_y \in \Gm\left(Y_{2}\right)}$ by $\beta_y\left(\nu,y'\right):=\beta\left(i_{y*}\nu,y,y'\right)$. Applying Lemma~\ref{lmm:unwinnable} to ${\beta_y}$ we get ${\nu \in \PM\left(Y_2\right)}$ s.t.}
...
\[\SV \Bd_b G_{m_0+1}\left(y_{:m_0},\BM_{:m_0+1};x\right) \geq \SVM G_{m_0}\left(y_{:m_0-1},\BM_{:m_0}\right) > -b\]
...
\begin{multline*}
\sup_{n \in \Nats} \max_{x' \in x_{:n-1}\OO} \left(\sum_{b \leq b_0} \zeta\left(b\right) {\SV \Bd_b G^F_{n}\left(x_{:n-1},x'\right)} + \right. \\ \left. \sum_{b > b_0} \zeta\left(b\right) \SV G^F_{n}\left(x_{:n-1},x'\right)\right) < +\infty
\end{multline*}
...
%The following equation looks kind of ugly... the \vphantom is to make the height of the left bracket the same size as the right one across the line break. Perhaps try using \Bigg instead?
\begin{multline*}
\sup_{n \geq k} \max_{x' \in x_{:n-1}\OO} \left(\xi\left(k\right) \SV G^{k F}_{n}\left(x_{:n-1},x'\right) \vphantom{\sum_{\substack{j \ne k\\ j \leq n}}} \right. + \\ \left. \sum_{\substack{j \ne k\\ j \leq n}} \xi\left(j\right) \SV G^{j F}_{n}\left(x_{:n-1},x'\right)\right) < +\infty
\end{multline*}
...
\[\E\left[X''_{n+1}-X''_n \mid \F_n\right] = \E\left[\E\left[X_{n+1} \mid \F_{n+1}\right]-\E\left[X_n \mid \F_n\right] \mid \F_n\right]\]
\[\E\left[X''_{n+1}-X''_n \mid \F_n\right] = \E\left[X_{n+1}-X_n \mid \F_n\right] \geq \alpha Y_n\]
...
\begin{multline*}\SV \PG{S}_{n+1}\left(y,\BM\right) - \SVM \PG{S}_{n+1}\left(y,\BM\right) \leq \\ \SV \PG{S}_n\left(y_{:n-1},\BM_{:n}\right) - \SVM \PG{S}_n\left(y_{:n-1},\BM_{:n}\right)
\end{multline*}

By the induction hypothesis, we get

\[\SV \PG{S}_{n+1}\left(y,\BM\right) - \SVM \PG{S}_{n+1}\left(y,\BM\right) \leq  2 \max_{m < n} \sup_{y \in \Ob^m} \N{S_m\left(y\right)}  + 1\]
\[\SV \PG{S}_{n+1}\left(y,\BM\right) - \SVM \PG{S}_{n+1}\left(y,\BM\right) \leq 2 \max_{m \leq n} \sup_{y \in \Ob^m} \N{S_m\left(y\right)}  + 1\]
...
By the assumption, the first two terms on the right hand side total to ${\leq 1}$, yielding the desired result.
\end{proof}
...
\[\E_\nu\left[f\right] \geq \varphi\left(\nu\right) - \frac{1}{16} r \left(r - r_0\right) \geq \varphi\left(\mu_0\right) + \frac{11}{16} r \left(r - r_0\right)\]
\[\E_\nu\left[f\right] \geq \E_{\mu_0}\left[f\right] +  \frac{5}{8} r \left(r - r_0\right) >  \E_{\mu_0}\left[f\right] +  \frac{1}{2} r \left(r - r_0\right)\]
...
\sloppy{Fix any $\mu^* \in M$. By Definition~\ref{def:update_incomplete}, for $\PO_{n*}\mu$-almost any $y\in\Ob^n$, ${\left(\mu^* \mid \PO_n\right)\left(y\right) \in M_n(y)}$. Since $S_n\left(y\right)$ is $\left(M_n(y),\epsilon\right)$-savvy, for any $\mu \in \PM\left(X\right)$, denoting $r_{\mu y}:=\DKR^n\left(\mu,\,M_n(y)\right)$:}
...
\sloppy{Consider $W_{1,2,3,4}$ Polish spaces, $X:=\prod_{i=1}^4 W_i$, $Y:=\prod_{i=1}^3 W_i$ and $Z = W_1 \times W_2$. Let $\pi^Y: X \rightarrow Y$, $\pi^Z: X \rightarrow Z$ and $\pi^W: X \rightarrow W_1$ be the projection mappings, $\mu \in \PM(X)$ and $K: Z \M W_3$. Assume that $\pi^Y_* \mu = \pi^Z_*\mu \ltimes K$. Then, for $\pi^W_* \mu$-almost any $w \in W_1$}
...
$$\limsup_{r \rightarrow 0} \E_{\mu}\left[f \mid \pi^{-1}\left(\B_r\left(y\right)\right)\right] \leq \limsup_{r \rightarrow 0} \E_{\mu}\left[f_k \mid \pi^{-1}\left(\B_r\left(y\right)\right)\right] + \epsilon$$
$$\limsup_{r \rightarrow 0} \E_{\mu}\left[f \mid \pi^{-1}\left(\B_r\left(y\right)\right)\right] \leq \E_{\mu}\left[f_k \mid \pi^{-1}\left(y\right)\right] + \epsilon$$
...
%%%if you want author-year style instead%%%
