...
%The form of inequality~(\ref{eqn:thm__regret_bound__regret}) is s.t. we can assume w.l.o.g. that $\eta < \frac{1}{N}$ and $\epsilon < \frac{1}{\Abs{\A}}$.

We start by giving an explicit description of an algorithm that computes the policy $\pi^\dagger$.

By condition~\ref{con:def__sane__bold} of Definition~\ref{def:sane}, for each $k\in[N]$ we can choose some $\pi^k: \St \rightarrow \A$ s.t. for any $s \in \St$, $\pi^k(s) \in \A_{M^k}^\star(s)$ and $\alpha^k\APM{\pi^k(s)}{s} > \epsilon$.
...
\Loop{}{
        ...
        \For{$m=0$ \KwTo $T-1$}{                
                ...
                \NFa$\leftarrow 0$\;
                \For{$k=0$ \KwTo $N-1$}{
                   $\Z(k) \leftarrow \Z(k)\T_{L^k}\APM{\NS,\AA}{\S,\RA}$\;
                   \NFa$\leftarrow\NFa+\Z(k)$
                }
                \NFb$\leftarrow 0$\;
                \For{$k=0$ \KwTo $N-1$}{
                   $\Z(k) \leftarrow \NFa^{-1}\Z(k)$\;
                   \If{$\Z(k) < \eta$}{
                        $\Z(k) \leftarrow 0$
                   }
