...
\item\label{con:prp__delegation__xa} $\CP{}{X_n = a}{K,\Theta_n}=\CP{}{X_n\ne\bot}{K,\Theta_n} \Ad^K\APM{a}{\Theta_n}$
...
For brevity, we denote $\Phi_n:=\AP{\Theta_n,\Psi_n,Z_n}$. Define the events $\{D_{nak} \subseteq \Omega\}_{n \in \Nats, a \in \A, k\in[N]}$ by

\[D_{nak}:=\AC{\CP{}{X_n=a}{K=k,\Phi_n} > 0 \land \AP{a = \Psi_n \lor \CP{}{X_n = \Psi_n}{K=k,\Phi_n} \leq \epsilon}}\]

Using assumption~\ref{con:prp__delegation__z}, for every $n\in\Nats$ and $a\in\A$

\begin{align*}
\CP{}{D_{naK}}{\Phi_n} = \CP{k\sim Z_n}{D_{nak}}{\Phi_n}
\end{align*}

Using assumption~\ref{con:prp__delegation__eta}, it follows that

\[\AP{\exists k \in \Supp{Z_n}: \neg D_{nak}} \implies \CP{}{D_{naK}}{\Phi_n} \leq 1 - \eta\]

Using assumption~\ref{con:prp__delegation__xa}, we have

\[D_{nak} = \AC{\Ad^k\APM{a}{\Theta_n} > 0 \land \AP{a = \Psi_n \lor \Ad^k\APM{\Psi_n}{\Theta_n} \leq \epsilon}}??\]
