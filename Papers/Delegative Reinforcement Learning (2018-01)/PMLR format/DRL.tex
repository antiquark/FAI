...
\Psi_n: \Omega &\rightarrow \A_\square\\
A_n: \Omega &\rightarrow \A_\square\\
X_n: \Omega &\rightarrow \A_\square\\
\Theta_n: \Omega &\rightarrow \St
\end{align*}

\Comment{We also define $H_n: \Omega \rightarrow \St^n$ by

$$H_n:= \AP{\Theta_0,\Theta_1 \dots \Theta_{n-1}}$$}
...
$$\Psi_{n} = \pi^{\star J_l}\AP{\Theta_n}$$

$$\Theta_0 = s_0$$

$$\CP{}{\Theta_{n+1} = s,X_{n}=a}{\Theta_{n},A_n} = \T_{L^K}\APM{s,a}{\Theta_n,A_n}$$

$$A_n = \begin{cases} \Psi_n \text{ if } \forall k \in \Supp{Z_n}: \Ad^k\APM{\Psi_n}{\Theta_n} > 0 \text{ and } Z_n\AP{J_n} > 0 \\ \text{some } a\in\A \text{ s.t. } \forall k \in \Supp{Z_n}: \Ad^k\APM{a}{\Theta_n} > 0 \text{ if such exists and } Z_n\AP{J_n}=0\\\bot \text{ otherwise} \end{cases}$$

$$\tilde{Z}_{n+1}(k)\sum_{j = 0}^{N-1} Z_n(j) \T_{L^j}\APM{\Theta_{n+1},X_n}{\Theta_n,A_n}=Z_{n}(k) \T_{L^k}\APM{\Theta_{n+1},X_n}{\Theta_n,A_n}$$
...
We now define $\pi^\dagger$ s.t. for any $n \in \Nats$, $h \in ?$ s.t. $\Pr\left[A\Theta_{:n}=h\right] > 0$ and $a \in ?$
