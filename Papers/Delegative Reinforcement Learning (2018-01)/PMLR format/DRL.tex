...
\newcommand{\MP}[2]{#1\AB{#2}}
\newcommand{\MA}[2]{#1\Chev{#2}}
...
The loss incurred during each \enquote{episode} of length $T$ can be divided into short-term (associated with the rewards during the episode) and long-term (associated with the rewards after the episode, or, equivalently, with the value of the state reached at the end of the episode.) To describe the short-term loss, we introduce the policies $\AC{\pi_n^{\star k}: \St^* \times \St \K \A}_{n \in \Nats}$ defined by

\begin{equation}
\pi_n^{\star k}(h,s):=\begin{cases} \IP\AB{\alpha^k}(h,s) \text{ if } \Abs{h} < nT \\ \pi^{\star k}(h,s) \text{ otherwise} \end{cases}
\end{equation}
...
\begin{equation}
\Rg_{L^k}^{\IP}(\gamma) = (1-\gamma)\sum_{n=0}^\infty \sum_{m=0}^{T-1} \gamma^{nT+m}\Ea{x\sim{\pi_n^{\star  k}}^{M^k}-\pi^{!k}\AB{\alpha^k}^{M^k}??}{\R\AP{x_{nT+m}}}+O\AP{\frac{\Tn_{M^k}(\gamma)}{T}}
\end{equation}

Here, the notation $\Ea{\mu-\nu}{f}$ means $\Ea{\mu}{f}-\Ea{\nu}{f}$.

In order to further analyze the short-term loss, we introduce the policies $\AC{\pi_n^{\sharp k}: \St^* \times \St \K \A}_{n \in \Nats}$. These policies result from modifying the algorithm as follows (starting from line~\ref{ln:loop_begin}.)
