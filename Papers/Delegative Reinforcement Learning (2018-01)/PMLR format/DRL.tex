...
\[A^\dagger_n = \begin{cases} \Psi^\dagger_n \text{ if } \forall k \in \Supp{Z^\dagger_n}: \Ad^k\APM{\Psi^\dagger_n}{\Theta^\dagger_n} > 0 \text{ and } Z^\dagger_n\AP{J^\dagger_n} > 0 \\ \text{some } a\in\A \text{ s.t. } \forall k \in \Supp{Z^\dagger_n}: \Ad^k\APM{a}{\Theta^\dagger_n} > 0 \text{ if such exists and } Z^\dagger_n\AP{J^\dagger_n}=0\\\bot \text{ otherwise} \end{cases}\\\]
%
\begin{align*}
...
\Psi^\dagger_{n} &= \pi^{\star J^\dagger_l}\AP{\Theta^\dagger_n}\\
\Theta^\dagger_0 &= s_0\\
X^\dagger_0 &= s_0\\
