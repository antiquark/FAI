...
Consider $N \in \Nats$, $\A$ a finite set, $\epsilon \in (0,\frac{1}{\Abs{\A}})$, $\eta \in (0,1)$, $B \subseteq \A$, $\zeta \in \Delta[N]$ and $\sigma: [N] \K \A$. Suppose that for every $a \in \A$

$$\Pa{k \sim \zeta}{\sigma(a \mid k) > 0 \land \AP{a \in B \lor \forall b \in B: \sigma(b \mid k) \leq \epsilon}} \leq 1 - \eta$$

Then

$$\Ia{(k,a)\sim\zeta\ltimes\sigma}{k;a} \geq \eta \ln\left(1 + \epsilon(1-\epsilon)^{(1-\epsilon)/\epsilon}\right)$$
