...
\Loop{}{\label{ln:loop_begin}
        ...
        \For{$m=0$ \KwTo $T-1$}{
                \eIf{$\Z(\J) > 0$}{
                        ...
                        \For{$k=0$ \KwTo $N-1$}{
                                \If{$\Z(k) > 0$ \bf{and} $\alpha^k\APM{\RA}{\S}=0$}{\label{ln:delegate_cond}
                                        \RA$\leftarrow\bot$
                                }
                        }                        
                }{
                        ...
                        \For{$a\in\A$}{
                                \GA$\leftarrow$ TRUE\;
                                \For{$k=0$ \KwTo $N-1$}{
                                        \If{$\Z(k) > 0$ \bf{and} $\alpha^k\APM{a}{\S}=0$}{
                                                \GA$\leftarrow$ FALSE
                                        }
                                }
                                \If{\GA}{
...
% What if the sampled hypothesis is falsified in the process of the episode?
% Doesn't this invalidate the reduction from the original delegation routine to lines 6-16?
% Seems that we should complicate the algorithm or weaken the # of delegations bound
