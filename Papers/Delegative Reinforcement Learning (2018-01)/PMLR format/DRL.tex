...
$$A_n: \Omega \rightarrow ?$$

$$\Theta_n: \Omega \rightarrow ?$$

We also define $A\Theta_{:n}: \Omega \rightarrow ?$ by

$$A\Theta_{:n}:= A_0\Theta_0A_1\Theta_1 \ldots A_{n-1}\Theta_{n-1}$$

(The following conditions on $A$ and $\Theta$ imply that the range of the above is indeed in $?$.) Let $?$ and $?^{!k}$ be as in Proposition B.1 (the form of the bound we are proving allows assuming w.l.o.g. that $\epsilon < \frac{1}{\Abs{\A}}$). By condition iii of Definition 2, there is $\pi^k : ? \rightarrow \A$ s.t. for any $h \in ?$, if $S^k(h) \in \UC^k$ then

$$\sigma^k\AP{\pi^k(h) \mid h} > \epsilon$$

$$\pi^k(h) \in \A_k^\omega\AP{S^k(h)}$$

We construct $\Omega$\Comment{, $\F$}, $K$, $\Z$, $\tilde{\Z}$, $\J$, $\Psi$, $A$ and $\Theta$ s.t $K$ is uniformly distributed and for any $k \in [N]$, $l \in \Nats$, $m \in [T]$ and $o \in ?$, denoting $n = lT+m$
...
$$A_n = ?\left(A\Theta_{:n}, \Psi_n\right)$$

$$\tilde{\Z}_{n+1}(k)\sum_{j = 0}^{N-1} \Z_n(j) [[A_n = ?^{!j}\left(A\Theta_{:n}, \Psi_n\right)]] \nu^j(\Theta_n \mid A\Theta_{:n}A_n)=\Z_{n}(k) [[A_n = ?^{!k}\left(A\Theta_{:n}, \Psi_n\right)]] \nu^k\left(\Theta_{n} \mid A\Theta_{:n}A_{n}\right)$$
...
We now construct the $?$-policy $\pi^*$ s.t. for any $n \in \Nats$, $h \in ?$ s.t. $\Pr\left[A\Theta_{:n}=h\right] > 0$ and $a \in ?$

$$\pi^*(a \mid h):=\Pr\left[A_n = a \mid A\Theta_{:n} = h\right]$$

That is, we perform Thompson sampling at time intervals of size $T$, moderated by the delegation routine $?$, and discard from our belief state hypotheses whose probability is below $\eta$ and hypotheses sampling which resulted in recommending "unsafe" actions i.e. actions that $?$ refused to perform.
...
$$A^!_n = ?^{!K}\left(A\Theta^!_{:n}, \Psi^!_n\right)$$

$$\tilde{Z}^!_{n+1}(k)=\frac{Z^!_{n}(k) [[A^!_n = ?^{!k}\left(A\Theta^!_{:n}, \Psi^!_n\right)]] \nu^k\left(\Theta^!_{n} \mid A\Theta^!_{:n}A^!_{n}\right)}{\sum_{j = 0}^{N-1} Z^!_n(j) [[A^!_n = ?^{!j}\left(A\Theta^!_{:n}, \Psi^!_n\right)]] \nu^j(\Theta^!_n \mid A\Theta^!_{:n}A^!_n)}$$

For any $k \in [N]$, we construct the $?$-policy $\pi^{?k}$ s.t. for any $n \in \Nats$, $h \in ?$ s.t. $\Pr\left[A\Theta^!_{:n}=h,\ K = k\right] > 0$ and $a \in ?$

$$\pi^{?k}(a \mid h):=\Pr\left[A^!_n = a \mid A\Theta^!_{:n} = h,\ K = k\right]$$

Given any $?$-policy $\pi$ and $?$-policy $\sigma$ we define $\alpha_{\sigma\pi}: ? \K ?$ by
...
$$\tau^k := \max_{s \in \UC^k} \sup_{\gamma \in [1 - \alpha, 1)} \Abs{\frac{\D\V_k(s,\gamma)}{\D\gamma}}+1$$

Condition iii of Proposition B.1 and condition ii of Definition 2 imply that for any $h \in ?$
...
Here, the $?$-policy $\pi^{*k}_n$ is defined as $\pi^*_n$ in Proposition B.2, with $\pi^k$ in place of $\pi^*$. We also define the $?$-policies $\pi^{!k}_n$ and $\pi^{!!k}_n$ by
...
Condition iv of Proposition B.1 implies that, given $h \in ?$ s.t. $\Abs{h} \geq nT$
...
$$\Abs{\EU^{!!k}_n-\EU^{!k}_n} \leq \Pr_{x\sim\rho^{!k}_n}\left[\exists m \in [T]: x_{nT+m} \in \bot?\right]$$

Since $\Z_{nT}^{!}(K) \geq \eta$, it follows that

$$\Abs{\EU^{!!k}_n-\EU^{!k}_n} \leq \frac{1}{\eta}\Pr_{x\sim\rho^{?k}}\left[\exists m \in [T]: x_{nT+m} \in \bot?\right] \leq \frac{1}{\eta}\Ea{x\sim\rho^{?k}}{\Abs{\AC{m \in [T] \mid x_{nT+m} \in \bot?}}}$$

$$\sum_{n=0}^\infty \Abs{\EU^{!!k}_n-\EU^{!k}_n} \leq \frac{1}{\eta}\Ea{x\sim\rho^{?k}}{\Abs{\AC{n \in \Nats \mid x_n \in \bot?}}}$$
