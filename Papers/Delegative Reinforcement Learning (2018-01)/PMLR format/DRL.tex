...
Consider some $T\in\Nats^+$, an MDP $M$, some $\pi^\star: \St_M \K \A_M$ and $\pi^0: \St_M^* \times \St_M \K \A_M$. For any $n \in \Nats$, define $\pi^\star_n: \St_M^* \times \St_M \K \A_M$ by

$$\pi^\star_n(h,s):=\begin{cases} \pi^0(h,s) \text{ if } \Abs{h} < nT \\ \pi^\star\AP{s} \text{ otherwise} \end{cases}$$

Assume that for any $h \in \FH_{M\pi^0}$ and $s \in \St_M$ 

\begin{equation}
\Supp{\pi^0(h,s)} \subseteq \A_{M}^0\AP{s}
\end{equation}
...
\begin{equation}
\EU^{\pi^\star}_M(\gamma)-\EU^{\pi^0}_M(\gamma) \leq \AP{1-\gamma^T}\sum_{n=0}^\infty \gamma^{nT}\AP{\EU^\star_n-\EU^0_n} + \frac{2\Tn_M(\gamma)\gamma^T(1-\gamma)}{1-\gamma^T}
\end{equation}
