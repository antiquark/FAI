...
Consider non-empty finite sets $\A$ and $\St$, $N \in \Nats^+$, $\epsilon\in\AP{0,\Abs{\A}^{-1}}$, $\eta\in(0,1)$ and\\ $\AC{\Ad^k: \St \K \A}_{k\in[N]}$. Consider also  a probability space $(\Omega,P)$ and random variables $K: \Omega \rightarrow [N]$, $\AC{\Theta_n: \Omega \rightarrow \St}_{n\in\Nats}$, $\AC{X_n,\Psi_n: \Omega \rightarrow \A \sqcup \{\bot\}}_{n \in \Nats}$, $\AC{Y_n: \Omega \rightarrow \{\top,\bot\}}_{n\in\Nats}$ and\\ $\AC{Z_n: \Omega \rightarrow \Delta[N]}_{n \in \Nats}$. Assume that for any $n\in\Nats$, $k\in[N]$ and $a\in\A$

\begin{enumerate}[i.]

\item\label{con:prp__delegation__x} $\CP{}{X_n = a}{K,\Theta_n}=\CP{}{X_n\ne\bot}{K,\Theta_n} \Ad^K\APM{a}{\Theta_n}$
\item\label{con:prp__delegation__y} $Y_n = \top \iff \exists a \in \A \forall k \in \Supp{Z_n}: \Ad^k\APM{a}{\Theta_n} > 0 \land \AP{a=\Psi_n \lor \Ad^k\APM{\Psi_n}{\Theta_n} \leq \epsilon}$
\item\label{con:prp__delegation__z} $Z_n(k)=\CP{}{K = k}{\Theta_0,\Theta_1\dots\Theta_{n},X_0,X_1 \dots X_{n-1}}$
...
\begin{equation}
\Ea{}{\Abs{\ACM{n\in\Nats}{Y_n=\bot}}} \leq \frac{\ln{N}}{\eta\ln{\AP{1+\epsilon(1-\epsilon)^{\frac{1}{\epsilon}-1}}}}
\end{equation}
...
\begin{align*}
\ln{N} &\geq \sum_{n=0}^\infty\Ea{}{\CI{}{K;\Theta_{n+1},X_n}{\Theta_n,\Psi_n,Z_n}} \\ 
&\geq\sum_{n=0}^\infty\Ea{}{\CI{}{K;X_n}{\Theta_n,\Psi_n,Z_n}} \\
&\geq\sum_{n=0}^\infty\Ea{}{\CI{}{K;X_n}{\Theta_n,\Psi_n,Z_n};Y_n=\bot}
\end{align*}

Using assumption... for every $n\in\Nats$... $a\in\A$

\[\CP{}{\CP{}{X_n=a}{K} > 0 \land \AP{a = \Psi_n \lor \CP{}{X_n = \Psi_n}{K} \leq \epsilon}}{Y_n=\bot,\Theta_n,\Psi_n,Z_n}??\]
