...
\Q_M(s,a,\gamma):=(1-\gamma)\Rew_M(s)+\gamma\Ea{t \sim \T_{M\pi}(s)}{\V_M(t,\gamma)}
\end{equation}
...
The limits above always exist, thanks to our assumptions that $\St$ and $\A$ are finite.
...
The above quantity is closely related to the \emph{bias span} parameter, which is known to figure in regret bounds in the no-traps setting (see \cite{Bartlett2009}). Intuitively, it measures how hard is it to recover from a non-fatal error. It can also be related to the mixing time of the Markov chain resulting from following the optimal policy in the MDP, but discussing this in detail is out of the present scope.
...
\emph{Section intentionally left blank in anonymous version.}
