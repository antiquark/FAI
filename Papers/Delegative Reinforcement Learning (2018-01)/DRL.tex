...
\item\label{con:def__sane__bold} There is $a \in \A_M^\star(s)$ s.t. $\sigma(a \mid h,s) > \epsilon$
\end{enumerate}
...
\begin{equation}
\EU_{M^k\AB{\sigma^k}}^\star(\gamma) - \EU_{M^k\AB{\sigma^k}}^{\pi^\dagger}(\gamma) \leq C\AP{(1-\gamma)N^5 \AP{\ln N} \AP{\frac{1}{\epsilon}+\Abs{\A}}\sum_{j=0}^{N-1} \Tn_{M^k}(\gamma)}^{\sfrac{1}{4}}
\end{equation}
