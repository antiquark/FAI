...
The meaning of the factor $\sum_{j=0}^{N-1} \Tn_{M^j}(\gamma)$ might seem somewhat unclear, however, we can bound this factor as follows.

Given a finite set $A$ and some $K: A \K A$, $\sigma(K)$ denotes the spectrum of the operator $K^*: \Reals^A \rightarrow \Reals^A$ defined by

\[\AP{K^*f}(x):=\Ea{y\sim K(x)}{f(y)}\]

\begin{samepage}
\begin{proposition}
\label{prp:mixing_time}

Let $M$ be an MDP and $\pi^\star: \St_M \rightarrow \A_M$ be s.t. for any $s \in \St_M$, $\pi^\star(s)\in\A_M^\star(s)$ (i.e. $\pi^\star$ is a Blackwell optimal policy.) Define $\lambda_M\in[0,1)$ by

\begin{equation}
\lambda_M := \max\ACM{\Abs{\lambda}}{\lambda \in \sigma\AP{\T_{M\pi^\star}},\ \Abs{\lambda} < 1}
\end{equation}
