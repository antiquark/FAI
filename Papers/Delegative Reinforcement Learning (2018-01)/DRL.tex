...
The algorithm we construct in order to show the regret bound is a variant of Thompson sampling. The timeline is divided into intervals of length $O\AP{(1-\gamma)^{-1/4}}$. At the start of each interval, the algorithm samples a hypothesis out of its current belief state, and starts carrying out an optimal policy for this hypothesis. On each round, it checks whether...
