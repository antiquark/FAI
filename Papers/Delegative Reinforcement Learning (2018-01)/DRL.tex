...
We start by reviewing some basic definitions and properties of Markov decision processes. See e.g. \cite{Feinberg2002} for a detailed overview with proofs. First, some notation. 
...
We define $\V^0_M : \St_M \rightarrow [0,1]$ and $\Q^0_M: \St_M \times \A_M \rightarrow [0,1]$ by
...
Indeed, the limits above are guaranteed to exist. We define $\A_M^0: \St_M \rightrightarrows \A_M$ by

\begin{equation}
\A_M^0(s) := \Argmax{a \in \A_M} \Q_M^0(s,a)
\end{equation}

Finally, it is well known that there are some $\A_M^*: \St_M \rightrightarrows \A_M$ (the set of Blackwell optimal actions) and $\gamma_M\in[0,1)$ s.t. for any $\gamma\in\AP{\gamma_M,1}$
...
We can now define the properties of a policy that make it a \enquote{satisfactory} advisor.
...
Consider $M$ an MDP, some $\epsilon\in(0,1)$ and some $\sigma: \St_M^* \times \St_M \K \A_M$. We think of $\sigma$ as a policy, where the first argument is the past history of states and the second argument is the current state. $\sigma$ is called \emph{$\epsilon$-sane for $M$} when for any $h \in \St_M^*$ and $s \in \St_M$,
...
% Alternatively we can bound the probability of \A^* as a whole?
\end{enumerate}
