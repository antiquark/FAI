...
Consider an MDP $M$ and some $\pi: \St_M^* \times \St_M \K \A_M$. We think of $\pi$ as a policy, where the first argument is the past history of states and the second argument is the current state... ?
...
? ...We define $\EU_M^\pi: [0,1) \rightarrow [0,1]$ by

\begin{equation}
\EU_M^\pi(\gamma) := (1-\gamma)\sum_{n=0}^\infty \gamma^n \Ea{?}{\R_M}
\end{equation}
...
Consider $M$ an MDP, some $\epsilon\in(0,1)$ and some $\sigma: \St_M^* \times \St_M \K \A_M$. $\sigma$ is called \emph{$\epsilon$-sane for $M$} when for any $h \in \St_M^*$ and $s \in \St_M$,
...
There is some constant $C \in (0,\infty)$ s.t. the following holds... ?
