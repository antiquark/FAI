...
Given a real vector space $\Y$, we will use $\PSD(\Y)$ to denote the set of positive semidefinite bilinear forms on $\Y$ and $\PD(\Y)$ to denote the set of positive definite bilinear forms on $\Y$. Given a bilinear form $Q:\Y\times\Y\rightarrow\Reals$, we will slightly abuse notation by also regarding it as a linear functional $Q:\Y\otimes\Y\rightarrow\Reals$. Thereby, we have $Q(v \otimes w) = Q(v, w)$ and $Qv^{\otimes 2}=Q(v,v)$. Also, if $\Y$ is finite-dimensional and $Q$ is non-degenerate, we will denote $Q^{-1}:\Y^*\times\Y^*\rightarrow\Reals$ the unique bilinear form which satisfies

$$\forall v\in\Y,\alpha\in\Y^*:\AP{\forall\beta\in\Y^*:Q^{-1}(\alpha,\beta)=\beta v}\iff\AP{\forall w\in\Y:Q(v,w)=\alpha w}$$ 

\textbf{Definition 1}\Co{b}

\textit{Consider a set $\X$, a real vector space $\Y$, some $\F\subseteq\AC{\X\rightarrow\Y}$ and a family $\AC{Q_x\in\PSD(\Y)}_{x\in\X}$. Consider also $n\in\Nats$, a sequence $\AC{x_k\in\X}_{k\in[n]}$ and $x^*\in\X$. $x^*$ is said to be} $\AP{\F,Q}$-dependant on $\AC{x_k}$ \textit{when, for any $f,\tilde{f}\in\F$}\Co{i}

$$\sum_{k=0}^{n-1}Q_{x_k}\AP{f\AP{x_k}-\tilde{f}\AP{x_k}}^{\otimes 2}\leq1\implies Q_{x^*}\AP{f\AP{x^*}-\tilde{f}\AP{x^*}}^{\otimes 2}\leq1$$

\textit{Otherwise, $x^*$ is said to be}\Co{i} $\AP{\F,Q}$-independent of $\AC{x_k}$.

\textbf{Definition 2}\Co{b}

\textit{Consider a set $\X$, a real vector space $\Y$ and some $\F\subseteq\AC{\X\rightarrow\Y}$. The}\Co{i} Russo-Van Roy-Osband dimension (RVO dimension) $\RVO{\F}$ \textit{is the supremum of the set of $n\in\Nats$ for which there is $\AC{x_k\in\X}_{k\in[n]}$ and $Q$ s.t. for all $m\in[n]$, $x_m$ is $\AP{\F,Q}$-independent of $\AC{x_k\in\X}_{k\in[m]}$.}\Co{i}
...
\textit{Consider a set $\X$, a real vector space $\Y$, some $\F\subseteq\AC{\X\rightarrow\Y}$ and a family $\AC{Q_x\in\PSD(\Y)}_{x\in\X}$. A set $A\subseteq\F$ is said to be a}\Co{i} $Q$-covering of $\F$ \textit{when}\Co{i}

$$\forall f\in\F\exists\tilde{f}\in A: \sup_{x\in\X}{Q_x\AP{f(x)-\tilde{f}(x)}^{\otimes 2}}<1$$

\textbf{Definition 4}\Co{b}

\textit{Consider a set $\X$, a real vector space $\Y$, some $\F\subseteq\AC{\X\rightarrow\Y}$ and a family $\AC{Q_x\in\PSD(\Y)}_{x\in\X}$. The}\Co{i} covering number $\N(\F,Q)$ \textit{is the infimum of the set of $n\in\Nats$ for which there is a $Q$-covering of $\F$ of size $n$.}\Co{i}

\textbf{Definition 5}\Co{b}

\textit{Consider a finite set $\X$, a finite-dimensional real vector product space $\Y$ and some $\F\subseteq\AC{\X\rightarrow\Y}$. Fix any $\AC{Q\in\PD(\Y)}_{x\in\X}$.  The}\Co{i} Minkowski–Bouligand dimension (MB dimension) of $\F$ \textit{is defined by}\Co{i}

$$\MB{\F}:=\limsup_{\epsilon \rightarrow 0}{\frac{\ln{\N\AP{\F,\epsilon^{-2} Q}}}{\ln\frac{1}{\epsilon}}}$$

It is easy to see the above is indeed well-defined, i.e. doesn't depend on the choice of $Q$. This is because given any $Q,\tilde{Q}$, there are constants $c_1,c_2\in\Reals^+$ s.t. for all $\F$ and $\epsilon$

$$c_1 \N\AP{\F,\epsilon^{-2} Q} \leq \N\AP{\F,\epsilon^{-2} \tilde{Q}} \leq c_2\N\AP{\F,\epsilon^{-2} Q}$$

For finite $\F$ and $\epsilon\ll1$, it's obvious that $\N\AP{\F,\epsilon^{-2}Q}=\Abs{\F}$, and in particular $\MB\F=0$. It is also possible to show that, for any $\F$, $\MB{\F}\leq\Abs{\X}\dim{\Y}$.
...
\textit{Consider a real finite-dimensional normed vector space $V$ and a linear subspace $W\subseteq V$. Then, there exists a $Q\in\PSD(V)$ s.t.}\Co{i}

1. \textit{For any $v\in V$, $Qv^{\otimes 2}\leq\Norm{v}^2$}\Co{i}

2. \textit{For any $w\in W$, $\AP{\dim{W}}^2 Qw^{\otimes 2}\geq \Norm{w}^2$}\Co{i}
...
\textit{Consider a finite-dimensional real vector space $\Y$, some $Q\in\PD(\Y)$ and a Borel probability measure $\mu\in\Delta\Y$ s.t. $\Pa{y\sim\mu}{Qy^{\otimes 2} \leq 1} = 1$. Let $y_0:=\Ea{y\sim\mu}{y}$ and $\sigma:=\sqrt{e}$. Then, $\mu$ is $\sigma$-sub-Gaussian w.r.t. $Q$, i.e., for any $\alpha\in\Y^*$}\Co{i}

$$\Ea{y\sim\mu}{\exp\AP{\alpha\AP{y-y_0}}} \leq \exp\AP{\frac{\sigma^2Q^{-1}\alpha^{\otimes 2}}{2}}$$
...
\textit{Consider a set $\X$, a finite-dimensional real vector space $\Y$, some $\F\subseteq\AC{\X\rightarrow\Y}$ and a family $\AC{Q_x\in\PSD(\Y)}_{x\in\X}$. Assume $\F$ is compact w.r.t. the product topology on $\X\rightarrow\Y\cong\prod_{x\in\X}\Y$. Consider also some $n\in\Nats$, $\bold{x}\in\X^n$, $\bold{y}\in\Y^n$ and $r\in\Reals^+$. We then use the notation}\Co{i}

$$\LS^{\F}_Q[\bold{xy}]:=\Argmin{f\in\F}{\sum_{m=0}^{n-1}Q_{\bold{x}_m}\AP{f\AP{\bold{x}_m}-\bold{y} }^{\otimes2}}$$

$$\CS^{\F}_{Q}[\bold{xy}]:=\ACM{f\in\F}{\sum_{m=0}^{n-1}Q_{\bold{x}_m}\AP{f\AP{\bold{x}_m}-\LS_Q^\F[\bold{xy}]\AP{\bold{x}_m}}^{\otimes2}\leq 1}$$
...
\textit{Consider finite sets $\X,\St$, some $\F\subseteq\AC{\X\K\St}$ and a family $\AC{Q_x\in\PSD\AP{\Reals^\St}}_{x\in\X}$. Let $\AC{\mathfrak{H}_n\subseteq\PS{\X^\omega\times\St^\omega}}_{n\in\Nats}$ be the natural filtration, i.e.}\Co{i}
...
$$\beta(t):=C_{\mathrm{A.N5}}\AP{\ln{\frac{\N(\F,\epsilon^{-2}Q)}{\delta}}+\epsilon t\ln{\frac{et}{\delta}}}$$

$$Q_n:=\frac{Q}{\beta(n+1)}$$

\textit{Then,}\Co{i}

$$\Pa{\bold{xs}\sim\mu}{f^*\not\in\bigcap_{n=0}^\infty\CS^\F_{Q_n}\AB{\bold{xs}_{:n}}} \leq \delta$$
...
\textit{Consider a set $\X$, some $x\in\X$, a real vector space $\Y$, some $\F\subseteq\AC{\X\rightarrow\Y}$ and a family $\AC{Q_x\in\PSD(\Y)}_{x\in\X}$. The}\Co{i} $Q$-width of $\F$ at $x$ \textit{is defined by}\Co{i}

$$\W^\F_Q(x):=\sup_{f,\tilde{f}\in\F}\sqrt{Q_x\AP{f(x)-\tilde{f}(x)}^{\otimes2}}$$  
