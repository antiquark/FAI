...
$$\LS^\F[\bold{x},\bold{y}]:=\Argmin{f\in\F}{\sum_{m=0}^{n-1}\Norm{f\AP{\bold{x}_m}-\bold{y}_m}^2}$$

$$\CS^\F_{r}[\bold{x},\bold{y}]:=\ACM{f\in\F}{\sum_{m=0}^{n-1}\Norm{f\AP{\bold{x}_m}-\LS^\F[\bold{x},\bold{y}]\AP{\bold{x}_m}}^2\leq r^2}$$
...
\textit{Consider a set $\X$, a real inner product space $\Y$ and some $\F\subseteq\AC{\X\rightarrow\Y}$. Consider also some $\bold{x}\in\X^\omega$, $\bold{y}\in\Y^\omega$, $T\in\Nats^+$, $\gamma\in(0,1)$, $\theta\in\Reals^+$, and $\eta_0,\eta_1,\eta_2\in\Reals^+$. For any $t\in\Reals^+$, define $\beta(t)$ by}\Co{i}

$$\beta(t):=\eta_0 + \eta_1t+\eta_2t\ln{t}$$

For any $n\in\Nats$, define $\F_n$ by

$$\F_n:=\CS^\F_{\sqrt{\beta(n+1)}}[\bold{x}_{:n},\bold{y}_{:n}]$$
...
\textit{Consider a set $\X$, a real inner product space $\Y$ and some $\F\subseteq\AC{\X\rightarrow\Y}$. Assume that for any $x\in\X$ and $f\in\F$, $\Norm{f(x)}\leq 1$. Consider also some $\bold{x}\in\X^\omega$, $\bold{y}\in\Y^\omega$, $T\in\Nats^+$, $\gamma\in(0,1)$, and $\eta_0,\eta_1,\eta_2\in\Reals^+$. Define $\beta$ and $\F_n$ the same way as in Proposition A.N2. Then,}\Co{i}

$$\sum_{l=0}^\infty\sum_{m=0}^{T-1}{\gamma^{lT+m}\W^{F_{lT}}\AP{\bold{x}_{lT+m}}} \leq C_{\mathrm{A.N3}}\AP{1+\RVO{\F}\cdot T\ln\frac{1}{1-\gamma}+\sqrt{\RVO{\F}\cdot\beta\AP{\frac{1}{1-\gamma}}\frac{1}{1-\gamma}\ln{\frac{1}{1-\gamma}}}}$$
