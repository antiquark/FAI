...
??? By the mean while theorem, for each $\T\in\Hy$, $s\in\St$ and $\gamma\in(0,1)$ we can choose some $\Gamma_{\T}(s,\gamma)\in(\gamma,1)$ s.t.

$$\V_{\T\R}(s,\gamma)=\V^0_{\T\R}\AP{s}-(1-\gamma)\frac{\D\V_{\T\R}(s,x)}{\D x}\Big\vert_{x=\Gamma_{\T}(s,\gamma)}$$

Moreover, we have

$$\frac{\D\V_{\T\R}(s,x)}{\D x}\Big\vert_{x=\Gamma_{\T}(s,\gamma)}=\V^1_{\T\R}(s)\pm O(1-\gamma)$$

...

$$\Reg(\gamma)=(1-\gamma)\sum_{n=0}^\infty\gamma^{n}\Ea{}{\Ea{s\sim \SHy_n\AP{\Theta_n,\AT_n}}{\V_{\SHy_n\R}(s,\gamma)}-\Ea{s\sim \THy\AP{\Theta_n,\AT_n}}{\V_{\SHy_n\R}(s,\gamma)}}$$
