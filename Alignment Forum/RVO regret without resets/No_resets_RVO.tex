...
\textit{Consider a set $\X$, a real vector space $\Y$, some $\F\subseteq\AC{\X\rightarrow\Y}$ and a positive semidefinite bilinear form $Q:\Y\times\Y\rightarrow\Reals$. Consider also $n\in\Nats$, a sequence $\AC{x_k\in\X}_{k\in[n]}$ and $x^*\in\X$. $x^*$ is said to be} $\AP{\F,Q}$-dependant on $\AC{x_k}$ \textit{when, for any $f,\tilde{f}\in\F$}\Co{i}

$$\sum_{k=0}^{n-1}Q\AP{f\AP{x_k}-\tilde{f}\AP{x_k},\ f\AP{x_k}-\tilde{f}\AP{x_k}}\leq1\implies Q\AP{f\AP{x^*}-\tilde{f}\AP{x^*},\ f\AP{x^*}-\tilde{f}\AP{x^*}}\leq1$$
...
\textit{Consider a set $\X$, a real vector space $\Y$ and some $\F\subseteq\AC{\X\rightarrow\Y}$ non-empty. Then}\Co{i}
...
\textit{Consider a set $\X$, a real vector space $\Y$, some $\F\subseteq\AC{\X\rightarrow\Y}$ and a positive semidefinite bilinear form $Q:\Y\times\Y\rightarrow\Reals$. A set $A\subseteq\F$ is said to be a}\Co{i} $Q$-covering of $\F$ \textit{when}\Co{i}

$$\forall f\in\F\exists\tilde{f}\in A: \sup_{x\in\X}{Q\AP{f(x)-\tilde{f}(x),\ f(x)-\tilde{f}(x)}}<1$$

\textbf{Definition 4}\Co{b}

\textit{Consider a set $\X$, a real vector space $\Y$, some $\F\subseteq\AC{\X\rightarrow\Y}$ and a positive semidefinite bilinear form $Q:\Y\times\Y\rightarrow\Reals$. The}\Co{i} covering number $\N(\F,Q)$ \textit{is the infimum of the set of $n\in\Nats$ for which there is a $Q$-covering of $\F$ of size $n$.}\Co{i}

\textbf{Definition 5}\Co{b}

\textit{Consider a set $\X$, a finite-dimensional real vector product space $\Y$ and some $\F\subseteq\AC{\X\rightarrow\Y}$. Let $Q:\Y\times\Y\rightarrow\Reals$ be any positive}\Co{i} definite \textit{bilinear form.  The}\Co{i} Minkowski–Bouligand dimension (MB dimension) of $\F$ \textit{is defined by}\Co{i}

$$\MB{\F}:=\limsup_{\epsilon \rightarrow 0}{\frac{\ln{\N\AP{\F,\epsilon^{-2} Q}}}{\ln\frac{1}{\epsilon}}}$$

It is easy to see the above is indeed well-defined, i.e. doesn't depend on the choice of $Q$. This is because given any $Q,\tilde{Q}$ positive definite, there are constants $c_1,c_2\in\Reals^+$ s.t. for all $\F$ and $\epsilon$
...
\textit{Consider a finite-dimensional real vector space $\Y$, a positive definite bilinear form $Q:\Y\times\Y\rightarrow\Reals$ and a Borel probability measure $\mu\in\Delta\Y$ s.t. $\Pa{y\sim\mu}{Q(y,y) \leq 1} = 1$. Let $y_0:=\Ea{y\sim\mu}{y}$ and $\sigma:=\sqrt{e}$. Then, $\mu$ is $\sigma$-sub-Gaussian w.r.t. $Q$, i.e., for any $\lambda\in\Y^*$}\Co{i}
...
\textit{Consider a set $\X$, a finite-dimensional real vector space $\Y$ some $\F\subseteq\AC{\X\rightarrow\Y}$ and a positive semidefinite bilinear form $Q:\Y\times\Y\rightarrow\Reals$. Assume $\F$ is compact w.r.t. the product topology on $\X\rightarrow\Y\cong\prod_{x\in\X}\Y$, where $\Y$ is equipped with the topology induced by the inner product. Consider also some $n\in\Nats$, $\bold{x}\in\X^n$, $\bold{y}\in\Y^n$ and $r\in\Reals^+$. We then use the notation}\Co{i}
