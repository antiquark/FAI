...
$$\Reg(\gamma)\leq(1-\gamma)\sum_{n=0}^\infty\gamma^{n}\Ea{}{\AP{\max_{s\in\St}{\partial_{\max}\V_{\SHy_n\R}(s,\gamma)}-\min_{s\in\St}{\partial_{\min}\V_{\SHy_n\R}(s,\gamma)}}\Dtva{\SHy_n\AP{\Theta_n,\AT_n},\THy\AP{\Theta_n,\AT_n}} }$$

Using the dominated convergence theorem, we conclude that there is some function $\delta\tau:[0,1]\rightarrow\Reals^+$ s.t. $\lim_{\gamma\rightarrow1}\delta\tau(\gamma)=0$ and

$$\Reg(\gamma)\leq(1-\gamma)\AP{\tau_{\zeta\R}+\delta\tau(\gamma)}\sum_{n=0}^\infty\gamma^{n}\Ea{}{\Dtva{\SHy_n\AP{\Theta_n,\AT_n},\THy\AP{\Theta_n,\AT_n}} }$$
