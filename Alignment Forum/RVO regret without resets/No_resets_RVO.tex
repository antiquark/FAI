...
$$\limsup_{\gamma \rightarrow 1}\frac{\Ea{\T\sim\zeta}{\EU^*_{\T\R}(\gamma)-\EU^{\pi^\dagger_\gamma}_{\T\R}(\gamma)}}{\tau_{\zeta\R}\DL\ln{\frac{1}{1-\gamma}}\sqrt{\AP{\DMB+\epsilon}\AP{\DRVO+\epsilon}(1-\gamma)}}\leq C$$
...
$$\Ea{\T\sim\zeta}{\EU^*_{\T\R}(\gamma)-\EU^{\pi_{\zeta\R T}^{\PSR}}_{\T\R}(\gamma)}=\sum_{n=0}^\infty\gamma^{n+1}\Ea{}{?-?
}$$

??? This seems wrong. We should apply both Bellman operators to the same value function, so it's not exactly $\Q$ ???
