...
$$\Reg(\gamma)\leq\sum_{n=0}^\infty\gamma^{n}\Ea{}{\AP{\max_{s\in\St}{\V_{\SHy_n\R}(s,\gamma)}-\min_{s\in\St}{\V_{\SHy_n\R}(s,\gamma)}}\Dtva{\SHy_n\AP{\Theta_n,\AT_n},\THy\AP{\Theta_n,\AT_n}} }$$
...
$$\De \V_{\T\R}(\gamma):=\max_{s\in\St}{\partial_{\max}\V_{\T\R}(s,\gamma)}-\min_{s\in\St}{\partial_{\min}\V_{\T\R}(s,\gamma)}$$

By the no-traps condition, $\V^0_{\T\R}(s)$ doesn't depend on $s$ and we get

$$\Reg(\gamma)\leq(1-\gamma)\sum_{n=0}^\infty\gamma^{n}\Ea{}{\De\V_{\SHy_n\R}(\gamma)\Dtva{\SHy_n\AP{\Theta_n,\AT_n},\THy\AP{\Theta_n,\AT_n}} }$$
...
$$\Reg(\gamma)\leq\frac{1}{2}\DL(1-\gamma)\sum_{n=0}^\infty\gamma^{n}\Ea{}{\De\V_{\SHy_n\R}(\gamma)\sqrt{B_{\Theta_n\AT_n}\AP{\SHy_n\AP{\Theta_n,\AT_n}-\THy\AP{\Theta_n,\AT_n}}^{\otimes2}} }$$

We now apply Proposition A.N5 with $\delta:=1-\gamma$ and $\epsilon:=(1-\gamma)^2$. We get

$$\Pa{}{\SHy_*\in\bigcap_{n=0}^\infty\CS^\Hy\AB{\Theta\AT_{:n}\Theta_n,\frac{\B }{\beta(n+1)}}} \geq \gamma$$
...
$$\Pa{}{\SHy_{lT}\in\CS^\Hy\AB{\Theta\AT_{:lT}\Theta_{lT},\frac{\B }{\beta(lT+1)}}} \geq \gamma$$

$$\Pa{}{\SHy_*,\SHy_{lT}\in\CS^\Hy\AB{\Theta\AT_{:lT}\Theta_{lT},\frac{\B }{\beta(lT+1)}}} \geq 2\gamma-1$$
...
$$\Pa{}{\sqrt{B_{\Theta_n\AT_n}\AP{\SHy_n\AP{\Theta_n,\AT_n}-\THy\AP{\Theta_n,\AT_n}}^{\otimes2}}\leq\W^{\Hy_{\Floor{n/T}}}\AP{\Theta_n\AT_n,B}}\geq2\gamma-1$$

% It seems easy to proceed if we have a uniform bound of the bias span. But using only the expected value?
% What if we use Cauchy-Shwartz? This will give us a term proportional to the 2nd moment times 1-\gamma
