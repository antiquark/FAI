...
$$\Reg(\gamma)\leq\sum_{n=0}^\infty\gamma^{n}\Delta\V(\gamma)\Ea{}{\Dtva{\SHy_n\AP{\Theta_n,\AT_n},\THy\AP{\Theta_n,\AT_n}} }$$
...
$$\Reg(\gamma)\leq\sum_{n=0}^\infty\gamma^{n}\Delta\V(\gamma)\AP{(1-\gamma)^2+\Ea{}{\Dtva{\SHy_n\AP{\Theta_n,\AT_n},\THy\AP{\Theta_n,\AT_n}};\CSE_n }}$$

$$\Reg(\gamma)\leq1-\gamma+\sum_{n=0}^\infty\gamma^{n}\Delta\V(\gamma)\Ea{}{\Dtva{\SHy_n\AP{\Theta_n,\AT_n},\THy\AP{\Theta_n,\AT_n}};\CSE_n }???$$

Using property 2 of B

$$\Reg(\gamma)\leq1-\gamma+\frac{1}{2}\DL\sum_{n=0}^\infty\gamma^{n}\Delta\V(\gamma)\Ea{}{\sqrt{B_{\Theta_n\AT_n}\AP{\SHy_n\AP{\Theta_n,\AT_n}-\THy\AP{\Theta_n,\AT_n}}^{\otimes2}};\CSE_n }???$$
...
$$\Reg(\gamma)\leq1-\gamma+\frac{1}{2}\DL\sum_{n=0}^\infty\gamma^{n}\Delta\V(\gamma)\Ea{}{\W^{\Hy_{\Floor{n/T}}}\AP{\Theta_n\AT_n,B} }???$$

% How do we separate \Delta\V and \W?
% Not clear how it works in Osband-Van Roy
% One option is assume a uniform bound on the bias span rather than just the expected value
% Does supremum of \T commute with \gamma->1?
