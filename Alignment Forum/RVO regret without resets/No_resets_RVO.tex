...
$$\forall v\in\Y,\alpha\in\Y^*:\AP{\forall\beta\in\Y^*:Q^{-1}(\alpha,\beta)=\beta v}\implies\forall w\in\Y:Q(v,w)=\alpha w$$ 
...
$$\sum_{k=0}^{n-1}Q_{x_k}\AP{f\AP{x_k}-\tilde{f}\AP{x_k},\ f\AP{x_k}-\tilde{f}\AP{x_k}}\leq1\implies Q_{x^*}\AP{f\AP{x^*}-\tilde{f}\AP{x^*},\ f\AP{x^*}-\tilde{f}\AP{x^*}}\leq1$$
...
$$\forall f\in\F\exists\tilde{f}\in A: \sup_{x\in\X}{Q_x\AP{f(x)-\tilde{f}(x),\ f(x)-\tilde{f}(x)}}<1$$
...
\textit{Consider a finite-dimensional real vector space $\Y$, some $Q\in\PD(\Y)$ and a Borel probability measure $\mu\in\Delta\Y$ s.t. $\Pa{y\sim\mu}{Q(y,y) \leq 1} = 1$. Let $y_0:=\Ea{y\sim\mu}{y}$ and $\sigma:=\sqrt{e}$. Then, $\mu$ is $\sigma$-sub-Gaussian w.r.t. $Q$, i.e., for any $\lambda\in\Y^*$}\Co{i}

$$\Ea{y\sim\mu}{\exp\AP{\lambda\AP{y-y_0}}} \leq \exp\AP{\frac{\sigma^2Q^{-1}(\lambda,\lambda)}{2}}$$
...
$$\LS^{\F}_Q[\bold{xy}]:=\Argmin{f\in\F}{\sum_{m=0}^{n-1}Q_{\bold{x}_m}\AP{f\AP{\bold{x}_m}-\bold{y},\ f\AP{\bold{x}_m}-\bold{y}}}$$

$$\CS^{\F}_{Q}[\bold{xy}]:=\ACM{f\in\F}{\sum_{m=0}^{n-1}Q_{\bold{x}_m}\AP{f\AP{\bold{x}_m}-\LS_Q^\F[\bold{xy}]\AP{\bold{x}_m},\ f\AP{\bold{x}_m}-\LS_Q^\F[\bold{xy}]\AP{\bold{x}_m}}\leq 1}$$
