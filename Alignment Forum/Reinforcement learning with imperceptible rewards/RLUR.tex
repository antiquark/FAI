...
\textit{Consider $(\Omega,P)$ a probability space, $T\in\Nats^+$ and $\AC{N_k,\Delta_k:\Omega\rightarrow\Nats}_{k\in\Nats}$ random variables s.t. $N_{k+1}-N_k\geq\Delta_k$ and the $\Delta_k$ are all independent and uniformly distributed between $1$ to $2T-1$. Consider also $\gamma\in(0,1)$ and assume that $\gamma^T\geq\frac{1}{2}$. Then,}\Co{i}

$$\sum_{k=0}^\infty\Ea{}{\gamma^{N_k}}\leq\frac{2}{(1-\gamma)T}$$
