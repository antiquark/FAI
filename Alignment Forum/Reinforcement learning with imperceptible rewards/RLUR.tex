...
\begin{Large}Instrumental States and Imperceptible Reward Functions\end{Large}
...
\textit{Given $\pi: \FH\K\A\sqcup\{\bot\}$ and $\mu\in\IS_\omega$, we define $\mu\pi\in\Delta\AP{\A\times\Ob}^{\leq\omega}$ by}\Co{i}
...
\begin{Large}A Regret Bound for RL with Imperceptible Rewards\end{Large}
...
Suppose that $\Ob=\St^2$. Then, given any MPD transition kernel $\T:\St\times\A\K\St$, we can define the POMDP transition kernel $\T^\sharp:\St\times\A\K\St\times\Ob$ by

$$\T^\sharp\APM{s',\AP{s_1,s_2}}{s,a}:=\boldsymbol{1}_{s_1=s}\boldsymbol{1}_{s_2=s'}\T\APM{s'}{s,a}$$

Fix $\R:\IS_\omega\rightarrow[0,1]$ continuous affine. For any $\T$ as above, we get the induced reward function $\R_\T:\St\rightarrow[0,1]$ given by $\R_\T:=\R\circ\In_{\T^\sharp}$. We can now consider the learning problem corresponding to a hypothesis class of MDP transition kernels
...
We can formulate the regret bound.

\textbf{Theorem 2}\Co{b}

\textit{There is some $C\in\Reals^+$ s.t. the following holds.}\Co{i}

\textit{Consider any finite non-empty sets $\St$ and $\A$, continuous affine function $\R:\IS_\omega\rightarrow[0,1]$, closed set $\Hy\subseteq\AC{\St\times\A\K\St}$ and Borel probability measure $\zeta$ on $\Hy$ (prior). Suppose $\pi_{\mathrm{est}}$ is a terminable policy and $r:\FH\rightarrow[0,1]$ is a bounded function s.t. $\R=\R_{\pi_{\mathrm{est}}r}$. Define the maximal expected reward estimation time $t_{\mathrm{est}}$ by}\Co{i}

$$t_{\mathrm{est}}:=\sup_{\mu\in\IS_\omega}\Ea{h\sim\mu\pi_{\mathrm{est}}}{\Abs{h}}$$
...
$$\limsup_{\gamma \rightarrow 1}\frac{\Reg(\gamma)}{\sqrt[3]{t_{\mathrm{est}}??\tau\AP{\DMB+1}\DRVO(1-\gamma)\ln{\frac{1}{1-\gamma}}}}\leq C$$

% Explain DRVO and DMB

\begin{Huge}Discussion\end{Huge}

TBD
% Bound is probably suboptimal for MDPs
% Kamikaze...
