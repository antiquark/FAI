...
$$\Abs{r\AP{x}-r'\AP{f(x)}} \leq \epsilon$$

\textbf{Proof of Proposition A.8}\Co{b}

Let $V$ be the vector space corresponding to $D$ (so that $D\subseteq V$) and $Y$ the (finite) set of vertices of $D$. Choose any $g:Y\rightarrow C$ s.t. for any $y\in Y$, $f\AP{g(y)}=y$. Define $v\in\Reals^Y$ by $v_y:=r\AP{g(y)}$. Let $\Reals^Y_0$ be the linear subspace of $\Reals^Y$ given by 

$$\Reals^Y_0:=\ACM{u\in\Reals^Y}{\sum_{y\in Y}u_y = 0}$$

Define the linear operator $A:\Reals^Y\rightarrow V$ by

$$Au:=\sum_{y\in Y}u_y y$$

Consider any $w\in\Ker{A}\cap\Reals^Y_0\setminus\boldsymbol{0}$. In particular, $w\in\Reals^Y_0\setminus\boldsymbol{0}$ and therefore

$$\sum_{y\in Y}\bold{1}_{w_y>0}w_y =-\sum_{y\in Y}\bold{1}_{w_y<0}w_y=\frac{\Norm{w}_1}{2}$$

This allows us to define $w^+,w^-\in\Delta Y$ by

$$w^+_y:=2\frac{\bold{1}_{w_y>0}w_y}{\Norm{w}_1}$$

$$w^-_y:=-2\frac{\bold{1}_{w_y<0}w_y}{\Norm{w}_1}$$

We have $w=\frac{1}{2}\Norm{w}_1\AP{w^+-w^-}$. Also, $Aw=0$ and therefore $Aw^+=Aw^-$...
