...
$$\theta(ha)=\Prj_n(\theta)(ha)$$

\textbf{Proof of Proposition A.2}\Co{b}

We prove the claim by induction on $n$. For $n=0$, there is nothing to prove. For $n>0$...

\textbf{Proof of Proposition 3}\Co{b}

??? It's easy to see that for any $n\in\Nats$, $\AP{\Prj^n\mu}\pi=\mu\pi_{:n}$. Therefore, $\mu\pi=\mu'\pi$ implies that $\big(\Prj^n\mu\big)\pi=\AP{\Prj^n\mu'}\pi$. By Proposition 1, we get $\Prj^n\mu=\Prj^n\mu'$, and therefore $\mu=\mu'$. $\blacksquare$
