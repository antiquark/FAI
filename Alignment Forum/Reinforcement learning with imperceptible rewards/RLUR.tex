...
Given a terminable policy $\pi$ and a function $r:\AP{\A\times\Ob}^*\rightarrow[0,1]$, we can define a reward function $\R:\IS_\omega\rightarrow[0,1]$ by

$$\R(\mu):=\Ea{h\sim\mu\pi}{r(h)}$$

This gives us a second definition of imperceptible reward functions. Now, it is easy to see that $\R$ defined as above is continuous and affine. Therefore, any reward function of the second type is also a reward function of the first type. The converse, however, requires more work:

\textbf{Theorem 1}\Co{b}

\textit{For any continuous affine $\R:\IS_\omega\rightarrow[0,1]$, there exists a terminable policy $\pi$ and $r:\AP{\A\times\Ob}^*\rightarrow[0,1]$ s.t.}\Co{i}
