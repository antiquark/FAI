...
Let $\Psi$ be a measurable space, $(\Omega,P)$ be some probability space, $\AC{X_n:\Omega\rightarrow\Psi}_{n\in\Nats}$, $\AC{M_k:\Omega\rightarrow\Nats}_{k\in\Nats}$ be random variables and $\AC{f_n:\Psi^{n+1}\rightarrow[0,1]}_{n\in\Nats}$ be measurable functions. Consider some $\gamma\in(0,1)$ and $C\in\Reals^+$ and assume that with probability 1
