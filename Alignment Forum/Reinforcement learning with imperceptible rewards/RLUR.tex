...
\textit{Let $C,D\in\Con$, $f\in\Mor(C,D)$ and $r\in\Mor\AP{C,[0,1]}$. Assume $D$ is a bounded closed finite-dimensional polytope, and $f$ is onto (as a mapping). Suppose $\epsilon\in\Reals^+$ s.t. for any $x_{1,2}\in C$, if $f\AP{x_1}=f\AP{x_2}$ then $\Abs{r\AP{x_1}-r\AP{x_2}} \leq \epsilon$. Then, there exists $r'\in\Mor\AP{D,[0,1]}$ s.t. for any $x\in C$}\Co{i} 

$$\Abs{r\AP{x}-r'\AP{f(x)}} \leq \epsilon$$

\textbf{Proof of Proposition A.8}\Co{b}

Let $V$ be the vector space corresponding to $D$ (so that $D\subseteq V$) and $Y$ the (finite) set of vertices of $D$. Let $\Reals^Y_0$ be the linear subspace of $\Reals^Y$ given by 

$$\Reals^Y_0:=\ACM{u\in\Reals^Y}{\sum_{y\in Y}u_y = 0}$$

Define the linear operator $A:\Reals_0^Y\rightarrow V$ by

$$Au:=\sum_{y\in Y}u_y y$$

For any $y\in Y$, choose some $x\in f^{-1}(y)$...
