...
$$\sum_{k=0}^\infty\sum_{n=N_k}^{N_{k+1}-1}{\gamma^{kT}\boldsymbol{1}_{\W^{F^k}\AP{\boldsymbol{x}_{n}}>\theta}} \leq C_{\mathrm{A.20}}(D+1)\AP{\frac{1}{\theta^2}\beta\AP{\frac{1}{1-\gamma}}+T}$$
...
$$\sum_{k=0}^{\infty}\gamma^{kT}\sum_{n=N_k}^{N_{k+1}-1}{\boldsymbol{1}_{\W^{\F^k}\AP{\boldsymbol{x}_{n}}>\theta}} \leq C_{\text{A.20}} (D+1)\AP{\frac{1}{\theta^2}\beta\AP{\frac{1}{1-\gamma}}+T}\ \blacksquare$$

\textbf{Proposition A.21}\Co{b}

\textit{In the setting of Proposition A.20, assume that for any $x\in\X$ and $f\in\F$, $\Norm{f(x)}\leq1$. Then,}\Co{i}

$$\sum_{k=0}^{\infty}\sum_{n=N_k}^{N_{k+1}-1}\gamma^{kT}\W^{\F^k}\AP{\boldsymbol{x}_{n}} \leq C_{\text{A.21}}...$$
