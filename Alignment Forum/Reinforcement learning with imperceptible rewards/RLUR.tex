...
$$\lim_{n\rightarrow\infty}\sup_{\mu\in\IS_\omega}\Pa{h\sim\mu\pi}{\Abs{h}>n}=0$$
...
$\IS_\omega$ is compact, therefore $\AC{\mu_k}$ has a convergent subsequence. Without loss of generality, we can assume that $\AC{\mu_k}$ itself converges and denote $\mu_*:=\lim_{k\rightarrow\infty}\mu_k$. For any $k\in\Nats$ and $j > k$, we have $n_k < n_j$ and therefore
...
$$\Pa{h\sim\mu_*\pi}{\Abs{h}=\infty}=\lim_{k\rightarrow\infty}\Pa{h\sim\mu_*\pi}{\Abs{h}>n_k} \geq \epsilon$$

This contradicts the assumption that $\pi$ is terminable. $\blacksquare$

\textbf{Proof of Theorem 1}\Co{b}

\textit{Direction 1:}\Co{i} $\R_{\pi r}$ is affine by Proposition 4. To verify it is continuous, consider a sequence $\AC{\mu_k\in\IS_\omega}_{k\in\Nats}$ s.t. $\mu_*:=\lim_{k\rightarrow\infty}\mu_k$ exists. Denote $\Delta r:=\sup r - \inf r$, $\mu_k^n:=\Prj^n\AP{\mu_k}$ and $\mu^n_*:=\Prj^n\AP{\mu_*}$. For any $n\in\Nats$, we can decompose the expected value into the contribution of histories at length at most $n$ and the contribution of longer histories. This yields???
...
$$\lim_{k\rightarrow\infty}\Abs{\R_{\pi r}\AP{\mu_k}-\R_{\pi r}\AP{\mu_*}} \leq 2\Delta r\sup_{\mu\in\IS_\omega}\Pa{\mu\pi}{\Abs{h}>n}$$

Since this holds for any $n\in\Nats$, we get

$$\lim_{k\rightarrow\infty}\Abs{\R_{\pi r}\AP{\mu_k}-\R_{\pi r}\AP{\mu_*}} \leq 2\Delta r\inf_{n\in\Nats}\sup_{\mu\in\IS_\omega}\Pa{\mu\pi}{\Abs{h}>n}$$
