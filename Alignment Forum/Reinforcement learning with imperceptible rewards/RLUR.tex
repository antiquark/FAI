...
$$+\sum_{k=0}^\infty\Ea{}{\sum_{n=\IT_k}^{\ET_{k+1}-1}\gamma^n\Abs{\CE{\Ev_{**}^{n-\IT_k}-\Ev_{*k}^{n-\IT_k}}{\R_*}{\Theta_{\IT_k}}}}$$
...
$$+\Delta\V(\gamma)\sum_{k=0}^\infty\Ea{}{{\gamma^{\IT_k}}}+\sum_{k=0}^\infty\Ea{}{\sum_{n=\IT_k}^{\ET_{k+1}-1}\gamma^n}+\Delta\V(\gamma)\sum_{k=0}^\infty\Ea{}{\gamma^{\ET_{k+1}}}$$

% Show that \sum E[\gamma^M] and \sum E[\gamma^N] are O(1/(1-\gamma^T))
% Use the decomposition of M and N into a sum of independent delta-s, by dropping the interlude terms
% Show that \E E[\sum_M^N \gamma^n] is O(t_est/T)
...
We now apply Proposition ?? with $\delta:=\frac{1}{2}(1-\gamma)^2$ and $\epsilon:=(1-\gamma)^2$. We get

$$\Pa{}{\SHy_*\in\bigcap_{n=0}^\infty\CS^\Hy\AB{\Theta\AT_{:n}\Theta_n,\beta(n+1)^{-1}?}} \geq 1-\frac{1}{2}(1-\gamma)^2$$

Here, $\beta$ was defined in Proposition ??.

Since the hypothesis $\SHy_n$ is sampled from the posterior, for any $l\in\Nats$ we also have

$$\Pa{}{\SHy_{lT}\in\CS^\Hy\AB{\Theta\AT_{:lT}\Theta_{lT},\beta(lT+1)^{-1}?}} \geq 1-\frac{1}{2}(1-\gamma)^2$$

$$\Pa{}{\SHy_*,\SHy_{lT}\in\CS^\Hy\AB{\Theta\AT_{:lT}\Theta_{lT},\beta(lT+1)^{-1}?}} \geq 1-(1-\gamma)^2$$
