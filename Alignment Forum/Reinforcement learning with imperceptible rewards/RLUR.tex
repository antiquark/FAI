...
If this value vanishes then $\theta_{a_*o_*}=\theta'_{a_*o_*}=\boldsymbol{0}$. Otherwise, consider any $\sigma:\AP{\A\times\Ob}^{<n-1}\rightarrow\A$ and define $\pi:\AP{\A\times\Ob}^{<n}\rightarrow\A$ by
...
Given $n\in\Nats^+$ and some $\alpha,\beta\in\Delta\AP{\A\times\Ob}^n$, to show that $\alpha=\beta$ it is sufficient to show that

1. For any $a_*\in\A$ and $o_*\in\Ob$, 

$$\Pa{ao\sim\alpha}{ao_{:1}=a_*o_*}=\Pa{ao\sim\beta}{ao_{:1}=a_*o_*}$$

2. For any $a_*\in\A$ and $o_*\in\Ob$, if $\Pa{ao\sim\alpha}{ao_{:1}=a_*o_*}>0$ then for any $h\in\AP{\A\times\Ob}^{n-1}$
...
By Definition 6
$$\Pa{ao\sim\AP{p\theta+(1-p)\theta}\pi}{ao_{:1}=a_*o_*}=\pi\APM{a_*}{\Estr}\Ht_{\IS_{n-1}}\AP{\AP{p\theta+(1-p)\theta}_{??}}$$
