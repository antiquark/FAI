...
\textit{Consider $\alpha\in\Delta\Nats$ and sequences }\Co{i}

$$\AC{\pi_n:\FH\K\AX}_{n\in\Nats}$$ 

$$\bigg\{r_n:\FH\rightarrow[0,1]\bigg\}_{n\in\Nats}$$

\textit{Assume $\pi_n$ is a terminable policy for every $n\in\Nats$. Then, there is $\pi$ a terminable policy and $r:\FH\rightarrow[0,1]$ s.t.}\Co{i}

$$\R_{\pi r}=\Ea{n\sim\alpha}{\R_{\pi_n r_n}}$$
...
$$\sup_{\mu\in\IS_\omega}\Abs{\R(\mu)-\tilde{\R}_n\AP{\Prj^n(\mu)}}\leq\frac{3}{2}\epsilon_n$$

It follows that there is a sequence $\AC{n_k\in\Nats}_{k\in\Nats}$ s.t. $n_{k+1}>n_k$ and

$$\sum_{k=0}^\infty \sup_{\mu\in\IS_\omega}\Abs{\tilde{\R}_{n_{k+1}}\AP{\Prj^{n_{k+1}}\AP{\mu}}-\tilde{\R}_{n_k}\AP{\Prj^{n_k}(\mu)}}<1$$

Define $\Delta\R_k\in\Mor\AP{\IS_{n_{k+1}},[0,1]}$ by

$$\Delta\R_k(\theta):=\tilde{\R}_{n_{k+1}}\big(\theta\big)-\tilde{\R}_{n_k}\AP{\Prj^{n_k}(\theta)}$$

By Proposition A.10, for every $k\in\Nats$, there is $\pi_k:\AO^{<n_{k+1}}\K\A$ and $r:\AO^{n_{k+1}}\rightarrow[0,1]$ s.t.

$$\Delta\R_k(\theta)=\Ea{\theta\pi_k}{r_k}$$

Denote $\delta_k:=\sup\Delta\R_k$. Define $\pi$ and $r$ by...
