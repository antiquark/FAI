...
\textit{Let $V$ be a finite-dimensional normed vector space and $W\subseteq V^*$ a linear subspace. Suppose $v\in V$ and $\epsilon\in\Reals^+$ are s.t. for any $\alpha\in W$, $\Abs{\alpha(v)} \leq \epsilon\Norm{\alpha}$ (we use the standard dual norm on $V^*$ here). Then, there is $v'\in W^\bot\subseteq V$ s.t. $\Norm{v-v'}\leq\epsilon$.}\Co{i}
...
TBD % descend of an affine function onto a polytope
