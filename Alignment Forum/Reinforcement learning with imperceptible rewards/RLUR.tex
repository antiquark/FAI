...
$$\Ea{o\sim\theta(a)}{j_o\AB{\theta_{ao}}}=\sum_{o\in\Ob}\Ht\AP{\theta_{ao}}\bigoplus_{o'\in\Ob}\AP{\boldsymbol{1}_{o'=o}\theta_{ao},\boldsymbol{1}_{o'=o}}$$

$$\Ea{o\sim\theta(a)}{j_o\AB{\theta_{ao}}}=\bigoplus_{o'\in\Ob}\AP{\sum_{o\in\Ob}\Ht\AP{\theta_{ao}}\boldsymbol{1}_{o'=o}\theta_{ao},\sum_{o\in\Ob}\Ht\AP{\theta_{ao}}\boldsymbol{1}_{o'=o}}$$
...
1. Each episode has random duration, uniformly distributed from $1$ to $T\in\Nats^+$.
...
$$\V_{kl}(s):=\V_{\SHy_{*}\Pi_{\Delta\Phi_k-l}\AP{\SHy_{k},\SHy_*}\R_*}\AP{s,\gamma}$$
...
By definition, $\SHy_{k}$ and $\SHy_*$ have the same distribution even when conditioned by the history up to $\Phi_k$. Therefore

$$\Ea{}{\V_{*}\AP{\Theta_{\Phi_k}}}=\Ea{}{\V_{k}\AP{\Theta_{\Phi_k}}}$$

It follows that

$$\Reg_0(\gamma)=\sum_{k=0}^\infty{\Ea{}{\gamma^{\ET_k}\AP{\V_{k}\AP{\Theta_{\Phi_k}}-\V_{k0}\AP{\Theta_{\Phi_k}}}}}$$

Denote $\Theta_{kl}:=\Theta_{\Phi_k+l}$ and $\Delta\V_{kl}:=\V_k\AP{\Theta_{kl}}-\V_{kl}\AP{\Theta_{kl}}$. We now prove by induction on $l\in\Nats$ that, with probability 1
...
Denote $\Ev_{kk}:=\SHy_k\Pi\AP{\SHy_k}$. We will also use the notation $\TE{X}:=\CE{}{X}{\ET_k,\IT_k}$. Substituting $s=\Theta_{kl}$ and subtracting the two identities, we get that, in the subspace of $\Omega$ defined by $l<\Delta\Phi_k$

$$\TE{\Delta\V_{kl}}=\TE{(1-\gamma)\Delta\R\AP{\Theta_{kl}}+\gamma\AP{\CE{\Ev_{kk}}{\V_k}{\Theta_{kl}}-\CE{\Ev_{*k}}{\V_{k,l+1}}{\Theta_{kl}}}}$$

Denote $\AT_{kl}:=\AT_{\Phi_k+l}$. Since $\Pi\AP{\SHy_{k}}$ is exactly the policy followed by PSRL at time $\Phi_k+l$, we get
...
Applying this to each term in the earlier expression for $\Reg_0(\gamma)$, we get the desired result.

\textbf{Proposition A.14}\Co{b}

Let $\X$ be a measurable space, $(\Omega,P)$ be some probability space, $\AC{X_n:\Omega\rightarrow\X}_{n\in\Nats}$ be random variables and $\AC{f_n:\X^n\rightarrow[0,1]}_{n\in\Nats}$ be measurable functions. Consider some $\gamma\in(0,1)$ and $C\in\Reals^+$...
