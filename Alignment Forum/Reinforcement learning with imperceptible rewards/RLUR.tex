...
$$\CP{}{M_{k+1}=n}{M_k}=\begin{cases} T^{-1} \text{ if } M_k + 1 \leq n \leq M_k+T\\ 0 \text{ otherwise} \end{cases}$$
...
$$\Pa{}{K_n\geq\frac{4n}{T}}\leq\exp\AP{-\frac{n}{2T}}$$

\textbf{Proof of Proposition A.14}\Co{b}

By Hoeffding's inequality, for any $k,m\in\Nats$
...
\textit{Finally, assume that, conditional on $M_k>n$, $M_k$ and $X_{n}$ are independent. Define the $\Fi_n$-measurable random variable $K_n:\Omega\rightarrow\Nats$ by}\Co{i}

$$K_n:=\min\ACM{k\in\Nats}{M_k\geq n}$$

\textit{Then,}\Co{i}

$$\sum_{n=0}^\infty\gamma^{\frac{4n}{T}}\Ea{}{f_{K_n}\AP{X_{M_0},X_{M_1}\ldots X_{M_{K_n-1}},X_n}}\leq CT+\frac{1}{1-\exp\AP{-\frac{1}{2T}}}$$
