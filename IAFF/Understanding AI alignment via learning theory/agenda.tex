...
It is possible to generalize this setting further by replacing "crisp" sets of environments by fuzzy sets. That is, we can define a "fuzzy model" to be a function $\phi: E \rightarrow [0,1]$ s.t. $\phi^{-1}(1)$ is non-empty. The performance of a policy $\pi$ on the model $\phi$ is then given by
