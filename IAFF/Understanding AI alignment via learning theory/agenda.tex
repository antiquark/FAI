...
Of course one would also have to take care that the implementation of this idea is sound, for example the key should be generated from quantum random and kept at a location sufficently remote from the AI's hardware (with a linking communication channels for encrypted inputs/outputs). More troubling is the large performance overhead this would impose...
