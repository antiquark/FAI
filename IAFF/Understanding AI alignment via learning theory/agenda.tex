...
% Using incomplete models to justify Cartesian assumptions in DRL and so forth

%* So far, we assumed that there are no side effects on the environment from the act of delegation itself.\ That is, the same action has exactly the same results whether carried out by the advisor or by the agent. Obviously, this is not entirely realistic since any physical isolation layer created to ensure this will not be entirely fool-proof. The sole exception is, perhaps, if both the agent and the advisor are programs running inside a [homeomorphic cryptography box](https://www.lesswrong.com/posts/2Wf3R4NZ77CLczLL2/cryptographic-boxes-for-unfriendly-ai). Therefore, it is desirable to derive a regret bound that assumes small (in some sense) but non-vanishing violations of this assumption. Similarly, the algorithm we analyzed (a form of Posterior-Sampling Reinforcement Learning) assumes access to a source of random which is invisible to the environment. This invisibility assumption should also be made more lax (although it is also possible to use deterministic algorithms instead).
