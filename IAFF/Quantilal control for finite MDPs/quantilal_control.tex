...
Using elementary linear algebra, it is easy to see that the minimum of a linear function inside a domain defined by a finite system of linear inequalities is a piecewise rational function of the coefficients of the linear function and the linear inequalities, with a finite number of pieces (using the fact that the minimum is always attained on a linear submanifold defined by taking a subset of the inequalities and replacing them by corresponding equalities). By Proposition A.N3, $\QV$ is exactly of this form, with the coefficients...
