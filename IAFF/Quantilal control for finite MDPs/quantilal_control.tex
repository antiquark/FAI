...
It is known that the value of an optimal policy depends on $\lambda$ as a piecewise rational function, and in particular it converges as $\lambda \rightarrow 1$ and has a Taylor expansion at $\lambda=1$. For quantilal policies, we derive the following, somewhat weaker result. Recall that a function is called *semialgebraic* when its graph is a finite union of sets defined by systems of polynomial inequalities.

\#Proposition 4

$\QV$ is a semialgebraic function of $\lambda$, $\eta$, the matrix elements of $\T$, the values of $\R$ and the values of $\zeta_0$.

\#Corollary 1

$\QV$ converges as $\lambda \rightarrow 1$, holding all other parameters fixed. It has a *Puiseux series* expansion in $\lambda$ at $\lambda=1$.
...
Just to tease a little, here are some developments of this work that I'm planning:
