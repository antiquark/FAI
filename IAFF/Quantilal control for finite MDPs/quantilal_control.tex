...
Assume geometric time discount. Assume further that $\R(s)$, $\T\APM{s}{t,a}$, $\zeta_0(s)$, $\lambda$, $\Z{\sigma}(s)$ and $\eta$ are rational numbers. Then:
...
Define the linear operator $T: \Reals^\St \rightarrow \Reals^\St$ by the matrix 

$$T_{ts} := \Ea{a\sim\pi^\dagger(s)}{\T\APM{t}{s,a}}$$

Viewing $\Delta\St$ as a subset of $\Reals^\St$, we get

$$\Z\pi^* = (1-\lambda)\zeta_0 + \lambda T\Z\pi^*$$

$$\Z\pi^* = (1-\lambda)\AP{\boldsymbol{1}-\lambda T}^{-1}\zeta_0$$

On the other hand, we have

$$\Z\pi^\dagger = (1-\lambda)\sum_{n=0}^\infty \lambda^n T^n \zeta_0 = (1-\lambda)\AP{\boldsymbol{1}-\lambda T}^{-1}\zeta_0$$
