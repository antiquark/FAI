...
$$\sum_{n=0}^\infty \gamma^{n+1} \Pa{x \sim \mu^\RMD \bowtie \pi}{S^\star\AP{x_{:n}}\not\in\SF_M,\ S^\star\AP{x_{:n+1}}\in\SF_M} \geq {\gamma^{\bar{\tau}}}-\frac{3}{2}\AP{\EU_{\UD}^{\pi^* S^\star}(\gamma)-\EU_{\UD}^{\pi}(\gamma)}-\frac{1}{2}$$
...
For any $x \in \HD{\mu^\RMD}$, define $\tau(x):=\min\{n \in \Nats \mid S^\star\AP{x_{:n}} \in \SF_M\} \in \Nats \sqcup \{\infty\}$. We have

$$\Ut^{r^\star}_\gamma(x) \leq (1-\gamma)\AP{\frac{1}{3}\sum_{n=0}^{\tau(x)-1}{\gamma^n} + \sum_{n=\tau(x)}^\infty \gamma^n} = (1-\gamma)\AP{\frac{1}{3} \cdot \frac{1 - \gamma^{\tau(x)}}{1-\gamma}+\frac{\gamma^{\tau(x)}}{1-\gamma}}=\frac{1}{3}+\frac{2}{3}\gamma^{\tau(x)}$$

It follows that

$$\EU_{\UD}^\pi(\gamma) \leq \frac{1}{3}+\frac{2}{3}\Ea{x\sim\mu^\RMD\bowtie\pi}{\gamma^{\tau(x)}}$$

If $x \in \HD{\mu^\RMD}$ is s.t. for all $n\in\Nats$, $S^\star\AP{x_{:n}}\ne\bot$, then

$$\Ut^{r^\star}_\gamma(x) \geq (1-\gamma)\cdot \frac{2}{3} \sum_{n=\tau(x)}^\infty \gamma^n = \frac{2}{3}\gamma^{\tau(x)}$$

Since $\pi^*$ is a mitigation policy, it follows that

$$\EU_{\UD}^{\pi^*S^\star}(\gamma) \geq \frac{2}{3}\Ea{x\sim\mu^\RMD\bowtie\pi^*S^\star}{\gamma^{\tau(x)}} \geq \frac{2}{3}\gamma^{\bar{\tau}}$$

Subtracting the two inequalities, we get

$$\EU_{\UD}^{\pi^*S^\star}(\gamma) - \EU_{\UD}^\pi(\gamma) \geq \frac{2}{3}\gamma^{\bar{\tau}} - \frac{1}{3} - \frac{2}{3}\Ea{x\sim\mu^\RMD\bowtie\pi}{\gamma^{\tau(x)}}$$

$$\Ea{x\sim\mu^\RMD\bowtie\pi}{\gamma^{\tau(x)}} \geq \gamma^{\bar{\tau}} - \frac{3}{2}\AP{\EU_{\UD}^{\pi^*S^\star}(\gamma) - \EU_{\UD}^\pi(\gamma)}-\frac{1}{2}$$
