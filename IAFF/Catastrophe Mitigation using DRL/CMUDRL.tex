...
$$\Dtva{\mu\bowtie\pi,\ \mu^{\tilde{M}}\bowtie\tilde{\pi}} \leq 1-\Ea{x\sim\mu^{\tilde{M}}\bowtie\tilde{\pi}}{\AP{1-2\delta}^{\tau_S(x)}}???$$
...
\Comment{Using Proposition A.N8, }It is straightforward to construct a probability space $(\Omega,P)$, measurable mappings $h, \tilde{h}: \Omega \rightarrow (\A \times \Ob)^\omega$ and measurable sets $\Omega = E_0 \supseteq E_1 \supseteq E_2 \supseteq \dots$ s.t.
...
Denote $E_\infty := \bigcap_{n = 0}^\infty E_n$. Clearly, for any $\omega \in E_\infty$, $h(\omega)=\tilde{h}(\omega)$. Also, we have

$$P\AP{E_\infty} \geq \Ea{x\sim\mu^{\tilde{M}}\bowtie\tilde{\pi}}{\AP{1-2\delta}^{\tau_S(x)}}???$$

% Shouldn't the distribution for the expected value be conditioned on E_*?

Applying the other direction of Proposition A.N8, we get the desired result.
