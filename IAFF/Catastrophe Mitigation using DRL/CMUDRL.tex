...
Let $\Ob:= \prod_k \St_{M_k}$ and $\In=(\A,\Ob)$. Define the $\In$-universe $\upsilon^k=(\mu^k,r^k)$ by... %reward functions should be compatible... The classical formulation is having a known state space, and each hypothesis corresponds to a different transition tensor + reward vector
