...
$$\sum_{n=0}^\infty \gamma^{n+1} \Pa{x \sim \mu^\RMD \bowtie \pi}{S^\star\AP{x_{:n}}\not\in\SF_M,\ S^\star\AP{x_{:n+1}}\in\SF_M} \geq 2{\gamma^{\tau+1}}-3\AP{\EU_{\UD}^{\pi^* \circ S^\star}(\gamma)-\EU_{\UD}^{\pi}(\gamma)}-1$$
...
$$\EU_{\upsilon}^{\pi^* \circ S}(\gamma)-\EU_{\upsilon}^{\pi^\Re???}(\gamma) \leq \EU_{\upsilon^\RME}^{\pi^* \circ S^\star}(\gamma)-\EU_{\upsilon^\RME}^{\pi}(\gamma)$$
