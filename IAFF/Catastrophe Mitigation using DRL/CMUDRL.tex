...
Here, $\pi^k S^k$ is the $\In$-policy defined by $\pi^k S^k(h):=\pi^k\AP{S^k(h)}$ and analogously for $\sigma^k S^k$. The $O$-notation refers to the asymptotics where $\epsilon$ is *fixed* (so we don't explicitly examine its effect on regret) whereas $\alpha$, $\tau_{1,2}$ and the $M_k$ are variable and $\alpha \rightarrow 0$, $\tau_{1,2} \rightarrow \infty$.
...
$$\V_M(s,\gamma) - \V_{M\pi^*}(s,\gamma) = O\AP{(1-\gamma)\AP{\tau + \max_{s\in\SF_M}\Abs{\V_{M}^2(s)}}}$$

???

% It is not clear that max V^2 is O(1) since the Blackwell optimal policy might not preserve \SF
...
$$\EU_{\upsilon^k}^{\pi^k S^{k}}(1-\alpha)-\EU_{\bar{\upsilon}^k\AB{\sigma^k S^{k}}}^{\pi^*}(1-\alpha) \leq O\AP{\max\AP{\frac{\delta}{\alpha},1}\cdot\AP{(\tau_1 \alpha)^{1/4} + \tau_2 \alpha}+\delta\tau_1}$$
