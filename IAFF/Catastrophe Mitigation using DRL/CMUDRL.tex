...
Fix $M$ a catastrophe MDP s.t. $\R_M \leq 1/2$. Suppose that $\pi^1$ is a mitigation policy for $M$ that has the 1st moment $\tau\in(0,\infty)$ at $s_0 \in \SD_M$. Consider any $\pi: \St_{\MD} \M \A_M$. Then, for any $\gamma\in(0,1)$

$$\sum_{n=0}^\infty \gamma^{n+1} \AP{\T_{\MD\pi}^{n+1}\AP{\top \mid s_0}-\T_{\MD\pi}^{n}\AP{\top \mid s_0}} \geq 2{\gamma^{\tau+1}}-2\AP{\V_{\MD\pi^1}\AP{s_0,\gamma}-\V_{\MD\pi}\AP{s_0,\gamma}}-1$$
