...
Consider $M$ a catastrophe MDP s.t. $\A_M = \A$ and a policy $\sigma: \St_M \M \A$. We then define the catastrophe MDP $\bar{M}[\sigma]$ as follows:

* $\SF_{\bar{M}[\sigma]} := \SF_M$, $\SD_{\bar{M}[\sigma]} := \SD_M$, $\SC_{\bar{M}[\sigma]} := \SC_M$.

* $\A_{\bar{M}[\sigma]} := \Ada$

* For any $s \in \St_M$ and $a \in \A$: $\T_{\bar{M}[\sigma]}(s,a) := \T_M(s,a)$.

* For any $s \in \St_M$: $\T_{\bar{M}[\sigma]}(s,\bot) := \T_{M\sigma}(s)$. 
