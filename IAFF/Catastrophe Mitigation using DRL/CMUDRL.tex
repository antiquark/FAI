...
Fix an interface $\In$ and an $\In$-universe $\upsilon=(\mu,r)$ which is an $\Ob$-realization of a catastrophe MDP $M$ with state function $S$. Assume that $\R_M \leq 1/2$. Suppose that $\pi^1$ is a mitigation policy for $M$ that has the 1st moment $\tau\in(0,\infty)$ at $S(\Estr)$. Let $\pi$ be any $\In'$-policy. Then, for any $\gamma\in(0,1)$

$$\sum_{n=0}^\infty \gamma^{n+1} \Pa{x \sim \mu^\RMD \bowtie \pi}{S^\RMD\AP{x_{:n}}\ne\top,\ S^\RMD\AP{x_{:n+1}}=\top} \geq 2{\gamma^{\tau+1}}-2\AP{\EU_{\UD}^{\pi^1 \circ S}(\gamma)-\EU_{\UD}^{\pi}(\gamma)}-1$$
...
Fix an interface $\In$ and an $\In$-universe $\upsilon=(\mu,r)$ which is an $\Ob$-realization of a catastrophe MDP $M$ with state function $S$. Suppose that $\pi^1$ is a mitigation policy for $M$. Let $\pi$ be any $\In'$-policy. Then, for any $\gamma\in(0,1)$

$$\EU_{\upsilon}^{\pi^1 \circ S}(\gamma)-\EU_{\upsilon}^{\pi}(\gamma) \leq \EU_{\upsilon^\RME}^{\pi^1 \circ S}(\gamma)-\EU_{\upsilon^\RME}^{\pi}(\gamma)$$
...
% Given mitigation time bound + regret of a perturbed policy wrt a perturbation of M
% We get regret for original policy and original M
