...
Consider $\gamma,\delta\in(0,1)$, a universe $\upsilon=(\mu,r)$ which is an $\Ob$-realization of a catastrophe MDP $M$ with state function $S$, and some $\pi: \FH \M \A$. Assume that for any $h \in S^{-1}\AP{\SF_M}$ and $a \in \Supp{\pi(h)}$, $\T_M\AP{\SF_M \mid s,a} = 1$. Let $\tilde{M}$ be a $\delta$-perturbation of $M$ and $\tilde{\pi}$ be a $\delta$-perturbation of $\pi$ over $S$. Then,

$$\Dtva{\mu\bowtie\pi,\ \mu^{\tilde{M}}\bowtie\tilde{\pi}} \leq 1-\Ea{x\sim\mu\bowtie\pi}{\AP{1-2\delta}^{\tau_S(x)}}$$
...
Using Proposition A.N8, it is straightforward to construct a probability space $(\Omega,P)$, measurable mappings $h, \tilde{h}: \Omega \rightarrow (\A \times \Ob)^\omega$ and measurable sets $\Omega = E_0 \supseteq E_1 \supseteq E_2 \supseteq \dots$ s.t.

i. $h_* P = \mu \bowtie \pi$

ii. $\tilde{h}_*P = \mu^{\tilde{M}} \bowtie \tilde{\pi}$

iii. For any $n \in \Nats$ and $\omega \in E_n$, $h(\omega)_{:n} = \tilde{h}(\omega)_{:n}$.

iv. For any $n \in \Nats$ and $x \in (\A \times \Ob)^n$, if $S(x)\in\SD_M$ then $P\AP{E_{n+1} \mid h^{-1}\AP{x(\A \times \Ob)^\omega} \cap E_n} \geq 1 - 2\delta$.

v. For any $n \in \Nats$ and $x \in (\A \times \Ob)^n$, if $S(x)\in\SF_M$ then $P\AP{E_{n+1} \mid h^{-1}\AP{x(\A \times \Ob)^\omega} \cap E_n} = 1$.

% Small total variation distance => conditioning on high probability event yields same distribution...
% Compute the probability of high probability event continuing to happen until \SF...

TBD

\#Proof of Proposition A.N5

% Relating perturbation effect to E[(1-\delta)^\tau]
