...
Consider $\gamma,\delta\in(0,1)$, a universe $\upsilon=(\mu,r)$ which is an $\Ob$-realization of a catastrophe MDP $M$ with state function $S$, and some $\pi: \FH \M \A$. Let $\tilde{M}$ be a $\delta$-perturbation of $M$ and $\tilde{\pi}$ be a $\delta$-perturbation of $\pi$ over $S$. Denote $\tilde{\mu}:=\mu^{\tilde{M}}$, $\tilde{\upsilon}:=\AP{\tilde{\mu},r}$. Then, there is some $C\in(0,\infty)$ which doesn't depend on anything s.t.

$$\Abs{\EU_{\tilde{\upsilon}}^{\tilde{\pi}}(\gamma)-\EU_{\upsilon}^{\pi}(\gamma)} \leq 1-\Ea{x\sim\mu\bowtie\pi}{\AP{1-C\delta}^{\tau_S(x)}}???$$

% This relies on never leaving \SF... ? 
% This works out in Proposition A.N5 since the reference regret is in \ME...
% Maybe we should redefine \bot to line in \SF_\ME so that any policy is automatically mitigation?
