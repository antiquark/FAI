...
i. $$\E{x \sim\Xi\left[\mu,\sigma^k,\D^{!k},\pi\right]}\left[\Abs{\{n \in \Nats \mid x_n \in 2^\A \times \bot \times \bar{\Ob}\}}\right] \leq \frac{\ln N}{\delta \ln\left(1 + \epsilon(1-\epsilon)^{\frac{1-\epsilon}{\epsilon}}\right)}=O\left(\frac{\ln N}{\delta \epsilon}\right)$$
...
Consider $N \in \Nats$, $\epsilon,\delta \in (0,1)$, $\A$ a finite set, $\B \subseteq \A$, $\zeta \in \Delta[N]$ and $\sigma: [N] \M \A$. Suppose that for every $a \in \A$
