...
(The following conditions on $A$ and $\Theta$ imply that the range of the above is indeed in $\Adfh$.) We construct $\Omega$\Comment{, $\F$}, $K$, $\Z$, $\tilde{\Z}$, $\Gamma$, $\Psi$, $A$ and $\Theta$ s.t $K$ is uniformly distributed and for any $k \in [N]$, $n \in \Nats$, $m \in [T]$, $a \in \A$ and $o \in \Ob$

$$\tilde{\Z}_0(k)=\Z_0(k)\equiv\frac{1}{N}$$
%
%$$\Gamma_{nT+m}=\Gamma_{nT}$$
%
$$\Pr\left[\Gamma_{n} = k \mid Z_{nT}\right] = \Z_{nT}\left(k\right)$$
%
$$\Psi_{nT+m} = \A^1_{\Gamma_n}\left(S^{\Gamma_n}(A\Theta_{:nT+m})\right)$$
%
...
% Add the environment, the actually selected actions and the sampling of $k$ from $[N]$ to $\Omega$? This would eliminate the need to condition in the definition of $\pi^*$...
