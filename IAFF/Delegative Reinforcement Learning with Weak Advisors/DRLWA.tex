...
ii. $\exists a \in \A_M^1\left(S(h)\right): \pi(a \mid h) \geq \epsilon$
...
iii. $\exists a \in \A_M^1\left(S(h)\right): \pi(a \mid h) \geq \epsilon$
...
Given any $\Adi$-policy $\pi$ and $\In$-policy $\sigma$, we denote $\left[\sigma\right]\underline{\pi}$ the $\In$-policy defined by

$$\left[\sigma\right]\underline{\pi}(h):=\begin{cases} \pi(h) \text{ if } \pi(h) \in \A \\ \sigma(\underline{h}) \text{ if } \pi(h) = \bot \end{cases}$$

Condition iii of Proposition A.N2 and the $\epsilon$-sanity of $\sigma^k$ imply that $\Supp{\left[\sigma^k\right]\underline{\pi}^{!k}(h)} \subseteq \A^0_k\left(S^k(h)\right)$. This means we can apply Proposition A.N4 and get

$$\EU^*_{\upsilon^k}(\gamma)-\EU^{\pi^{!k}}_{\bar{\upsilon}^k[\sigma^k]}(\gamma) \leq (1-\gamma)\sum_{n=0}^\infty \sum_{m=0}^{T-1} \gamma^{nT+m}\left(\E{x\sim\mu\bowtie\pi^{!k}_n}\left[r\left(x_{:nT+m}\right)\right]-\E{x\sim\mu\bowtie\pi^{!k}}\left[r\left(x_{:nT+m}\right)\right]\right) + O\left(\frac{1}{T}\right)$$

Here, $\pi^{!k}_n$ is defined as $\pi^*_n$ in Proposition A.N4.
