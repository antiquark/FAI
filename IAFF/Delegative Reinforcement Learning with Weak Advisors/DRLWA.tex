...
Consider an MDP $M$ and $\UC \subseteq \St_M$. The MDP $M/\UC^\complement$ is defined by by

$$\St_{M / \UC^\complement} := \UC \sqcup \{\bot\}$$
%
$$\A_{M / \UC^\complement} := \A_M$$
%
$$\T_{M / \UC^\complement}(t \mid s, a) := \begin{cases} \T_M(t \mid s,a) \text{ if } t,s \in \UC \\ \T_M(\St_M \setminus \UC \mid s,a) \text{ if } t = \bot,\ s \in \UC \\ 0 \text { if } t\in\UC,\ s = \bot  \\ 1 \text{ if } t=s=\bot\end{cases}$$
%
$$\R_{M / \UC^\complement}(s) := \begin{cases} \R_M(s) \text{ if } s \in \UC \\ 0 \text{ if } s=\bot \end{cases}$$

\#Definition A.N3

Fix an interface $\In=(\A,\Ob)$. Consider an $\In$-universe $\upsilon=(\mu,r)$ which is an $\Ob$-realization of $M$ with state function $S$, and $\UC \subseteq \St_M$ s.t. $S(\Estr) \in \UC$. Denote $\Ob':=\Ob\sqcup\{\bot\}$ and $\In':=(\A,\Ob')$. The $\In'$-universe $\upsilon/_S\UC^\complement=(\mu/_S\UC^\complement,r')$ is defined by

$$\mu/_S\UC^\complement(o \mid ha) = \begin{cases} \mu(o \mid ha) \text{ if } hao \in \FH \text{ and } S(hao)\in\UC \\ 0 \text{ if } hao \in \FH \text{ and } S(hao)\not\in\UC \\ \T_{M/\UC^\complement}\AP{\bot \mid S(h),a} \text{ if } h \in \FH \text{ and } o = \bot \\ 0 \text{ if } h \not\in \FH \text{ and } o \ne \bot \\ 1 \text{ if } h\not\in\FH \text{ and } o = \bot \end{cases}$$

$$r'(h):=\begin{cases} r(h) \text{ if } h \in \FH \\ 0 \text{ otherwise} \end{cases}$$

It is easy to see that $\upsilon/_S \UC^\complement$ is an $\Ob'$-realization of $M/\UC^\complement$ with state function $S'$ defined by
