...
For any $n \in \Nats$ and $h \in \Do_n$, the notation $\Gamma(h)$ means $\Gamma_n(h)$ and the same applies to $\Z$, $\tilde{\Z}$, $\Pi$. We construct $\Omega$, $\F$, $\Z$, $\tilde{\Z}$, $\Gamma$ and $\Pi$ s.t. for any $k \in [N]$, $n \in \Nats$, $m \in [T]$, $h \in \Do_{nT+m}$, $a \in \A$ and $o \in \Ob$
...
$$\tilde{\Z}(k \mid hao)=\frac{\Z(k \mid h) \mu^k(o \mid ha)}{\sum_{j \in [N]} \Z(j \mid h) \mu^j(o \mid ha)}$$

$$\Z(k \mid hao) = \frac{[[\tilde{\Z}(k \mid hao) \geq \delta]]\cdot[[a \in \Pi(h) \lor k \ne \Gamma(h)]]}{\sum_{j \in [N]}[[\tilde{\Z}(j \mid hao) \geq \delta]]\cdot[[a \in \Pi(h) \lor j \ne \Gamma(h)]]}$$
...
$$\pi^*(a \mid h)=\Pr\left[\D\left(h,\Pi\left(\underline{h}\right)\right)=a \mid \forall m \in [n]: h_m \in \D\left(h_{:m},\Pi\left(\underline{h_{:m}}\right)\right) \times \Ado\right]$$
