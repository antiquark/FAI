...
Given $\pi: \FH \M \Delta\A!$, an $\In$-environment $\mu$ and $k \in [N]$, we can define $\Xi\left[\mu,\sigma^k,\D,\pi\right]\in \Delta\left(\A! \times \Adao\right)^\omega$ as follows
...
Fix $\gamma \in (0,1)$ and $T \in \Nats$. Define $\pi^!: \FH \rightarrow \Delta\A!$ as follows.
