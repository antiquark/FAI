...
A *stationary policy* for $M$ is any $\pi: \St_M \M \A_M$. The space of stationary policies is denoted $\Pi_M$. Given $\pi \in \Pi_M$, we define $\pi^M: \St_M \M \St_M^\omega$ by requiring that for any $s_0 \in \St_M$, $s \in \St_M$, $n \in \Nats$ and $h \in \St_M^{n+1}$
...
$$\V_M(s,\gamma):=\max_{\pi \in \Pi_M} \frac{?}{?}$$
...
Given an MDP $M$, $\epsilon > 0$, a stationary policy $\pi$ for $M$ is said to be *strictly $\epsilon$-sane* when...
