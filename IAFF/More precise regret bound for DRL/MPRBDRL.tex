...
(The following conditions on $A$ and $\Theta$ imply that the range of the above is indeed in $\Adfh$.) Let $\Dl$ and $\Dl^{!k}$ be as in Proposition B.1 (the form of the bound we are proving allows assuming w.l.o.g. that $\epsilon < \frac{1}{\Abs{\A}}$). By condition iii of Definition 2, there is $\pi^k : \HD{\mu^k} \rightarrow \A$ s.t. for any $h \in \HD{\mu^k}$, if $S^k(h) \in \UC^k$ then
...
Condition iii of Proposition B.1 and condition ii of Definition 2 imply that for any $h \in \HD{\mu^k}$
...
Here, the $\In$-policy $\pi^{*k}_n$ is defined as $\pi^*_n$ in Proposition B.2, with $\pi^k$ in place of $\pi^*$. We also define the $\Adi$-policies $\pi^{!k}_n$ and $\pi^{!!k}_n$ by
...
Condition iv of Proposition B.1 implies that, given $h \in \HD{\nu^k}$ s.t. $\Abs{h} \geq nT$
...
$$\Abs{\frac{\D{\V_M(s,\gamma)}}{\D\gamma}} \leq \Ea{k \sim \zeta(s)}{k} + F\frac{1+\lambda}{1-\lambda} \leq \max \Pd + F\frac{1+\lambda}{1-\lambda}$$
