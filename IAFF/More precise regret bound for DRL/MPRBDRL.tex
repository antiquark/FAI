...
$$\EU_{\upsilon^k}^*(1-\alpha) - \EU_{\bar{\upsilon}^k\left[\sigma^k\right]}^{\pi^*}(1-\alpha) \leq C\AP{\frac{\alpha N^5 \ln{N} \sum_{j = 0}^{N-1} \max_{s \in \UC_k} \Abs{\V^1_{M^j}(s)}}{\epsilon}}^{1/4}$$

***

The factor $\max_{s \in \UC_j} \Abs{\V^1_{M^j}(s)}$ might seem difficult to understand. However, it can be bounded as follows.
