...
Consider some $\gamma\in(0,1)$, $\tau\in(0,\infty)$, $T\in\Nats^+$, a universe $\upsilon=(\mu,r)$ that is an $\Ob$-realization of $M$ with state function $S$, a policy $\pi^*: \St_M \rightarrow \A$ and an arbitrary $\In$-policy $\pi^0: \FH \M \A$. Assume that for any $s \in \St_M$

$$\pi^*(s) \in \Argmax{a \in \A}{\Q_M(s,a,\gamma)}$$

For any $n \in \Nats$, let $\pi^*_n$ be an $\In$-policy s.t. for any $h \in \HD{\mu}$
...
ii. For any $s \in \St_M$ and $\theta\in(\gamma,1)$ $$\Abs{\frac{\D\V_{M\pi^*}\AP{s,\theta}}{\D\theta}} \leq \tau$$
