...
Let $\upsilon = (\mu,r)$ be a universe and $\epsilon > 0$. A policy $\pi$ is said to be *locally $\epsilon$-sane for $\upsilon$* when there are $M$, $S$ and $\SC_M \subseteq \St_M$ (the set of corrupt states) s.t. $\upsilon$ is an $\Ob$-realization of $M$ with state function $S$, $S(\Estr) \not\in \SC_M$ and for any $h \in \HD{\mu}$, if $S(h) \not\in \SC_M$ then

i. If $a \in \Supp{\pi(h)}$ then $$\T_M\AP{\SC_M \mid S(h),a} = 0$$
...
Consider $\In$ an interface, $\epsilon \in (0,1)$ and $\Hy = \{\upsilon^k = (\mu^k,r^k) \in \Upsilon_{\In}\}_{k \in [N]}$ for some $N \in \Nats$. Assume that for each $k \in [N]$, $\sigma^k$ is locally $\epsilon$-sane for $\upsilon^k$. For each $k \in [N]$, let $M^k$ be the corresponding MDP and $\SC_k \subseteq \St_{M^k}$ the corresponding set of corrupt states. Assume further that for any $k,j \in \Nats$ and $h \in \HD{\mu^k} \cap \HD{\mu^j}$, if $S^k(h) \not\in \SC_k$ and $S^j(h) \not\in \SC_j$, then $r^k(h)=r^j(h)$. Then, there is an $\bar{\In}$-policy $\pi^*$ s.t. for any $k \in [N]$
...
...$F^k$ and $\lambda^k$ constants as in Proposition 1...
