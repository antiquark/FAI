...
Fix an interface $\I$ and consider a universe $\upsilon=(\mu,r)$. Let $t,\beta \in (0,\infty)$. An autonomous $\Adi$-policy $\alpha$ is called *strictly $\beta$-rational* for $(\upsilon,t)$ when for any $h \in \HD{\bar{\mu}[\alpha]}$ and $a \in \A$

$$\alpha(h)(a) \leq \exp{[\beta(\W^{\upsilon}_t(\underline{h}a)-\V^\upsilon_t(\underline{h}))]} \max_{a^* \in \A^\upsilon_t(\underline{h})} \alpha(h)(a^*) $$
