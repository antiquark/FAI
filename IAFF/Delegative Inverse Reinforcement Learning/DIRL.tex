...
ii. $\lim_{t \rightarrow \infty} \E_{x \sim \mu_t^{\pi^*_t}}[\lim_{n \rightarrow \infty} \hat{\kappa}_t(x_{:n})] = 0$
...
i. For each $k \in \Hy$, there is $a^*_k \in \A$ s.t. $L(k,a_k^*)=0$.

ii. For each $k \in \Hy$, $\zeta(k) \geq \beta\epsilon$.

iii. For each $a \in A$, $\E_{k \in \zeta}[L(k,a)] \geq \epsilon$.

iv. For each $k \in \Hy$ and $a \in A$, $\alpha(k)(a) \leq \exp[-\beta L(k,a)] \max_{b \in \A} \alpha(k)(b)$
...
Consider the setting of Theorem, but assume that $\Hy = \{\upsilon^k = (\mu^k,r^k)\}_{k < N}$ for some $N \in \Nats$ (i.e. it is finite) and that $\alpha^k(t)$ is *strictly* $\beta(t)$-rational for $\bar{\upsilon}^k[\alpha^k]$. Denote $\nu^k:=\bar{\mu}^k[\alpha^k]$ and $\HD{\nu}:= \bigcup_{k < N} \HD{\nu^k}$. Define $L_t: \HD{\nu} \times \A \rightarrow [0,1]$ by

$$L_t(ha):=\frac{\sum_{k < N} \hat{\nu}^k(h)(\max_{b \in A}{\W^{\upsilon^k}_t(\underline{h}b)}-\W^{\upsilon^k}_t(\underline{h}a))}{\sum_{k < N} \hat{\nu}^k(h)}$$

Denote $\epsilon_t:=\beta(t)^{-1}t^{-1/3}$. Define the following $\Adi$-metapolicy $\pi^*$:

$$\pi^*_t(h):=\begin{cases} \Argmin{a \in \A} L_t(ha) \text{ if } h \in \HD{\nu},\, \min_{a \in \A}{L_t(ha)} < \epsilon_t \\ \bot \text{ if } h \in \HD{\nu},\,\min_{a \in \A}{L_t(ha)} \geq \epsilon_t \\ \bot \text{ if } h \not\in \HD{\nu}\end{cases}$$
