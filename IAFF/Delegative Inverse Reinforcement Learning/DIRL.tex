...
Consider the setting of Theorem, but assume that $\Hy = \{\upsilon^k = (\mu^k,r^k)\}_{k < N}$ for some $N \in \Nats$ (i.e. it is finite) and that $\alpha^k(t)$ is *strictly* $\beta(t)$-rational for $\bar{\upsilon}^k[\alpha^k]$. Denote $\nu^k:=\bar{\mu}^k[\alpha^k]$ and $\HD{\bar{\Hy}}:= \bigcup_{k < N} \HD{\nu^k}$. Denote $\zeta^0 \in \Delta[N]$ the uniform probability distribution. For any $t > N^3$, define $\zeta_t,\zeta'_t: \HD{\bar{\Hy}} \rightarrow \Delta[N]$ recursively as follows

$$\zeta'_t(\Estr):=\zeta^0$$

$$\zeta'_t(hao)(k):=\frac{\zeta_t(h)(k) \cdot \nu^k(ha)(o)}{\sum_{j < N} \zeta_t(h)(j) \cdot \nu^j(ha)(o)}$$

$$\zeta_t(h)(k):=\frac{\zeta'_t(h)(k)[[\zeta'_t(h)(k) > t^{-1/3}]]}{\sum_{j < N}\zeta'_t(h)(j)[[\zeta'_t(h)(j) > t^{-1/3}]]}$$

That is, $\zeta_t(h)$ is obtained by starting from prior $\zeta^0$, updating on every observation and removing from consideration any universe whose probability drops below $t^{-1/3}$. Define $L_t: \HD{\bar{\Hy}} \times \A \rightarrow [0,1]$ by
...
$$\EE{k \sim \zeta^0}{x \sim {\nu^k_t}\bowtie{\pi^*_t}}[\Ent(\zeta_t(x_{:\Abs{h}+1})) \mid x_{:\Abs{h}}=h] = \Ent(\zeta_t(h)) - \EE{k \sim \zeta^0}{x \sim {\nu^k_t}\bowtie{\pi^*_t}}[\KL{\zeta_t(x_{:n+1})}{\zeta_t(x_{:n})} \mid x_{:\Abs{h}}=h]$$
...
$$0 \leq \EE{k \sim \zeta^0}{x \sim {\nu^k_t}\bowtie{\pi^*_t}}[\Ent(\zeta_t(x_{:n}))] \leq N \ln{N} - \frac{(1-e^{-1})^2}{8\Abs{\A}^2} \beta(t)^2 \epsilon_t^2\EE{k \sim \zeta^0}{x \sim {\nu^k_t}\bowtie{\pi^*_t}}[\Abs{\{m \in [n] \mid x_m \in \bot \times \Ob\}}]$$

$$\EE{k \sim \zeta^0}{x \sim {\nu^k_t}\bowtie{\pi^*_t}}[\Abs{\{m \in [n] \mid x_m \in \bot \times \Ob\}}] \leq \frac{8 \Abs{\A}^2 N \ln{N}}{(1-e^{-1})^2}t^{2/3}$$
