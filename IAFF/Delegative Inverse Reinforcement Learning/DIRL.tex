...
That is, $\xi^k$ behaves as if, at each moment of time, we update on a bit of information saying whether the true universe is in the set of low probability universes. Denote $\rho_t:= \zeta^0 \ltimes ({\nu_t}\bowtie{\pi^*_t}) \in \Delta([N] \times \Adih)$, where the dependence of ${\nu^k_t}\bowtie{\pi^*_t}$ on $k$ is used to view it as Markov kernel from $[N]$ to $\Adih$. It is easy to see that, since the probability of ever receiving a "low probability" bit is at most $(N-1)t^{-1/3}$, we have

$$\Prb_{(k,x)\sim\rho_t}[\exists n \in \Nats: \zeta_t(x_{:n}) \ne \xi_t^k(x_{:n})] \leq (N-1)t^{-1/3}$$

For any $h \in \HD{\bar{\Hy}}$, we have

$$\E_{(k,x)\sim\rho_t}[\Ent(\xi^k_t(x_{:\Abs{h}+1})) \mid x_{:\Abs{h}}=h] = \Ent(\xi^k_t(h)) -\E_{(k,x)\sim\rho_t}[\KL{\tilde{\xi}^k_t(x_{:\Abs{h}+1})}{\xi^k_t(x_{:\Abs{h}})} +H(\tilde{\xi}^k_t(x_{:\Abs{h}+1}))-H(\xi^k_t(x_{:\Abs{h}+1}))\mid x_{:\Abs{h}}=h]$$
