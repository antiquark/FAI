...
$$\E{(k,x)\sim\rho_t}[\Ent(\xi^k_t(x_{:\Abs{h}+1})) \mid h] = \Ent(\xi^k_t(h)) -\E{(k,x)\sim\rho_t}[\KL{\tilde{\xi}^k_t(x_{:\Abs{h}+1})}{\xi^k_t(x_{:\Abs{h}})} +\Ent(\tilde{\xi}^k_t(x_{:\Abs{h}+1}))-\Ent(\xi^k_t(x_{:\Abs{h}+1}))\mid h]$$

Taking expected value over $h$, we conclude that for any $n \in \Nats$

$$\E{(k,x)\sim\rho_t}[\Ent(\xi^k_t(x_{:n+1}))] = \E{(k,x)\sim\rho_t}[\Ent(\xi^k_t(x_{:n}))] - \E{(k,x)\sim\rho_t}[\KL{\tilde{\xi}^k_t(x_{:n+1})}{\xi^k_t(x_{:n})} +\Ent(\tilde{\xi}^k_t(x_{:n+1}))-\Ent(\xi^k_t(x_{:n+1}))]$$

$$\E{(k,x)\sim\rho_t}[\Ent(\xi^k_t(x_{:n}))] = \Ent(\zeta^0) - \E{(k,x)\sim\rho_t}[\sum_{m < n} (\KL{\xi^k_t(x_{:m+1})}{\xi^k_t(x_{:m})}+\Ent(\tilde{\xi}^k_t(x_{:m+1}))-\Ent(\xi^k_t(x_{:m+1})))]$$
