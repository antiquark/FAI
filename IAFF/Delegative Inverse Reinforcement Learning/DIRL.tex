...
$$\E{(k,x)\sim\rho_t}[\Ent(\zeta^{!k}_t(x_{:\Abs{h}+1})) \mid h] = \Ent(\zeta^{!k}_t(h)) -\E{(k,x)\sim\rho_t}[\KL{\tilde{\zeta}^{!k}_t(x_{:\Abs{h}+1})}{\zeta^{!k}_t(x_{:\Abs{h}})} +\Ent(\tilde{\zeta}^{!k}_t(x_{:\Abs{h}+1}))-\Ent(\zeta^{!k}_t(x_{:\Abs{h}+1}))\mid h]$$

Taking expected value over $h$, we conclude that for any $n \in \Nats$

$$\E{(k,x)\sim\rho_t}[\Ent(\zeta^{!k}_t(x_{:n+1}))] = \E{(k,x)\sim\rho_t}[\Ent(\zeta^{!k}_t(x_{:n}))] - \E{(k,x)\sim\rho_t}[\KL{\tilde{\zeta}^{!k}_t(x_{:n+1})}{\zeta^{!k}_t(x_{:n})} +\Ent(\tilde{\zeta}^{!k}_t(x_{:n+1}))-\Ent(\zeta^{!k}_t(x_{:n+1}))]$$

$$\E{(k,x)\sim\rho_t}[\Ent(\zeta^{!k}_t(x_{:n}))] = \Ent(\zeta^0) - \E{(k,x)\sim\rho_t}[\sum_{m < n} (\KL{\tilde{\zeta}^{!k}_t(x_{:m+1})}{\zeta^{!k}_t(x_{:m})}+\Ent(\tilde{\zeta}^{!k}_t(x_{:m+1}))-\Ent(\zeta^{!k}_t(x_{:m+1})))]$$

Using the relationship between $\xi_t^k$ and $\zeta_t$, we get

$$\E{(k,x)\sim\rho_t}[\Ent(\zeta_t(x_{:n}))] = \Ent(\zeta^0) - \E{(k,x)\sim\rho_t}[\sum_{m < n} (\KL{\tilde{\zeta}_t(x_{:m+1})}{\zeta_t(x_{:m})}+\Ent(\tilde{\zeta}_t(x_{:m+1}))-\Ent(\zeta_t(x_{:m+1})))]+O(t^{-1/3}n)???$$
