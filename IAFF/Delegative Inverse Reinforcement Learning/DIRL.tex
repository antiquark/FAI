...
$$\tilde{\zeta}_t(hao)(k):=\frac{\zeta_t(h)(k) \cdot \nu^k(ha)(o)}{\sum_{j < N} \zeta_t(h)(j) \cdot \nu^j(ha)(o)}$$

$$\zeta_t(h)(k):=\frac{\tilde{\zeta}_t(h)(k)[[\tilde{\zeta}_t(h)(k) > t^{-1/3}]]}{\sum_{j < N}\tilde{\zeta}_t(h)(j)[[\tilde{\zeta}_t(h)(j) > t^{-1/3}]]}$$

That is, $\zeta_t(h)$ is obtained by starting from prior $\zeta^0$, updating on every observation, and setting to 0 the probability of any universe whose probability drops below $t^{-1/3}$. Define $L_t: \HD{\bar{\Hy}} \times \A \rightarrow [0,1]$ by

$$L_t(ha):=\E_{k \sim \zeta_t(h)}[\max_{b \in A}{\W^{\upsilon^k_t}_t(\underline{h}b)}-\W^{\upsilon^k_t}_t(\underline{h}a)]$$
...
For every $k \in [N]$, we define $\xi^k_t, \tilde{\xi}^k_t: \HD{\bar{\Hy}} \rightarrow \Delta[N]$ and $S_t^k: \HD{\bar{\Hy}} \rightarrow 2^{[N]}$ recursively as follows

$$\tilde{\xi}^k_t(\Estr):=\zeta^0$$

$$\tilde{\xi}^k_t(hao)(i):=\frac{\xi^k_t(h)(i) \cdot \nu^i(ha)(o)}{\sum_{j < N} \xi^k_t(h)(j) \cdot \nu^j(ha)(o)}$$

$$S_t^k(h):=\{i \in [N] \mid \tilde{\xi}^k_t(h)(i) > t^{-1/3}\}$$

$$\xi^k_t(h)(i):= [[i,k \in S_t^k(h)]]\frac{\tilde{\xi}^k_t(h)(i)}{\sum_{j \in S_t^k(h)}\tilde{\xi}^k_t(h)(j)}+[[i,k \not\in S_t^k(h)]]\frac{\tilde{\xi}^k_t(h)(i)}{\sum_{j \in [N] \setminus S_t^k(h)}\tilde{\xi}^k_t(h)(j)}$$

That is, $\xi^k$ behaves as if, at each moment of time, we update on a bit of information saying whether the true universe is in the set of low probability universes. It is easy to see that, since the probability of ever receiving a "low probability" bit is at most $(N-1)t^{-1/3}$, we have

$$\PP{k \sim \zeta^0}{x \sim {\nu^k_t}\bowtie{\pi^*_t}}[\exists n \in \Nats: \zeta_t(x_{:n}) \ne \xi_t^k(x_{:n})] \leq (N-1)t^{-1/3}$$

...For any $h \in \HD{\bar{\Hy}}$, we have

$$\EE{k \sim \zeta^0}{x \sim {\nu^k_t}\bowtie{\pi^*_t}}[\Ent(\zeta'_t(x_{:\Abs{h}+1})) \mid x_{:\Abs{h}}=h] = \Ent(\zeta_t(h)) - \EE{k \sim \zeta^0}{x \sim {\nu^k_t}\bowtie{\pi^*_t}}[\KL{\zeta'_t(x_{:n+1})}{\zeta_t(x_{:n})} \mid x_{:\Abs{h}}=h]$$

Taking expected value over $h$, we conclude??? that for any $n \in \Nats$

$$\EE{k \sim \zeta^0}{x \sim {\nu^k_t}\bowtie{\pi^*_t}}[\Ent(\zeta_t(x_{:n+1}))] = \EE{k \sim \zeta^0}{x \sim {\nu^k_t}\bowtie{\pi^*_t}}[\Ent(\zeta_t(x_{:n}))] - \EE{k \sim \zeta^0}{x \sim {\nu^k_t}\bowtie{\pi^*_t}}[\KL{\zeta_t(x_{:n+1})}{\zeta_t(x_{:n})}]$$

$$\EE{k \sim \zeta^0}{x \sim {\nu^k_t}\bowtie{\pi^*_t}}[\Ent(\zeta_t(x_{:n}))] = \Ent(\zeta^0) - \EE{k \sim \zeta^0}{x \sim {\nu^k_t}\bowtie{\pi^*_t}}[\sum_{m < n} \KL{\zeta_t(x_{:m+1})}{\zeta_t(x_{:m})}]$$
