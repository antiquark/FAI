...
Without loss of generality, we can assume that $\E_\zeta[L] = \epsilon$, because otherwise we can rescale $L$ by a constant in $(0,1)$ which will only make $\E_{\zeta}[e^{-\beta L}]$ larger. It now follows that for any $k$, $L(k) \leq \beta^{-1}$...
