...
$$\EE{k \sim \zeta^0}{x \sim {\nu^k_t}\bowtie{\pi^*_t}}[\Ent(\xi^k_t(x_{:\Abs{h}+1})) \mid x_{:\Abs{h}}=h] = \Ent(\xi^k_t(h)) - \EE{k \sim \zeta^0}{x \sim {\nu^k_t}\bowtie{\pi^*_t}}[\KL{\xi^k_t(x_{:n+1})}{\xi^k_t(x_{:n})} \mid x_{:\Abs{h}}=h]$$

Taking expected value over $h$, we conclude that for any $n \in \Nats$

$$\EE{k \sim \zeta^0}{x \sim {\nu^k_t}\bowtie{\pi^*_t}}[\Ent(\xi^k_t(x_{:n+1}))] = \EE{k \sim \zeta^0}{x \sim {\nu^k_t}\bowtie{\pi^*_t}}[\Ent(\xi^k_t(x_{:n}))] - \EE{k \sim \zeta^0}{x \sim {\nu^k_t}\bowtie{\pi^*_t}}[\KL{\xi^k_t(x_{:n+1})}{\xi^k_t(x_{:n})}]$$

$$\EE{k \sim \zeta^0}{x \sim {\nu^k_t}\bowtie{\pi^*_t}}[\Ent(\xi^k_t(x_{:n}))] = \Ent(\zeta^0) - \EE{k \sim \zeta^0}{x \sim {\nu^k_t}\bowtie{\pi^*_t}}[\sum_{m < n} \KL{\xi^k_t(x_{:m+1})}{\xi^k_t(x_{:m})}]$$
