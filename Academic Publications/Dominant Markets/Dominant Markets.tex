...
We remind that $\PO$ is the projection mapping from $\OO$ to $\Ob^n$ and $y\OO:= (\PO)^{-1}(y)$.
...
\begin{equation}
\PG{S}_n(y,\BM):=\begin{cases} S_n(y) \text{ if } \max_{y\OO}{\SV{\PG{S}}_n(y_{:n-1},\BM)}-\min_{y\OO}{\SV{\PG{S}}_n(y_{:n-1},\BM)} \leq 1 \\ 0 \text{ otherwise} \end{cases}
\end{equation}
...
\item $\inf_n X_n(\omega) > -\infty$
\item If $\sup_n X_n(\omega) < +\infty$ then $\sum_n Y_n(\omega) < +\infty$
...
For each $k \in \Nats$, $N_k$ is a stopping time w.r.t. ${\F}$. As easy to see, for each $k \in \Nats$, ${\Sqn{X^0_{\min(n,N_k)}}}$ is a uniformly integrable submartingale. Using the fact that ${N_{k} \leq N_{k+1}}$ to apply Theorem C, we get
...
By Proposition B.12, ${\A{X^0_{N_{k+1}}-X^0_{N_k}}} \leq \alpha + 2$. Applying the Azuma-Hoeffding inequality, we conclude that for any positive integer ${k}$:

$$\Pr[X^0_{N_k} - S_0 < -\alpha k] \leq \exp(-\frac{(\alpha k)^2}{2(\alpha+2)^2 k})=\exp(-\frac{\alpha^2 k}{2(\alpha+2)^2})$$

Since ${\sum_k \exp(-\frac{\alpha^2 k}{2(\alpha+2)^2}) < \infty}$, it follows that

$$\Pr[\exists k \in \Nats \forall l > k: X^0_{N_l} - S_0 \geq -\alpha l]=1$$

$$\Pr[\exists k \in \Nats \forall l > k: S_{N_l} - S_0 \geq \alpha (\sum_{n=0}^{N_l-1} Y_n -  l)]=1$$

By Proposition B.13

$$\Pr[\sum_{n=0}^\infty  Y_n = \infty \implies \exists k \in \Nats \forall l > k: S_{N_l} - S_0 \geq 0]=1$$

It remains to show that if ${x \in \Omega}$ is s.t. ${\inf_n S_n(x) = -\infty}$ then the condition above fails. Consider any such ${x \in \Omega}$. ${\A{S_n(x) - S_0(x)} \leq \sum_{m=0}^{n-1} Y_n(x) + \alpha}$, therefore ${\sum_{n=0}^\infty  Y_n(x) = \infty}$. On the other hand, by Proposition B.14, ${\inf_{k} S_{N_k(x)}(x) = -\infty}$.

Define $\Sqn{X^0_n: \Omega \rightarrow \Reals}$ by

$$X^0_n := S_n - \alpha \sum_{m=0}^{n-1} Y_m$$

Let $\Sqn{N_n}$ be the accumulation times of $Y$. Consider any $x \in \Omega$ s.t. $\sup_n S_n(x) = s(x) < \infty$ but $\sum_n Y_n(x) = \infty$. Proposition B.13 implies that

$$X^0_{N_k(x)}(x) \leq S_{N_k(x)}(x) - \alpha k \leq s(x) - \alpha k$$

As in the proof of Proposition A.3, we can apply the Azuma-Hoeffding inequality to $X^0_N$ and get that for any positive integer $k$

$$\Pr[X^0_{N_k} - S_0 < -k^{\frac{3}{4}}] \leq \exp(-\frac{k^{\frac{3}{2}}}{2(\alpha + 2)^2k})=\exp(-\frac{k^{\frac{1}{2}}}{2(\alpha + 2)^2})$$

It follows that

$$\sum_{k=1}^\infty \Pr[X^0_{N_k} - S_0 < -k^{\frac{3}{4}}] < \infty$$

$$\Pr[\exists k \in \Nats \forall l > k: X^0_{N_l} - S_0 < -l^{\frac{3}{4}}] = 0$$
