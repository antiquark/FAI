...
Let $\Sqn{\Ob_n}$ be a sequence of compact Polish spaces. $\Ob_n$ represents the space of possible observations at time $n$. Denote $\Ob^n := \prod_{m < n} \Ob_n$  and $\Ob^\omega:=\prod_{n \in \Nats} \Ob_n$. $\PO_n: \Ob^\omega \rightarrow \Ob^n$ is the projection mapping. $\Ob^n$ will be regarded as a topological space using the product topology and as a measurable space with the $\sigma$-algebra of \emph{universally measurable} sets. $\PM(\OO)$ will denote the space of probability measures on $\OO$, which will be regarded as a topological space using the weak topology and as a measurable space using the $\sigma$-algebra of \emph{Borel} sets. Given $\mu \in \PM(\OO)$, we denote $\Sp \mu \subseteq \OO$ the support of $\mu$.

\begin{definition}

A \emph{forecaster} $F$ is a family of measurable mappings

\[\Sqn{F_n: \Ob^n \rightarrow \PM(\OO)}\]

s.t. $\Sp {F_n(y)} \subseteq (\pi^\Ob_n)^{-1}(y)$.

\end{definition}

Given a forecaster $F$ and $y \in \Ob^n$, $F_n(y)$ represents the forecast corresponding to observation history $y$. The condition $\Sp {F_n(y)} \subseteq (\PO_n)^{-1}(y)$ reflects the obvious requirement of consistency with past observations.

In order to formulate a claim about forecast convergence, we will need a metric on $\PM(\OO)$. Consider $\rho: \OO \times \OO \rightarrow \Reals$ a metrization of $\OO$. Let $\Lp(\OO;\rho)$ be the Banach space of $\rho$-Lipschitz functions on $\OO$, equipped with the norm

\begin{equation}
\N{f}_\rho:=\max_{x}{\A{f(x)}} + \sup_{x \ne y} \frac{\A{f(x)-f(y)}}{\rho(x,y)}
\end{equation}

$\PM(\OO)$ can be regarded as a compact subset of the dual space $\Lp(\OO;\rho)'$, yielding the following metrization of $\PM(\OO)$:
...
\begin{equation}
\lim_{n \rightarrow \infty} \max_{x' \in (\PO_n)^{-1}(x)} \rho(x', x) = 0
\end{equation}

Denote $\delta_x \in \PM(\OO)$ the unique probability measure s.t. $\delta_x(\{x\})=1$. It follows that for any sequence $\Sqn{\mu_n \in \PM(\OO)}$ s.t. $\Sp{\mu_n} \subseteq \pi_n^{-1}(x)$
...
Consider any $M \subseteq \PM(\OO)$. Given $y \in \Ob^n$, we define
...
\begin{equation}
M'_n:=\{(y,\mu) \in Y_n \times \PM(\OO) \mid \mu \textnormal{ is in the convex hull of } M''_y\}
\end{equation}

\begin{equation}
M_y:=\{\mu \in \PM(\OO) \mid (y,\mu) \in \bar{M}'_n\}
\end{equation}
...
Fix any $\MC \subseteq 2^{\PM(\OO)}$ countable. Then, there exists a forecaster $F^\MC$ s.t. for any $M \in \MC$, $\mu \in M$ and $\mu$-almost any $x \in \OO$
