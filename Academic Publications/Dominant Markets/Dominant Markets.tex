...
The forecasting method we demonstrate is based on the principles introduced in \cite{Garrabrant_2016}. The forecast may be regarded as the pricing of a certain combinatorial prediction market, where each incomplete model $M \in \MC$ corresponds to a gambler that plays in the market. This gambler makes \enquote{bets} (continuous functions $f$ on $X$) s.t. assuming the environment is sampled from some $\mu \in M$, the expected value $\E_\mu[f] - \E_F[f]$ of the bet is positive. The market pricing is then defined by the requirement that the aggregate of all gamblers doesn't make a net profit (the existence of such a pricing is established using the Kakutani-Glicksberg-Fan fixed point theorem). The fact that the aggregate of all gamblers cannot make a net profit implies that each individual gambler can only make a bounded profit, which implies the main result in turn.
...
Note that $F^\MC$ as above exists for any choice of $\Sqn{\rho_n}$ but it depends on the choice.
...
$$\forall k \in \Nats: \lim_{r \rightarrow 0} \E_{\mu}[f_k \mid \pi^{-1}(\B_r(y_0))] = \E_{\mu}[f_k \mid \pi^{-1}(y_0)]$$

Now consider any $f \in C(X)$. For any $\epsilon > 0$, there is $k \in \Nats$ s.t. $\N{f-f_k} < \epsilon$ and therefore

$$\limsup_{r \rightarrow 0} \E_{\mu}[f \mid \pi^{-1}(\B_r(y_0))] \leq \limsup_{r \rightarrow 0} \E_{\mu}[f_k \mid \pi^{-1}(\B_r(y_0))] + \epsilon = \E_{\mu}[f_k \mid \pi^{-1}(y_0)] + \epsilon  \leq \E_{\mu}[f \mid \pi^{-1}(y_0)] + 2\epsilon$$

Similarly

$$\liminf_{r \rightarrow 0} \E_{\mu}[f \mid \pi^{-1}(\B_r(y_0))] \geq \E_{\mu}[f \mid \pi^{-1}(y_0)] - 2\epsilon$$

Taking $\epsilon$ to 0, we conclude that

$$\lim_{r \rightarrow 0} \E_{\mu}[f \mid \pi^{-1}(\B_r(y_0))] = \E_{\mu}[f \mid \pi^{-1}(y_0)]$$

$$\lim_{r \rightarrow 0} \mu \mid \pi^{-1}(\B_r(y_0)) = (\mu \mid \pi)(y_0)$$
\end{proof}
...
$$\lim_{r \rightarrow 0} {\mu^* \mid \B_r(y)} = (\mu^* \mid \PO_n)(y)$$

Therefore, ${(\mu^* \mid \PO_n)(y) \in M_y \subseteq M \mid y}$ (see Definition~\ref{def:update_incomplete}). Since $S_n(y)$ is $(M \mid y,\, \epsilon)$-savvy, for any $\mu \in \PM(X)$, denoting $r_{\mu y}:=\DKR^n(\mu,\,M \mid y)$:
...
We wish to thank Michael Greinecker for pointing out Theorem~\ref{thm:selection} to us. TBD
