...
We will now introduce a way to transform a gambler in a way that enforces a \enquote{finite spending budget.} Consider a gambler $G$ and fix $b > 0$ (the size of the \enquote{budget}). Define the measurable functions $\SVM G^F_n: \Ob^n \rightarrow \Reals$ by

\begin{equation}
\SVM G^F_n(y) := \min_{y\OO}{\SV G^F_n(y)}
\end{equation}

Define the universally measurable sets

\begin{equation}
\Ob^n_{GFb}:=\{y \in \Ob^n \mid \SVM G^F_n(y) > -b\}
\end{equation}

Define the measurable functions $\Nr_b G^F_n: \Ob^n_{GFb} \rightarrow C(\PMO)$ by

\begin{equation}
\Nr_b G^F_n(y,\mu):=\max(1,\max_{y\OO} \frac{-\V G^F_n(y,\mu)}{b + \SV G^F_n(y)})^{-1}
\end{equation}

Finally, define the gambler??? $\B_b G^F$...
