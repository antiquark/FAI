...
Fix any $\mu^* \in M$. By Lemma~\ref{lmm:update_incomplete}, for $\pi_{n*} \mu$-almost any $y \in Y$

$$\lim_{r \rightarrow 0} \mu^* \mid \B_r(y) = (\mu^* \mid \pi)(y)$$

Therefore, ${(\mu^* \mid \pi)(y) \in M_y \subseteq M \mid y}$ (see Definition~\ref{def:update_incomplete}). Since $S_n(y)$ is $(M \mid y,\, \epsilon)$-savvy, for any $\mu \in \PM(X)$, denoting $r_{\mu y}:=\DKR^n(\mu,\,M \mid y)$:
