...
Consider any $M \in \MC$ and a positive integer $k$. By Lemma~\ref{lmm:savvy}, for any $n \in \Nats$, there exists ${S^{Mk}_{n}: \Ob^n \rightarrow \Gm(X)}$ measurable s.t. for all $y \in \Ob^n$, $S^{Mk}_{n}(y)$ is a $(M \mid y,\,\frac{1}{k})$-savvy... We fix such a $\upsilon^{M}_{kn}(y)$ for each $M$ and $k$. It is easy to see that the $\DKR^n$-diameter of $\PM(X)$ is at most 2, therefore $\N{\upsilon^{M}_{kn}(y)} \leq 2$ and $\Sqn{\upsilon^{M}_{kn}}$ is a trading metastrategy. By Theorem B.1, there is a stochastic market $F^\MC$ that dominates $T_{\upsilon^{M}_k}$ for all $M$ and $k$. By Corollary 1, for any $M \in \MC$, $\mu^* \in M$, positive integer $k$ and $\mu^*$-almost any $x \in X$

$$\limsup_{n \rightarrow \infty} {\DKR^n(M^H_n(x(n)),M_{x(n)})} \leq \frac{1}{k}$$

It follows that

$$\lim_{n \rightarrow \infty} {\DKR^n(M^H_n(x(n)),M_{x(n)})} = 0$$
