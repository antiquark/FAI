...
In this section we construct a tool for proving asymptotic convergence results about dominant forecasters. We start by introducing a notion of \enquote{prudent gambling,} which requires that gambles are only made when the expected payoff is a certain fraction of the risk.
...
Given a gambling strategy $S$ and $\mu \in \PMO$, $S$ is said to be \emph{$\mu$-prudent} when there are $\alpha > 0$ and...
