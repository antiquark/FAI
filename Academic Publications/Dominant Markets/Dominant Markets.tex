...
The \enquote{correct} way for a gambler to use a prudent gambling strategy is not employing it all the time: in order to avoid running out of budget, a gambler shouldn’t place too many bets simultaneously. To address this, we define a gambler that employs a strategy only when all previous bets are close to being settled.
...
\begin{equation}
\SV \PG{S}_n - \SVM \PG{S}_n \leq 2 \sup_{m \in \Nats} \sup_{y \in \Ob^m} \N{S(y)} + 1
\end{equation}
...
We prove by induction. For $n=0$ the claim is obvious. Consider any ${n \in \Nats}$ and ${x \in X}$. First, assume that

$$\max_{x' \in x_{:n}\OO} \SV \PG{S}^F_n(x_{:n-1},x')-\min_{x' \in x_{:n}\OO} \SV \PG{S}^F_n(x_{:n-1},x') > 1$$

Then, by definition of ${\PG{S}}$, ${\PG{S}^F_n}(x_{:n}) \equiv 0$ and therefore

$$\SV \PG{S}^F_{n+1}(x_{:n})=\SV \PG{S}^F_n(x_{:n-1})$$

$$\SVM \PG{S}^F_{n+1}(x_{:n}) \geq \SVM \PG{S}^F_n(x_{:n-1})$$

This obviously implies the induction step.

Now, assume that 

$$\max_{x' \in x_{:n}\OO} \SV \PG{S}^F_n(x_{:n-1},x')-\min_{x' \in x_{:n}\OO} \SV \PG{S}^F_n(x_{:n-1},x') \leq 1$$

Then, ${\PG{S}^F_n}(x_{:n}) = S_n(x_{:n})$ and therefore

$$\SV \PG{S}^F_{n+1}(x_{:n})=\SV \PG{S}^F_n(x_{:n-1}) + (\V{S}_n(x_{:n}))(F_{n}(x_{:n}))$$

We get

$$\SV \PG{S}^F_{n+1}(x?) - \SVM \PG{S}^F_{n+1}(x?) \leq \max_{x' \in x_{:n}\OO} \SV \PG{S}^F_n(x_{:n-1},x') - \min_{x' \in x_{:n}\OO}\SV \PG{S}^F_n(x_{:n-1},x') + 2 \N{S_n(x_{:n})}$$
...
$$X'_n(x):= \min_{x' \in x_{:n-1}\OO} \SV \PG{S}^F_{n}(x_{:n-1},x')$$

$$Y_n(x):=\max_{x' \in x_{:n}\OO} \overline{\PG{S}}^F_{n}(x_{:n},x') - \min_{x' \in x_{:n}\OO} \overline{\PG{S}}^F_{n}(x_{:n},x')$$
...
By Proposition B.15, ${\A{X_n-X'_n}}$ are uniformly bounded. ${X_0}$ is bounded and in particular ${\E[\A{X_0}] < \infty}$. We have

$$\A{X_{n+1}(x)-X_n(x)} = \A{\V \bar{T}_{\upsilon n}^M(x_{:n},x)} \leq Y_n(x)$$

Let $\beta > 0$ and $K_n \in \mu^* \mid \pi_{n\omega}$ be as in Definition 4. 

$$\E[X_{n+1} - X_n \mid \F_n] = \E_{x' \sim K_n(x_{:n})}[\V \bar{T}_{\upsilon n}^M(x_{:n},x')]$$

$$\E[X_{n+1} - X_n \mid \F_n] = \E_{x' \sim K_n(x_{:n})}[\bar{T}_{\upsilon n}^M(x_{:n},x')] - \E_{x' \sim M_n(x_{:n})}[\bar{T}_{\upsilon n}^M(x_{:n},x')]$$

By definition of $\PG{S}$, $\bar{T}_{\upsilon n}^M(x_{:n})$ is equal to either $\upsilon_n(x_{:n},M_n(x_{:n}))$ or 0. In either case, we get (almost everywhere)

$$\E[X_{n+1} - X_n \mid \F_n] \geq \beta (\max_{x_{:n}\OO} \bar{T}_{\upsilon n}^M(x_{:n}) - \min_{x_{:n}\OO} \bar{T}_{\upsilon n}^M(x_{:n})) = \beta Y_n$$

Applying Proposition A.4, we get...

Let $\F$, $S$, $S'$ and $\Delta$ be as in the proof of Proposition A.5. Using Proposition A.5 and the assumption that $M$ dominates $\PG{S}$, we conclude that for $\mu^*$-almost any $x \in X$, $\sup_n X_n(x) < \infty$. As in the proof of Proposition A.5, the conditions of Proposition A.4 are satisfied, and therefore the conditions of Proposition A.7 are also satisfied. Applying Proposition A.7, we conclude that $\sum_n Y_n(x) < \infty$. By Proposition B.17, it follows that for any $n \gg 0$, $T_{\upsilon n}^M(x_{:n}) = \upsilon(x_{:n})$. We get

$$\E_{K_n(x_{:n})}[\upsilon(x_{:n},M_n(x_{:n}))]-\E_{M_n(x_{:n})}[\upsilon(x_{:n},M_n(x_{:n}))] = \E_{K_n(x_{:n})}[\bar{T}_{\upsilon n}^M(x_{:n})]-\E_{M_n(x_{:n})}[\bar{T}_{\upsilon n}^M(x_{:n})]$$

$$\E_{K_n(x_{:n})}[\upsilon(x_{:n},M_n(x_{:n}))]-\E_{M_n(x_{:n})}[\upsilon(x_{:n},M_n(x_{:n}))] \leq Y_n(x)$$

$$\lim_{n \rightarrow \infty} (\E_{K_n(x_{:n})}[\upsilon(x_{:n},M_n(x_{:n}))]-\E_{M_n(x_{:n})}[\upsilon(x_{:n},M_n(x_{:n}))]) = 0$$
