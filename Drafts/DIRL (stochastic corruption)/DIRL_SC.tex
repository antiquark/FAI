...
By Lemma A and Proposition 2, $\bar{\Hy}^\kappa$ is learnable. Let $\pi^*$ be a metapolicy that learns it. For any $k \in \Nats$, $\pi^*$ learns $\bar{\psi}^k[\alpha^{k*}]$. It is easy to see that for any policy $\pi$, $\EU_{\bar{\psi}^k[\alpha^{k*}]}^\pi(t)=\EU_{\bar{\psi}^k[\alpha^{k}]}^\pi(t)$ ($\psi^k$ is defined s.t. once the corruption occurs, the reward is constant, so the advisor has no effect)???, therefore $\pi^*$ learns $\bar{\psi}^k[\alpha^{k}]$ . By Proposition A.7, we conclude that $\pi^*$ learns $\bar{\upsilon}^k[\alpha^k]$.

TODO: Probability of past corruption has to update on past behavior of $\alpha$!!!
