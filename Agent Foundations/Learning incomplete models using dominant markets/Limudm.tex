...
By Proposition N14, $\Supp \mu \subseteq \Argmax{} \tau(\mu)$ iff ${\E_\mu[\tau(\mu)]=\max \tau(\mu)}$. Consider ${\tau_k \rightarrow \tau}$, ${\mu_k \rightarrow \mu}$, ${(\tau_k,\mu_k) \in C}$. By Proposition N8, ${\tau_k(\mu_k) \rightarrow \tau(\mu)}$ and hence 
...
Combining, we conclude that $\E_{\mu_k}[\tau_k(\mu_k)] \rightarrow \E_{\mu}[\tau(\mu)]$ and ${(\tau,\mu) \in C}$.
...
Fix ${X}$ a compact Polish metric space and ${Z \subseteq X}$ closed. Consider any ${\mu \in \Prob(X)}$. Then, ${\mu \in \Prob(Z)}$ (equivalently ${\Supp \mu \subseteq Z}$) iff ${\E_\mu[d_Z] = 0}$.
