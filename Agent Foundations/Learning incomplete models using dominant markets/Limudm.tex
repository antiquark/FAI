...
Given ${X}$ a topological space, ${\Prob(X)}$ is the space of Borel probability measures on ${X}$ equipped with the weak* topology, ${\mathcal{B}}(X)$ is the Borel ${\sigma}$-algebra on ${X}$ and ${\UM(X)}$ is the ${\sigma}$-algebra of universally measurable sets on ${X}$. Given ${\mu \in \Prob(X)}$, ${\Supp \mu}$ denotes the support of ${\mu}$. ${\I_X: X \rightarrow X}$ is the identity mapping.

Given ${X}$ and ${Y}$ measurable spaces, ${K: X \Markov\ Y}$ is a Markov kernel from ${X}$ to ${Y}$. For any ${x \in X}$, we have ${K(x) \in \Prob(Y)}$.
...
A *metastrategy* is a family of bounded measurable mappings ${\{\upsilon: Y_n \rightarrow \T(X)\}_{n \in \Nats}}$. Given ${\mu^* \in \Prob(X)}$, ${\upsilon}$ is said said to be *profitable for ${\mu^*}$*, when there are ${c > 0}$ and ${\{K_n: Y_n \Markov X\}_{n \in \Nats}}$ s.t. ...
