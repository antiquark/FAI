...
Note that $\Supp \mu \subseteq \Argmax{} \tau(\mu)$ iff ${\E_\mu[\tau(\mu)]=\max \tau(\mu)}$. Consider ${\tau_k \rightarrow \tau}$, ${\mu_k \rightarrow \mu}$, ${(\tau_k,\mu_k) \in C}$. By Proposition N8, ${\tau_k(\mu_k) \rightarrow \tau(\mu)}$ and hence 
