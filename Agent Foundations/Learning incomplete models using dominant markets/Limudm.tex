...
By Proposition N21, ${S^0}$ is a submartingale. Let ${\Sqn{N_n:X \rightarrow \Nats \sqcup \{\infty\}}}$ be the accumulation times of ${T}$. It is easy to see that they are stopping times. By proposition N23, ${\Sqn{S^0_{\min(n,N_k)}}}$ are submartingales for all ${k}$. By Proposition\ N24, each of them is uniformly integrable. Using the fact that ${N_{k} \leq N_{k+1}}$ to apply Theorem N22, we get
...
We prove by induction on ${k}$. For ${k = 0}$, the claim is obvious. Consider any ${x \in X}$. If ${N_k(x) = \infty}$ then ${N_{k+1}(x)=\infty}$ and, by the induction hypothesis

$$\sum_{n = 0}^{N_{k+1}(x) - 1} T_n(x) = \sum_{n = 0}^{\infty} T_n(x) = \sum_{n = 0}^{N_{k}(x) - 1} T_n(x) \leq 2k \leq 2(k+1)$$

Now assume ${N_k(x) < \infty}$. Then ${N_{k+1}(x) > N_k(x)}$ (because for the sum in the definition of ${N}$ to be ${\geq 1}$ it has to be non-empty), therefore

$$\sum_{n = 0}^{N_{k+1}(x) - 1} T_n(x) = \sum_{n = 0}^{N_{k}(x) - 1} T_n(x) + \sum_{n = N_k(x)}^{N_{k + 1}(x) - 1} T_n(x)$$

By the induction hypothesis

$$\sum_{n = 0}^{N_{k+1}(x) - 1} T_n(x) \leq 2k + \sum_{n = N_k(x)}^{N_{k + 1}(x) - 1} T_n(x)$$

If ${N_{k+1}(x) < \infty}$ then

$$\sum_{n = 0}^{N_{k+1}(x) - 1} T_n(x) \leq 2k + \sum_{n = N_k(x)}^{N_{k + 1}(x) - 2} T_n(x) + T_{N_{k+1}(x)-1}(x)$$

By definition of ${N}$, the middle term is ${< 1}$. By definition of ${T}$, the last term is ${\leq 1}$. We get

$$\sum_{n = 0}^{N_{k+1}(x) - 1} T_n(x) < 2k + 1 + 1 = 2(k+1)$$

Finally, assume ${N_{k+1}(x) = \infty}$. We have

$$\sum_{n = 0}^{N_{k+1}(x) - 1} T_n(x) \leq 2k + \sum_{n = N_k(x)}^{\infty} T_n(x)$$

By definition of ${N}$, we get that for any ${m \in \Nats}$:

$$\sum_{n = N_k(x)}^{m} T_n(x) < 1$$

It follows that

$$\sum_{n = N_k(x)}^{\infty} T_n(x) \leq 1$$

Combining, we have

$$\sum_{n = 0}^{N_{k+1}(x) - 1} T_n(x) \leq 2k+1 <\ 2(k+1)$$
