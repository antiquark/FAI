...
Consider ${X}$ a probability space, ${\{\F_n \subseteq 2^X\}_{n \in \Nats}}$ a filtration of ${X}$, ${\{S_n:X \rightarrow \Reals\}_{n \in \Nats}}$ and ${\{T_n:X \rightarrow [0,1]\}_{n \in \Nats}}$ stochastic processes adapted to ${\F}$. Assume that there are ${\alpha, \beta > 0}$ s.t.:

$$\E[\Abs{S_0}] < \infty$$

$$\forall n' \geq n:\Abs{S_{n'}-S_{n}} \leq \sum_{m=n}^{n'-1} T_m + \alpha$$

$$\E[S_{n+1} - S_n \mid \F_n] \geq \beta T_n$$
...
$$S^0_n := S_n - \beta \sum_{m=0}^{n-1} T_m$$
...
$$\Prb[\exists k \in \Nats \forall l > k: S_{N_l} - S_0 \geq \beta (\sum_{n=0}^{N_l-1} T_n -  l)]=1$$

By Proposition N28

$$\Prb[\sum_{n=0}^\infty  T_n = \infty \implies \exists k \in \Nats \forall l > k: S_{N_l} - S_0 \geq 0]=1$$

It remains to show that if ${x \in X}$ is s.t. ${\inf_n S_n(x) = -\infty}$ then the condition above fails. Consider any such ${x \in X}$. ${\Abs{S_n(x) - S_0(x)} \leq \sum_{m=0}^{n-1} T_n(x) + \alpha}$, therefore ${\sum_{n=0}^\infty  T_n(x) = \infty}$. On the other hand, by Proposition N29, ${\inf_{k} S_{N_k(x)}(x) = -\infty}$.
 
\#Proposition N30

Consider ${X}$ a probability space, ${\{\F_n \subseteq 2^X\}_{n \in \Nats}}$ a filtration of ${X}$, ${\{S'_n:X \rightarrow \Reals\}_{n \in \Nats}}$ and ${\{T_n:X \rightarrow [0,1]\}_{n \in \Nats}}$ stochastic processes adapted to ${\F}$ and ${\Sqn{S_n:X \rightarrow \Reals}}$ an arbitrary stochastic process. Assume that there are ${\alpha,\beta > 0}$ s.t.:
...
$$\forall n' \geq n: \Abs{S''_{n'}-S''_n} \leq \Abs{S_{n'}-S_n} + \alpha \leq \sum_{m=n}^{n'-1} T_m + \alpha$$

$$\E[S''_{n+1}-S''_n \mid \F_n] = \E[\E[S_{n+1} \mid \F_{n+1}]-\E[S_n \mid \F_n] \mid \F_n] = \E[S_{n+1}-S_n \mid \F_n] \geq \beta T_n$$
...
We regard ${X}$ as a probability space using the ${\sigma}$-algebra ${\UM(X)}$ and the probability measure ${\mu^*}$. For any ${n \in \Nats}$, we define ${\F_n \subseteq \UM(X)}$, ${S_n,S'_n: X \rightarrow \Reals}$ and ${T_n: X \rightarrow [0,1]}$ by 
...
$$T_n(x):=\max_{z \in \pi_{n\omega}^{-1}(\pi_{n\omega}(x))} \bar{T}^M_{\upsilon n}(\pi_{n\omega}(x),z) - \min_{z \in \pi_{n\omega}^{-1}(\pi_{n\omega}(x))} \bar{T}^M_{\upsilon n}(\pi_{n\omega}(x),z)$$
...
$$\Abs{S_{n}-S_0} \leq \sum_{m=0}^{n-1} T_m + \alpha$$
...
Consider ${X}$ a probability space, ${\{\F_n \subseteq 2^X\}_{n \in \Nats}}$ a filtration of ${X}$, ${\{S_n:X \rightarrow \Reals\}_{n \in \Nats}}$ and ${\{T_n:X \rightarrow [0,1]\}_{n \in \Nats}}$ stochastic processes adapted to ${\F}$. Define ${\Sqn{U_n: X \rightarrow \Reals}}$ by
...
Let ${\{N_k: X \rightarrow \Nats \sqcup \{\infty\}\}_{k \in \Nats}}$ be the accumulation times of ${T}$. Then, for any ${k \in \Nats}$, ${\Sqn{S_{\min(n,N_k)}}}$ and ${\Sqn{U_{\min(n,N_k)}}}$ are uniformly integrable.
