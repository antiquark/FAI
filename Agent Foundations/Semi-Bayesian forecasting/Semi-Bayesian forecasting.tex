...
Define ${U: \Prob(S) \times \Prob(E) \rightarrow \Reals}$ by ${U(\pi,\mu):=\E_{\pi \times \mu}[u]}$. By Proposition A.3, ${U}$ is continuous. Define ${U_\Phi: \Prob(S) \rightarrow \Reals}$ by ${U_\Phi(\pi):= \inf_{\mu \in \Phi} U(\pi,\mu)}$. Given any ${\pi_\infty \in \Prob(S)}$ and a sequence ${\pi_n \rightarrow \pi_\infty}$, Proposition A.1 implies that ${U(\pi_n,\mu)}$ converges to ${U(\pi_\infty,\mu)}$ uniformly in ${\mu}$. It follows that ${U_\Phi(\pi_n) \rightarrow U_\Phi(\pi_\infty)}$, and thus ${U_\Phi}$ is continuous. Therefore, ${\pi^*}$ exists.
