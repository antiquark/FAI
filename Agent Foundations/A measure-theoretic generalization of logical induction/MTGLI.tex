...
$$\SWm \B T_m(\{\mu_l \in \Prob(X)\}_{l<m})=\SWm T_m(\{\mu_l \in \Prob(X)\}_{l<m}) > -b$$

(Note that the sum in the definition of ${\SWm \B T_m}$ only involves ${\B T_l}$ for ${l < m}$)

For ${m=m_0+1}$, we are still in the second case in the definition of ${\B T_n(\{\mu_m \in \Prob(X)\}_{m<n})}$??? We get

$$\SWm \B T_m(\{\mu_l \in \Prob(X)\}_{l<m}) = \min_{x \in X_m} \SW \B T_m(\{\mu_l \in \Prob(X)\}_{l<m},x)$$

$$\SWm \B T_m(\{\mu_l \in \Prob(X)\}_{l<m}) = \min_{x \in X_m} (\SW \B T_{m-1}(\{\mu_l \in \Prob(X)\}_{l<{m-1}},x) + \W T_{m-1}(\{\mu_l \in \Prob(X)\}_{l \leq {m-1}},x))$$
