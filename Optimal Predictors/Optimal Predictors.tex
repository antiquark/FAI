...
\label{eqn:thm__exists_smp__a_alg}A^K(\prod_{i \in [l(K)^4]}y_i z_i,w) &\in \Argmin{a \in \Bool^{l(K)}} L^K(a,\prod_{i \in [l(K)^4]}y_i z_i,w)
\end{align}
...
Consider any ${\alpha: \Nats^n \rightarrow \Words}$ s.t. ${\Abs{\alpha(K)}=l(K)}$. Define ${h_\alpha^K: \Words \Markov \Reals}$ by
...
\[\E_{\Un_G^K}[\E[(h_\alpha^K(G^K(y)_0)-G^K(y)_1)^2]] \overset{\alpha}{\equiv} \E_{\mu^{\eta(K)}}[\E[(h_\alpha^K(x)-f(x))^2]] + \E_{\Un_G^K}[(f(G^K(y)_0)-G^K(y)_1)^2] \pmod \Fall\]

\begin{equation}
\label{eqn:thm__exists_smp__prf1}
\E_{\Un_G^K}[\E[(h_\alpha^K(G^K(y)_0)-G^K(y)_1)^2]] \overset{\alpha}{\equiv} \varrho^K(\alpha(K)) + \varrho_0^K \pmod \Fall
\end{equation}

${L^K(\alpha(K),y)}$ is the average of ${l(K)^4}$ independent and and identically distributed random variables. By \ref{eqn:thm__exists_smp__prf1}, there is ${\varepsilon \in \Fall}$ that doesn't depend on ${\alpha}$ s.t. the expected value of these random variables is in ${[\varrho^K(\alpha(K)) + \varrho_0^K - \varepsilon(K), \varrho^K(\alpha(K)) + \varrho_0^K + \varepsilon(K)]}$. Applying Hoeffding's inequality we conclude that for some ${\epsilon \in \Reals^{>0}}$

\[\forall b \in \Bool^{l(K)}: \Prb_{\Un_L^K}[L^K(b,y) > \varrho^K(b) + \varrho_0^K  + \varepsilon(K) + l(K)^{-1}] \leq 2^{-\epsilon l(K)^2}\]

In particular, since for any ${b \in \Bool^{l(K)}}$, ${L^K(A^K(y),y) \leq L^K(b,y)}$

\begin{equation}
\label{eqn:thm__exists_smp__prf2}
\forall b \in \Bool^{l(K)}: \Prb_{\Un_L^K}[L^K(A^K(y),y) > \varrho^K(b) + \varrho_0^K + \varepsilon(K) + l(K)^{-1}] \leq 2^{-\epsilon l(K)^2}
\end{equation}
...
\[\Prb_{\Un_L^K}[\exists b \in \Bool^{l(K)}: L^K(b,y) < \varrho^K(b) + \varrho_0^K - \varepsilon(K) - l(K)^{-1}] \leq 2^{-\epsilon l(K)^2+l(K)}\]

\begin{equation}
\label{eqn:thm__exists_smp__prf3}
\Prb_{\Un_L^K}[L^K(A^K(y),y) < \varrho^K(A^K(y)) + \varrho_0^K - \varepsilon(K) - l(K)^{-1}] \leq 2^{-\epsilon l(K)^2+l(K)}
\end{equation}

Combining \ref{eqn:thm__exists_smp__prf2} and \ref{eqn:thm__exists_smp__prf3}, we conclude

\[\forall b \in \Bool^{l(K)}: \Prb_{\Un_L^K}[\varrho^K(A^K(y)) + \varrho_0^K - \varepsilon(K) - l(K)^{-1} > \varrho^K(b) + \varrho_0^K + \varepsilon(K) + l(K)^{-1}] \leq 2^{-\epsilon l(K)^2} + 2^{-\epsilon l(K)^2+l(K)}\]

\[\forall b \in \Bool^{l(K)}: \Prb_{\Un_L^K}[\varrho^K(A^K(y)) > \varrho^K(b) + 2(\varepsilon(K) + l(K)^{-1})] \leq 2^{-\epsilon l(K)^2+l(K)+1}\]
