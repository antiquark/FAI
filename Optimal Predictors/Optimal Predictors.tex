...
\subsubsection{Growth Spaces and \texorpdfstring{$\Gamma$-}{}Schemes}
...
Assume $\Fall$ is $\GrowA$-ample. Consider $(\mu,f)$ a distributional estimation problem, $P$ an $\EG$-optimal predictor for $(\mu,f)$ and $A,B: \bm{1} \Scheme \Rats$ s.t. $\R_A \equiv 0$ and $\R_B \equiv 0$. Denote ${\alpha(K):=\Prb_{\mu^{K_0} \times \U_P^{K}}[A^{K} \leq P^{K} \leq B^{K}]}$. Then, there is $\varepsilon \in \Fall$ s.t. 
...
\[\E_{\mu^{K_0} \times \U_{P_{\chi f}}^K \times \U_S^K}[(P_{\chi f}^K - \chi_D f)S^K] = \E_{\mu^{K_0} \times \U_{P_{\chi f}}^K \times \U_S^K}[(P_D^K - \chi_D) P_{f \mid D}^KS^K)] + \E_{\mu^{K_0} \times \U_{P_{\chi f}}^K \times \U_S^K}[\chi_D (P_{f \mid D}^K - f)S^K]\]

Using the fact $P_D^K$ is $\ESG$-optimal for $(\mu,\chi_D)$,

\[\E_{\mu^{K_0} \times \U_{P_{\chi f}}^K \times \U_S^K}[(P_{\chi f}^K - \chi_D f)S^K] \equiv  \E_{\mu^{K_0} \times \U_{P_{\chi f}}^K \times \U_S^K}[\chi_D (P_{f \mid D}^K - f)S^K] \pmod \Fall\]

\[\E_{\mu^{K_0} \times \U_{P_{\chi f}}^K \times \U_S^K}[(P_{\chi f}^K - \chi_D f)S^K] \equiv \mu^{K_0}(D)  \E_{(\mu^{K_0} \mid D) \times \U_{P_{\chi f}}^K \times \U_S^K}[(P_{f \mid D}^K - f)S^K] \pmod \Fall\]
 
Using the fact $P_{f \mid D}^K$ is $\Fall_D^\sharp(\Gamma)$-optimal for $(\mu \mid D, f)$, we conclude
 
 \[\Abs{\E_{\mu^{K_0} \times \U_{P_{\chi f}}^K \times \U_S^K}[(P_{\chi f}^K - \chi_D f)S^K]} \equiv 0 \pmod \Fall\]
%
\end{proof}
...
\[\E_{\mu_P^K \times \U_S^K}[(\tilde{P}_{\chi f}^K - \chi_Df) S^K(x,P_D^K,P_{\chi f}^K)]=\E_{\mu_P^K \times \U_S^K}[(\tilde{P}_{\chi f}^K - P_{\chi f}^K + P_{\chi f}^K - \chi_Df) S^K(x,P_D^K,P_{\chi f}^K)]\]

\[\E_{\mu_P^K \times \U_S^K}[(\tilde{P}_{\chi f}^K - \chi_D f) S^K]=\E_{\mu_P^K \times \U_S^K}[(\tilde{P}_{\chi f}^K - P_{\chi f}^K) S^K]+\E_{\mu_P^K \times \U_S^K}[( P_{\chi f}^K - \chi_Df) S^K]\]

Using the fact $P_{\chi f}$ is an $\ESG$-optimal predictor for $(\mu, \chi_D f)$, we get

\[\E_{\mu_P^K \times \U_S^K}[(\tilde{P}_{\chi f}^K - \chi_D f) S^K] \equiv \E_{\mu_P^K \times \U_S^K}[(\tilde{P}_{\chi f}^K - P_{\chi f}^K) S^K] \pmod \Fall\]

\[\Abs{\E_{\mu_P^K \times \U_S^K}[(\tilde{P}_{\chi f}^K - \chi_D f) S^K]} \leq \E_{\mu_P^K \times \U_S^K}[\Abs{\tilde{P}_{\chi f}^K - P_{\chi f}^K}] \sup S \pmod \Fall\]

Applying \ref{eqn:thm__cond__lemma__prf5} we conclude that

\[\E_{\mu_P^K \times \U_S^K}[(\tilde{P}_{\chi f}^K - \chi_D f) S^K] \equiv 0 \pmod \Fall\]
%
\end{proof}
...
Consider any $S: \Words \times \Rats \Scheme \Rats$ bounded. Denote ${\mu_{PS}^K:=\mu^{K_0} \times \U_{P_D}^K \times \U_{P_{\chi f}}^K \times \U_S^K}$.
...
\[\mu^{K_0}(D) \E_{(\mu^{K_0} \mid D)\times \U_{P_D}^K \times \U_{P_{\chi f}}^K \times \U_S^K}[(P_{f \mid D}^K - f)S^K(x,P_{f \mid D}^K)] \equiv 0 \pmod \Fall\]

\[\E_{(\mu^{K_0} \mid D)\times \U_{P_D}^K \times \U_{P_{\chi f}}^K \times \U_S^K}[(P_{f \mid D}^K - f)S^K(x,P_{f \mid D}^K)] \equiv 0 \pmod {\Fall_D}\]
%
\end{proof}
\subsection{\texorpdfstring{$\MGrow$}{M}-Schemes and Samplers}
...
\subsubsection{\texorpdfstring{$\MGrow$}{M}-Schemes}
