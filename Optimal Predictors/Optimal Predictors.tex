...
Assume that for any non-constant ${p \in \Nats[k]}$, ${\Fall p \subseteq \Fall}$. Consider an encoded set ${E}$ which is prefix-free, i.e. for all ${\phi,\psi \in E}$ and ${z \in \Bool^{>0}}$, ${\En_E(\phi) \ne \En_E(\psi)z}$. Consider ${\mathfrak{F}: E \times \Words \rightarrow \Reals}$ bounded. Define ${\eta^k: \Words^3 \rightarrow \Words^3}$ by ${\eta^k(a,b,w)=(a,b,\Ev^{k}(a;\En_\Nats(k),w))}$. Define the distributional estimation problem $({\mu_{\mathfrak{F}}},f_{\mathfrak{F}})$ by 

\begin{align}
\mu_{\mathfrak{F}}^k&:=\En_*^3 \eta_*^k(\Un^k \times \Un^k \times \Un^{k}) \\
f_{\mathfrak{F}}(\Chev{a,b,x}) &:= \begin{cases}\mathfrak{F}(\phi, x) \textnormal{ if } \exists z \in \Words: b=\En_E(\phi)z \\ 0 \textnormal{ if } \forall \phi \in E, z \in \Words: b \ne \En_E(\phi)z\end{cases}
\end{align}

For any ${\phi \in E}$, define ${f_\phi: \Words \rightarrow \Reals}$ by ${f_\phi(x)=\mathfrak{F}(\phi,x)}$. Then, for any ${\phi \in E}$ and ${\EG}$-samplable world ensemble ${\mu}$, there is a precise pseudo-invertible ${\EG}$-reduction from ${(\mu,f_\phi)}$ to ${(\mu_{\mathfrak{F}}, f_{\mathfrak{F}})}$ over a polynomial.
...
Let ${\sigma}$ be an ${\EG}$-sampler of ${\mu}$. Choose ${p \in \Nats[k]}$ and ${a \in \Words}$ s.t. for any ${k \in \Nats}$, ${z \in \Words}$, ${w_1 \in \Bool^{\R_\sigma(k)}}$ and ${w_2 \in \Words}$, ${\R_\sigma(k) \leq p(k)}$ and ${\Ev^{p(k)}(az;\En_\Nats(p(k)),w_1 w_2)=\sigma^k(w_1)}$. Denote ${b=\En_E(\phi)}$. We construct ${\pi: \Words \Scheme \Words}$ s.t. for any ${k \gg 0}$, ${x \in \Supp \sigma_{\bullet}^k}$, ${z_1 \in \Bool^{p(k)-\Abs{a}}}$ and ${z_2 \in \Bool^{p(k)-\Abs{b}}}$

\begin{align}
\R_\pi(k) &= 2p(k)-\Abs{a}-\Abs{b} \\
\label{eqn:thm__complete__red}\pi^k(x,z_1 z_2)&=\Chev{az_1,bz_2,x} 
\end{align}

We also ensure that for any ${k \gg 0}$, ${x \in \Words}$ and ${z_1,z_2}$ as above, either \ref{eqn:thm__complete__red} holds or ${\Abs{\pi^k(x,z_1 z_2)}=0}$.

To verify condition~\ref{con:def__pp_reduce__dist} of Definition~\ref{def:pp_reduce}, construct ${W: \Words \Scheme \Rats^{\geq 0}}$ s.t. 

\[W^k(y)=\begin{cases}2^{\Abs{a}+\Abs{b}} \text{ if } \exists z_1,z_2,x \in \Bool^{\leq p(k)}: y=\Chev{az_1, bz_2,x}\\0 \text{ otherwise}\end{cases}\]
...
\[\sum_{y \in \Words} \Abs{\mu_{\mathfrak{F}}^{p(k)}(y)W^k(y)-(\pi_*^k\mu^k)(y)} \equiv \sum_{z_1,z_2,x \in \Bool^{\leq p(k)}} \Abs{\mu_{\mathfrak{F}}^{p(k)}(\Chev{az_1, bz_2,x})2^{\Abs{a}+\Abs{b}}-(\pi_*^k\sigma_\bullet^k)(\Chev{az_1, bz_2,x})} \pmod \Fall\]

\[\sum_{y \in \Words} \Abs{\mu_{\mathfrak{F}}^{p(k)}(y)W^k(y)-\pi_*^k\mu^k(y)} \equiv \sum_{\substack{z_1 \in \Bool^{p(k)-\Abs{a}}\\z_2 \in \Bool^{p(k)-\Abs{b}}\\x \in \Bool^{\leq p(k)}}} \Abs{2^{-p(k)}2^{-p(k)}\sigma_\bullet^k(x) 2^{\Abs{a}+\Abs{b}}-(\pi_*^k\sigma_\bullet^k)(\Chev{az_1, bz_2,x})} \pmod \Fall\]

\[\sum_{y \in \Words} \Abs{\mu_{\mathfrak{F}}^{p(k)}(y)W^k(y)-\pi_*^k\mu^k(y)} \equiv \sum_{\substack{z_1 \in \Bool^{p(k)-\Abs{a}}\\z_2 \in \Bool^{p(k)-\Abs{b}}\\x \in \Bool^{\leq p(k)}}} \Abs{2^{-2p(k)+\Abs{a}+\Abs{b}}\sigma_\bullet^k(x) -(\pi_*^k\sigma_\bullet^k)(\Chev{az_1, bz_2,x})} \pmod \Fall\]

\[\sum_{y \in \Words} \Abs{\mu_{\mathfrak{F}}^{p(k)}(y)W^k(y)-\pi_*^k\mu^k(y)} \equiv \sum_{\substack{z_1 \in \Bool^{p(k)-\Abs{a}}\\z_2 \in \Bool^{p(k)-\Abs{b}}\\x \in \Bool^{\leq p(k)}}} \Abs{2^{-2p(k)+\Abs{a}+\Abs{b}}\sigma_\bullet^k(x) -2^{-(p(k)-\Abs{a})}2^{-(p(k)-\Abs{b})}\sigma_\bullet^k(x)} \pmod \Fall\]

\[\sum_{y \in \Words} \Abs{\mu_{\mathfrak{F}}^{p(k)}(y)W^k(y)-\pi_*^k\mu^k(y)} \equiv 0 \pmod \Fall\]
...
\[\E_{\mu^k \times \Un_\pi^k}[\Abs{f_\phi(x)-f_{\mathfrak{F}}(\pi^k(x,z))}] \equiv \E_{\sigma_\bullet^k \times \Un_\pi^k}[\Abs{f_\phi(x)-f_{\mathfrak{F}}(\Chev{az_1,bz_2,x})}] \pmod \Fall\]
...
To verify condition~\ref{con:def__pp_reduce__smp} of Definition~\ref{def:pp_reduce}, construct ${\tau: \Words \Scheme \Words}$ s.t. for any ${z_1,z_2 \in \Bool^{p(k)}}$ and ${x \in \Supp \sigma_\bullet^k}$, ${\tau^k(\Chev{z_1,z_2,x})=x}$. By Proposition~\ref{prp:prob_cong_push} and Proposition~\ref{prp:prob_cong_ev}
...
\[\E[\Dtv(\mu^k \mid (\pi^k)^{-1}(y),\tau_y^k)] \equiv \E_{(x,z_1,z_2) \sim \sigma_\bullet^k \times \Un^{p(k)-\Abs{a}} \times \Un^{p(k)-\Abs{b}}}[\Dtv(\mu^k \mid (\pi^k)^{-1}(\Chev{az_1,bz_2,x}),\tau_{\Chev{az_1,bz_2,x}}^k)] \pmod \Fall\]

\[\E[\Dtv(\mu^k \mid (\pi^k)^{-1}(y),\tau_y^k)] \equiv \E_{(x,z_1,z_2) \sim \sigma_\bullet^k \times \Un^{p(k)-\Abs{a}} \times \Un^{p(k)-\Abs{b}}}[\Dtv(\delta_x,\delta_x)] \pmod \Fall\]

\[\E[\Dtv(\mu^k \mid (\pi^k)^{-1}(y),\tau_y^k)] \equiv 0 \pmod \Fall\]
%
\end{proof}

Obviously ${\mu_{\mathfrak{F}}}$ is ${\EG}$-samplable. Therefore, if there is ${\phi^* \in E}$ s.t. ${f_{\phi^*} = f_{\mathfrak{F}}}$ then ${(\mu_{\mathfrak{F}},f_{\mathfrak{F}})}$ can be regarded as a complete problem for the corresponding class. This is the case for many interesting classes, e.g. ${\mathsf{NP}}$, ${\mathsf{PSPACE}}$, ${\mathsf{EXP}}$ (where we interpret decision problems as functions taking values in ${\Bool}$)???
