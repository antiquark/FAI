...
Consider $(\mu,f)$ a distributional estimation problem, ${P}$ an ${\ESG}$-optimal predictor for ${(\mu,f)}$ and ${d,l: \Nats \Alg \Nats}$. Assume that ${d,l \in \Gamma_{\textnormal{poly}}^n}$ as functions, ${\T_d, \T_l \in \Gamma_{\textnormal{poly}}^n}$ and ${l(d(K))=K}$. Then, ${P^d}$ is an ${\Fall d^\sharp(\Gamma d)}$-optimal predictor for ${(\mu^d,f)}$.
...
Consider any ${S: \Words \times \Rats \xrightarrow{\Gamma d} \Rats}$ bounded. We can construct ${\tilde{S}: \Words \times \Rats \Scheme \Rats}$ s.t. for any ${K \in \Nats^n}$ that satisfies ${d(l(K))=K}$, ${x,y,z \in \Words}$ and ${t \in \Rats}$ we have

\begin{align*}
\A_{\tilde{S}}(K)&=\A_S(l(K)) \\
\R_{\tilde{S}}^K(z)&=\R_S^{l(K)}(z) \\
\tilde{S}^K(x,t,y,z)&=S^{l(K)}(x,t,y,z)
\end{align*}

There is ${\varepsilon \in \Fall}$ s.t. for any ${K \in \Nats^n}$
...
Consider any ${Q: \Words \xrightarrow{\Gamma d} \Rats}$ bounded. We can construct ${\tilde{Q}: \Words \Scheme \Rats}$ s.t. for any ${K \in \Nats^n}$ that satisfies ${d(l(K))=K}$ and ${x,y,z \in \Words}$ we have

\begin{align*}
\A_{\tilde{Q}}(K)&=\A_Q(l(K)) \\
\R_{\tilde{Q}}^K(z)&=\R_Q^{l(K)}(z) \\
\tilde{Q}^K(x,y,z)&=Q^{l(K)}(x,y,z)
\end{align*}

There is ${\varepsilon \in \Fall}$ s.t.
...
Consider $(\mu,f)$, $(\nu,g)$ distributional estimation problems and ${\pi: \Words \Scheme \Words}$. $\pi$ is called a \emph{precise pseudo-invertible $\EG$-reduction of $(\mu,f)$ to $(\nu,g)$} when there are ${d,l: \Nats^n \Alg \Nats^n}$ s.t.

\begin{enumerate}[(i)]

\item As functions, ${d,l \in \Gamma_{\textnormal{poly}}^n}$

\item ${\T_d, \T_l \in \Gamma_{\textnormal{poly}}^n}$

\item $\forall K \in \Nats^n: l(d(K))=K$
...
By Corollary~\ref{crl:psp_reduce_sharp}, ${P \circ \pi}$ is an ${\ESG}$-optimal predictor for ???
