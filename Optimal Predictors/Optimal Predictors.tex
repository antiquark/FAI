...
Given $f,g: \NatFun \Reals$, the notation $f(K) \equiv g(K) \pmod \Fall$ means $\Abs{f-g} \in \Fall$. $f(K) \leq g(K) \pmod \Fall$ means $\exists \varepsilon \in \Fall \forall K \in \Nats^n: f(K) \leq g(K) + \varepsilon(K)$. Similarly, $f(K) \geq g(K) \pmod \Fall$ means $\exists \varepsilon \in \Fall \forall K \in \Nats^n: f(K) \geq g(K) - \varepsilon(K)$.
...
Consider a word ensemble $\mu$, an encoded set $Y$, a family ${\{\pi^K: \Supp \mu^{K} \Markov Y\}_{K \in \Nats^n}}$ and a uniformly bounded family ${\{h^K: (\Supp \mu) \times Y \rightarrow \Reals\}_{K \in \Nats^n}}$. Suppose $\sigma$ is an $\EMG$-sampler of $\mu$ relative to $\pi$. Then

\begin{equation}
\label{eqn:prp__smp}
\E_{(x,y) \sim \mu^{K} \ltimes \pi^K}[h^K(x,y)] \equiv \E_{(y,z) \sim \pi_*^K\mu^{K} \times \UM_\sigma^K}[h^K(\sigma^K(y,z),y)] \pmod \Fall
\end{equation}
...
$$\E_{(x,y) \sim \mu^{K} \ltimes \pi^K}[h^K(x,y)] = \E_{(x,y) \sim \mu^{K} \ltimes \pi^K}[\E_{x' \sim \mu_y^K}[h^K(x',y)]]$$

$$\E_{\mu^{K} \ltimes \pi^K}[h^K(x,y)] - \E_{\pi_*^K\mu^{K} \times \UM_\sigma^K}[h^K(\sigma^K(y,z),y)] = \E_{\mu^{K} \ltimes \pi^K}[\E_{\mu_y^K}[h^K(x',y)]] - \E_{\pi_*^K\mu^{K} \times \UM_\sigma^K}[h^K(\sigma^K(y,z),y)]$$

$$\E_{\mu^{K} \ltimes \pi^K}[h^K(x,y)] - \E_{\pi_*^K\mu^{K} \times \UM_\sigma^K}[h^K(\sigma^K(y,z),y)] = \E_{\mu^{K} \ltimes \pi^K}[\E_{\mu_y^K}[h^K(x',y)]-\E_{\UM_\sigma^K}[h^K(\sigma^K(y,z),y)]]$$

$$\E_{\mu^{K} \ltimes \pi^K}[h^K(x,y)] - \E_{\pi_*^K\mu^{K} \times \UM_\sigma^K}[h^K(\sigma^K(y,z),y)] = \E_{\mu^{K} \ltimes \pi^K}[\E_{\mu_y^K}[h^K(x',y)]-\E_{\sigma_y^K}[h^K(x',y)]]$$

$$\Abs{\E_{\mu^{K} \ltimes \pi^K}[h^K(x,y)] - \E_{\pi_*^K\mu^{K} \times \UM_\sigma^K}[h^K(\sigma^K(y,z),y)]} \leq \E_{\mu^{K} \ltimes \pi^K}[\Abs{\E_{\mu_y^K}[h^K(x',y)]-\E_{\sigma_y^K}[h^K(x',y)]}]$$

$$\Abs{\E_{\mu^{K} \ltimes \pi^K}[h^K(x,y)] - \E_{\pi_*^K\mu^{K} \times \UM_\sigma^K}[h^K(\sigma^K(y,z),y)]} \leq (\sup h - \inf h) \E_{\mu^{K} \ltimes \pi^K}[\Dtv(\mu_y^K,\sigma_y^K)]$$
...
Consider a distributional estimation problem $(\mu,f)$, an encoded set $Y$, a family ${\{\pi^K: \Supp \mu^{K} \Markov Y\}_{K \in \Nats^n}}$ and a uniformly bounded family ${\{h^K: (\Supp \mu) \times Y \rightarrow \Reals\}_{K \in \Nats^n}}$. Denote $\mu_\pi^K:=\mu^{K} \ltimes \pi^K$. Suppose $G$ is an $\EMG$-sampler of $(\mu,f)$ relative to $\pi$. Then

\begin{equation}
\E_{\mu_\pi^K}[h^K(x,y)f(x)] \equiv \E_{\pi_*^K\mu^{K} \times \UM_G^K}[h^K(G^K(y,z)_0,y)G^K(y,z)_1] \pmod \Fall
\end{equation}
...
$$\E_{\mu_\pi^K}[h^K(x,y)f(x)]-\E_{\mu_\pi^K}[h^K(x,y)f_G^K(x,y)]=\E_{\mu_\pi^K}[h^K(x,y)(f(x)-f_G^K(x,y))]$$

$$\Abs{\E_{\mu_\pi^K}[h^K(x,y)f(x)]-\E_{\mu_\pi^K}[h^K(x,y)f_G^K(x,y)]} \leq \E_{\mu_\pi^K}[\Abs{h^K(x,y)} \cdot \Abs{f(x)-f_G^K(x,y)}]$$

$$\Abs{\E_{\mu_\pi^K}[h^K(x,y)f(x)]-\E_{\mu_\pi^K}[h^K(x,y)f_G^K(x,y)]} \leq (\sup \Abs{h}) \E_{\mu_\pi^K}[\Abs{f(x)-f_G^K(x,y)}]$$

By property (ii) of Definition~\ref{def:smp_prob_rel}

$$\E_{\mu_\pi^K}[h^K(x,y)f(x)] \equiv \E_{\mu_\pi^K}[h^K(x,y)f_G^K(x,y)] \pmod \Fall$$

Using property (i) of Definition~\ref{def:smp_prob_rel} we can apply Proposition~\ref{prp:smp} to the right hand side and get

$$\E_{\mu_\pi^K}[h^K(x,y)f(x)] \equiv \E_{\pi_*^K\mu^{K} \times \UM_G^K}[h^K(G^K(y,z)_0,y) f_G^K(G^K(y,z)_0,y)] \pmod \Fall$$

$$\E_{\mu_\pi^K}[h^K(x,y)f(x)] \equiv \E_{\pi_*^K\mu^{K} \times \UM_G^K}[h(G^K(y,z)_0,y) \E_{z' \sim\UM_G^K}[G^K(y,z')_1 \mid G^K(y,z')_0 = G^K(y,z)_0]] \pmod \Fall$$
...
Fix ${l: \NatFun \Nats^{>0}}$ steadily growing. Denote ${\Gamma_{\textnormal{adv}}^n:=(\Gamma_0^n,\Gamma_l)}$. Fix ${r: \bm{1} \xrightarrow{\Gamma_{\textnormal{adv}}}\Nats}$ steadily growing. Assume ${\GrowR=\Gamma_r}$, ${\GrowA=\Gamma_l}$. Define ${\eta: \NatFun \Nats^{n-1}}$ by

\[\forall J \in \Nats^{n-1}, k \in \Nats: \eta(J,k)=J\]

Consider ${(\mu,f)}$ a distributional estimation problem of rank ${n-1}$. Then, there exists an ${\FallU(\Gamma)}$-optimal estimator for ${(\mu^\eta,f)}$.
...
\label{eqn:thm__exists_all__q_adv}\exists a_Q^K \in \Bool^{l(q(K))} \forall i \in \Nats, x,z \in \Words, y \in \BoolR{Q}: D(\Ev^{q(K)+i}(a_Q^K;x,yz))&=Q^K(x,y)
\end{align}
...
In particular, ${r}$ is steadily growing. Assume ${\Fall \supseteq \FallU}$, ${\GrowR=\Gamma_r}$ and ${\GrowA=\Gamma_\textnormal{log}^n}$. Consider ${(\mu,f)}$ an distributional estimation problem of rank ${n-1}$ and ${G}$ an ${\EG}$-sampler of ${(\mu^\eta,f)}$. Then, there exists ${P}$ an ${\EG}$-optimal estimator for ${(\mu^\eta,f)}$ s.t. ${\A_P=\A_G}$. In particular, if ${G}$ is uniform (i.e. ${\A_G \equiv \Estr}$) then so is ${P}$.
...
Consider ${(\mu,f)}$ a distributional estimation problem, ${G}$ an ${\EG}$-sampler of ${(\mu,f)}$ and ${\{h^K: \Words \Markov \Reals\}_{K \in \Nats^n}}$ uniformly bounded. Then

\begin{equation}
\label{eqn:prp__est_err_smp}
\E_{\Un_G^K}[\E[(h^K \circ G^K_0-G^K_1)^2]] \equiv \E_{\mu^K}[\E[(h^K-f)^2]] + \E_{\Un_G^K}[(f \circ G^K_0-G^K_1)^2] \pmod \Fall
\end{equation}
...
Denote ${h_G^K := h^K \circ G^K_0}$, ${f_G^K := f \circ G^K_0}$. Proposition~\ref{prp:smp} implies

\[\E_{\Un_G^K}[(\E[h_G^K]-f_G^K)f_G^K] \equiv \E_{\mu^K}[(\E[h^K]-f)f] \pmod \Fall\]

Applying Proposition~\ref{prp:gen} to the right hand side

\begin{equation}
\label{eqn:prp__est_err_smp__prf}
\E_{\Un_G^K}[(\E[h_G^K]-f_G^K)f_G^K]] \equiv \E_{\Un_G^K}[(\E[h_G^K]-f_G^K)G^K_1] \pmod \Fall
\end{equation}

On the other hand

\[\E_{\Un_G^K}[\E[(h_G^K-G^K_1)^2]] = \E_{\Un_G^K}[\E[(h_G^K-f_G^K+f_G^K-G^K_1)^2]]\]

\[\E_{\Un_G^K}[\E[(h_G^K-G^K_1)^2]] = \E_{\Un_G^K}[\E[(h_G^K-f_G^K)^2]]+2\E_{\Un_G^K}[(\E[h_G^K]-f_G^K)(f_G^K-G^K_1)]]+\E_{\Un_G^K}[\E[(f_G^K-G^K_1)^2]]\]
...
Fix ${D}$ an ${f}$-compatible decoding. Denote ${l(K):=\Floor{\log (K_{n-1} + 2)}}$. Construct ${L: \Words \Scheme \Rats}$ s.t. for any ${K \in \Nats^n}$, $w \in \Words$, ${a \in \Bool^{l(K)}}$, ${\{y_i \in \Bool^{r_G(K,w)}\}_{i \in [l(K)^3]}}$ and\\ ${\{z_i \in \Bool^{r(K)}\}_{i \in [l(K)^3]}}$

\begin{align}
\label{eqn:thm__exists_smp__l_adv}\A_L(K) &= \A_G(K) \\
\label{eqn:thm__exists_smp__l_rnd}\R_L(K,w) &= l(K)^3 (\R_G(K,w) + r(K)) \\
\label{eqn:thm__exists_smp__l_alg}L^K(a,\prod_{i \in [l(K)^3]}y_i z_i,w) &= \frac{1}{l(K)^3}\sum_{i \in [l(K)^3]}(D(\Ev^{K_{n-1}}(a;G^K(y_i,w)_0,z_i))-G^K(y_i,w)_1)^2 
\end{align}

That is, ${L}$ generates ${l(K)^3}$ estimates of ${f}$ using ${G}$ and computes the \enquote{empirical risk} of the program ${a}$ w.r.t. these estimates. Here, \ref{eqn:thm__exists_smp__l_rnd} is legitimate due to Proposition~\ref{prp:std_grow_log}. 

Construct ${A: \bm{1} \Scheme \Words}$ s.t. for any ${K \in \Nats^n}$, ${w \in \Words}$, ${\{y_i \in \Bool^{r_G(K,w)}\}_{i \in [l(K)^3]}}$ and\\ ${\{z_i \in \Bool^{r(K)}\}_{i \in [l(K)^3]}}$

\begin{align}
\label{eqn:thm__exists_smp__a_adv}\A_A(K) &= \A_G(K) \\
\label{eqn:thm__exists_smp__a_rnd}\R_A(K,w) &= \R_L(K,w) \\
\label{eqn:thm__exists_smp__a_alg}A^K(\prod_{i \in [l(K)^3]}y_i z_i,w) &\in \Argmin{a \in \Bool^{l(K)}} L^K(a,\prod_{i \in [l(K)^3]}y_i z_i,w)
\end{align}

Finally, construct ${P: \Words \Scheme \Rats}$ s.t. for any ${K \in \Nats^n}$, ${w \in \Words}$, ${\{y_i \in \Bool^{r_G(K,w)}\}_{i \in [l(K)^3]}}$,\\ ${\{z_i \in \Bool^{r(K)}\}_{i \in [l(K)^3]}}$ and ${z_* \in \Bool^{r(K)}}$

\begin{align}
\label{eqn:thm__exists_smp__p_adv}\A_P(K) &= \A_G(K) \\
\label{eqn:thm__exists_smp__p_rnd}\R_P(K,w) &= \R_L(K,w) + r(K) \\
\label{eqn:thm__exists_smp__p_alg}P^K(x,(\prod_{i \in [l(K)^3]}y_i z_i)z_{*},w) &= D(\Ev^{K_{n-1}}(A^K(\prod_{i \in [l(K)^3]}y_i z_i,w);x,z_{*}))
\end{align}

Consider any ${a: \Nats^n \rightarrow \Words}$ s.t. ${\Abs{a(K)}=l(K)}$. We can use Proposition~\ref{prp:est_err_smp} to show

\[\E_{\Un_L^K}[L^K(a(K),y)] \equiv \E_{\mu^{\eta(K)} \times \Un^{r(K)}}[(D(\Ev^{K_{n-1}}(a(K);x,z))-f(x))^2]+\E_{\Un_G^K}[(f(G^K(y)_0)-G^K(y)_1)^2] \pmod \Fall\]

Denote the first term on the right hand side ${\varrho^K(a)}$ and the second term on the right hand side ${\varrho_0^K}$. ${L^K(a(K),y)}$ is the average of ${l(K)^3}$ independent and identically distributed random variables. By Proposition~\ref{prp:est_err_smp}, there is ${\varepsilon \in \Fall}$ s.t. the expected value of these random variables is at most ${\varrho_0^K + \varrho^K(a) + \varepsilon(K)}$. Applying Hoeffding's inequality we conclude that for some ${\epsilon \in \Reals^{>0}}$

\[\Prb_{\Un_L^K}[L^K(a(K),y) > \varrho_0^K + \varrho^K(a) + \varepsilon(K) + l(K)^{-1}] \leq 2^{-\epsilon l(K)^3 l(K)^{-2}}\]

\[\Prb_{\Un_L^K}[L^K(a(K),y) > \varrho_0^K + \varrho^K(a) + \varepsilon(K) + l(K)^{-1}] \leq 2^\epsilon (K_{n-1}+2)^{-\epsilon} \pmod \Fall\]

In particular, since ${L^K(A^K(y),y) \leq L^K(a(K),y)}$

\[\Prb_{\Un_L^K}[L^K(A^K(y),y) > \varrho_0^K + \varrho^K(a) + \varepsilon(K) + l(K)^{-1}] \leq 2^\epsilon (K_{n-1}+2)^{-\epsilon} \pmod \Fall\]
...
% Explain relation between ${\FallUni}$ and quasi-polynomial time (perfect estimators)
