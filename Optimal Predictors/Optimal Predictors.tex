...
\item Denote ${\bar{g}: \Words \rightarrow \Reals}$ the extension of $g$ by $0$.  We require ${\E_{(x,z) \sim \mu^{K_0} \times \Un_\pi^{K}}[\Abs{f_1(x)-\bar{g}(\pi^{K}(x,z))}] \in \Fall}$
...
\item Denote ${\bar{g}: \Words \rightarrow \Reals}$ the extension of $g$ by 0. We require $\E_{(x,z) \sim \mu^{K_0} \times \Un_\pi^{K}}[\Abs{f(x)-\bar{g}(\pi^{K}(x,z))}] \in \Fall$
...
\item Denote ${\bar{g}: \Words \rightarrow \Reals}$ the extension of $g$ by 0. We require $\E_{(x,z) \sim \mu^{K_0}}[\Abs{f(x)-\E_{\Un_\pi^{K}}[g(\pi^{K}(x,z))]}] \in \Fall$
...
Suppose $\gamma \in \Gamma_{\textnormal{poly}}^n$ is s.t. $\gamma^{-1} \in \Fall$. Consider $(\mu,f)$, $(\nu,g)$ distributional estimation problems, $\pi$ a strict pseudo-invertible $\EG$-reduction of $(\mu, f)$ to $(\nu, g)$ and $P_g$ an $\ESG$-optimal predictor for $(\nu, g)$. Assume $\gamma (\R_P + \R_\pi) \in \GrowR$. Construct ${P_f}$ s.t.

\begin{align}
\R_{P_f}(K) &= \gamma(K) (\R_{P_g}(K) + \R_\pi(K)) \\
P_f^K(x, \prod_{i \in [\gamma(K)]} y_i z_i) &= \frac{1}{\gamma(K)}\sum_{i \in [\gamma(K)]} P_g^K(\pi^K(x,z_i),y_i)
\end{align}
...
Suppose $\gamma \in \Gamma_{\textnormal{poly}}^n$ is s.t. $\gamma^{-1} \in \Fall$. Consider $(\mu,f)$ a distributional estimation problem, $\bar{g}: \Words \rightarrow \Reals$, $\pi: \Words \Scheme \Words$. Assume ${\E_{(x,z) \sim \mu^{K_0}}[\Abs{f(x)-\E_{\Un_\pi^{K}}[\bar{g}(\pi^{K}(x,z))]}] \in \Fall}$. Then,

\begin{equation}
%\label{eqn:tbd}
\E_{(x,z_0, z_1 \ldots z_{\gamma(K)-1}) \sim \mu^{K_0} \times \prod_{i \in [\gamma(K)]} U_\pi^K}[\Abs{f(x)-\frac{1}{\gamma(K)} \sum_{i \in [\gamma(K)]} g(\pi^K(x,z_i))}] \in \Fall
\end{equation}
...
Consider any $S: \Words \times \Rats \Scheme \Rats$ bounded.

\[\E_{\mu^{K_0} \times \Un_{P_f}^K \times \Un_S^K}[(P_f^K(x) - f(x))S(x,P_f^K(x))] = \frac{1}{\gamma(K)}\sum_{i \in [\gamma(K)]} \E_{\mu^{K_0} \times \Un_{P_f}^K \times \Un_S^K}[(P_g^K(\pi^K(x,z_i),y_i)-f(x))S(x,P_f^K(x))]\]
