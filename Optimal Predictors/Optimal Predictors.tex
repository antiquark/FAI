...
Consider any ${S: \Words \times \Rats \xrightarrow{\Gamma d} \Rats}$ bounded. We can construct ${\tilde{S}: \Words \times \Rats \Scheme \Rats}$ s.t. if ${d(l(K))=K}$ then ${\R_{\tilde{S}}(K)=\R_S(l(K))}$ and ${\tilde{S}^K(x,y)=S^{l(K)}(x,y)}$? There is ${\varepsilon \in \Fall}$ s.t.

\[\Abs{\E_{\mu^{K} \times \Un_P^K \times \Un_{S^l}^K}[(P^K(x,y) - f(x))(S^l)^K(x,P^K(x,y),z)]}=\varepsilon(K)\]

Substituting ${d(K)}$ for ${K}$, we get

\[\Abs{\E_{\mu^{d(K)} \times \Un_P^{d^n(K)} \times \Un_{S^l}^{d^n(K)}}[(P^{d^n(K)}(x,y) - f(x))(S^l)^{d^n(K)}(x,P^{d^n(K)}(x,y),z)]}=\varepsilon(d^n(K))\]

\[\Abs{\E_{(\mu^d)^{K} \times \Un_{P^d}^{K} \times \Un_{S}^{K}}[((P^d)^{K}(x,y) - f(x))S^{K}(x,(P^d)^{K}(x,y),z)]}=\varepsilon(d^n(K))\]
%
\end{proof}
...
\[\E_{\mu^{K} \times \Un_P^K}[(P^K(x,y)-f(x))^2] \leq \E_{\mu^{K} \times \Un_{Q^l}^K}[((Q^l)^K(x,y)-f(x))^2] + \varepsilon(K)\]

Substituting ${d^n(K)}$ for ${K}$, we get

\[\E_{\mu^{d(K)} \times \Un_P^{d^n(K)}}[(P^{d^n(K)}(x,y)-f(x))^2] \leq \E_{\mu^{d(K)} \times \Un_{Q^l}^{d^n(K)}}[((Q^l)^{d^n(K)}(x,y)-f(x))^2] + \varepsilon({d^n(K)})\]

\[\E_{(\mu^d)^{K} \times \Un_{P^d}^K}[((P^d)^K(x,y)-f(x))^2] \leq \E_{(\mu^d)^{K} \times \Un_Q^K}[(Q^K(x,y)-f(x))^2] + \varepsilon(d^n(K))\]
%
\end{proof}
...
%In many cases of interest, ${\Fall d \subseteq \Fall}$, ${\GrowR d = \GrowR}$ and ${\GrowA d = \GrowA}$ for \emph{any} admissible ${d}$ so ${P^d}$ is actually an ${\ESG}$-optimal predictor for ${(\mu^d,f)}$. Naturally, the same remark applies to the subsequent results of this form.
TBD
