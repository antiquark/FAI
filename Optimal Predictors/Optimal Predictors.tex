...
\item\label{con:def__family__time} There is a polynomial $p_1: \Nats^n \rightarrow \Nats$ s.t. for any $S \in F$, $K \in \Nats^n$, $x \in X$ and $y \in \WordsLen{\R_S(K)}$

\[\T_S(K,x,y,\A_S(K)) \leq p_1(K)\]

\item\label{con:def__family__rtime} There is a polynomial $p_2: \Nats^n \rightarrow \Nats$ s.t. for any $S \in F$ and $K \in \Nats^n$

\[\T_{\R_S}(K,\A_S(K)) \leq p_2(K)\]

\item\label{con:def__family__rand} $\max_{S \in F} \R_S \in \GrowR$

\item\label{con:def__family__adv} There is $l \in \GrowA$ s.t. for any $S \in F$ and $K \in \Nats^n$
...
Note that the concept of a uniform family is non-trivial only when $1 \in \GrowA$, otherwise condition~\ref{con:def__family__adv} allows only families of at most one member.
...
Consider $\mu$ a word ensemble, $\bar{g}: \Words \rightarrow \Reals$ bounded and $\pi: \Words \Scheme \Words$. Suppose $\gamma \in \Gamma_{\textnormal{poly}}^n$ is s.t. $\gamma^{-\frac{1}{2}} \in \Fall$. Then,

\begin{equation}
%\label{eqn:tbd}
\E_{(x,z_0, z_1 \ldots z_{\gamma(K)-1}) \sim \mu^{K_0} \times \prod_{i \in [\gamma(K)]} \Un_\pi^K}[\Abs{\E_{z \sim \Un_\pi^{K}}[\bar{g}(\pi^{K}(x,z))]-\frac{1}{\gamma(K)} \sum_{i \in [\gamma(K)]} \bar{g}(\pi^K(x,z_i))}] \in \Fall
\end{equation}
...
Using the construction of $P_f$ and Proposition~\ref{prp:ev_equiv_mean}, we get
...
Suppose $\gamma \in \Gamma_{\textnormal{poly}}^n$ is s.t. $\gamma^{-\frac{1}{2}} \in \Fall$. Consider $(\mu,f)$, $(\nu,g)$ distributional estimation problems, $\pi$ a strict pseudo-invertible $\EG$-reduction of $(\mu, f)$ to $(\nu, g)$ and $P_g$ an $\EG$-optimal predictor for $(\nu, g)$. Assume $\gamma (\R_P + \R_\pi) \in \GrowR$. Construct ${P_f}$ according to \ref{eqn:thm__sp_reduce__rpf} and \ref{eqn:thm__sp_reduce__pf}. Then, $P_f$ is an $\EG$-optimal predictor for ${(\mu,g)}$.
...
Let ${L}$ be an ${\EMG}$-sampler of ${\mu}$ relative to ${\pi}$. Consider any ${Q_f: \Words \Scheme \Rats}$ bounded. Construct ${Q_g: \Words \MScheme \Rats}$ s.t. for any ${z_L \in \UM_L^K}$, ${z_Q \in \Supp \UM_{Q_f }^K}$, ${z_f \in \Supp \UM_{P_f}^K}$ and ${z_g \in \Supp \UM_{P_g}^K}$

\begin{align*}
\M_{Q_g}^K &= c_*^4(\M_L^K \times \M_{Q_f}^K \times \M_{P_f}^K \times \M_{P_g}^K) \\
\R_{Q_g}^K(\Chev{z_{L1}, z_{Q1},z_{f1},z_{g1}}) &= \R_L^K(z_{L1}) + \R_{Q_f}^K(z_{Q1}) + \R_{P_f}^K(z_{f1}) + \R_{P_g}^K(z_{g1}) \\
Q_g^K(x,z_{L0} z_{Q0} z_{f0} z_{g0}, \Chev{z_{L1}, z_{Q1},z_{f1},z_{g1}}) &= Q_f^K(L^K(x,z_L),z_Q)-P_f^K(L^K(x,z_L),z_f)+P_g^K(x,z_g)
\end{align*}
...
\[\E_{\mu^{K_0} \times \Un_\pi^K \times \Un_{P_g}^K}[((P_g \circ \pi)^K-\bar{g} \circ \pi^K)^2] \leq \E_{\mu^{K_0} \times \Un_\pi^K \times \Un_{Q_f}^K \times \Un_{P_f}^K \times \Un_{P_g}^K}[(Q_f ^K - P_f^K+(P_g \circ \pi)^K - \bar{g} \circ \pi^K)^2] \pmod \Fall\]
...
\[\E_{\mu^{K_0} \times \Un_{P_g}^K}[\E_{\Un_\pi^K}[(P_g \circ \pi)^K-\bar{g} \circ \pi^K]^2] \leq \E_{\mu^{K_0} \times \Un_{Q_f}^K \times \Un_{P_f}^K \times \Un_{P_g}^K}[(Q_f ^K - P_f^K+\E_{\Un_\pi^K}[(P_g \circ \pi)^K - \bar{g} \circ \pi^K])^2] \pmod \Fall\]

Denote $y_i:=\pi^K(x,z_i)$ where the ${z_i}$ are sampled independently from ${\Un_\pi^K}$. Applying Proposition~\ref{prp:sq_diff_cong} via Proposition~\ref{prp:ev_equiv_mean} we get

% TODO: Don't multisample P_g's coin-flips in definition P_f (only \pi's coinflips)

\[\E[(\frac{1}{\gamma(K)}\sum_{i \in [\gamma(K)]}(P_g^K(y_i,w_i???)-\bar{g}(y_i)))^2] \leq \E[(Q_f ^K - P_f^K+\frac{1}{\gamma(K)}\sum_{i \in [\gamma(K)]}(P_g^K(y_i,w_i???)-\bar{g}(y_i)))^2] \pmod \Fall\]
