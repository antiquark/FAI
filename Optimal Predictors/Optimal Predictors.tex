...
Then, ${P}$ is an ${\FallU(\Gamma)}$-perfect predictor for ${(\mu,f)}$.
...
In the setting of Proposition~\ref{prp:tally_perfect}, any ${\EG}$-optimal predictor for ${(\mu,f)}$ has to be an ${\EG}$-perfect predictor. In particular, if no ${\EG}$-perfect predictor exists then no ${\EG}$-optimal predictor exists. Similarly, if no \emph{uniform} ${\EG}$-perfect predictor exists then no uniform ${\EG}$-optimal predictor exists (and likewise for any other condition on the predictor).
...
Suppose ${\Fall^{\frac{1}{2}}}$ is ${\GrowA}$-ample. Consider ${D \subseteq \Nats^n}$ and ${\mu}$ a word ensemble s.t. given ${K_1, K_2 \in \Nats^n}$, if ${K_1 \ne K_2}$ then ${\Supp \mu^{K_1} \cap \Supp \mu^{K_2} = \varnothing}$. Define ${\chi: \Supp \mu \rightarrow \Bool}$ by requiring that for any ${K \in \Nats^n}$ and ${x \in \Supp \mu^K}$, ${\chi(x)=\chi_D(K)}$. Assume ${L}$ is an ${\EG}$-sampler of ${\mu}$ and ${P}$ is an ${\EG}$-optimal predictor for ${(\mu, \chi)}$. Then there is ${A: \bm{1} \Scheme \Bool}$ s.t. ${\A_A(K)=\Chev{\A_L(K),\A_P(K)}}$ and
...
\begin{align*}
\R_A(K) &= \R_L(K)+\R_P(K) \\
A^K(y_1 y_2) &= \begin{cases}0 \text{ if } P^K(L^K(y_1),y_2) \leq \frac{1}{2} \\ 1 \text{ if } P^K(L^K(y_1),y_2) > \frac{1}{2} \end{cases}
\end{align*}

We get

\[\Prb_{y \sim \Un_A^K}[A^K(y) \neq \chi_D(K)] \leq \Prb_{y_1 \sim \Un_L^K,y_2 \sim \Un_P^K}[\Abs{P^K(L^K(y_1),y_2) - \chi_D(K)} \geq \frac{1}{2}]\]

\[\Prb_{y \sim \Un_A^K}[A^K(y) \neq \chi_D(K)] \leq \Prb_{y_1 \sim \Un_L^K,y_2 \sim \Un_P^K}[(P^K(L^K(y_1),y_2) - \chi_D(K))^2 \geq \frac{1}{4}]\]

\[\Prb_{y \sim \Un_A^K}[A^K(y) \neq \chi_D(K)] \leq 4 \E_{y_1 \sim \Un_L^K,y_2 \sim \Un_P^K}[(P^K(L^K(y_1),y_2) - \chi_D(K))^2]\]
...
By Proposition~\ref{prp:tally_perfect}, ${P}$ is an ${\EG}$-perfect predictor for ${(\mu,\chi)}$, therefore

\[\Prb_{y \sim \Un_A^K}[A^K(y) \neq \chi_D(K)] \equiv 0 \pmod \Fall\]
%
\end{proof}

Again, the statement can be reversed to disprove existence of ${\EG}$-optimal estimators.

Now we consider the special case ${\Fall=\FallU}$, ${\GrowR = \GammaPoly}$. 

\begin{samepage}
\begin{proposition}
\label{prp:tbd}

Suppose ${\GrowR = \GammaPoly}$ and ${\FallU^{\frac{1}{2}}}$ is ${\GrowA}$-ample. Consider ${D \subseteq \Nats^n}$ and ${\mu}$ a word ensemble s.t. given ${K_1, K_2 \in \Nats^n}$, if ${K_1 \ne K_2}$ then ${\Supp \mu^{K_1} \cap \Supp \mu^{K_2} = \varnothing}$. Define ${\chi: \Supp \mu \rightarrow \Bool}$ by requiring that for any ${K \in \Nats^n}$ and ${x \in \Supp \mu^K}$, ${\chi(x)=\chi_D(K)}$. Assume ${L}$ is an ${\FallU(\Gamma)}$-sampler of ${\mu}$ and ${P}$ is an ${\FallU(\Gamma)}$-optimal predictor for ${(\mu, \chi)}$. Then...
