...
$\Words$ is the set of all finite binary strings (words), i.e. $\Words:=\bigsqcup_{n \in \Nats} \Bool^n$. For any $x \in \Words$, $\Abs{x}$ is the length of $x$ i.e. $x \in \WordsLen{\Abs{x}}$. For any $n \in \Nats$, $\Bool^{\leq n}:=\{x \in \Words \mid \Abs{x} \leq n\}$, $\Bool^{>n}:=\{x \in \Words \mid \Abs{x} > n\}$. For any $x \in \Words$ and $n \in \Nats$, $x_{\leq n}$ stands for the prefix of $x$ of length $n$ if $\Abs{x} \geq n$ and $x$ otherwise. Given $x,y \in \Words$, $xy$ stands for the concatenation of $x$ and $y$ (in particular $\Abs{xy}=\Abs{x}+\Abs{y}$). Given $n \in \Nats$ and $x,y \in \WordsLen{n}$, $x \cdot y$ stands for $\bigoplus_{i \in [n]} x_i y_i$. For any $n \in \Nats$, $\U^n$ is the uniform probability distribution on $\WordsLen{n}$.
...
Fix $n \in \Nats$, $\Gamma$ a pair of growth spaces of rank $n$ and $\Fall$ a fall space of rank $n$. Consider $(\mu,f)$ a distributional estimation problem and $P: \Words \Scheme \Rats$ with bounded range. $P$ is called an \emph{$\Fall(\Gamma)$-optimal predictor for $(\mu,f)$} when for any $Q: \Words \Scheme \Rats$

\begin{equation}
\label{eqn:op}
\E_{\mu^{K_0} \times \U_P^K}[(P^K - f)^2] \leq \E_{\mu^{K_0} \times \U_Q^K}[(Q^K - f)^2] \pmod \Fall
\end{equation}
...
\begin{equation}
\E_{\mu^{K_0} \times \U_P^{K}}[(P^{K} - f)^2] \leq \E_{\mu^{K_0} \times \U_Q^{K}}[(Q^{K} - f)^2] + \varepsilon(K)
\end{equation}
...
For any $K \in \Nats^n$, $\{\E_{\mu^{K_0} \times \U_Q^{K}}[(Q^{K} - f)^2] \mid Q \in F\}$ is a finite set because $F$ is a uniform family so the runtime of $Q^{K}$ is bounded by a polynomial in $K$ that doesn't depend on $Q$. Therefore we can choose ${Q_{K} \in \Argmin{Q \in F} \E_{\mu^{K_0} \times \U_Q^{K}}[(Q^{K} - f)^2]}$. By Proposition~\ref{prp:fam_diag}, there is $\bar{Q}: \Words \Scheme \Rats$ s.t. $\bar{Q}^{K}(x)$ is distributed the same as $Q_{K}^{K}(x)$.

Since $P$ is an $\Fall(\Gamma)$-optimal predictor, there is $\varepsilon \in \Fall$ s.t.

\begin{equation}
\label{eqn:prp__unif__prf1}
\E_{\mu^{K_0} \times \U_P^{K}}[(P^{K} - f)^2] \leq \E_{\mu^{K_0} \times \U_{\bar{Q}}^{K}}[(\bar{Q}^{K} - f)^2] + \varepsilon(K)
\end{equation}

For any $Q \in F$, we have 

$$\E_{\mu^{K_0} \times \U_{\bar{Q}}^{K}}[(\bar{Q}^{K} - f)^2]=\E_{\mu^{K_0} \times \U_{Q_{K}}^{K}}[(Q_{K}^{K} - f)^2]$$

\begin{equation}
\label{eqn:prp__unif__prf2}
\E_{\mu^{K_0} \times \U_{\bar{Q}}^{K}}[(\bar{Q}^{K} - f)^2] \leq \E_{\mu^{K_0} \times \U_Q^{K}}[(Q^{K} - f)^2]
\end{equation}
...
$$\E_{\mu^{K_0} \times \U_P^{K}}[(P^{K} - f)^2] \leq \E_{\mu^{K_0} \times U^{r_{\mathfrak{R}}(K)}}[(\bar{Q}^{K} - f)^2] + \varepsilon(K)$$

Obviously $\E_{\mu^{K_0} \times U^{r_{\mathfrak{R}}(K)}}[(\bar{Q}^{K} - f)^2] \leq \E_{\mu^{K_0} \times \U_Q^{K}}[(Q^{K} - f)^2]$ therefore
...
Suppose $\R_{\bar{P}}=r_{\mathfrak{R}}+r_{\mathfrak{A}}$ where $r_{\mathfrak{R}} \in \GrowR$ and $r_{\mathfrak{A}} \in \GrowA$. For any ${K \in \Nats^n}$, choose ${\bar{\A}_P(K) \in \Argmin{y \in \WordsLen{r_{\mathfrak{A}}(K)}} \E_{(x,z) \sim \mu^{K_0} \times U^{r_{\mathfrak{R}}(K)}}[(\bar{P}^{K}(x,yz) - f(x))^2]}$. We can construct $P: \Words \Scheme \Rats$ so that for all $K \in \Nats^n$, $\R_P(K) = r_{\mathfrak{R}}(K)$, $\A_P(K):=\Chev{\A_{\bar{P}}(K),\bar{\A}_P(K)}$ and for any $x \in \Supp \mu^{K_0}$ and $z \in \WordsLen{r_{\mathfrak{R}}(K)}$, ${P^{K}(x,z)=\bar{P}^{K}(x,\bar{\A}_P(K)z)}$. Clearly ${\E_{\mu^{K_0} \times U^{r_{\mathfrak{R}}(K)}}[(P^{K} - f)^2] \leq \E_{\mu^{K_0} \times \U_{\bar{P}}^{K}}[(\bar{P}^{K} - f)^2]}$ and therefore $P$ is an $\Fall(\Gamma)$-optimal predictor for $(\mu,f)$.
\end{proof}
...
Assume $\Fall$ is $\GrowA$-ample. Consider $(\mu,f)$ a distributional estimation problem, $P$ an $\Fall(\Gamma)$-optimal predictor for $(\mu,f)$, $Q: \Words \Scheme \Rats$ and ${W: \Words \Scheme \Rats^{\geq 0}}$ bounded s.t. $\R_W \geq \max(\R_P, \R_Q)$. Denote ${\mu^{K}_{P,W}:=\mu^{K_0} \times \U_P^{K} \times U^{\R_W(K)-\R_P(K)}}$, ${\mu^{K}_{Q,W}:=\mu^{K_0} \times \U_Q^{K} \times U^{\R_W(K)-\R_Q(K)}}$. Then
...
$${\Fall_W:=\{\varepsilon_W: \Nats^2 \rightarrow \Reals^{\geq 0} \textnormal{ bounded} \mid  \E_{\mu^k \times \U_W^k}[W^k]\varepsilon_W(K) \in \Fall\}}$$
...
$$\E_{\mu^{K_0} \times \U_P^{K} \times \U_W^{K_0}}[W^{K_0}(P^{K} - f)^2] \leq \\ \E_{\mu^{K_0} \times \U_Q^{K} \times \U_W^{K_0}}[W^{K_0}(Q^{K} - f)^2] + \varepsilon(K)$$

$$\E_{\mu^{K_0} \times \U_P^{K}}[\E_{\U_W^{K_0}}[W^{K_0}](P^{K} - f)^2] \leq \\ \E_{\mu^{K_0} \times \U_Q^{K}}[\E_{\U_W^{K_0}}[W^{K_0}](Q^{K} - f)^2] + \varepsilon(K)$$

Dividing both sides of the inequality by $\E_{\mu^{K_0} \times \U_W^{K_0}}[W^{K_0}(x)]$ we get

$$\E_{\nu^{K_0} \times \U_P^{K}}[(P^{K} - f)^2] \leq \\ \E_{\nu^{K_0} \times \U_Q^{K}}[(Q^{K} - f)^2] + \frac{\varepsilon(K)}{\E_{\mu^{K_0} \times \U_W^{K_0}}[W^{K_0}(x)]}$$

Let $M$ be the supremum of the left hand side.

$$\E_{\nu^{K_0} \times \U_P^{K}}[(P^{K} - f)^2] \leq \\ \E_{\nu^{K_0} \times \U_Q^{K}}[(Q^{K} - f)^2] + \min(\frac{\varepsilon(K)}{\E_{\mu^{K_0} \times \U_W^{K_0}}[W^{K_0}(x)]},M)$$
...
$$\E_{\mu^{K_0} \times \U_P^{K}}[(P^{K}-f)^2] \leq \E_{\mu^{K_0} \times \U_W^{K}}[(Q_t^{K}-f)^2] + \varepsilon(K)$$

$$\E_{(x,y) \sim \mu^{K_0} \times \U_W^{K}}[(P^{K}(x,y_{\leq \R_P(K)})-f(x))^2-(Q_t^{K}(x,y)-f(x))^2] \leq \varepsilon(K)$$
...
$$\E_{(x,y) \sim \mu^{K_0} \times \U_W^{K}}[\theta(W^{K}(x,y)-\rho_\zeta^{K}(t)) \cdot ((P^{K}(x,y_{\leq \R_P(K)})-f(x))^2-(Q^{K}(x,y_{\leq \R_Q(K)})-f(x))^2)] \leq \varepsilon(K)$$
...
$$\E_{\mu^{K_0} \times \U_P^{K}}[(P^{K} - f)^2] \leq \E_{\mu^{K_0} \times \U_P^{K} \times \U_S^{K}}[(Q_t^{K} - f)^2] + \varepsilon(K)$$

$$\E_{\mu^{K_0} \times \U_P^{K}}[(P^{K} - f)^2] \leq \E_{\mu^{K_0} \times \U_P^{K} \times \U_S^{K}}[(P^{K} + t(K)S^{K}  - f)^2] + \varepsilon(K)$$

$$\E_{\mu^{K_0} \times \U_P^{K} \times \U_S^{K}}[(P^{K} - f)^2 - (P^{K} + t(K)S^{K} - f)^2] \leq \varepsilon(K)$$

$$-\E_{\mu^{K_0} \times \U_P^{K} \times \U_S^{K}}[(t(K)S^{K} + 2 (P^{K} - f)) S^{K}] t(K) \leq \varepsilon(K)$$

$$-\E_{\mu^{K_0} \times \U_P^{K} \times \U_S^{K}}[(S^{K})^2] t(K)^2 + 2 \E_{\mu^{K_0} \times \U_P^{K} \times \U_S^{K}}[(P^{K} - f) S^{K}] t(K) \leq \varepsilon(K)$$

$$2 \E_{\mu^{K_0} \times \U_P^{K} \times \U_S^{K}}[(P^{K} - f) S^{K}] t(K) \leq \E_{\mu^{K_0} \times \U_P^{K} \times \U_S^{K}}[(S^{K})^2] t(K)^2 + \varepsilon(K)$$

$$2 \E_{\mu^{K_0} \times \U_P^{K} \times \U_S^{K}}[(P^{K} - f) S^{K}] t(K) \leq (\sup \Abs{S^{K}})^2 t(K)^2 + \varepsilon(K)$$

$$2 \E_{\mu^{K_0} \times \U_P^{K} \times \U_S^{K}}[(P^{K} - f) S^{K}] \sigma(K) 2^{-m(K)} \leq (\sup \Abs{S^{K}})^2 4^{-m(K)} + \varepsilon(K)$$

Multiplying both sides by $2^{m(K)-1}$ we get

$$\E_{\mu^{K_0} \times \U_P^{K} \times \U_S^{K}}[(P^{K} - f) S^{K}] \sigma(K) \leq \frac{1}{2}((\sup \Abs{S^{K}})^2 2^{-m(K)} + \varepsilon(K) 2^{m(K)})$$

Let $\sigma(K):=\Sgn \E_{\mu^{K_0} \times \U_S^{K}}[(P^{K} - f) S^{K}]$.

$$\Abs{\E_{\mu^{K_0} \times \U_P^{K} \times \U_S^{K}}[(P^{K} - f) S^{K}]} \leq \frac{1}{2}((\sup \Abs{S^{K}})^2 2^{-m(K)} + \varepsilon(K) 2^{m(K)})$$
...
$$\E_{\mu^{K_0} \times \U_Q^{K}}[(Q^{K}-f)^2]=\E_{\mu^{K_0} \times \U_Q^{K} \times \U_P^{K}}[(Q^{K}-P^{K}+P^{K}-f)^2]$$
...
$$ \forall k \in I: \E_{\mu_f^k}[(\frac{1}{2}-\chi_B)^2] \geq \E_{\mu_f^k \times \U_Q^k}[(Q^k-\chi_B)^2] +\frac{\epsilon}{k^d}$$

$$ \forall k \in I: \E_{\mu_f^k \times \U_Q^{k}}[(Q^{k}-\chi_B)^2] \leq \frac{1}{4} - \frac{\epsilon}{k^d} $$

$$ \forall k \in I: \E_{\mu_f^k}[(\E_{\U_Q^{k}}[Q^{k}]-\chi_B)^2] \leq \frac{1}{4} - \frac{\epsilon}{k^d} $$
...
Assume $\Fall$ is $\GrowA$-ample. Consider $(\mu,f)$ a distributional estimation problem, $P$ an $\Fall(\Gamma)$-optimal predictor for $(\mu,f)$ and ${W: \Words \Scheme \Rats^{\geq 0}}$ bounded s.t. $\R_W \geq \R_P$ and for every $K \in \Nats^n$ there is $x \in \Supp \mu^{K_0}$ and $y \in \U_W^{K}$ with $W^{K}(x,y) > 0$. Denote ${\alpha(K):=\E_{\mu^{K_0} \times \U_W^{K}}[W^{K}]}$ and ${\delta(K):=\E_{\mu^{K_0} \times \U_W^{K}}[W^{K}(P^{K}-f)]}$. Then, $\frac{\delta^2}{\alpha} \in \Fall$.
...
Assume $\Fall$ is $\GrowA$-ample. Consider $(\mu,f)$ a distributional estimation problem, $P$ an $\Fall(\Gamma)$-optimal predictor for $(\mu,f)$ and $A,B: \bm{1} \Scheme \Rats$ s.t. $\R_A \equiv 0$ and $\R_B \equiv 0$. Denote ${\alpha(K):=\Prb_{\mu^{K_0} \times \U_P^{K}}[A^{K} \leq P^{K} \leq B^{K}]}$. Then, there is $\varepsilon \in \Fall$ s.t. 
...
Construct $W: \Words \Scheme \Bool$ s.t. ${W^{K}(x,y)=\theta(P^{K}(x,y)-A^{K})\theta(B^{K}-P^{K}(x,y))}$. Denote $\delta(K):=\E_{\mu^{K_0} \times \U_W^{K}}[W^{K}(P^{K}-f)]$ and $\varepsilon:=\frac{\delta^2}{\alpha}$. According to Theorem~\ref{thm:calib}, $\varepsilon \in \Fall$.
We get

$$\frac{\E_{\mu^{K_0} \times \U_W^{K}}[W^{K}(P^{K}-f)]^2}{\alpha(K)} = \varepsilon(K)$$

$$\frac{\E_{\mu^{K_0} \times \U_W^{K}}[\theta(P^{K}(x,y)-A^{K})\theta(B^{K}-P^{K}(x,y))(P^{K}-f)]^2}{\alpha(K)} = \varepsilon(K)$$

$$\frac{(\E_{\mu^{K_0} \times \U_W^{K}}[\theta(P^{K}(x,y)-A^{K})\theta(B^{K}-P^{K}(x,y))]\E[P^{K}-f \mid A^{K} \leq P^{K} \leq B^{K}])^2}{\alpha(K)} = \varepsilon(K)$$
...
$$\E_{\mu^{K_0} \times \U_W^{K}}[W^{K}(P^{K} - f)^2] \leq \E_{\mu^{K_0} \times \U_W^{K}}[W^{K}(Q^{K}-f)^2] + \varepsilon(K)$$

$$\E_{\mu^{K_0} \times \U_W^{K}}[W^{K}(P^{K} - f)^2] \leq \E_{\mu^{K_0} \times \U_W^{K}}[W^{K}(P^{K}-f-\epsilon(K))^2] + \varepsilon(K)$$

$$\E_{\mu^{K_0} \times \U_W^{K}}[W^{K}((P^{K} - f)^2 - (P^{K}-f-\epsilon(K))^2] \leq \varepsilon(K)$$

$$ \epsilon(K) \E_{\mu^{K_0} \times \U_W^{K}}[W^{K}(2(P^{K} - f) - \epsilon(K))] \leq \varepsilon(K)$$

$$ \epsilon(K) (2 \E_{\mu^{K_0} \times \U_W^{K}}[W^{K}(P^{K} - f)]-\E_{\mu^{K_0} \times \U_W^{K}}[W^{K}]\epsilon(K)) \leq \varepsilon(K)$$
...
Consider $\mu$ a word ensemble, $f_1,f_2: \Supp \mu \rightarrow \Reals$ bounded and $t_1,t_2 \in \Rats$. Denote $f: = t_1 f_1 + t_2 f_2$. Suppose $P_1$ is an $\Fall^\sharp(\Gamma)$-optimal predictor for $(\mu,f_1)$ and $P_2$ is an $\Fall^\sharp(\Gamma)$-optimal predictor for $(\mu,f_2)$. Construct $P: \Words \Scheme \Rats$ s.t. $\R_P(K) = \R_{P_1}(K) + \R_{P_2}(K)$ and for any $x \in \Supp \mu^{K_0}$, $y_1 \in \WordsLen{\R_{P_1}(K)}$, $y_2 \in \WordsLen{\R_{P_1}(K)}$, $P^{K}(x,y_1 y_2)=t_1 P_1^{K}(x,y_1) + t_2 P_2^{K}(x, y_2)$. Then, $P$ is an $\Fall^\sharp(\Gamma)$-optimal predictor for $(\mu, f)$.
...
The following is a useful generalization of the concept of a $\Gamma$-scheme.
...
A word ensemble $\mu$ is called \emph{$\Fall(\MGrow)$-samplable} (resp. \emph{$\Fall(\Gamma)$-samplable}) when there is an $\MGrow$-scheme (resp. $\Gamma$-scheme) $L$ of signature ${\bm{1} \rightarrow \Words}$  s.t. $\mu^{K_0} \equiv L_\bullet^K \pmod \Fall$.
...
A distributional estimation problem $(\mu,f)$ is called \emph{$\Fall(\MGrow)$-generatable (resp. $\Fall(\Gamma)$-generatable)} when there is an $\MGrow$-scheme (resp. $\Gamma$-scheme) $G$ of signature $\bm{1} \rightarrow \Words \times \Rats$ s.t. 
...
Assume $\Fall$ is $\GrowA$-ample. Consider a distributional estimation problem $(\mu,f)$ s.t. for some $l \in \GrowA$, $\mu^{K_0}(\Bool^{\leq l(K)}) \equiv 1 \pmod \Fall$. Then, $(\mu,f)$ is $\Fall(\MGrow)$-generatable. 
...
We now introduce the notions of samplability of generatability over a given \enquote{base space} $Y$.

\begin{definition}

Consider a word ensemble $\mu$, an encoded set $Y$ and a family ${\{\pi^K: \Supp \mu^{K_0} \Markov Y\}_{K \in \Nats^n}}$. $\mu$ is called \emph{$\Fall(\MGrow)$-samplable (resp. $\Fall(\Gamma)$-samplable) relative to $\pi$} when there is an $\MGrow$-scheme (resp. $\Gamma$-scheme) $L$ of signature $Y \rightarrow \Words$ s.t. the function ${\varepsilon(K):=\E_{(x,y) \sim \mu^{K_0} \ltimes \pi^K}[\Dtv(\mu \mid (\pi^K)^{-1}(y),L_y^K)]}$ is in $\Fall$.
...
\item $\mu^{K_0} \ltimes \pi^{K} \equiv \nu^{K_0} \pmod \Fall$

\item $\E_{(x,z) \sim \mu^{K_0} \times \U_\pi^{K}}[\Abs{f_2(x)-g(\pi^{K}(x,z))}] \in \Fall$

\item $(\mu, f_1)$ is $\Fall(\MGrow)$-generatable relative to $\pi$.

\end{enumerate}

Suppose $P_1$ is an $\Fall^\sharp(\Gamma)$-optimal predictor for $(\mu,f_1)$ and $P_2$ is an $\Fall^\sharp(\Gamma)$-optimal predictor for $(g,\nu)$. Construct $P: \Words \Scheme \Rats$ s.t. $\R_P=\R_{P_1}+\R_{P_2}+\R_{\pi}$ and for any $z_1 \in \WordsLen{\R_{P_1}(K)}$, $z_2 \in \WordsLen{\R_{P_2}(K)}$ and $z_3 \in \WordsLen{\R_\pi(K)}$

\begin{equation}
P^{K}(x,z_1 z_2 z_3)=P_1^{K}(x,z_1) P_2^{K}(\pi^{K}(x,z_3),z_2)
\end{equation}
...
Consider a word ensemble $\mu$, an encoded set $Y$, a family $\{\pi^K: \Supp \mu^{K_0} \Markov Y\}_{K \in \Nats^n}$ and ${h: (\Supp \mu) \times Y \rightarrow \Reals}$ bounded. Suppose $L$ is an $\Fall(\MGrow)$-sampler of $\mu$ relative to $\pi$. Then

\begin{equation}
E_{\mu^{K_0} \ltimes \pi^K}[h(x,y)] \equiv \E_{\mu^{K_0} \ltimes \pi^K \times \UM_L^K}[h(L^K(y,z),y)] \pmod \Fall
\end{equation}
...
$$\E_{\mu^{K_0} \ltimes \pi^K}[h(x,y)] - \E_{\mu^{K_0} \ltimes \pi^K \times \UM_L^K}[h(L^K(y,z),y)] = \E_{\mu^{K_0} \ltimes \pi^K}[\E_{\mu_y^K}[h(x',y)]] - \E_{\mu^{K_0} \ltimes \pi^K \times \UM_L^K}[h(L^K(y,z),y)]$$

$$\E_{\mu^{K_0} \ltimes \pi^K}[h(x,y)] - \E_{\mu^{K_0} \ltimes \pi^K \times \UM_L^K}[h(L^K(y,z),y)] = \E_{\mu^{K_0} \ltimes \pi^K}[\E_{\mu_y^K}[h(x',y)]-\E_{\UM_L^K}[h(L^K(y,z),y)]]$$

$$\E_{\mu^{K_0} \ltimes \pi^K}[h(x,y)] - \E_{\mu^{K_0} \ltimes \pi^K \times \UM_L^K}[h(L^K(y,z),y)] = \E_{\mu^{K_0} \ltimes \pi^K}[\E_{\mu_y^K}[h(x',y)]-\E_{L_y^K}[h(x',y)]]$$

$$\Abs{\E_{\mu^{K_0} \ltimes \pi^K}[h(x,y)] - \E_{\mu^{K_0} \ltimes \pi^K \times \UM_L^K}[h(L^K(y,z),y)]} \leq \E_{\mu^{K_0} \ltimes \pi^K}[\Abs{\E_{\mu_y^K}[h(x',y)]-\E_{L_y^K}[h(x',y)]}]$$

$$\Abs{\E_{\mu^{K_0} \ltimes \pi^K}[h(x,y)] - \E_{\mu^{K_0} \ltimes \pi^K \times \UM_L^K}[h(L^K(y,z),y)]} \leq (\sup h - \inf h) \E_{\mu^{K_0} \ltimes \pi^K}[\Dtv(\mu_y^K,L_y^K)]$$
...
Consider a distributional estimation problem $(\mu,f)$, an encoded set $Y$, a family ${\{\pi^K: \Supp \mu^{K_0} \Markov Y\}_{K \in \Nats^n}}$ and ${h: (\Supp \mu) \times Y \rightarrow \Reals}$ bounded. Denote $\mu_\pi^K:=\mu^{K_0} \ltimes \pi^K$. Suppose $G$ is an $\Fall(\MGrow)$-generator of $(\mu,f)$ relative to $\pi$. Then
...
\E_{\mu^{K_0} \times \U_P^{K} \times \UM_S^{K}}[(P^{K}(x,y) - f(x))S^{K}(x,P^{K}(x,y),z,w)] \equiv 0 \pmod \Fall
\end{equation}
...
$$w^{K} \in \Argmax{w \in \Supp \M_S^{K}} \Abs{\E_{\mu^{K_0} \times \U_P^{K} \times \U^{\R_S^{K}(w)}}[(P^{K}(x,y) - f(x))S^{K}(x,P^{K}(x,y),z,w)]}$$

Construct $\bar{S}: \Words \times \Rats \Scheme \Rats$ s.t. $\R_{\bar{S}}(K)=\R_S^{K}(w^{K})$ and ${\bar{S}^{K}(x,t,z)=S^{K}(x,t,z,w^{K})}$. $P$ is an $\Fall^\sharp(\Gamma)$-optimal predictor for $(\mu,f)$, therefore

$$\E_{\mu^{K_0} \times \U_P^{K} \times \U_{\bar{S}}^{K}}[(P^{K}(x,y) - f(x))\bar{S}^{K}(x,P^{K}(x,y),z)] \equiv 0 \pmod \Fall$$

$$\E_{\mu^{K_0} \times \U_P^{K} \times \U^{\R_S^{K}(w)}}[(P^{K}(x,y) - f(x))S^{K}(x,P^{K}(x,y),z,w^{K})] \equiv 0 \pmod \Fall$$
...
$$P^{K}(x,z_1 z_2 z_3)-f_1(x)f_2(x)=f_1(x)(P_2^{K}(\pi^{K}(x,z_3),z_2)-f_2(x))+(P_1^{K}(x,z_1)-f_1(x))P_2^{K}(\pi^{K}(x,z_3),z_2)$$

Denote $\bar{P}_2^{K}:=P_2^{K} \circ \pi^{K}$. For any bounded $S: \Words \times \Rats \Scheme \Rats$ we get

$$\Abs{\E[(P^{K}-f_1 f_2)S^{K}]} \leq \Abs{\E[(\bar{P}_2^{K}-f_2)f_1 S^{K}]} + \Abs{\E[(P_1^{K}-f_1) \bar{P}_2^{K} S^{K}]}$$

$P_1$ is an $\Fall^\sharp(\Gamma)$-optimal predictor for $(\mu,f_1)$ therefore the second term on the right hand side is in $\Fall$.

$$\Abs{\E[(P^{K}-f_1 f_2)S^{K}]} \leq \Abs{\E[(\bar{P}_2^{K}-f_2)f_1 S^{K}]} \pmod \Fall$$

$$\Abs{\E[(P^{K}-f_1 f_2)S^{K}]} \leq \Abs{\E[(\bar{P}_2^{K}-f_2)f_1 S^{K}] - \E[(\bar{P}_2^{K}-g \circ \pi^{K})f_1 S^{K}] + \E[(\bar{P}_2^{K}-g \circ \pi^{K})f_1 S^{K}]} \pmod \Fall$$

$$\Abs{\E[(P^{K}-f_1 f_2)S^{K}]} \leq \Abs{\E[(\bar{P}_2^{K}-f_2)f_1 S^{K}] - \E[(\bar{P}_2^{K}-g \circ \pi^{K})f_1 S^{K}]} + \Abs{\E[(\bar{P}_2^{K}-g \circ \pi^{K})f_1 S^{K}]} \pmod \Fall$$

$$\Abs{\E[(P^{K}-f_1 f_2)S^{K}]} \leq \Abs{\E[(g \circ \pi^{K}-f_2)f_1 S^{K}]} + \Abs{\E[(\bar{P}_2^{K}-g \circ \pi^{K})f_1 S^{K}]} \pmod \Fall$$

$$\Abs{\E[(P^{K}-f_1 f_2)S^{K}]} \leq (\sup \Abs{f_1}) (\sup \Abs{S}) \E[\Abs{g \circ \pi^{K} - f_2}] + \Abs{\E[(\bar{P}_2^{K}-g \circ \pi^{K})f_1 S^{K}]} \pmod \Fall$$

Condition (ii) implies the first term on the right hand side is in $\Fall$.

$$\Abs{\E[(P^{K}-f_1 f_2)S^{K}]} \leq \Abs{\E[(\bar{P}_2^{K}-g \circ \pi^{K})f_1 S^{K}]} \pmod \Fall$$

Denote $\U_{\text{tot}}^{K}:= \U_{P_1}^{K} \times \U_{P_2}^{K} \times \U_S^{K}$. We change variables inside the expected value on the right hand side by $y:=\pi^{K}(x,z_3)$. Observing that $(x,y)$ obeys the distribution $\mu^{K_0} \ltimes \pi^{K}$ we get

$$\Abs{\E[(P^{K}-f_1 f_2)S^{K}]} \leq \Abs{\E_{\mu^{K_0} \ltimes \pi^{K} \times \U_{\text{tot}}^{K}}[(P_2^{K}(y,z_{2})-g(y))f_1(x) S^{K}(x,P_1^{K}(x,z_1)P_2^{K}(y,z_2), z_4)]} \pmod \Fall$$

$$\Abs{\E[(P^{K}-f_1 f_2)S^{K}]} \leq \Abs{\E_{\mu^{K_0} \ltimes \pi^{K}}[\E_{\U_{\text{tot}}^{K}}[(P_2^{K}(y,z_{2})-g(y))S^{K}(x,P_1^{K}(x,z_1)P_2^{K}(y,z_2), z_4)]f_1(x)]} \pmod \Fall$$

Let $G$ be an $\Fall(\MGrow)$-generator of $(\mu,f_1)$ relative to $\pi$. Applying Proposition~\ref{prp:gen} to the right hand side we get

$$\Abs{\E[(P^{K}-f_1 f_2)S^{K}]} \leq \Abs{\E_{\mu^{K_0} \ltimes \pi^{K} \times \UM_G^{K}}[\E[(P_2^{K}(y)-g(y))S^{K}(G^{K}(y)_1,P_1^{K}(G^{K}(y)_1)P_2^{K}(y))]G^{K}(y)_2]} \pmod \Fall$$

Using condition (i) we conclude that

$$\Abs{\E[(P^{K}-f_1 f_2)S^{K}]} \leq \Abs{\E_{\nu^k \times \UM_G^{K}}[\E[(P_2^{K}(y)-g(y))S^{K}(G^{K}(y)_1,P_1^{K}(G^{K}(y)_1)P_2^{K}(y))]G^{K}(y)_2]} \pmod \Fall$$

$$\Abs{\E[(P^{K}-f_1 f_2)S^{K}]} \leq \Abs{\E_{\nu^k \times \U_{\text{tot}}^{K} \times \UM_G^{K}}[(P_2^{K}(y)-g(y))S^{K}(G^{K}(y)_1,P_1^{K}(G^{K}(y)_1)P_2^{K}(y))G^{K}(y)_2]} \pmod \Fall$$
...
Consider $\mu_1$, $\mu_2$ word ensembles and $L_1$, $L_2$ which are $\Fall(\MGrow)$-samplers for $\mu_1$ and $\mu_2$ respectively. Define ${\mu^k:=(\En_{\Words \times \Words})_*(\mu_1^k \times \mu_2^k)}$. Suppose $\pi: \Words \Scheme \Words$ is s.t. for any $K \in \Nats^n$, $x_1 \in \Supp L_{1\bullet}^{K}$, ${x_2 \in \Supp L_{2\bullet}^{K}}$ and $z \in \Bool^{\R_\pi(K)}$, $\pi^{K}(\Chev{x_1,x_2},z)=x_2$. Then $\mu^{K_0} \ltimes \pi^{K} \equiv \mu_2^{K_0} \pmod \Fall$.
...
$L_{1\bullet}^{K} \equiv \mu_1^{K_0} \pmod \Fall$ and $L_{2\bullet}^{K} \equiv \mu_2^{K_0} \pmod \Fall$ therefore ${L_{1\bullet}^{K} \times L_{2\bullet}^{K} \equiv \mu_1^{K_0} \times \mu_2^{K_0} \pmod \Fall}$ (by Proposition~\ref{prp:prob_cong_dir}). Denote $\mu_L^{K}:=(\En_{\Words \times \Words})_*(L_{1\bullet}^{K} \times L_{2\bullet}^{K})$. We get ${\mu_L^{K} \equiv \mu^{K_0} \pmod \Fall}$ and therefore $\mu_L^{K} \ltimes \pi^K \equiv \mu^{K_0} \ltimes \pi^K \pmod \Fall$ (by Proposition~\ref{prp:prob_cong_semidir}). Obviously $\mu_L^{K}\ltimes \pi^{K}=L_{2\bullet}^{K}$. We conclude that ${L_{2\bullet}^{K} \equiv \mu^{K_0} \ltimes \pi^K \pmod \Fall}$ and therefore $\mu_2 \equiv \mu^{K_0} \ltimes \pi^K \pmod \Fall$ (by Proposition~\ref{prp:prob_cong_eq}).
\end{proof}
...
Consider $\mu_1$, $\mu_2$ word ensembles and $L_1$, $L_2$ which are $\Fall(\MGrow)$-samplers for $\mu_1$ and $\mu_2$ respectively. Suppose $\pi: \Words \Scheme \Words$ is s.t. for any $K \in \Nats^n$, $x_1 \in \Supp L_{1\bullet}^{K}$, ${x_2 \in \Supp L_{2\bullet}^{K}}$ and $z \in \Bool^{\R_\pi(K)}$, $\pi^{K}(\Chev{x_1,x_2},z)=x_2$. Then, for any $g: \Supp \mu_2 \rightarrow \Reals$ bounded we have 

$$\E_{(x_1,x_2,z) \sim\mu_1^{K_0} \times \mu_2^{K_0} \times \U_\pi^{K}}[\Abs{g(x_2)-g(\pi^{K}(\Chev{x_1,x_2},z))}] \in \Fall$$
...
$$\E[\Abs{g(x_2)-g(\pi^{K}(\Chev{x_1,x_2}))}] \leq M \Prb_{ \mu_1^{K_0} \times \mu_2^{K_0}}[(x_1,x_2) \not\in \Supp L_{1\bullet}^{K} \times \Supp L_{2\bullet}^{K}]$$

$$\E[\Abs{g(x_2)-g(\pi^{K}(\Chev{x_1,x_2}))}] \leq M\Prb_{ L_{1\bullet}^{K} \times L_{2\bullet}^{K}}[(x_1,x_2) \not\in \Supp L_{1\bullet}^{K} \times \Supp L_{2\bullet}^{K}] \pmod \Fall$$

$$\E[\Abs{g(x_2)-g(\pi^{K}(\Chev{x_1,x_2}))}] \equiv 0 \pmod \Fall$$
\end{proof}
...
Consider  word ensembles $\mu_1$ and $\mu_2$ with $\Fall(\MGrow)$-samplers $L_1$ and $L_2$ respectively. Define ${\mu^k:=(\En_{\Words \times \Words})_*(\mu_1^k \times \mu_2^k)}$. Suppose ${\pi: \Words \Scheme \Words}$ is s.t. 

\begin{enumerate}[(i)]

\item For any $K \in \Nats^n$, $x \in \Supp L_{1\bullet}^{K}$, ${y \in \Supp L_{2\bullet}^{K}}$ and $z \in \Bool^{\R_\pi(K)}$, ${\pi^{K}(\Chev{x,y},z)=y}$

\item For any $K \in \Nats^n$, $x,y \in \Words$ and $z \in \Bool^{\R_\pi(K)}$, either ${\pi^{K}(\Chev{x,y},z)=y}$ or ${\pi^{K}(\Chev{x,y},z) \not\in \Supp L_{2\bullet}^{K}}$.

\end{enumerate}

Consider $L: \Words \MScheme \Words$ s.t. $\UM_L^{K}=\UM_{L_1}^{K}$ and ${L^{K}(y,z,w)=\Chev{L_1^{K}(z,w),y}}$. Then, $L$ is an $\Fall(\MGrow)$-sampler of $\mu$ relative to $\pi$. In particular, since such an $L$ can always be constructed, $\mu$ is $\Fall(\MGrow)$-samplable relative to $\pi$.
...
Denote $\mu_\pi^K := \mu^{K_0} \ltimes \pi^K$, $\mu_y^K:=\mu \mid (\pi^K)^{-1}(y)$, ${\mu_L^K:=(\En_{\Words \times \Words})_*(L_{1\bullet}^k \times L_{2\bullet}^k)}$. We have $\mu^{K_0} \equiv \mu_L^K \pmod \Fall$, $\mu_\pi^K \equiv \mu_L^K \ltimes \pi^K \equiv L_{2\bullet}^K \pmod \Fall$.

\[\E_{(x,y) \sim \mu_\pi^K}[\Dtv(\mu_y^K,L_y^K)] \equiv \E_{y \sim L_{2\bullet}^K}[\Dtv(\mu_y^K,L_y^K)] \pmod \Fall\]

\[\E_{(x,y) \sim \mu_\pi^K}[\Dtv(\mu_y^K,L_y^K)] \equiv \frac{1}{2}\E_{y \sim L_{2\bullet}^K}[\sum_{x \in \Words}\Abs{\mu_y^K(x)-L_y^K(x)}] \pmod \Fall\]

\[\E_{(x,y) \sim \mu_\pi^K}[\Dtv(\mu_y^K,L_y^K)] \equiv \frac{1}{2}\E_{y \sim L_{2\bullet}^K}[\sum_{x,y' \in \Words}\Abs{\mu_y^K(\Chev{x,y'})-L_y^K(\Chev{x,y'})}] \pmod \Fall\]

\[\E_{(x,y) \sim \mu_\pi^K}[\Dtv(\mu_y^K,L_y^K)] \equiv \frac{1}{2}\E_{y \sim L_{2\bullet}^K}[\sum_{x,y' \in \Words}\Abs{\mu_y^K(\Chev{x,y})\delta_{yy'}-L_{1\bullet}^K(x)\delta_{yy'}}] \pmod \Fall\]

\[\E_{(x,y) \sim \mu_\pi^K}[\Dtv(\mu_y^K,L_y^K)] \equiv \frac{1}{2}\E_{y \sim L_{2\bullet}^K}[\sum_{x \in \Words}\Abs{\mu_y^K(\Chev{x,y})-L_{1\bullet}^K(x)}] \pmod \Fall\]

\[\E_{(x,y) \sim \mu_\pi^K}[\Dtv(\mu_y^K,L_y^K)] \equiv \frac{1}{2}\E_{y \sim L_{2\bullet}^K}[\sum_{x \in \Supp L_{1\bullet}^K}\Abs{\mu_y^K(\Chev{x,y})-L_{1\bullet}^K(x)}+\sum_{x \not\in \Supp L_{1\bullet}^K}\Abs{\mu_y^K(\Chev{x,y})-L_{1\bullet}^K(x)}] \pmod \Fall\]

\[\E_{(x,y) \sim \mu_\pi^K}[\Dtv(\mu_y^K,L_y^K)] \equiv \frac{1}{2}\E_{y \sim L_{2\bullet}^K}[\sum_{x \in \Supp L_{1\bullet}^K}\Abs{\mu_y^K(\Chev{x,y})-L_{1\bullet}^K(x)}+\sum_{x \not\in \Supp L_{1\bullet}^K}\mu_y^K(\Chev{x,y})] \pmod \Fall\]

\[\E_{(x,y) \sim \mu_\pi^K}[\Dtv(\mu_y^K,L_y^K)] \equiv \frac{1}{2}\E_{y \sim L_{2\bullet}^K}[\sum_{x \in \Supp L_{1\bullet}^K}\Abs{\mu_y^K(\Chev{x,y})-L_{1\bullet}^K(x)}+\Prb_{\Chev{x,y} \sim \mu_y^K}[x \not\in \Supp L_{1\bullet}^K]] \pmod \Fall\]

The properties of $\pi$ imply that for any $y \in \Supp L_{2\bullet}^K$, ${\Prb_{\Chev{x,y} \sim \mu_y^K}[x \not\in \Supp L_{1\bullet}^K] \leq \Prb_{x \sim \mu_1^{K_0}}[x \not\in \Supp L_{1\bullet}^K]}$.

\[\E_{(x,y) \sim \mu_\pi^K}[\Dtv(\mu_y^K,L_y^K)] \leq \frac{1}{2}\E_{y \sim L_{2\bullet}^K}[\sum_{x \in \Supp L_{1\bullet}^K}\Abs{\mu_y^K(\Chev{x,y})-L_{1\bullet}^K(x)}+\Prb_{x \sim \mu_1^{K_0}}[x \not\in \Supp L_{1\bullet}^K]] \pmod \Fall\]

\[\E_{(x,y) \sim \mu_\pi^K}[\Dtv(\mu_y^K,L_y^K)] \leq \frac{1}{2}\E_{y \sim L_{2\bullet}^K}[\sum_{x \in \Supp L_{1\bullet}^K}\Abs{\mu_y^K(\Chev{x,y})-L_{1\bullet}^K(x)}+\Prb_{x \sim L_{1\bullet}^K}[x \not\in \Supp L_{1\bullet}^K]] \pmod \Fall\]

\begin{equation}
\label{eqn:prp__smp_base_change__prf1}
\E_{\mu_\pi^K}[\Dtv(\mu_y^K,L_y^K)] \leq \frac{1}{2}\E_{L_{2\bullet}^K}[\sum_{x \in \Supp L_{1\bullet}^K}\Abs{\mu_y^K(\Chev{x,y})-L_{1\bullet}^K(x)}] \pmod \Fall
\end{equation}

For any $x,y \in \Words$

\[\mu_y^K(\Chev{x,y})=\frac{\mu^K(\Chev{x,y}) \Prb_{\U_\pi^K}[\pi^K(\Chev{x,y})=y]}{\sum_{x',y' \in \Words} \mu^K(\Chev{x',y'}) \Prb_{\U_\pi^K}[\pi^K(\Chev{x',y'})=y]}\]

Assuming $x \in \Supp L_{1\bullet}^K$, $y \in \Supp L_{2\bullet}^K$

\[\mu_y^K(\Chev{x,y})=\frac{\mu^K(\Chev{x,y})}{\sum_{x' \in \Words} \mu^K(\Chev{x',y}) \Prb_{\U_\pi^K}[\pi^K(\Chev{x',y})=y]}\]

\[\mu_y^K(\Chev{x,y})=\frac{\mu_1^{K_0}(x)\mu_2^{K_0}(y)}{\sum_{x' \in \Words} \mu_1^{K_0}(x')\mu_2^{K_0}(y)\Prb_{\U_\pi^K}[\pi^K(\Chev{x',y})=y]}\]

\[\mu_y^K(\Chev{x,y})=\frac{\mu_1^{K_0}(x)}{\sum_{x' \in \Words} \mu_1^{K_0}(x') \Prb_{\U_\pi^K}[\pi^K(\Chev{x',y})=y]}\]

\[\mu_y^K(\Chev{x,y})=\frac{\mu_1^{K_0}(x)}{\sum_{x' \in \Supp L_{1\bullet}^K} \mu_1^{K_0}(x') \Prb_{\U_\pi^K}[\pi^K(\Chev{x',y})=y]+\sum_{x' \not\in \Supp L_{1\bullet}^K} \mu_1^{K_0}(x') \Prb_{\U_\pi^K}[\pi^K(\Chev{x',y})=y]}\]

\[\mu_y^K(\Chev{x,y})=\frac{\mu_1^{K_0}(x)}{\sum_{x' \in \Supp L_{1\bullet}^K} \mu_1^{K_0}(x') + \sum_{x' \not\in \Supp L_{1\bullet}^K} \mu_1^{K_0}(x') \Prb_{\U_\pi^K}[\pi^K(\Chev{x',y})=y]}\]

Denote $\varepsilon(K):=\sum_{x' \not\in \Supp L_{1\bullet}^K} \mu_1^{K_0}(x')$. Clearly $\varepsilon \in \Fall$ and there is ${\alpha^K(y) \in [0,1]}$ s.t. ...
...
Construct $\bar{G}: \Words \MScheme \Words \times \Rats$ s.t. $\UM_{\bar{G}}^{K}=\UM_G^{K}$ and ${\bar{G}^{K}(x,y,z)=G^{K}(y,z)}$. 
...
\begin{equation}
\Prb_{(x,y) \sim \U^k \times \U_S^k}[f(S^k(f(x),y))=x] \in \Fall_{\text{neg}}
\end{equation}
...
\begin{equation}
\Prb_{x \sim \U^k}[f(S^k(f(x)))=x] \in \Fall_{\text{neg}}
\end{equation}
