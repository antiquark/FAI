...
\item\label{con:def__sgrow__poly} There is ${q \in \NatPoly}$ s.t. ${\forall J \in \Nats^{n-1}, k \in \Nats: \gamma(J,k) \leq \frac{1}{2}\gamma(J,q(J,k))}$.
\end{enumerate}
\end{definition}
\end{samepage}
...
To verify condition~\ref{con:def__grow__add}, consider ${\gamma_1}$, ${\gamma_2}$ s.t. ${\gamma(J,k) \leq \gamma^*(J,p_1(J,k))}$ and ${\gamma_2(J,k) \leq \gamma^*(J,p_2(J,k))}$. Choose ${p,q \in \NatPoly}$ s.t. ${p \geq \max(p_1,p_2)}$ and ${q}$ is as in condition~\ref{con:def__sgrow__poly} of Definition~\ref{def:sgrow}.
...
For any ${q \in \NatPolyJ}$ s.t. ${q \geq 1}$ there are ${\{\omega_q^K \in \mathcal{P}(\Nats)\}_{K \in \Nats^n}}$ s.t. for any ${\zeta: \NatFun \Reals}$ bounded, if 

\begin{equation}
\label{eqn:prp__fall_uni_weak__prem}
\forall J \in \Nats^{n-1}, k,k' \in \Nats: k' \geq (k+2)^{\Floor{\log q(J)}} \implies \zeta(J,k') \leq \zeta(J,k)
\end{equation}
...
\zeta(J,k) \equiv \E_{i \sim \omega_p^{Jk}}[\zeta(J,(k+2)^{\Floor{\log q(J)}}+i)] \pmod \FallU
\end{equation}
...
Fix ${p \in \NatPoly}$. Choose ${q \in \NatPolyJ}$ s.t. ${p(J,k) \leq (k+2)^{\Floor{\log q(J)}}}$. Let ${\{\omega_q^K \in \mathcal{P}(\Nats)\}_{K \in \Nats^n}}$ be as in Proposition~\ref{prp:fall_uni_weak}. Define ${\{\omega_p^K \in \mathcal{P}(\Nats)\}_{K \in \Nats^n}}$ by

\[\Prb_{i \sim \omega_p^{Jk}}[i \geq k] = \Prb_{i \sim \omega_q^{Jk}}[i + (k+2)^{\Floor{\log q(J)}} - p(J,k) \geq k]\]
...
\[\zeta(J,k) \equiv \E_{i \sim \omega_q^{Jk}}[\zeta(J, p(J,k) + i+ (k+2)^{\Floor{\log q(J)}} - p(J,k))] \pmod \FallU\]
...
Consider ${p \in \NatPoly}$, ${(\mu,f)}$ a distributional estimation problem and\\ ${P,Q: \Words \Scheme \Rats}$. Suppose that
