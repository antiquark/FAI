...
Consider $\Dist_1$ a word ensemble with polynomial-time $\EMG$-sampler $\sigma$ and $(\Dist_2, f)$ a distribution estimation problem with polynomial-time $\EMG$-sampler $\sigma$. Define the distributional estimation problem $(\Dist,\bar{f})$ by 
...
Construct $\bar{\sigma}: \Words \MScheme \Words \times \Rats$ s.t. $\UM_{\bar{\sigma}}^K=\UM_\sigma^K$ and for any ${x \in \Supp \sigma_\bullet^K}$

\[\bar{\sigma}^K(x,y,z)=(\Chev{x,\sigma^K(y,z,w)_0},\sigma^K(y,z,w)_1)\] 

By Proposition~\ref{prp:smp_base_change}, $\bar{\sigma}_0$ is an polynomial-time $\EMG$-sampler of $\Dist$ relative to $\pi$.

\[\Dist^{K} \equiv \En_*^2(\sigma_\bullet^K \times \sigma_{0\bullet}^K)\pmod \Fall\]

\[\Dist^{K} \ltimes \pi^K \equiv \En_*^2(\sigma_\bullet^K \times \sigma_{0\bullet}^K) \ltimes \pi^K \pmod \Fall\]

Let ${f_\sigma^K}$ and ${f_{\bar{\sigma}}^K}$ be defined as in Definition~\ref{def:smp_prob_rel}.

\[\E_{(x,y) \sim \Dist^{K} \ltimes \pi^K}[\Abs{f_{\bar{\sigma}}^K(x,y)-\bar{f}(x)}] \equiv \E_{(x,y) \sim \En_*^2(\sigma_\bullet^K \times \sigma_{0\bullet}^K) \ltimes \pi^K}[\Abs{f_{\bar{\sigma}}^K(x,y)-\bar{f}(x)}] \pmod \Fall\]

\[\E_{(x,y) \sim \Dist^{K} \ltimes \pi^K}[\Abs{f_{\bar{\sigma}}^K(x,y)-\bar{f}(x)}] \equiv \E_{(x_1,x_2) \sim \sigma_\bullet^K \times \sigma_{0\bullet}^K}[\Abs{f_{\bar{\sigma}}^K(\Chev{x_1,x_2},x_1)-\bar{f}(\Chev{x_1,x_2})}] \pmod \Fall\]

\[\E_{(x,y) \sim \Dist^{K} \ltimes \pi^K}[\Abs{f_{\bar{\sigma}}^K(x,y)-\bar{f}(x)}] \equiv \E_{(x_1,x_2) \sim \sigma_\bullet^K \times \sigma_{0\bullet}^K}[\Abs{\E_{\UM_{\bar{\sigma}}^K}[\bar{\sigma}_1^K(x_1) \mid \bar{\sigma}^K(x_1)_0 = \Chev{x_1,x_2}]-f(x_2)}] \pmod \Fall\]

\[\E_{(x,y) \sim \Dist^{K} \ltimes \pi^K}[\Abs{f_{\bar{\sigma}}^K(x,y)-\bar{f}(x)}] \equiv \E_{(x_1,x_2) \sim \sigma_\bullet^K \times \sigma_{0\bullet}^K}[\Abs{\E_{\UM_\sigma^K}[\sigma_1^K \mid \Chev{x_1,\sigma^K} = \Chev{x_1,x_2}]-f(x_2)}] \pmod \Fall\]

\[\E_{(x,y) \sim \Dist^{K} \ltimes \pi^K}[\Abs{f_{\bar{\sigma}}^K(x,y)-\bar{f}(x)}] \equiv \E_{(x_1,x_2) \sim \sigma_\bullet^K \times \sigma_{0\bullet}^K}[\Abs{\E_{\UM_\sigma^K}[\sigma_1^K \mid \sigma^K = x_2]-f(x_2)}] \pmod \Fall\]

\[\E_{(x,y) \sim \Dist^{K} \ltimes \pi^K}[\Abs{f_{\bar{\sigma}}^K(x,y)-\bar{f}(x)}] \equiv \E_{x_2 \sim \sigma_{0\bullet}^K}[\Abs{f_\sigma^K(x_2)-f(x_2)}] \pmod \Fall\]

\[\E_{(x,y) \sim \Dist^{K} \ltimes \pi^K}[\Abs{f_{\bar{\sigma}}^K(x,y)-\bar{f}(x)}] \equiv \E_{x_2 \sim \Dist_2^{K}}[\Abs{f_\sigma^K(x_2)-f(x_2)}] \pmod \Fall\]

\[\E_{(x,y) \sim \Dist^{K} \ltimes \pi^K}[\Abs{f_{\bar{\sigma}}^K(x,y)-\bar{f}(x)}] \equiv 0 \pmod \Fall\]
\end{proof}
