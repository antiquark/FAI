...
\begin{proposition}
%\label{prp:tbd}
Consider ${(\mu,f)}$ a distributional estimation problem, ${P: \Words \Scheme \Rats}$ bounded,\\ $m \in \Nats^{>0}$ and ${p \in \NatPolyJ}$ s.t. ${p \geq 2}$. Define ${q: \NatFun \Nats}$ by ${q(J,k):=2^{\Floor{\log p(J)\log (k+2)}^m}}$. Suppose that 
...
Substitute ${t=2^{(\log p(J) \log s)^m}}$. For some ${s_0 \in \Reals^{\geq 0}}$
