...
Consider ${(M,f)}$ a worst-case estimation problem. ${A: \Words \Scheme \Rats}$ is said to be an \emph{${\EG}$-solution to ${(M,f)}$} when...
...
Consider $(M,f)$, $(N,g)$ worst-case estimation problems and ${\pi: \Words \Scheme \Words}$. $\pi$ is called a \emph{precise $\EG$-reduction of $(M,f)$ to $(N,g)$} when there is ${\alpha: \Nats^n \Alg \Nats^n}$ s.t.
...
\item\label{con:def__wc_reduce__dist} $\sup_{x \in M^K} \Prb_{z \sim \Un_\pi^{K}}[\pi^K(x,z) \not\in N^{\alpha(K)}] \in \Fall$

\item\label{con:def__wc_reduce__fun} Denote ${\bar{g}: \Words \rightarrow \Reals}$ the extension of $g$ by 0. We require \[\sup_{x \in M^K} \E_{z \sim \Un_\pi^{K}}[\Abs{f(x)-\bar{g}(\pi^{K}(x,z))}] \in \Fall\]
...
Consider $(M,f)$, $(N,g)$ worst-case estimation problems and $\pi$ a precise $\EG$-reduction of $(M,f)$ to $(N,g)$.
