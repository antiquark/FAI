...
Next, we consider a scenario in which the identity mapping can be regarded as a reduction between distributional estimation problems that have the same function but different word ensembles.
...
\item ${\mu^{K_0}(\Supp \nu^{K_0}) \equiv 1 \pmod \Fall}$

\item There is ${W: \Words \Scheme \Rats^{\geq 0}}$ bounded s.t. ${\E_{\nu^{K_0} \times \Un_W^K}[\Abs{W^K(x)-\frac{\mu^{K_0}(x)}{\nu^{K_0}(x)}}] \in \Fall}$

\end{enumerate}

In this case, ${W}$ is called the \emph{Radon-Nikodym ${\EG}$-derivative of ${\mu}$ w.r.t. ${\nu}$}.
