...
\[\forall b \in \Bool^{\leq l(K)}: \E_{\mu^{\eta(K)} \times \Un_P^K}[(P^K(x,y)-f(x))^2] \leq \varrho^K(b) + 2(\varepsilon(K) + l(K)^{-1}) + 2^{-\epsilon l(K)^2+l(K)+2} M_0\]

%\[\E_{\mu^{\eta(K)} \times \Un_P^K}[(P^K(x,y)-f(x))^2] \overset{\alpha}{\leq} \varrho^K(\alpha(K)) \pmod \FallM\]

Consider ${Q: \Words \Scheme \Rats}$ bounded. Without loss of generality we can assume ${\sup \Abs{Q} \leq M}$ where ${M}$ is as in condition~\ref{con:def__decod__inv} of Definition~\ref{def:decod}. Choose ${q \in \NatPoly}$ s.t. ${q(K) \geq K_{n-1}}$ and for all ${K \in \Nats^n}$, \ref{eqn:thm__exists_all__q_rnd} and \ref{eqn:thm__exists_all__q_adv} hold. Denote ${\varepsilon_1(K):=2(\varepsilon(K) + l(K)^{-1}) + 2^{-\epsilon l(K)^2+l(K)+1} M_0}$. It is easy to see that ${\varepsilon_1 \in \FallM}$.  %Note that ${\varepsilon \in \FallM}$ implies ${\varepsilon \circ \alpha_q \in \FallM}$, therefore

\[\E_{\mu^{\eta(K)} \times \Un_P^{\alpha_{q+i}(K)}}[(P^{\alpha_{q+i}(K)}(x,y)-f(x))^2] \leq \varrho^{\alpha_{q+i}(K)}(a_Q^K) + \varepsilon_1(\alpha_{q+i}(K))\]

\[\E_{\mu^{\eta(K)} \times \Un_P^{\alpha_{q+i}(K)}}[(P^{\alpha_{q+i}(K)}(x,y)-f(x))^2] \leq \E_{\mu^{\eta(K)} \times \Un_Q^K}[(Q^K(x,y)-f(x))^2] + \varepsilon_1(\alpha_{q+i}(K))\]
...
\[\sup_{i \in \Nats} \E_{\mu^{\eta(K)} \times \Un_P^{\alpha_{q+i}(K)}}[(P^{\alpha_{q+i}(K)}(x,y)-f(x))^2] \leq \E_{\mu^{\eta(K)} \times \Un_Q^K}[(Q^K(x,y)-f(x))^2] \pmod \FallU\]
...
Perfect estimators are essentially objects of \enquote{classical} average-case complexity theory. It is easy to see that existence of perfect estimators for distributional decision problems of rank 1 is equivalent to existence of heuristic algorithms in the sense of \cite{Bogdanov_2006}, whereas perfect estimators for distributional decision problems of rank 2 correspond to heuristic schemes assuming the word ensemble is of the form ${\mu^\eta}$ with ${\mu}$ of rank 1.
...
% Add to previous sections analysis of the properties of ${\FallU}$
