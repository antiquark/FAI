...
Consider any ${Q: \Words \Scheme \Rats}$ bounded. Choose ${q \in \NatPoly}$ s.t.

\begin{align*}
\forall K \in \Nats^n: \R_Q(K) &\leq r(\alpha_q(K)) \\
\forall K \in \Nats^n \exists a_Q^K \in \Bool^{l(q(K))} \forall i \in \Nats, x,z \in \Words, y \in \BoolR{Q}: D(\Ev^{q(K)+i}(a_Q^K;x,yz))&=Q^K(x,y)
\end{align*}
...
\item There is ${q \in \NatPoly}$ s.t. ${\forall K \in \Nats^n: \log(K_{n-1}+4)r(K) \leq r(\alpha_q(K))}$.
...
\[\gamma'(K):=\Floor{\log(K_{n-1}+1)}\gamma(K)\]
...
\[\gamma'(K) \leq \Floor{\log(K_{n-1}+1)} r(\alpha_p(K))\]

\[\gamma'(K) \leq \Floor{\log(p(K)+4)} r(\alpha_p(K))\]

\[\gamma'(K) \leq r(\alpha_q(\alpha_p(K)))\]
%
\end{proof}

\begin{proof}[Proof of Theorem \ref{thm:exists_smp}]

Let ${D: \Words \Alg \Rats}$ be s.t. ${D(\En_\Rats(t))=t}$ and ${\T_D(x) \leq p_D(\Abs{x})}$ for some ${p_D \in \Nats[k]}$. Denote ${l(K):=\Floor{\log (K_{n-1} + 1)}}$. Construct ${A: \bm{1} \Scheme \Words}$ s.t. for any ${K \in \Nats^n}$, ${w \in \Words}$, ${\{y_i \in ...\}}$...

\begin{align}
\label{eqn:thm__exists_smp__tbd}\A_A(K) &= \A_G(K) \\
\label{eqn:thm__exists_smp__tbd}\R_A(K,w) &= l(K)^2 (\R_G(K,w) + r(K)) \\
\label{eqn:thm__exists_smp__tbd}A^K(\prod_{i \in [l(K)^2]}y_i z_i,w) &\in \Argmin{a \in \Bool^{l(K)}} \sum_{i \in [l(K)^2]}(D(\Ev^{K_{n-1}}(a;G^K(y_i,w)_0,z_i))-G^K(y_i,w)_1)^2 
\end{align}

\ref{eqn:thm__exists_smp__rnd} is legitimate due to Proposition~\ref{prp:std_grow_log}. Construct ${P: \Words \Scheme \Rats}$ s.t.

\begin{align}
\label{eqn:thm__exists_smp__adv}\A_P(K) &= \A_G(K) \\
\label{eqn:thm__exists_smp__rnd}\R_P(K,w) &= m(K) (\R_G(K,w) + r(K)) + r(K) \\
\label{eqn:thm__exists_smp__alg}P^K(x,(\prod_{i \in [m(K)]}y_i z_i)z_{m(K)},w) &= D(\Ev^{K_{n-1}}(A^K(\prod_{i \in [m(K)]}y_i z_i,w);x,z_{m(K)}))
\end{align}
TBD
%
\end{proof}
