...
\newcommand{\FallUt}[1]{{\Fall_{\textnormal{uni}}^{(t)}}}
...
For any ${n \in \Nats}$ and ${\zeta: \Nats^n \rightarrow \Reals^{\geq 0}}$, we define ${\mathcal{E}_{\zeta}}$ to be the set of ${\varepsilon: \NatFun \Reals^{\geq 0}}$ bounded s.t. there is ${M \in \Reals}$ for which ${\varepsilon \leq M \zeta}$. If there is ${h \in \NatPoly}$ s.t. ${\zeta \geq 2^{-h}}$ then ${\mathcal{E}_\zeta}$ is a fall space of rank ${n}$.
...
\begin{example}
\label{exm:e_uni}
For any ${n \in \Nats^{>0}}$ and ${t: \Nats^{n-1} \rightarrow \Nats \sqcup \{ \infty \}}$, we define ${\FallUt{t}}$, a fall space of rank ${n}$. For any ${\varepsilon: \NatFun \Reals^{\geq 0}}$ bounded, $\varepsilon \in \FallUt{t}$ iff there are ${M \in \Reals^{>0}}$ and ${p \in \NatPolyJ}$ s.t.

\begin{equation}
%\label{eqn:tbd}
\forall J \in \Nats^{n-1}: \sum_{k=2}^{t(J)-1}\frac{\varepsilon(J,k)}{k \log k} \leq M \log \log p(J)
\end{equation}

To verify condition \ref{con:def__fall__pol} note that ${2^{-K_{n-1}} \in \FallUt{t}}$.

For ${t \equiv \infty}$ we use the notation ${\FallU:=\FallUt{t}}$.
...
In particular, the conditions of Corollary~\ref{crl:complete_sharp} and Corollary~\ref{crl:complete} can hold for ${\Fall=\Fall_\zeta}$ and ${\Fall^{(m)}=\FallUt{t}}$.

\begin{samepage}
\begin{proposition}
\label{prp:tbd}

% t \zeta \leq M \log \log p
% \beta_p should increase t in a bounded way: t' \leq t^{(\log p)^M}
...
\[\sum_{k=2}^{t(J)-1} \frac{\Prb_{z \sim \Un_A^{Jk}}[A^{Jk}(z) \neq \chi_D(J)]}{k \log k} \leq M \log \log p(J)\]
...
Denote $I:=\{J \in \Nats^{n-1} \mid t(J) < q(J)\}$. We get
...
By the assumption on ${t}$, ${I}$ is a finite set therefore we got the desired result.
%
\end{proof}

For ${n=2}$, we can think of ${D}$ as a language using \emph{unary} encoding of natural numbers. Proposition~\ref{prp:tally_fall_uni}  tells us that if this language cannot be decided in quasi-polynomial time by a bounded-error probabilistic algorithm, then there is no ${\FallUt{t}(\Delta)}$-optimal estimator for ${(\mu,\chi)}$.
...
% Add extension of ${\FallU}$ and enhance negative result
% After Corollary 3.6, add special case of ${\FallU}$ (and extensions of ${\FallU}$?)
