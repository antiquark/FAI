...
Finally, we consider another scenario in which the identity mapping is a valid reduction. This scenario is a simple re-indexing of the word ensemble. For the remainder of subsection, we fix some ${m \in \Nats}$.
...
Consider ${\mu}$ a word ensemble of rank ${n}$ and ${\alpha: \Nats^m \rightarrow \Nats^n}$. We denote ${\mu^\alpha}$ the word ensemble of rank ${m}$ defined by ${(\mu^\alpha)^k:=\mu^{\alpha(k)}}$.
...
\item\label{con:def__pp_reduce__dist} ${\pi_*\mu}$ is ${\EG}$-dominated by ${\nu^\alpha}$.
