...
\[\E_{\Un_L^K}[L^K(a(K),y)] \overset{a}{\equiv} \E_{\mu^{\eta(K)} \times \Un^{r(K)}}[(D(\Ev^{K_{n-1}}(a(K);x,z))-f(x))^2]+\E_{\Un_G^K}[(f(G^K(y)_0)-G^K(y)_1)^2] \pmod \Fall\]

Denote the first term on the right hand side ${\varrho^K(a)}$ and the second term on the right hand side ${\varrho_0^K}$. ${L^K(a(K),y)}$ is the average of ${l(K)^3}$ independent and identically distributed random variables. By Proposition~\ref{prp:est_err_smp}, there is ${\varepsilon \in \Fall}$ that doesn't depend on ${a}$ s.t. the expected value of these random variables is in ${[\varrho_0^K + \varrho^K(a) - \varepsilon(K),\varrho_0^K + \varrho^K(a) + \varepsilon(K)]}$. Applying Hoeffding's inequality we conclude that for some ${\epsilon \in \Reals^{>0}}$
...
\[\Prb_{\Un_L^K}[\exists b \in \Bool^{l(K)}: L^K(b,y) < \varrho_0^K + \varrho^K(a) - \varepsilon(K) - l(K)^{-1}] \leq 2^\epsilon (K_{n-1}+2)^{-\epsilon} 2^{l(K)} \pmod \Fall\]
