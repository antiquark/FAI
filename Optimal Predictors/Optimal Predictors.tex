...
\begin{example}
\label{exm:e_uni}
For any ${n \in \Nats^{>0}}$, We define ${\FallU}$, a fall space of rank ${n}$. 

Given any ${l \in \Nats \setminus [2]}$, we denote ${\LLU^l}$ the probability measure on ${\Nats}$ given by

\begin{equation}
%\label{eqn:tbd}
\LLU^l(j):=\begin{cases}\frac{\log\log (j+1) - \log\log j}{\log\log l} \text{ if } 2 \leq j \leq l-1 \\ 0 \text{ if } j < 2 \text{ or } j \geq l\end{cases}
\end{equation}

${h: \Nats^{n-1} \rightarrow \Nats}$ is called \emph{superquasi-polynomial} when for every ${p \in \Nats[K_0, K_1 \ldots K_{n-2}]}$ and ${\epsilon \in \Reals^{>0}}$, there are only finitely many ${K \in \Nats^{n-1}}$ s.t. ${\frac{\log\log p(K)}{\log\log h(K)} > \epsilon}$.

For any ${\varepsilon: \Nats^n \rightarrow \Reals^{\geq 0}}$ bounded, $\varepsilon \in \FallU$ iff for every ${h: \Nats^{n-1} \rightarrow \Nats \setminus [2]}$ superquasi-polynomial there is ${M \in \Reals^{>0}}$ s.t.

\begin{equation}
%\label{eqn:tbd}
\forall K \in \Nats^{n-1}: \E_{j \sim \LLU^{h(K)}}[\varepsilon(K,j)] \leq \frac{M}{h(K)}
\end{equation}

\end{example}
\end{samepage}

\begin{proof}[Proof that Example~\ref{exm:e_uni} is valid]

Conditions \ref{con:def__fall__add} and \ref{con:def__fall__ineq} are obvious... TBD
\end{proof}

Example~\ref{exm:e_uni} might seem poorly motivated. Its justification lies in the existence theorems proven in Section \ref{sec:e_and_u}.
