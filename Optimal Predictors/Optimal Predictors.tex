...
Consider ${d \in \Gamma_{\textnormal{poly}}^1}$. Define ${d^n: \Nats^n \rightarrow \Nats^n}$ by ${d^n(K):=(d(K_0),K_1 \ldots K_{n-1})}$. The \emph{pullback of ${\Fall}$ by ${d}$}, denoted ${d^{-1}\Fall}$, is defined by

\begin{equation}
\label{eqn:tbd}
d^{-1}\Fall:=\{\varepsilon_d: \Nats^n \rightarrow \Reals^{\geq 0} \textnormal{ bounded} \mid \exists \varepsilon \in \Fall: \varepsilon_d \leq \varepsilon \circ d^n\}
\end{equation}
...
For any ${d \in \Gamma_{\textnormal{poly}}^1}$, ${d^{-1} \Fall}$ is a fall space
...
Consider a word ensemble ${\mu}$ and ${d: \Nats \rightarrow \Nats}$. The \emph{pullback of ${\mu}$ by ${d}$}, denoted ${d^{-1} \mu}$, is defined by ${(d^{-1}\mu)^k:=\mu^{d(k)}}$.
...
Consider $(\mu,f)$ a distributional estimation problem, ${P}$ an ${\ESG}$-optimal predictor for ${(\mu,f)}$ and ${d,l: \Nats \Alg \Nats}$. Assume that ${d \in \Gamma_{\textnormal{poly}}^1}$ as a function, ${\T_d, \T_l \in \Gamma_{\textnormal{poly}}^1}$ and ${l(d(k))=k}$. Construct ${P_d: \Words \Scheme \Rats}$ s.t.

\begin{align}
\R_{P_d}(K) &= \R_P(d^n(K)) \\
P_d^K(x,y) &= P^{d^n(K)}(x,y)
\end{align}

Then, ${P_d}$ is an ${(d^{-1}\Fall)^\sharp(\Gamma)}$-optimal predictor for ${(d^{-1}\mu,f)}$.
...
It is easy to verify that ${d^{-1} \Fall}$ is a fall space.

Define ${l^n: \Nats^n \rightarrow \Nats^n}$ by ${l^n(K):=(l(K_0),K_1 \ldots K_{n-1})}$. Consider any ${S: \Words \times \Rats \Scheme \Rats}$ bounded. Define ${S_l: \Words \times \Rats \Scheme \Rats}$ by

\begin{align*}
\R_{S_l}(K) &= \R_S(l^n(K)) \\
S_l^K(x,t,y) &= S^{l^n(K)}(x,t,y)
\end{align*}

There is ${\varepsilon \in \Fall}$ s.t.

\[\Abs{\E_{\mu^{K_0} \times \Un_P^K \times \Un_{S_l}^K}[(P^K(x,y) - f(x))S_l^K(x,P^K(x,y),z)]}=\varepsilon(K)\]

Substituting ${d^n(K)}$ for ${K}$, we get

\[\Abs{\E_{\mu^{d(K_0)} \times \Un_P^{d^n(K)} \times \Un_{S_l}^{d^n(K)}}[(P^{d^n(K)}(x,y) - f(x))S_l^{d^n(K)}(x,P^{d^n(K)}(x,y),z)]}=\varepsilon(d^n(K))\]

\[\Abs{\E_{(d^{-1}\mu)^{K_0} \times \Un_{P_d}^{K} \times \Un_{S}^{K}}[(P_d^{K}(x,y) - f(x))S^{K}(x,P_d^{K}(x,y),z)]}=\varepsilon(d^n(K))\]
%
\end{proof}
...
Consider $(\mu,f)$ a distributional estimation problem, ${P}$ an ${\EG}$-optimal predictor for ${(\mu,f)}$ and ${d,l: \Nats \Alg \Nats}$. Assume that ${d \in \Gamma_{\textnormal{poly}}^1}$ as a function, ${\T_d, \T_l \in \Gamma_{\textnormal{poly}}^1}$ and ${l(d(k))=k}$. Then, using the same notation as in Proposition~\ref{prp:idx_reduce_sharp}, ${P_d}$ is an ${(d^{-1}\Fall)(\Gamma)}$-optimal predictor for ${(d^{-1}\mu,f)}$.
...
Consider any ${Q: \Words \Alg \Rats}$ bounded. Define ${Q_l}$ in the obvious way. There is ${\varepsilon \in \Fall}$ s.t.

\[\E_{\mu^{K_0} \times \Un_P^K}[(P^K(x,y)-f(x))^2] \leq \E_{\mu^{K_0} \times \Un_{Q_l}^K}[(Q_l^K(x,y)-f(x))^2] + \varepsilon(K)\]

Substituting ${d^n(K)}$ for ${K}$, we get

\[\E_{\mu^{d(K_0)} \times \Un_P^{d^n(K)}}[(P^{d^n(K)}(x,y)-f(x))^2] \leq \E_{\mu^{d(K_0)} \times \Un_{Q_l}^{d^n(K)}}[(Q_l^{d^n(K)}(x,y)-f(x))^2] + \varepsilon({d^n(K)})\]

\[\E_{(d^{-1}\mu)^{K_0} \times \Un_{P_d}^K}[(P_d^K(x,y)-f(x))^2] \leq \E_{(d^{-1}\mu)^{K_0} \times \Un_Q^K}[(Q^K(x,y)-f(x))^2] + \varepsilon(d^n(K))\]
%
\end{proof}
...
\item As a function, ${d \in \Gamma_{\textnormal{poly}}^1}$

\item ${\T_d, \T_l \in \Gamma_{\textnormal{poly}}^1}$

\item $\forall k \in \Nats: l(d(k))=k$

\item\label{con:def__psp_reduce__dist} ${\pi_*(\mu)}$ is ${\EG}$-dominated by ${d^{-1}\nu}$.
