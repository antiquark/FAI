...
\item $G_0$ is an $\Fall(\MGrow)$-sampler (resp. $\Fall(\Gamma)$-sampler) of $\mu$.
...
When $\sup{\Abs{G_1}} < \infty$ (since $f$ is bounded, this can always be assumed without loss of generality), $G$ is called an \emph{$\Fall(\MGrow)$-generator (resp. $\Fall(\Gamma)$-generator) of $(\mu,f)$}.
...
Let $L$ be an $\Fall(\MGrow)$-sampler of $\mu$ from Proposition~\ref{prp:adv_mgamma_smp}. Clearly $L_{\bullet}^K=G_{0\bullet}^K$ therefore $G_0$ is also an $\Fall(\MGrow)$-sampler of $\mu$.
...
\item $G_0$ is an $\Fall(\MGrow)$-sampler (resp. $\Fall(\Gamma)$-sampler) of $\mu$ relative to $\pi$.
...
When $\sup{\Abs{G_1}} < \infty$, $G$ is called an \emph{$\Fall(\MGrow)$-generator (resp. $\Fall(\Gamma)$-generator) of $(\mu,f)$ relative to $\pi$}.
...
\item $\E_{(x,z) \sim \mu^{K_0} \times \U_\pi^{K}}[\Abs{f_1(x)-g(\pi^{K}(x,z))}] \in \Fall$

\item $(\mu, f_2)$ is $\Fall(\MGrow)$-generatable relative to $\pi$.

\end{enumerate}

Suppose $P_1$ is an $\Fall^\sharp(\Gamma)$-optimal predictor for $(g,\nu)$ and $P_2$ is an $\Fall^\sharp(\Gamma)$-optimal predictor for $(\mu,f_2)$. Construct $P: \Words \Scheme \Rats$ s.t. $\R_P=\R_{P_1}+\R_{P_2}+\R_{\pi}$ and for any $z_1 \in \WordsLen{\R_{P_1}(K)}$, $z_2 \in \WordsLen{\R_{P_2}(K)}$ and $z_3 \in \WordsLen{\R_\pi(K)}$
...
$$P^{K}(x,z_1 z_2 z_3)-f_1(x)f_2(x)=(P_1^{K}(\pi^{K}(x,z_3),z_1)-f_1(x))f_2(x)+P_1^{K}(\pi^{K}(x,z_3),z_1)(P_2^{K}(x,z_2)-f_2(x))$$

Denote $\bar{P}_1^{K}:=P_1^{K} \circ \pi^{K}$. For any bounded $S: \Words \times \Rats \Scheme \Rats$ we get

$$\Abs{\E[(P^{K}-f_1 f_2)S^{K}]} \leq \Abs{\E[(\bar{P}_1^{K}-f_1)f_2 S^{K}]} + \Abs{\E[(P_2^{K}-f_2) \bar{P}_1^{K} S^{K}]}$$

$P_2$ is an $\Fall^\sharp(\Gamma)$-optimal predictor for $(\mu,f_2)$ therefore the second term on the right hand side is in $\Fall$.
...
$$\Abs{\E[(P^{K}-f_1 f_2)S^{K}]} \leq \Abs{\E_{\mu^{K_0} \ltimes \pi^{K} \times \U_{\text{tot}}^{K}}[(P_1^{K}(y,z_1)-g(y))f_2(x) S^{K}(x,P_1^{K}(y,z_1)P_2^{K}(x,z_2), z_4)]} \pmod \Fall$$

$$\Abs{\E[(P^{K}-f_1 f_2)S^{K}]} \leq \Abs{\E_{\mu^{K_0} \ltimes \pi^{K}}[\E_{\U_{\text{tot}}^{K}}[(P_1^{K}(y,z_1)-g(y))S^{K}(x,P_1^{K}(y,z_1)P_2^{K}(x,z_2), z_4)]f_2(x)]} \pmod \Fall$$

Let $G$ be an $\Fall(\MGrow)$-generator of $(\mu,f_2)$ relative to $\pi$. Applying Proposition~\ref{prp:gen} to the right hand side we get
...
$$\Abs{\E[(P^{K}-f_1 f_2)S^{K}]} \leq \Abs{\E_{\nu^k \times \U_{\text{tot}}^{K} \times \UM_G^{K}}[(P_1^{K}(y)-g(y))S^{K}(G^{K}(y)_0,P_1^{K}(y)P_2^{K}(G^{K}(y)_0))G^{K}(y)_1]} \pmod \Fall$$
...
Consider $\mu_1$, $\mu_2$ word ensembles and $L_1$, $L_2$ which are $\Fall(\MGrow)$-samplers for $\mu_1$ and $\mu_2$ respectively. Define ${\mu^k:=(\En_{\Words \times \Words})_*(\mu_1^k \times \mu_2^k)}$. Suppose $\pi: \Words \Scheme \Words$ is s.t. for any $K \in \Nats^n$, $x_1 \in \Supp L_{1\bullet}^{K}$, ${x_2 \in \Supp L_{2\bullet}^{K}}$ and $z \in \Bool^{\R_\pi(K)}$, $\pi^{K}(\Chev{x_1,x_2},z)=x_1$. Then $\pi_*^K(\mu^{K_0}) \equiv \mu_1^{K_0} \pmod \Fall$
...
$$\E_{(x_1,x_2,z) \sim\mu_1^{K_0} \times \mu_2^{K_0} \times \U_\pi^{K}}[\Abs{g(x_1)-g(\pi^{K}(\Chev{x_1,x_2},z))}] \in \Fall$$
...
Consider  word ensembles $\mu_1$ and $\mu_2$ with $\Fall(\MGrow)$-samplers $L_1$ and $L_2$ respectively. Define ${\mu^k:=(\En_{\Words \times \Words})_*(\mu_1^k \times \mu_2^k)}$. Suppose ${\pi: \Words \Scheme \Words}$ is s.t. for any $K \in \Nats^n$, $x_1 \in \Supp L_{1\bullet}^{K}$, ${x_2 \in \Words}$ and $z \in \Bool^{\R_\pi(K)}$, ${\pi^{K}(\Chev{x_1,x_2},z)=x_1}$. Consider $L: \Words \MScheme \Words$ s.t. $\UM_L^{K}=\UM_{L_2}^{K}$ and for any $x_1 \in \Supp L_{1\bullet}^{K}$, ${L^{K}(x_1,z,w)=\Chev{x_1,L_2^{K}(z,w)}}$. Then, $L$ is an $\Fall(\MGrow)$-sampler of $\mu$ relative to $\pi$. In particular, since such an $L$ can always be constructed, $\mu$ is $\Fall(\MGrow)$-samplable relative to $\pi$.
...
Denote $\mu_y^K:=\mu \mid (\pi^K)^{-1}(y)$, ${\mu_L^K:=(\En_{\Words \times \Words})_*(L_{1\bullet}^k \times L_{2\bullet}^k)}$. We have $\mu^{K_0} \equiv \mu_L^K \pmod \Fall$, $\pi_*^K(\mu^{K_0}) \equiv \pi_*^K(\mu_L^K) \equiv L_{1\bullet}^K \pmod \Fall$.

\[\E_{x \sim \pi_*^K(\mu^{K_0})}[\Dtv(\mu_x^K,L_x^K)] \equiv \E_{x \sim L_{1\bullet}^K}[\Dtv(\mu_x^K,L_x^K)] \pmod \Fall\]

For any $x \in \Supp L_{1\bullet}^{K}$, $\mu_x^K = (\En_{\Words \times \Words})_*(\delta_x \times \mu_2^{K_0})$...
