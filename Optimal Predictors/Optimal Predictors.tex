...
\subsubsection{\texorpdfstring{Polynomial-Time $\MGrow$}{MΓ}-Schemes}
...
A word ensemble $\Dist$ is called \emph{$\EMG$-samplable} (resp. \emph{$\EG$-samplable}) when there is a polynomial-time $\MGrow$-scheme (resp. polynomial-time $\Gamma$-scheme) $\sigma$ of signature ${\bm{1} \rightarrow \Words}$  s.t. $\Dist^{K} \equiv \sigma_\bullet^K \pmod \Fall$.
...
A distributional estimation problem $(\Dist,f)$ is called \emph{$\EMG$-samplable (resp. $\EG$-samplable)} when there is a polynomial-time $\MGrow$-scheme (resp. polynomial-time $\Gamma$-scheme) $G$ of signature $\bm{1} \rightarrow \Words \times \Rats$ s.t. 
