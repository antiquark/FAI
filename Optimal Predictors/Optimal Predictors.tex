...
Consider $(\mu,f)$ a distributional estimation problem, ${P}$ an ${\ESG}$-optimal predictor for ${(\mu,f)}$ and ${d,l: \Nats \Alg \Nats}$. Assume that ${d \in \Gamma_{\textnormal{poly}}^1}$ as a function, ${\T_d, \T_l \in \Gamma_{\textnormal{poly}}^1}$ and ${l(d(k))=k}$. Define ${d^n: \Nats^n \rightarrow \Nats^n}$ by ${d^n(K):=(d(K_0),K_1 \ldots K_{n-1})}$. Define ${\mu_d^k:=\mu^{d(k)}}$ and

\begin{equation}
\label{eqn:tbd}
\Fall_d:=\{\varepsilon_d: \Nats^n \rightarrow \Reals^{\geq 0} \textnormal{ bounded} \mid \exists \varepsilon \in \Fall: \varepsilon_d \leq \varepsilon \circ d^n\}
\end{equation}

Construct ${P_d: \Words \Scheme \Rats}$ s.t.
...
Then, ${P_d}$ is an ${\Fall_d^\sharp(\Gamma)}$-optimal predictor for ${(\mu_d,f)}$.
...
It is easy to verify that ${\Fall_d}$ is a fall space.
...
\[\Abs{\E_{\mu_d^{K_0} \times \Un_{P_d}^{K} \times \Un_{S}^{K}}[(P_d^{K}(x,y) - f(x))S^{K}(x,P_d^{K}(x,y),z)]}=\varepsilon(d^n(K))\]
%
\end{proof}
...
\[\E_{\mu_d^{K_0} \times \Un_{P_d}^K}[(P_d^K(x,y)-f(x))^2] \leq \E_{\mu_d^{K_0} \times \Un_Q^K}[(Q^K(x,y)-f(x))^2] + \varepsilon(d^n(K))\]
%
\end{proof}
