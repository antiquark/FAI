...
\begin{equation}
\label{eqn:op_sharp}
\E_{(x,y,z) \sim \mu_P^{kj} \times \U_S^{kj}}[(P^{kj}(x,y) - f(x))S^{kj}(x,P^{kj}(x,y),z)] \equiv 0 \pmod \Fall
\end{equation}
...
\begin{equation}
\E_{\mu^k \times \U_P^{kj} \times \UM_S^{kj}}[(P^{kj}(x,y) - f(x))S^{kj}(x,P^{kj}(x,y),z)] \equiv 0 \pmod \Fall
\end{equation}
...
Denote $U_{\text{tot}}^{kj}:= \U_{P_1}^{kj} \times \U_{P_2}^{kj} \times U^{\R_{S}(k,j)}$. We change variables inside the expected value on the right hand side by $y:=\pi^{kj}(x,z_3)$. Observing that $(x,y)$ obeys the distribution $\mu^k \ltimes \pi^{kj}$ we get
