...
Fix ${D}$ an ${f}$-compatible decoding. Denote\\ ${l(K):=\Floor{\log (K_{n-1} + 2)}}$. Construct ${R: \Words \Scheme \Rats}$ s.t. for any ${K \in \Nats^n}$, $w \in \Words$, ${a \in \Bool^{l(K)}}$, ${\{y_i \in \Bool^{r_G(K,w)}\}_{i \in [l(K)^4]}}$ and ${\{z_i \in \Bool^{r(K)}\}_{i \in [l(K)^4]}}$

\begin{align}
\label{eqn:thm__exists_smp__l_adv}\A_R(K) &= \A_G(K) \\
\label{eqn:thm__exists_smp__l_rnd}\R_R(K,w) &= l(K)^4 (\R_G(K,w) + r(K)) \\
\label{eqn:thm__exists_smp__l_alg}R^K(a,\prod_{i \in [l(K)^4]}y_i z_i,w) &= \frac{1}{l(K)^4}\sum_{i \in [l(K)^4]}(D(\Ev^{K_{n-1}}(a;G^K(y_i,w)_0,z_i))-G^K(y_i,w)_1)^2 
\end{align}

That is, ${R}$ generates ${l(K)^4}$ estimates of ${f}$ using ${G}$ and computes the \enquote{empirical risk} of the program ${a}$ w.r.t. these estimates. Here, \ref{eqn:thm__exists_smp__l_rnd} is legitimate due to Proposition~\ref{prp:std_grow_log}. 
...
Perfect estimators are essentially objects of \enquote{classical} average-case complexity theory. In particular, perfect estimators for distributional decision problems of rank 1 are closely related to heuristic algorithms in the sense of \cite{Bogdanov_2006} (their existence is equivalent under mild assumptions), whereas perfect estimators for rank 2 problems of the form ${(\Dist^\eta,\chi_D)}$ with ${\Dist}$ of rank 1 are related to heuristic schemes.
