...
Given ${n \in \Nats}$, ${\Nats[K_0, K_1 \ldots K_{n-1}]}$ will stand for the set of polynomials with natural coefficients in the ${n}$ variables ${K_0, K_1 \ldots K_{n-1}}$.
...
\item For any $\gamma \in \Gamma$ there is a $p \in \Nats[K_0, K_1 \ldots K_{n-1}]$ s.t. $\gamma \leq p$.
...
For any $n \in \Nats$, $\Gamma_{\text{poly}}^n$ is a growth space of rank $n$. 

\[\Gamma_{\text{poly}}^n := \{\gamma: \Nats^n \rightarrow \Nats \mid \exists p \in \Nats[K_0, K_1 \ldots K_{n-1}]: \gamma \leq p\}\]
...
\item\label{con:def__fall__pol} There is $h \in \Nats[K_0, K_1 \ldots K_{n-1}]$ s.t. $2^{-h} \in \Fall$.
...
Let $\Gamma:=(\Gamma_{\textnormal{poly}}^1,\Gamma_0^1)$. Let $P: \Words \Scheme \Rats$ satisfy $P \equiv \frac{1}{2}$. Then, $P$ is an $\Fall_{\textnormal{neg}}(\Gamma)$-optimal estimator for $(\mu_f, \chi_B)$.
...
Let $\Gamma:=(\Gamma_{\textnormal{poly}}^1,\Gamma_0^1)$. Let $P: \Words \Scheme \Rats$ satisfy $P \equiv \frac{1}{2}$. Then, $P$ is an $\Fall_{\textnormal{neg}}(\Gamma)$-optimal estimator for $(\mu_{(f)}, \chi_f)$.
...
Let $\Gamma:=(\Gamma_{\textnormal{poly}}^1,\Gamma_{\textnormal{poly}}^1)$. Let $P: \Words \Scheme \Rats$ satisfy $P \equiv \frac{1}{2}$. Then, $P$ is an $\Fall_{\textnormal{neg}}(\Gamma)$-optimal estimator for $(\mu_f, \chi_B)$.
...
Let $\Gamma:=(\Gamma_{\textnormal{poly}}^1,\Gamma_{\textnormal{poly}}^1)$. Let $P: \Words \Scheme \Rats$ satisfy $P \equiv \frac{1}{2}$. Then, $P$ is an $\Fall_{\textnormal{neg}}(\Gamma)$-optimal estimator for $(\mu_f, \chi_f)$.
...
Consider encoded sets $X$, $Y$, $Z$ and $S: X \MScheme Y$, $T: Y \MScheme Z$. Choose\\ ${p \in \Nats[K_0, K_1 \ldots K_{n-1}]}$ s.t. 
...
Conditions \ref{con:def__fall__add} and \ref{con:def__fall__ineq} are obvious. To verify condition \ref{con:def__fall__pol}, consider ${h \in \Nats[K_0, K_1 \ldots K_{n-1}]}$ s.t. ${2^{-h} \in \Fall}$. Without loss of generality, we can assume all the coefficients of ${h}$ are non-negative and in particular, it is non-decreasing in all arguments. Consider ${p: \Nats^m \rightarrow \Nats^n}$ a polynomial map s.t. for any ${i \in [n]}$, ${\alpha_i \leq p_i}$. Clearly, ${2^{-h \circ p} \in \Fall \alpha}$.
%
\end{proof}
...
Consider  ${X}$, ${Y}$ encoded sets, ${\alpha: \Nats^m \Alg \Nats^n}$ and ${\beta \in \Gamma_{\textnormal{poly}}^{nm}}$. Assume that ${\T_\alpha \in \Gamma_{\textnormal{poly}}^m}$ and ${\forall L \in \Nats^m: \beta(\alpha(L))=L}$. Then, for any  ${S: X \xrightarrow{\Gamma \alpha} Y}$ there is ${\tilde{S}: X \Scheme Y}$ s.t. for all ${K \in \Nats^n}$ that satisfy ${\alpha(\beta(K))=K}$, ${x \in X}$ and ${y,z \in \Words}$
...
${\alpha: \Nats^m \Alg \Nats^n}$ is called an \emph{efficient injection} when ${\alpha \in \Gamma_{\textnormal{poly}}^{mn}}$ as a function, ${\T_\alpha \in \Gamma_{\textnormal{poly}}^m}$ and there is ${\beta \in \Gamma_{\textnormal{poly}}^{nm}}$ s.t. ${\forall L \in \Nats^m: \beta(\alpha(L))=L}$.
...
Assume ${\Fall \alpha}$ is ${\GrowA \alpha}$-ample. Consider $(\mu,f)$ a distributional estimation problem of rank ${m}$, $(\nu,g)$ distributional estimation problem of rank ${n}$, ${\alpha: \Nats^m \Alg \Nats^n}$ an efficient injection and $\pi$ a pseudo-invertible $\EG$-reduction of $(\mu, f)$ to $(\nu, g)$ over ${\alpha}$. Suppose there exist ${P}$ an $\EG$-optimal estimator for $(\nu, g)$ and ${\gamma \in \Gamma_{\textnormal{poly}}^m}$ s.t. ${\gamma^{-\frac{1}{2}} \in \Fall \alpha}$ and ${\gamma(\R_P \circ \alpha + \R_\pi) \in \GrowR \alpha}$. Then, there exists an $\Fall \alpha (\Gamma \alpha)$-optimal estimator for $(\mu, f)$.
...
Assume ${\Gamma_*}$ is a growth space of rank ${n}$ s.t. for any ${\gamma \in \Gamma_*}$ and ${\alpha \in \Gamma_{\textnormal{poly}}^{nn}}$, ${\gamma \circ \alpha \in \Gamma_*}$. Let ${\alpha^*, \beta^* \in \Gamma_{\textnormal{poly}}^{nn}}$ be s.t. ${\beta^*(\alpha^*(K))=K}$. Then, ${\Gamma_* \alpha^* = \Gamma_*}$.
...
In this subsection we show that certain classes of functions paired with ${\EG}$-samplable word ensembles have a distributional estimation problem which is complete w.r.t. precise pseudo-invertible ${\EG}$-reductions. In particular, ${\ESG}$-optimal estimators (or, assuming ${\Fall}$ is ${\GrowA}$-ample, ${\EG}$-optimal estimators) exist for all problems in this class iff one exists for the complete problem.

\begin{samepage}
\begin{definition}
%\label{def:tbd}

${\alpha: \Nats^n \Alg \Nats^n}$ is called an \emph{${\Fall}$-re-indexing} when

\begin{enumerate}[(i)]

\item If ${K,L \in \Nats^n}$ are s.t. ${\forall i \in [n]: K_i \leq L_i}$ then ${\forall i \in [n]: \alpha(K)_i \leq \alpha(L)_i}$

\item ${\alpha}$ is an efficient injection
...
For any ${r \in \Gamma_{\textnormal{poly}}^1}$, we define ${\Gamma_{r}}$ by 
\begin{equation}
%\label{eqn:tbd}
\Gamma_{r}:=\{\gamma \in \Gamma_{\textnormal{poly}}^n \mid \exists p \in \Nats[k] \forall k \in \Nats: \gamma(k) \leq r(p(k)) \}
\end{equation}
...
Consider any ${r \in \Gamma_{\textnormal{poly}}^1}$ non-decreasing. Then, ${\Gamma_{r}}$ is a growth space???
...
For any polynomial ${p: \Nats \rightarrow \Nats}$, define ${\alpha_p: \Nats^n \rightarrow \Nats^n}$ by ${\alpha_p(K)=(p(K_0),K_1,K_2 \ldots K_{n-1})}$. Assume that for any non-constant polynomial ${p: \Nats \rightarrow \Nats}$, ${\Fall \alpha_p \subseteq \Fall}$.

Fix  ${r \in \Gamma_{\textnormal{poly}}^n}$ non-decreasing in all arguments and ${t \in \Gamma_{\textnormal{poly}}^n}$. Let ${\Gamma:=(\Gamma_{\Fall,r},\Gamma_0^n)}$. Assume ${\zeta: \Nats^n \rightarrow (0,\frac{1}{2}]}$ is s.t. ${\zeta \in \Fall}$ and ${\Floor{\log \frac{1}{\zeta}} \in \Gamma_{\Fall,r}}$ (i.e. ${\Fall}$ is ${\Gamma_{\Fall,r}}$-ample). Consider an encoded set ${E}$ which is prefix-free, i.e. for all ${\phi,\psi \in E}$ and ${z \in \Bool^{>0}}$, ${\En_E(\phi) \ne \En_E(\psi)z}$. Consider ${\mathfrak{F}: E \times \Words \rightarrow \Reals}$ bounded. Define ${\tau^K: \Words^3 \rightarrow \Words^3}$ by ${\tau^K(a,z,w)=(a,z,\Ev^{t(K)}(a;\En_\Nats(K),w))}$. Define the distributional estimation problem $({\mu_{\mathfrak{F}}},f_{\mathfrak{F}})$ by 

\begin{align}
\mu_{\mathfrak{F}}^K&:=\En_*^3 \tau_*^K(\Un^{\Floor{\log \frac{1}{\zeta(K)}}} \times \Un^{\Floor{\log \frac{1}{\zeta(K)}}} \times \Un^{r(K)}) \\
f_{\mathfrak{F}}(\Chev{a,z,x}) &:= \begin{cases}\mathfrak{F}(\phi, x) \textnormal{ if } z=\En_E(\phi) \\ 0 \textnormal{ if } z \not\in \Img \En_E\end{cases}
\end{align}

For any ${\phi \in E}$, define ${f_\phi: \Words \rightarrow \Reals}$ by ${f_\phi(x)=\mathfrak{F}(\phi,x)}$. Then, for any ${\phi \in E}$ and ${\EG}$-samplable world ensemble ${\mu}$???, there is a precise pseudo-invertible ${\EG}$-reduction from ${(\mu,f_\phi)}$ to ${(\mu_{\mathfrak{F}}, f_{\mathfrak{F}})}$ over an ${\EG}$-re-indexing.
...
We start by constructing the reduction ${\pi}$ and the re-indexing ${\alpha}$. 

Let ${\sigma}$ be an ${\EG}$-sampler of ${\mu}$.
