...
A word ensemble $\mu$ is called \emph{$\Fall(\MGrow)$-samplable} (resp. \emph{$\Fall(\Gamma)$-samplable}) when there is an $\MGrow$-scheme (resp. $\Gamma$-scheme) $L$ of signature ${\bm{1} \rightarrow \Words}$  s.t. the function $\varepsilon(K):=\Dtv(\mu^{K_0},L_\bullet^K)$ is in $\Fall$.
...
\item $\Dtv(\nu^k,\mu^{K_0} \ltimes \pi^{K}) \in \Fall$
...
$$\Dtv(\mu_2^{K_0}, \mu^{K_0} \ltimes \pi^{K}) \leq \Dtv(\mu_2^{K_0},L_{2\bullet}^{K})+\Dtv(L_{2\bullet}^{K}, \mu^{K_0} \ltimes \pi^{K})$$

$$\Dtv(\mu_2^{K_0}, \mu^{K_0} \ltimes \pi^{K}) \leq \Dtv(L_{2\bullet}^{K}, \mu^{K_0} \ltimes \pi^{K}) \pmod \Fall$$

Denote $\mu_L^{K}:=(\En_{\Words \times \Words})_*(L_{1\bullet}^{K} \times L_{2\bullet}^{K})$. Obviously $\mu_L^{K}\ltimes \pi^{K}=L_{2\bullet}^{K}$.

$$\Dtv(\mu_2^{K_0}, \mu^{K_0} \ltimes \pi^{K}) \leq \Dtv( \mu_L^{K} \ltimes \pi^{K},  \mu^{K_0} \ltimes \pi^{K}) \pmod \Fall$$

Semi-direct products contract the total variation distance, therefore

$$\Dtv(\mu_2^{K_0}, \mu^{K_0} \ltimes \pi^{K}) \leq \Dtv(\mu_L^{K},  \mu^{K_0}) \pmod \Fall$$

$$\Dtv(\mu_2^{K_0}, \mu^{K_0} \ltimes \pi^{K}) \leq \Dtv(L_{1\bullet}^{K} \times L_{2\bullet}^{K}, \mu_1^{K_0} \times \mu_2^{K_0}) \pmod \Fall$$

Total variation distance is subadditive w.r.t. direct product, giving

$$\Dtv(\mu_2^{K_0}, \mu^{K_0} \ltimes \pi^{K}) \leq \Dtv(L_{1\bullet}^{K}, \mu_1^k) + \Dtv(L_{2\bullet}^{K}, \mu_2^k) \pmod \Fall$$

$$\Dtv(\mu_2^{K_0}, \mu^{K_0} \ltimes \pi^{K}) \equiv 0 \pmod \Fall$$
...
$$\E[\Abs{g(x_2)-g(\pi^{K}(\Chev{x_1,x_2}))}] \leq M(\Prb_{ L_{1\bullet}^{K} \times L_{2\bullet}^{K}}[(x_1,x_2) \not\in \Supp L_{1\bullet}^{K} \times \Supp L_{2\bullet}^{K}]+\Dtv(\mu_1^{K_0} \times \mu_2^{K_0},L_{1\bullet}^{K} \times L_{2\bullet}^{K}))$$

$$\E[\Abs{g(x_2)-g(\pi^{K}(\Chev{x_1,x_2}))}] \leq M \Dtv(\mu_1^{K_0} \times \mu_2^{K_0},L_{1\bullet}^{K} \times L_{2\bullet}^{K})$$

$$\E[\Abs{g(x_2)-g(\pi^{K}(\Chev{x_1,x_2}))}] \leq M (\Dtv(\mu_1^{K_0},L_{1\bullet}^{K})+\Dtv(\mu_2^{K_0},L_{2\bullet}^{K})) $$

$$\E[\Abs{g(x_2)-g(\pi^{K}(\Chev{x_1,x_2}))}] \in \Fall$$
...
Consider  word ensembles $\mu_1$ and $\mu_2$ with $\Fall(\MGrow)$-samplers $L_1$ and $L_2$ respectively. Define ${\mu^k:=(\En_{\Words \times \Words})_*(\mu_1^k \times \mu_2^k)}$. Suppose ${\pi: \Words \Scheme \Words}$ is s.t. for any $K \in \Nats^n$, $x_1 \in \Supp L_{1\bullet}^{K}$, ${x_2 \in \Supp L_{2\bullet}^{K}}$ and $z \in \Bool^{\R_\pi(K)}$, $\pi^{K}(\Chev{x_1,x_2},z)=x_2$. Consider $\bar{L}: \Words \MScheme \Words$ s.t. $\UM_{\bar{L}}^{K}=\UM_{L_1}^{K}$ and ${\bar{L}^{K}(x,y,z)=L_1^{K}(y,z)}$. Then, $\bar{L}$ is an $\Fall(\MGrow)$-sampler of $\mu$ relative to $\pi$. In particular, since such an $\bar{L}$ can always be constructed, $\mu$ is $\Fall(\MGrow)$-samplable relative to $\pi$.
