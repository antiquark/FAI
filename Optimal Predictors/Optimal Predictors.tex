...
Given $X,Y$ measurable spaces, the notation $f: X \xrightarrow{\textnormal{mk}} Y$ signifies $f$ is a Markov kernel with source $X$ and target $Y$.
...
We will use the notation $S: X \MScheme Y$ to signify $S$ is a $\MGrow$-scheme of signature $X \rightarrow Y$.

\end{definition}

\begin{definition}

Fix $n \in \Nats$, $\Grow$ a pair of growth spaces of rank $n$ and $\Fall$ a fall space of rank $n$. A word ensemble $\mu$ is called \emph{$\Fall(\MGrow)$-samplable} (resp. \emph{$\Fall(\Gamma)$-samplable}) when there is an $\MGrow$-scheme (resp. $\Gamma$-scheme) $L$ of signature $\bm{1} \rightarrow \Words$  s.t. the function $\varepsilon(K):=\Dtv(\mu^{K_1},L_*^K(\UM_L^K))$ is in $\Fall$.
...
Fix $n \in \Nats$, $\Grow$ a pair of growth spaces of rank $n$ and $\Fall$ a fall space of rank $n$. Consider word ensembles $\mu$, $\nu$ an a family ${\{\pi^K: \Supp \mu^K \rightarrow \Supp \nu^K\}_{K \in \Nats^n}}$. $\mu$ is called \emph{$\Fall(\MGrow)$-samplable (resp. $\Fall(\Gamma)$-samplable) relative to $(\nu,\pi)$} when there is an $\MGrow$-scheme (resp. $\Gamma$-scheme) $L$ of signature $\Words \rightarrow \Words$ s.t. $\E_{}[] \in \Fall$.

\end{definition}

\begin{definition}

Fix $n \in \Nats$, $\Grow$ a pair of growth spaces of rank $n$ and $\Fall$ a fall space of rank $n$. Consider a distributional estimation problem $(\mu,f)$, a word ensemble $\nu$ an a family $\{\pi^K: \Supp \mu^K \rightarrow \Supp \nu^K\}_{K \in \Nats^n}$. $(\mu,f)$ is called \emph{$\Fall(\MGrow)$-generatable (resp. $\Fall(\Gamma)$-generatable) relative to $(\nu,\pi)$} when there is an $\MGrow$-scheme (resp. $\Gamma$-scheme) $G$ of signature $\Words \rightarrow \Words \times \Rats$ s.t.
