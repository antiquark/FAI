...
Note that the last results involved passing from fall space ${\Fall}$ and growth spaces ${\Gamma}$ to fall space ${\Fall \alpha}$ and growth spaces ${\Gamma \alpha}$, however in many natural examples ${m = n}$, ${\Fall \alpha = \Fall}$ and ${\Gamma \alpha = \Gamma}$. In particular, we have the following:
...
Assume that for any ${\varepsilon \in \Fall}$ and ${\alpha \in \Gamma_{\textnormal{poly}}^{nn}}$, ${\varepsilon \circ \alpha \in \Gamma_*}$. Let ${\alpha^*, \beta^* \in \Gamma_{\textnormal{poly}}^{nn}}$ be s.t. ${\beta^*(\alpha^*(K))=K}$. Then, ${\Fall \alpha^* = \Fall}$.
...
Completely analogous to Proposition~\ref{prp:stable_growth_space}.
%
\end{proof}
...
${\alpha: \Nats^n \Alg \Nats^n}$ is called an \emph{${\EG}$-re-indexing} when it is an efficient injection, ${\Gamma \alpha = \Gamma}$ and ${\Fall \alpha \subseteq \Fall}$.
...
Assume ${\zeta: \Nats^n \rightarrow (0,\frac{1}{2}]}$ is s.t. ${\zeta \in \Fall}$ and ${\Floor{\log \frac{1}{\zeta}} \in \GrowR}$ (i.e. ${\Fall}$ is ${\GrowR}$-ample), ${\gamma \in \GrowR}$ is s.t. for any ${\gamma' \in \GrowR}$ there is an ${\EG}$-re-indexing ${\alpha}$ with ${\gamma' \leq \gamma \circ \alpha}$ and ${p: \Nats^n \rightarrow \Nats}$ is a polynomial s.t. for any polynomial ${p': \Nats^n \rightarrow \Nats}$ there is an ${\EG}$-re-indexing ${\alpha}$ with ${p' \leq p \circ \alpha}$. Consider an encoded set ${X}$ which is prefix-free, i.e. for all ${x,y \in X}$ and ${z \in \Bool^{>0}}$, ${\En_X(x) \ne \En_X(y)z}$. Consider ${\mathfrak{F}: X \times \Words \rightarrow \Reals}$ bounded. Define ${\tau^K: \Words^3 \rightarrow \Words^3}$ by ${\tau^K(x,y,z)=(x,y,\Ev^{p(K)}(x;\En_\Nats(K),z))}$. Define the distributional estimation problem $({\mu_{\mathfrak{F}}},f_{\mathfrak{F}})$ by 

\begin{align}
\mu_{\mathfrak{F}}^K&:=\En_*^3 \tau_*^K(\Un^{\Floor{\log \frac{1}{\zeta(K)}}} \times \Un^{\Floor{\log \frac{1}{\zeta(K)}}} \times \Un^{\gamma(K)}) \\
f_{\mathfrak{F}}(\Chev{x,y,z}) &:= \mathfrak{F}(y???, z) 
\end{align}
