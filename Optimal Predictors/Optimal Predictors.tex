...
A word ensemble $\mu$ is called \emph{$\EMG$-samplable} (resp. \emph{$\EG$-samplable}) when there is an $\MGrow$-scheme (resp. $\Gamma$-scheme) $L$ of signature ${\bm{1} \rightarrow \Words}$  s.t. $\mu^{K} \equiv L_\bullet^K \pmod \Fall$.
...
Denote ${I:=\{K \in \Nats^n \mid \mu^{K}(\Bool^{\leq l(K)}) > 0\}}$. Consider ${L: \bm{1} \MScheme \Words}$ s.t. for any ${K \in I}$

\begin{align*}
\M_L^K&:=\mu^{K} \mid \Bool^{\leq l(K)} \\
L^K(y,z)&=z
\end{align*}
...
Given $K \in I$, ${L_\bullet^K = \mu^{K} \mid \Bool^{\leq l(K)}}$ and we get

$$\Dtv(\mu^{K}, L_\bullet^K) = \Dtv(\mu^{K}, \mu^{K} \mid \Bool^{\leq l(K)})$$

$$\Dtv(\mu^{K}, L_\bullet^K) = \frac{1}{2} \sum_{x \in \Words} \Abs{\mu^{K}(x)-(\mu^{K} \mid \WordsLen{\leq l(K)})(x)}$$

Denote $\chi^K:=\chi_{\WordsLen{\leq l(K)}}$.

$$\Dtv(\mu^{K}, L_\bullet^K) = \frac{1}{2} \sum_{x \in \Words} \Abs{\mu^{K}(x)-\frac{\chi^K(x)\mu^{K}(x)}{\mu^{K}(\WordsLen{\leq l(K)})}}$$

$$\Dtv(\mu^{K}, L_\bullet^K) = \frac{1}{2} \sum_{x \in \Words} \mu^{K}(x) \Abs{1-\frac{\chi^K(x)}{\mu^{K}(\Bool^{\leq l(K)})}}$$

$$\Dtv(\mu^{K}, L_\bullet^K) = \frac{1}{2} (\sum_{x \in \Bool^{\leq l(K)}} \mu^{K}(x) \Abs{1-\frac{\chi^K(x)}{\mu^{K}(\Bool^{\leq l(K)})}}+\sum_{x \in \Bool^{>l(K)}} \mu^{K}(x) \Abs{1-\frac{\chi^K(x)}{\mu^{K}(\Bool^{\leq l(K)})}})$$

$$\Dtv(\mu^{K}, L_\bullet^K) = \frac{1}{2} (\sum_{x \in \Bool^{\leq l(K)}} \mu^{K}(x)(\frac{1}{\mu^{K}(\Bool^{\leq l(K)})}-1)+\sum_{x \in \Bool^{>l(K)}} \mu^{K}(x))$$

$$\Dtv(\mu^{K}, L_\bullet^K) = \frac{1}{2} (\mu^{K}(\Bool^{\leq l(K)})(\frac{1}{\mu^{K}(\Bool^{\leq l(K)})}-1)+1 - \mu^{K}(\Bool^{\leq l(K)}))$$

$$\Dtv(\mu^{K}, L_\bullet^K) = 1-\mu^{K}(\Bool^{\leq l(K)})$$

Given arbitrary $K \in \Nats^n$,

$$\Dtv(\mu^{K}, L_\bullet^K) \leq \max(1-\mu^{K}(\Bool^{\leq l(K)}), 1-\chi_I)$$
\end{proof}
...
Consider a word ensemble $\mu$, an encoded set $Y$ and a family ${\{\pi^K: \Supp \mu^{K} \Markov Y\}_{K \in \Nats^n}}$. $\mu$ is called \emph{$\EMG$-samplable (resp. $\EG$-samplable) relative to $\pi$} when there is an $\MGrow$-scheme (resp. $\Gamma$-scheme) $L$ of signature $Y \rightarrow \Words$ s.t. ${\E_{y \sim \pi_*^K\mu^{K}}[\Dtv(\mu \mid (\pi^K)^{-1}(y),L_y^K)] \in \Fall}$.
...
Consider a word ensemble $\mu$, an encoded set $Y$, a family ${\{\pi^K: \Supp \mu^{K} \Markov Y\}_{K \in \Nats^n}}$ and ${h: (\Supp \mu) \times Y \rightarrow \Reals}$ bounded. Suppose $L$ is an $\EMG$-sampler of $\mu$ relative to $\pi$. Then

\begin{equation}
\label{eqn:prp__smp}
\E_{(x,y) \sim \mu^{K} \ltimes \pi^K}[h(x,y)] \equiv \E_{(y,z) \sim \pi_*^K\mu^{K} \times \UM_L^K}[h(L^K(y,z),y)] \pmod \Fall
\end{equation}
...
$$\E_{\mu^{K} \ltimes \pi^K}[h(x,y)] - \E_{\pi_*^K\mu^{K} \times \UM_L^K}[h(L^K(y,z),y)] = \E_{\mu^{K} \ltimes \pi^K}[\E_{\mu_y^K}[h(x',y)]] - \E_{\pi_*^K\mu^{K} \times \UM_L^K}[h(L^K(y,z),y)]$$

$$\E_{\mu^{K} \ltimes \pi^K}[h(x,y)] - \E_{\pi_*^K\mu^{K} \times \UM_L^K}[h(L^K(y,z),y)] = \E_{\mu^{K} \ltimes \pi^K}[\E_{\mu_y^K}[h(x',y)]-\E_{\UM_L^K}[h(L^K(y,z),y)]]$$

$$\E_{\mu^{K} \ltimes \pi^K}[h(x,y)] - \E_{\pi_*^K\mu^{K} \times \UM_L^K}[h(L^K(y,z),y)] = \E_{\mu^{K} \ltimes \pi^K}[\E_{\mu_y^K}[h(x',y)]-\E_{L_y^K}[h(x',y)]]$$

$$\Abs{\E_{\mu^{K} \ltimes \pi^K}[h(x,y)] - \E_{\pi_*^K\mu^{K} \times \UM_L^K}[h(L^K(y,z),y)]} \leq \E_{\mu^{K} \ltimes \pi^K}[\Abs{\E_{\mu_y^K}[h(x',y)]-\E_{L_y^K}[h(x',y)]}]$$

$$\Abs{\E_{\mu^{K} \ltimes \pi^K}[h(x,y)] - \E_{\pi_*^K\mu^{K} \times \UM_L^K}[h(L^K(y,z),y)]} \leq (\sup h - \inf h) \E_{\mu^{K} \ltimes \pi^K}[\Dtv(\mu_y^K,L_y^K)]$$
...
Consider $\mu_1$, $\mu_2$ word ensembles and $L_1$, $L_2$ which are $\EMG$-samplers for $\mu_1$ and $\mu_2$ respectively. Define ${\mu^k:=\En_*^2(\mu_1^k \times \mu_2^k)}$. Suppose $\pi: \Words \Scheme \Words$ is s.t. for any $K \in \Nats^n$, $x_1 \in \Supp L_{1\bullet}^{K}$, ${x_2 \in \Supp L_{2\bullet}^{K}}$ and $z \in \Bool^{\R_\pi(K)}$, $\pi^{K}(\Chev{x_1,x_2},z)=x_1$. Then $\pi_*^K\mu^{K} \equiv \mu_1^{K} \pmod \Fall$
...
$L_{1\bullet}^{K} \equiv \mu_1^{K} \pmod \Fall$ and $L_{2\bullet}^{K} \equiv \mu_2^{K} \pmod \Fall$.  By Proposition~\ref{prp:prob_cong_dir},

\[L_{1\bullet}^{K} \times L_{2\bullet}^{K} \equiv \mu_1^{K} \times \mu_2^{K} \pmod \Fall\]

Denote $\mu_L^{K}:=\En_*^2(L_{1\bullet}^{K} \times L_{2\bullet}^{K})$. We get ${\mu_L^{K} \equiv \mu^{K} \pmod \Fall}$ and therefore ${\pi_*^K\mu_L^{K} \equiv \pi_*^K\mu^{K} \pmod \Fall}$ (by Proposition~\ref{prp:prob_cong_push}). Obviously $\pi_*^K\mu_L^{K}=L_{1\bullet}^{K}$. We conclude that ${\pi_*^K\mu^{K} \equiv L_{1\bullet}^{K} \pmod \Fall}$ and therefore ${\pi_*^K\mu^{K} \equiv \mu_1 \pmod \Fall}$ (by Proposition~\ref{prp:prob_cong_eq}).
\end{proof}
...
\[\mu^{K} \equiv \En_*^2(L_{1\bullet}^K \times L_{2\bullet}^K)\pmod \Fall\]

\[\pi_*^K\mu^{K} \equiv \pi_*^K\En_*^2(L_{1\bullet}^K \times L_{2\bullet}^K) \pmod \Fall\] 

\[\pi_*^K\mu^{K} \equiv  L_{1\bullet}^K \pmod \Fall\]

Denote $\mu_x^K:=\mu \mid (\pi^K)^{-1}(x)$.

\[\E_{x \sim \pi_*^K\mu^{K}}[\Dtv(\mu_x^K,L_x^K)] \equiv \E_{x \sim L_{1\bullet}^K}[\Dtv(\mu_x^K,L_x^K)] \pmod \Fall\]

For any $x \in \Supp L_{1\bullet}^{K}$, $\mu_x^K = \En_*^2(\delta_x \times \mu_2^{K})$ and $L_x^K=\En_*^2(\delta_x \times L_{2\bullet}^K)$.

\[\E_{x \sim \pi_*^K\mu^{K}}[\Dtv(\mu_x^K,L_x^K)] \equiv \E_{x \sim L_{1\bullet}^K}[\Dtv(\En_*^2(\delta_x \times \mu_2^{K}),\En_*^2(\delta_x \times L_{2\bullet}^K))] \pmod \Fall\]

\[\E_{x \sim \pi_*^K\mu^{K}}[\Dtv(\mu_x^K,L_x^K)] \equiv \E_{x \sim L_{1\bullet}^K}[\Dtv(\mu_2^{K},L_{2\bullet}^K)] \pmod \Fall\]

\[\E_{x \sim \pi_*^K\mu^{K}}[\Dtv(\mu_x^K,L_x^K)] \equiv \Dtv(\mu_2^{K},L_{2\bullet}^K) \pmod \Fall\]

\[\E_{x \sim \pi_*^K\mu^{K}}[\Dtv(\mu_x^K,L_x^K)] \equiv 0 \pmod \Fall\]
\end{proof}
...
\[\mu^{K} \equiv \En_*^2(L_\bullet^K \times G_{0\bullet}^K)\pmod \Fall\]

\[\mu^{K} \ltimes \pi^K \equiv \En_*^2(L_\bullet^K \times G_{0\bullet}^K) \ltimes \pi^K \pmod \Fall\]

\[\E_{(x,y) \sim \mu^{K} \ltimes \pi^K}[\Abs{f_{\bar{G}}^K(x,y)-\bar{f}(x)}] \equiv \E_{(x,y) \sim \En_*^2(L_\bullet^K \times G_{0\bullet}^K) \ltimes \pi^K}[\Abs{f_{\bar{G}}^K(x,y)-\bar{f}(x)}] \pmod \Fall\]

\[\E_{(x,y) \sim \mu^{K} \ltimes \pi^K}[\Abs{f_{\bar{G}}^K(x,y)-\bar{f}(x)}] \equiv \E_{(x_1,x_2) \sim L_\bullet^K \times G_{0\bullet}^K}[\Abs{f_{\bar{G}}^K(\Chev{x_1,x_2},x_1)-\bar{f}(\Chev{x_1,x_2})}] \pmod \Fall\]

\[\E_{(x,y) \sim \mu^{K} \ltimes \pi^K}[\Abs{f_{\bar{G}}^K(x,y)-\bar{f}(x)}] \equiv \E_{(x_1,x_2) \sim L_\bullet^K \times G_{0\bullet}^K}[\Abs{\E_{\UM_{\bar{G}}^K}[\bar{G}_1^K(x_1) \mid \bar{G}^K(x_1)_0 = \Chev{x_1,x_2}]-f(x_2)}] \pmod \Fall\]

\[\E_{(x,y) \sim \mu^{K} \ltimes \pi^K}[\Abs{f_{\bar{G}}^K(x,y)-\bar{f}(x)}] \equiv \E_{(x_1,x_2) \sim L_\bullet^K \times G_{0\bullet}^K}[\Abs{\E_{\UM_G^K}[G_1^K \mid \Chev{x_1,G^K} = \Chev{x_1,x_2}]-f(x_2)}] \pmod \Fall\]

\[\E_{(x,y) \sim \mu^{K} \ltimes \pi^K}[\Abs{f_{\bar{G}}^K(x,y)-\bar{f}(x)}] \equiv \E_{(x_1,x_2) \sim L_\bullet^K \times G_{0\bullet}^K}[\Abs{\E_{\UM_G^K}[G_1^K \mid G^K = x_2]-f(x_2)}] \pmod \Fall\]
...
\[\E_{\mu_{PS}^K}[(\bar{P}^{K} - \bar{f})S^{K}] \equiv \E_{\UM_L^K}[\E_{\mu_2^{K} \times \Un_{PS}^K}[(\bar{P}^K(\Chev{L^K,x_2}) - f(x_2))S^K(\Chev{L^K,x_2},\bar{P}^K(\Chev{L^K,x_2}))]] \pmod \Fall\]

\[\E_{\mu_{PS}^K}[(\bar{P}^{K} - \bar{f})S^{K}] \equiv \E_{\UM_L^K}[\E_{\mu_2^{K} \times \Un_{PS}^K}[(P^K(x_2) - f(x_2))S^K(\Chev{L^K,x_2},P^K(x_2))]] \pmod \Fall\]

\[\E_{\mu_{PS}^K}[(\bar{P}^{K} - \bar{f})S^{K}] \equiv \E_{\mu_2^{K} \times \Un_{PS}^K \times \UM_L^K}[(P^K(x_2) - f(x_2))S^K(\Chev{L^K,x_2},P^K(x_2))] \pmod \Fall\]
...
\[\E_{\nu^{K} \times \Un_P^K}[(P^K-g)^2] \leq \E_{\nu^{K} \times \Un_Q^K \times \UM_L^K}[((Q \circ L)^K - g)^2] \pmod \Fall\]

Using condition~\ref{con:def__psp_reduce__dist} of Definition~\ref{def:psp_reduce}

\[\E_{\pi_*^K\mu^{K} \times \Un_P^K}[(P^K-\bar{g})^2] \leq \E_{\pi_*^K\mu^{K} \times \Un_Q^K \times \UM_L^K}[((Q \circ L)^K - \bar{g})^2] \pmod \Fall\]

\[\E_{\pi_*^K\mu^{K} \times \Un_P^K}[(P^K-\bar{g})^2] \leq \E_{\pi_*^K\mu^{K} \times \UM_L^K}[\E_{\Un_Q^K}[((Q \circ L)^K - \bar{g})^2]] \pmod \Fall\]
...
\[\E[(P_f^K - f)S] \equiv \frac{1}{\gamma(K)} \sum_{i \in [\gamma(K)]} \E_{\nu^{K} \times \Un_i^K}[(P_g^K - g)S(L^K,\frac{1}{\gamma(K)}(P_g^K+\sum_{j \in [\gamma(K)] \setminus i} P_g^K(\pi^K(L^K,z_j))))] \pmod \Fall\]
...
\[\E_{\pi_*^K\mu^{K} \times \Un_{P_g}^K}[(P_g^K-\bar{g})^2] \leq \E_{\pi_*^K\mu^{K} \times \UM_{Q_g}^K}[((Q_f \circ L)^K - (P_f \circ L)^K + P_g^K - \bar{g})^2] \pmod \Fall\]
...
Consider ${\Gamma_*}$ a growth space of rank ${n}$ and ${d \in \Gamma_{\textnormal{poly}}^n}$. We introduce the notation

\begin{equation}
\label{eqn:tbd}
\Gamma_*d:=\{\gamma_d: \Nats^n \rightarrow \Reals^{\geq 0} \textnormal{ bounded} \mid \exists \gamma \in \Gamma_*: \gamma_d \leq \gamma \circ d\}
\end{equation}

Obviously ${\Gamma_*d}$ is also a growth space of rank ${n}$.

We also denote ${\Gamma d := (\GrowR d, \GrowA d)}$.
...
Consider ${d \in \Gamma_{\textnormal{poly}}^n}$. We introduce the notation

\begin{equation}
\label{eqn:tbd}
\Fall d:=\{\varepsilon_d: \Nats^n \rightarrow \Reals^{\geq 0} \textnormal{ bounded} \mid \exists \varepsilon \in \Fall: \varepsilon_d \leq \varepsilon \circ d\}
\end{equation}
...
For any ${d \in \Gamma_{\textnormal{poly}}^1}$, ${\Fall d}$ is a fall space
...
Consider a ${\mu}$ word ensemble and ${d: \Nats^n \rightarrow \Nats^n}$. We denote ${\mu^d}$ the word ensemble defined by ${(\mu^d)^k:=\mu^{d(k)}}$.
...
Consider ${X}$, ${Y}$ encoded sets, ${S: X \Scheme Y}$ and ${d: \Nats^n \Alg \Nats^n}$ s.t. ${d \in \Gamma_{\textnormal{poly}}^n}$ as a function and ${\T_d \in \Gamma_{\textnormal{poly}}^n}$. We define ${S^d: X \xrightarrow{\Gamma d} Y}$ by ${\R_{S^d}(K)=\R_S(d(K))}$ and ${(S^d)^K(x,y)=S^{d(K)}(x,y)}$.
...
Consider  ${X}$, ${Y}$ encoded sets and ${d,l: \Nats^n \Alg \Nats^n}$. Assume that ${d,l \in \Gamma_{\textnormal{poly}}^n}$ as functions, ${\T_d, \T_l \in \Gamma_{\textnormal{poly}}^n}$ and ${l(d(K))=K}$. Then, for any  ${S: X \xrightarrow{\Gamma d} Y}$ there is ${\tilde{S}: X \Scheme Y}$ s.t. for all ${K \in \Nats^n}$ that satisfy ${d(l(K))=K}$, ${x \in X}$ and ${y,z \in \Words}$

\begin{align}
\A_{\tilde{S}}(K)&=\A_S(l(K)) \\
\R_{\tilde{S}}^K(z)&=\R_S^{l(K)}(z) \\
\tilde{S}^K(x,y,z)&=S^{l(K)}(x,y,z)
\end{align}
...
There is ${\gamma_{\mathfrak{R}} \in \GrowR}$ s.t. ${\R_S(K) \leq \gamma_{\mathfrak{R}}(d(K))}$ and ${\gamma_{\mathfrak{A}} \in \GrowA}$ s.t. ${\Abs{\A_S(K)} \leq \gamma_{\mathfrak{A}}(d(K))}$. In particular, if ${K \in \Nats^n}$ is s.t. ${d(l(K))=K}$ then ${\R_S(l(K)) \leq \gamma_{\mathfrak{R}}(K)}$ and ${\Abs{\A_S(l(K))} \leq \gamma_{\mathfrak{A}}(K)}$.
%
\end{proof}
...
Consider $(\mu,f)$ a distributional estimation problem, ${P}$ an ${\ESG}$-optimal predictor for ${(\mu,f)}$ and ${d,l: \Nats^n \Alg \Nats^n}$. Assume that ${d,l \in \Gamma_{\textnormal{poly}}^n}$ as functions, ${\T_d, \T_l \in \Gamma_{\textnormal{poly}}^n}$ and ${l(d(K))=K}$. Then, ${P^d}$ is an ${\Fall d^\sharp(\Gamma d)}$-optimal predictor for ${(\mu^d,f)}$.
...
Consider any ${S: \Words \times \Rats \xrightarrow{\Gamma d} \Rats}$ bounded. Construct ${\tilde{S}: \Words \times \Rats \Scheme \Rats}$ by applying Proposition~\ref{prp:rev_sch_idx} to ${S}$. There is ${\varepsilon \in \Fall}$ s.t. for any ${K \in \Nats^n}$

\[\Abs{\E_{\mu^{K} \times \Un_P^K \times \Un_{\tilde{S}}^K}[(P^K(x,y) - f(x))\tilde{S}^K(x,P^K(x,y),z)]}=\varepsilon(K)\]

Substituting ${d(K)}$ for ${K}$, we get

\[\Abs{\E_{\mu^{d(K)} \times \Un_P^{d(K)} \times \Un_{\tilde{S}}^{d(K)}}[(P^{d(K)}(x,y) - f(x))\tilde{S}^{d(K)}(x,P^{d(K)}(x,y),z)]}=\varepsilon(d(K))\]

\[\Abs{\E_{(\mu^d)^{K} \times \Un_{P^d}^{K} \times \Un_{\tilde{S}}^{d(K)}}[((P^d)^{K}(x,y) - f(x))\tilde{S}^{d(K)}(x,(P^d)^{K}(x,y),z)]}=\varepsilon(d(K))\]

We have ${d(l(d(K))=d(K)}$, therefore

\[\Abs{\E_{(\mu^d)^{K} \times \Un_{P^d}^{K} \times \Un_{S}^{l(d(K))}}[((P^d)^{K}(x,y) - f(x))S^{l(d(K))}(x,(P^d)^{K}(x,y),z)]}=\varepsilon(d(K))\]

\[\Abs{\E_{(\mu^d)^{K} \times \Un_{P^d}^{K} \times \Un_{S}^{K}}[((P^d)^{K}(x,y) - f(x))S^{K}(x,(P^d)^{K}(x,y),z)]}=\varepsilon(d(K))\]
%
\end{proof}
...
Consider $(\mu,f)$ a distributional estimation problem, ${P}$ an ${\EG}$-optimal predictor for ${(\mu,f)}$ and ${d,l: \Nats^n \Alg \Nats^n}$. Assume that ${d,l \in \Gamma_{\textnormal{poly}}^n}$ as functions, ${\T_d, \T_l \in \Gamma_{\textnormal{poly}}^n}$ and ${l(d(K))=K}$. Then, ${P^d}$ is an ${\Fall d(\Gamma d)}$-optimal predictor for ${(\mu^d,f)}$.
...
Consider any ${Q: \Words \xrightarrow{\Gamma d} \Rats}$ bounded. Construct ${\tilde{Q}: \Words \Scheme \Rats}$ by applying Proposition~\ref{prp:rev_sch_idx} to ${Q}$. There is ${\varepsilon \in \Fall}$ s.t.

\[\E_{\mu^{K} \times \Un_P^K}[(P^K(x,y)-f(x))^2] \leq \E_{\mu^{K} \times \Un_{\tilde{Q}}^K}[(\tilde{Q}^K(x,y)-f(x))^2] + \varepsilon(K)\]

Substituting ${d(K)}$ for ${K}$, we get

\[\E_{\mu^{d(K)} \times \Un_P^{d(K)}}[(P^{d(K)}(x,y)-f(x))^2] \leq \E_{\mu^{d(K)} \times \Un_{\tilde{Q}}^{d(K)}}[(\tilde{Q}^{d(K)}(x,y)-f(x))^2] + \varepsilon({d(K)})\]

\[\E_{(\mu^d)^{K} \times \Un_{P^d}^K}[((P^d)^K(x,y)-f(x))^2] \leq \E_{(\mu^d)^{K} \times \Un_{\tilde{Q}}^{d(K)}}[(\tilde{Q}^{d(K)}(x,y)-f(x))^2] + \varepsilon(d(K))\]

We have ${d(l(d(K))=d(K)}$, therefore

\[\E_{(\mu^d)^{K} \times \Un_{P^d}^K}[((P^d)^K(x,y)-f(x))^2] \leq \E_{(\mu^d)^{K} \times \Un_Q^{l(d(K))}}[(Q^{l(d(K))}(x,y)-f(x))^2] + \varepsilon(d(K))\]

\[\E_{(\mu^d)^{K} \times \Un_{P^d}^K}[((P^d)^K(x,y)-f(x))^2] \leq \E_{(\mu^d)^{K} \times \Un_Q^K}[(Q^K(x,y)-f(x))^2] + \varepsilon(d(K))\]
%
\end{proof}
...
Consider $(\mu,f)$, $(\nu,g)$ distributional estimation problems, ${d: \Nats^n \Alg \Nats^n}$ and\\ ${\pi: \Words \Scheme \Words}$. $\pi$ is called a \emph{degree ${d}$ precise pseudo-invertible $\EG$-reduction of $(\mu,f)$ to $(\nu,g)$} when there is ${l: \Nats^n \Alg \Nats^n}$ s.t.

\begin{enumerate}[(i)]

\item\label{con:def__pp_reduce__pol_deg} As functions, ${d,l \in \Gamma_{\textnormal{poly}}^n}$

\item\label{con:def__pp_reduce__eff_deg} ${\T_d, \T_l \in \Gamma_{\textnormal{poly}}^n}$

\item\label{con:def__pp_reduce__inv_deg} $\forall K \in \Nats^n: l(d(K))=K$

\item\label{con:def__pp_reduce__dist} ${(\pi_*\mu)^d}$ is ${\EG}$-dominated by ${\nu}$.
...
Consider $(\mu,f)$, $(\nu,g)$ distributional estimation problems, ${d: \Nats^n \Alg \Nats^n}$ and $\pi$ a degree ${d}$ precise pseudo-invertible $\EG$-reduction of $(\mu, f)$ to $(\nu, g)$. Suppose $P$ is an $\ESG$-optimal predictor for $(\nu, g)$. Then, $(P \circ \pi)^d$ is an $\Fall d^\sharp (\Gamma d)$-optimal predictor for $(\mu, f)$.
...
By Corollary~\ref{crl:psp_reduce_sharp} and conditions \ref{con:def__pp_reduce__fun} and \ref{con:def__pp_reduce__smp} of Definition~\ref{def:pp_reduce}, ${P \circ \pi}$ is an ${\ESG}$-optimal predictor for ${(\pi_* \mu, g)}$.   By Proposition~\ref{prp:idx_reduce_sharp} and conditions \ref{con:def__pp_reduce__pol_deg}, \ref{con:def__pp_reduce__eff_deg} and \ref{con:def__pp_reduce__inv_deg} of Definition~\ref{def:pp_reduce}, ${(P \circ \pi)^d}$ is an ${\Fall d^\sharp(\Gamma d)}$-optimal predictor for ${((\pi_* \mu)^d, g)}$. By Proposition~\ref{prp:dom_reduce_sharp} and condition \ref{con:def__pp_reduce__dist} of Definition~\ref{def:pp_reduce}, ${(P \circ \pi)^d}$ is also an ${\ESG}$-optimal predictor for ${(\nu, g)}$.
%
\end{proof}

%When \Gamma is invariant under any appropriate ${d}$, it follows that ${\Gamma d = d}$
%Often \Fall d = \Fall as well
