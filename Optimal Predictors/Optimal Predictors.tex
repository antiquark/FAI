...
In this subsection we study notions of Karp reduction between distributional estimation problems such that the pull-back of an optimal estimator is an optimal estimator. It is also interesting to study Cook reductions but we avoid it in the present work.
...
We will describe several types of reductions that preserve optimal estimators. After that, we will characterize reductions that can be constructed by composing those types.

\subsubsection{Strict Pseudo-Invertible Reductions}
...
\subsubsection{Ensemble Pullbacks}
...
In this subsection we assume ${n = 1}$ and $\Gamma=(\GammaPoly^1,\Gamma_0^1)$.

We will show that certain classes of functions paired with ${\EG}$-samplable word ensembles have a distributional estimation problem which is complete w.r.t. precise pseudo-invertible ${\EG}$-reductions. In particular, ${\ESG}$-optimal estimators (or, assuming ${\Fall}$ is ${\GrowA}$-ample, ${\EG}$-optimal estimators) exist for all problems in this class iff one exists for the complete problem.

% TODO: Insert unary string of length t(\phi,k) in definition of complete problem

\begin{samepage}
\begin{theorem}
\label{thm:complete}

Assume that for any non-constant ${p \in \Nats[k]}$, ${\Fall p \subseteq \Fall}$. Consider an encoded set ${E}$ which is prefix-free, i.e. for all ${\phi,\psi \in E}$ and ${z \in \Bool^{>0}}$, ${\En_E(\phi) \ne \En_E(\psi)z}$. Consider ${\mathfrak{F}: E \times \Words \rightarrow \Reals}$ bounded and ${\{D_\phi: \Nats \Alg \Words\}_{\phi \in E}}$ s.t. for all ${\phi \in E}$, ${\T_{D_\phi} \in \GammaPoly^1}$. Define ${\zeta^k: \Words^2 \rightarrow \Words^2}$ and ${\xi^k: \Words \rightarrow \Words^2}$ by 
...
For any ${\phi \in E}$, define ${f_\phi: \Words \rightarrow \Reals}$ by ${f_\phi(x)=\mathfrak{F}(\phi,x)}$. Then, for any ${\phi \in E}$ and ${\EG}$-samplable world ensemble ${\mu}$, there is a precise pseudo-invertible ${\EG}$-reduction from ${(\mu,f_\phi)}$ to ${(\mu_{\mathfrak{F},D}, f_{\mathfrak{F}})}$ over a polynomial.
...
Let ${\sigma}$ be an ${\EG}$-sampler of ${\mu}$. Choose ${p \in \Nats[k]}$ and ${a \in \Words}$ s.t. for any ${k \in \Nats}$, ${z \in \Words}$, ${w_1 \in \Bool^{\R_\sigma(k)}}$ and ${w_2 \in \Words}$, ${\R_\sigma(k) \leq p(k)}$ and ${\Ev^{p(k)}(az;\En_\Nats(p(k)),w_1 w_2)=\sigma^k(w_1)}$. Denote ${b=\En_E(\phi)}$. We construct ${\pi: \Words \Scheme \Words}$ s.t. for any ${k \gg 0}$, ${x \in \Supp \sigma_{\bullet}^k}$,  ${z_b \in \Bool^{p(k)-\Abs{b}}}$ and ${z_a \in \Bool^{p(k)-\Abs{a}}}$
...
To verify condition~\ref{con:def__pp_reduce__dist} of Definition~\ref{def:pp_reduce}, fix ${q \in \Nats[k]}$ s.t.
...
Construct ${W: \Words \Scheme \Rats^{\geq 0}}$ s.t. 
...
${\mu^k \equiv \sigma_\bullet^k \pmod \Fall}$ since ${\sigma}$ is an ${\EG}$-sampler of ${\mu}$. By Proposition~\ref{prp:prob_cong_push}, ${\pi_*^k\mu^k \equiv \pi_*^k\sigma_\bullet^k \pmod \Fall}$. It follows that

\[\sum_{y \in \Words} \Abs{\mu_{\mathfrak{F}}^{p(k)}(y)W^k(y)-(\pi_*^k\mu^k)(y)} \equiv \sum_{y \in \Words} \Abs{\mu_{\mathfrak{F}}^{p(k)}(y)W^k(y)-(\pi_*^k\sigma_\bullet^k)(y)} \pmod \Fall\]

Given ${k \gg 0}$, for any ${y \in \Words}$, if ${W^k(y)=0}$ then ${(\pi_*^k\sigma_\bullet^k)(y) = 0}$. We get

\[\sum_{\Words} \Abs{\mu_{\mathfrak{F}}^{p(k)}W^k-(\pi_*^k\mu^k)} \equiv \sum_{\substack{z_b,z_a \in \Bool^{\leq q(k)}\\x \in \Bool^{\leq q(k)}}} \Abs{\mu_{\mathfrak{F}}^{p(k)}(\Chev{bz_b, D_\phi(p(k)), az_a, x})2^{\Abs{a}+\Abs{b}}-(\pi_*^k\sigma_\bullet^k)(\Chev{bz_b,D_\phi(p(k)),az_a,x})} \pmod \Fall\]

\[\sum_{\Words} \Abs{\mu_{\mathfrak{F}}^{p(k)}W^k-\pi_*^k\mu^k} \equiv \sum_{\substack{z_b \in \Bool^{p(k)-\Abs{b}}\\z_a \in \Bool^{p(k)-\Abs{a}}\\x \in \Bool^{\leq q(k)}}} \Abs{2^{-p(k)}2^{-p(k)}\sigma_\bullet^k(x) 2^{\Abs{a}+\Abs{b}}-(\pi_*^k\sigma_\bullet^k)(\Chev{bz_b, D_\phi(p(k)),az_a,x})} \pmod \Fall\]

\[\sum_{\Words} \Abs{\mu_{\mathfrak{F}}^{p(k)}W^k-\pi_*^k\mu^k} \equiv \sum_{\substack{z_1 \in \Bool^{p(k)-\Abs{a}}\\z_2 \in \Bool^{p(k)-\Abs{b}}\\x \in \Bool^{\leq q(k)}}} \Abs{2^{-2p(k)+\Abs{a}+\Abs{b}}\sigma_\bullet^k(x) -(\pi_*^k\sigma_\bullet^k)(\Chev{bz_b, D_\phi(p(k)),az_a,x})} \pmod \Fall\]

\[\sum_{\Words} \Abs{\mu_{\mathfrak{F}}^{p(k)}W^k-\pi_*^k\mu^k} \equiv \sum_{\substack{z_1 \in \Bool^{p(k)-\Abs{a}}\\z_2 \in \Bool^{p(k)-\Abs{b}}\\x \in \Bool^{\leq q(k)}}} \Abs{2^{-2p(k)+\Abs{a}+\Abs{b}}\sigma_\bullet^k(x) -2^{-(p(k)-\Abs{a})}2^{-(p(k)-\Abs{b})}\sigma_\bullet^k(x)} \pmod \Fall\]
...
\[\E_{\mu^k \times \Un_\pi^k}[\Abs{f_\phi(x)-f_{\mathfrak{F}}(\pi^k(x,z))}] \equiv \E_{\sigma_\bullet^k \times \Un_\pi^k}[\Abs{f_\phi(x)-f_{\mathfrak{F}}(\Chev{bz_b,D_\phi(p(k)),az_a,x})}] \pmod \Fall\]
...
To verify condition~\ref{con:def__pp_reduce__smp} of Definition~\ref{def:pp_reduce}, construct ${\tau: \Words \Scheme \Words}$ s.t. for any ${z_1,z_2 \in \Bool^{p(k)}}$ and ${x \in \Supp \sigma_\bullet^k}$, ${\tau^k(\Chev{z_1,D_\phi(p(k)),z_2,x})=x}$. By Proposition~\ref{prp:prob_cong_push} and Proposition~\ref{prp:prob_cong_ev}
...
\[\E[\Dtv(\mu^k \mid (\pi^k)^{-1}(y),\tau_y^k)] \equiv \E_{(z_b,z_a,x) \sim \Un_{ba}^k \times \sigma_\bullet^k}[\Dtv(\mu^k \mid (\pi^k)^{-1}(\Chev{bz_b,D_\phi(p(k)),az_a,x}),\tau_{\Chev{bz_b,D_\phi(p(k)),az_a,x}}^k)] \pmod \Fall\]
...
Obviously ${\mu_{\mathfrak{F}}}$ is ${\EG}$-samplable. Therefore, if there is ${\phi^* \in E}$ s.t. ${f_{\phi^*}}$ is an extension of ${f_{\mathfrak{F}}}$ then ${(\mu_{\mathfrak{F},D},f_{\phi^*})}$ is as a complete problem for the corresponding class. 
...
${D_{\mathsf{EXP}}}$ is constructed s.t. ${D_{\mathsf{EXP},\phi}(k)=\En_\Nats(p_\phi(k))}$.

\end{example}
\end{samepage}

% Lifting to rank n and non-trivial advice
% 
% Examples of classes
