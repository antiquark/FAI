...
Let ${D: \Words \Alg \Rats}$ be s.t. ${D(\En_\Rats(t))=t}$ and ${\T_D(x) \leq p_D(\Abs{x})}$ for some ${p_D \in \Nats[k]}$. Choose ${a^*: \Nats^n \rightarrow \Words}$ s.t.
...
Consider any ${Q: \Words \Scheme \Rats}$ bounded. Choose ${p \in \NatPoly}$ s.t.

\begin{align*}
\forall K \in \Nats^n: \R_Q(K) &\leq r(\alpha_p(K)) \\
\forall K \in \Nats^n \exists a_Q^K \in \Bool^{l(p(K))} \forall i \in \Nats, x,z \in \Words, y \in \BoolR{Q}: D(\Ev^{p(K)+i}(a_Q^K;x,yz))&=Q^K(x,y)
\end{align*}

Take any ${i \in \Nats}$.

\[\E_{ \mu^{\eta(K)} \times \Un_P^{\alpha_{p+i}(K)}}[(P^{\alpha_{p+i}(K)}(x,y)-f(x))^2]=\E_{ \mu^{\eta(K)} \times \Un^{r(\alpha_{p+i}(K))}}[(D(\Ev^{p(K)+i}(a^*(\alpha_{p+i}(K));x,y))-f(x))^2]\]

Using \ref{eqn:thm__exists_all__astar}

\[\E_{ \mu^{\eta(K)} \times \Un_P^{\alpha_{p+i}(K)}}[(P^{\alpha_{p+i}(K)}(x,y)-f(x))^2] \leq \E_{\mu^{\eta(K)} \times \Un^{r(\alpha_{p+i}(K))}}[(D(\Ev^{p(K)+i}(a_Q^K;x,y))-f(x))^2]\]

\[\E_{ \mu^{\eta(K)} \times \Un_P^{\alpha_{p+i}(K)}}[(P^{\alpha_{p+i}(K)}(x,y)-f(x))^2] \leq \E_{\mu^{\eta(K)} \times \Un^{r_Q(K)}}[(Q^K(x,y)-f(x))^2]\]
...
% ERM (agnostic PAC learning) and Levin's universal search
