...
Construct $\bar{S}: \Words \times \Rats \Scheme \Rats$ s.t. $\R_{\bar{S}}(k,j)=\R_S^{kj}(w^{kj})$ and ${\bar{S}^{kj}(x,q,z)=S^{kj}(x,q,z,w^{kj})}$. $P$ is an $\Fall^\sharp(\Gamma)$-optimal predictor for $(\mu,f)$, therefore
...
\begin{equation}
\label{eqn:thm__mult__prf2}
\E[(\bar{P}_2^{kj}-f_2)f_1 S^{kj}] \equiv 0 \pmod \Fall
\end{equation}

For the second term on the right hand side of \ref{eqn:thm__mult__prf1}, we have


$$\Abs{\E[(P_1^{kj}-f_1) \bar{P}_2^{kj} S^{kj}]}...$$
