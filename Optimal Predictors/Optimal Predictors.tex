...
\begin{definition}
\label{def:sgrow}
Fix ${n \in \Nats^{>0}}$. ${\gamma: \Nats^n \rightarrow \Nats}$ is said to be \emph{steadily growing} when
\begin{enumerate}[(i)]
\item ${\gamma \in \GammaPoly^n}$
\item ${\forall J \in \Nats^{n-1}, k,l \in \Nats: k < l \implies \gamma(J,k) < \gamma(J,l)}$
\item\label{con:def__sgrow__poly} There is ${q \in \NatPoly{1}}$ s.t. ${\forall J \in \Nats^{n-1}, k \in \Nats: \gamma(J,k) \leq \frac{1}{2}\gamma(J,q(J,k))}$.
\end{enumerate}
\end{definition}
\end{samepage}
...
For any ${n \in \Nats^{>0}}$ and ${\gamma^*}$ steadily growing, ${\Gamma_{\gamma^*}}$ is a growth space of rank ${n}$. ${\gamma \in \Gamma_{\gamma^*}}$ iff there is ${p \in \NatPoly{1}}$ s.t. ${\gamma(J,k) \leq \gamma^*(J,p(J,k))}$. 

To verify condition~\ref{con:def__grow__add}, consider ${\gamma_1}$, ${\gamma_2}$ s.t. ${\gamma(J,k) \leq \gamma^*(J,p_1(J,k))}$ and ${\gamma_2(J,k) \leq \gamma^*(J,p_2(J,k))}$. Choose ${p,q \in \NatPoly{1}}$ s.t. ${p \geq \max(p_1,p_2)}$ and ${q}$ is as in condition~\ref{con:def__sgrow__poly} of Definition~\ref{def:sgrow}.
...
Let ${\sigma}$ be an ${\Fall(\Gamma_{\text{smp}})}$-sampler of ${\mu}$. Consider some ${\phi \in E}$ and denote ${b=\En_E(\phi)}$. Choose\\ ${p_\phi \in \NatPoly{1}}$ increasing in the last argument and ${a \in \Words}$ s.t. for any ${K \in \Nats^{n}}$, ${z \in \Words}$, ${w_1 \in \Bool^{\R_\sigma(K)}}$ and ${w_2 \in \Words}$: ${r(\alpha_\phi(K)) \geq \Abs{b}}$, ${r(\alpha_\phi(K)) \geq \Abs{a}}$, ${s(\alpha_\phi(K)) \geq \R_\sigma(K)}$ and 
...
To verify condition~\ref{con:def__pp_reduce__dist} of Definition~\ref{def:pp_reduce} (with ${\alpha_\phi}$ playing the role of the efficient injection), fix ${q \in \NatPoly{1}}$ s.t. $q \geq r_\phi$ and $\Supp \sigma_\bullet^K \subseteq \Bool^{q(K)}$. Construct ${W: \Words \xrightarrow{\Gamma_{\textnormal{red}}} \Rats^{\geq 0}}$ s.t. 
...
Fix ${m \in \Nats}$ s.t. ${m \geq n}$, ${\Fall^{(m)}}$ a fall space of rank ${m}$ and ${\Gamma^m=(\GrowR^m, \GrowA^m)}$ growth spaces of rank ${m}$. For any ${p \in \NatPoly{1}}$, define ${\beta_p: \Nats^m \rightarrow \Nats^m}$ by 
...
Fix ${l: \Nats^n \rightarrow \Nats^{>0}}$ steadily growing. Denote ${\Gamma_{\textnormal{adv}}^n:=(\Gamma_0^n,\Gamma_l)}$. Fix ${r: \bm{1} \xrightarrow{\Gamma_{\textnormal{adv}}}\Nats}$ steadily growing. Assume ${\GrowR=\Gamma_r}$, ${\GrowA=\Gamma_l}$. Define ${\eta: \Nats^n \rightarrow \Nats^{n-1}}$ by
...
For any ${p \in \NatPoly{1}}$ there is ${\{\omega_p^K\}_{K \in \Nats^n}}$ a family of probability measures on ${\Nats}$ s.t. for any ${\zeta: \Nats^n \rightarrow \Reals}$ bounded, if 
...
Consider ${p \in \NatPoly{1}}$, ${(\mu,f)}$ a distributional estimation problem and\\ ${P,Q: \Words \Scheme \Rats}$. Suppose that

\begin{align}
%\label{eqn:tbd}
\sup_{i \in \Nats} \E_{ \mu^{Jk} \times \Un_P^{Jk}}[(P^{J,p(J,k)+i}(x,y)-f(x))^2] &\leq \E_{ \mu^{Jk} \times \Un_P^{Jk}}[(P^{Jk}(x,y)-f(x))^2] \pmod \FallU \\
\sup_{i \in \Nats} \E_{\mu^{Jk} \times \Un_P^{Jk}}[(P^{J,p(J,k)+i}(x,y)-f(x))^2] &\leq \E_{\mu^{Jk} \times \Un_Q^{Jk}}[(Q^{Jk}(x,y)-f(x))^2] \pmod \FallU
\end{align}
