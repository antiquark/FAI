...
Consider $(\mu, f)$ a distributional estimation problem  and ${D \subseteq \Words}$. Let $P_D$ be an $\ESG$-optimal predictor for $(\mu, \chi_D)$ and $P_{f \mid D}$ be an $\ESG$-optimal predictor for $(\mu \mid D, f)$. Construct $P_f: \Words \Scheme \Rats$ s.t. $\R_{P_f}=\R_{P_D} + \R_{P_{f \mid D}}$ and for any $y \in \BoolR{P_D}$ and $z \in \BoolR{P_{f \mid D}}$, $P_f^K(x,yz)=P_D^K(x,y) P_{f \mid D}^K(x,z)$. Then, $P_f$ is an $\ESG$-optimal predictor for $(\mu,f)$.
...
\subsection{\texorpdfstring{$\MGrow$}{MGamma}-Schemes and Samplers}
...
\subsubsection{\texorpdfstring{$\MGrow$}{MGamma}-Schemes}
...
A distributional estimation problem $(\mu,f)$ is called \emph{$\Fall(\MGrow)$-samplable (resp. $\Fall(\Gamma)$-samplable)} when there is an $\MGrow$-scheme (resp. $\Gamma$-scheme) $G$ of signature $\bm{1} \rightarrow \Words \times \Rats$ s.t. 
...
When $\sup{\Abs{G_1}} < \infty$ (since $f$ is bounded, this can always be assumed without loss of generality), $G$ is called an \emph{$\Fall(\MGrow)$-sampler (resp. $\Fall(\Gamma)$-sampler) of $(\mu,f)$}.

\end{definition}
\end{samepage}

For sufficiently large $\GrowA$ the requirements of $\Fall(\MGrow)$-samplability and $\Fall(\MGrow)$-samplability become very weak, as seen in the following propositions.
...
Assume $\Fall$ is $\GrowA$-ample. Consider a distributional estimation problem $(\mu,f)$ s.t. for some $l \in \GrowA$, $\mu^{K_0}(\Bool^{\leq l(K)}) \equiv 1 \pmod \Fall$. Then, $(\mu,f)$ is $\Fall(\MGrow)$-samplable. 
...
Consider a distributional estimation problem $(\mu,f)$, an encoded set $Y$ and a family $\{\pi^K: \Supp \mu^{K_0} \Markov Y\}_{K \in \Nats^n}$. $(\mu,f)$ is called \emph{$\Fall(\MGrow)$-samplable (resp. $\Fall(\Gamma)$-samplable) relative to $\pi$} when there is an $\MGrow$-scheme (resp. $\Gamma$-scheme) $G$ of signature $Y \rightarrow \Words \times \Rats$ s.t.
...
When $\sup{\Abs{G_1}} < \infty$, $G$ is called an \emph{$\Fall(\MGrow)$-sampler (resp. $\Fall(\Gamma)$-sampler) of $(\mu,f)$ relative to $\pi$}.
...
\item $(\mu, f_2)$ is $\Fall(\MGrow)$-samplable relative to $\pi$.
...
Consider a distributional estimation problem $(\mu,f)$, an encoded set $Y$, a family ${\{\pi^K: \Supp \mu^{K_0} \Markov Y\}_{K \in \Nats^n}}$ and ${h: (\Supp \mu) \times Y \rightarrow \Reals}$ bounded. Denote $\mu_\pi^K:=\mu^{K_0} \ltimes \pi^K$. Suppose $G$ is an $\Fall(\MGrow)$-sampler of $(\mu,f)$ relative to $\pi$. Then
...
Let $G$ be an $\Fall(\MGrow)$-sampler of $(\mu,f_2)$ relative to $\pi$. Applying Proposition~\ref{prp:gen} to the right hand side we get
...
Consider word ensembles $\mu_1$ and $\mu_2$ with $\Fall(\MGrow)$-samplers $L_1$ and $L_2$ respectively. Define ${\mu^k:=\En_*^2(\mu_1^k \times \mu_2^k)}$. Suppose ${\pi: \Words \Scheme \Words}$ is s.t. for any $K \in \Nats^n$, $x_1 \in \Supp L_{1\bullet}^{K}$, ${x_2 \in \Words}$ and $z \in \Bool^{\R_\pi(K)}$, ${\pi^{K}(\Chev{x_1,x_2},z)=x_1}$. Consider $L: \Words \MScheme \Words$ s.t. $\UM_L^{K}=\UM_{L_2}^{K}$ and for any $x \in \Supp L_{1\bullet}^{K}$, ${L^{K}(x,z,w)=\Chev{x,L_2^{K}(z,w)}}$. Then, $L$ is an $\Fall(\MGrow)$-sampler of $\mu$ relative to $\pi$. In particular, since such an $L$ can always be constructed, $\mu$ is $\Fall(\MGrow)$-samplable relative to $\pi$.
...
Consider $\mu_1$ a word ensemble with $\Fall(\MGrow)$-sampler $L$ and $(\mu_2, f)$ a distribution estimation problem with $\Fall(\MGrow)$-sampler $G$. Define ${\mu^k:=\En_*^2(\mu_1^k \times \mu_2^k)}$ and $\bar{f}: \Supp \mu \rightarrow \Reals$ given by ${\bar{f}(\Chev{x_1,x_2})=f(x_2)}$. Suppose $\pi: \Words \Scheme \Words$ is s.t. for any $K \in \Nats^n$, ${x_1 \in \Supp L_\bullet^{K}}$, $x_2 \in \Words$ and $z \in \Bool^{\R_\pi(K)}$, $\pi^{K}(\Chev{x_1,x_2},z)=x_1$. Then, $(\mu,\bar{f})$ is $\Fall(\MGrow)$-samplable relative to $\pi$.
...
Construct $\bar{G}: \Words \MScheme \Words \times \Rats$ s.t. $\UM_{\bar{G}}^K=\UM_G^K$ and for any ${x \in \Supp L_\bullet^K}$, ${\bar{G}^K(x,y,z)=(\Chev{x,G^K(y,z,w)_0},G^K(y,z,w)_1)}$. By Proposition~\ref{prp:smp_base_change}, $\bar{G}_0$ is an $\Fall(\MGrow)$-sampler of $\mu$ relative to $\pi$.
...
Consider word ensemble $\mu_1$  with $\Fall(\MGrow)$-sampler $L$ and $(\mu_2,f)$ a distributional estimation problem. Define ${\mu^k:=\En_*^2(\mu_1^k \times \mu_2^k)}$ and ${\bar{f}: \Supp \mu \rightarrow \Reals}$ given by ${\bar{f}(\Chev{x_1,x_2})=f(x_2)}$. Suppose $P$ is an $\Fall^\sharp(\Gamma)$-optimal predictor for $(\mu_2,f)$. Let $\bar{P}: \Words \Scheme \Rats$ be s.t. $\R_{\bar{P}}=\R_P$ and for any $K \in \Nats^n$, $x_1 \in \Supp L_\bullet^K$, $x_2 \in \Supp \mu_2^K$ and $z \in \Bool^{\R_P(K)}$, $\bar{P}^K(\Chev{x_1,x_2},z)=P^K(x_2,z)$. Then, $\bar{P}$ is an $\Fall^\sharp(\Gamma)$-optimal predictor for $(\mu,\bar{f})$.
...
Consider any $S: \Words \times \Rats \Scheme \Rats$ bounded. Denote $\U_{PS}^K:=\U_P^K \times \U_S^K$, ${\mu_{PS}^K:=\mu^{K_0} \times \U_{PS}^K}$.

\[\E_{\mu_{PS}^K}[(\bar{P}^{K}(x) - \bar{f}(x))S^{K}(x,\bar{P}^{K}(x))]=\E_{\mu_1^{K_0} \times \mu_2^{K_0} \times \U_{PS}^K}[(\bar{P}^K(\Chev{x_1,x_2}) - \bar{f}(\Chev{x_1,x_2}))S^K(\Chev{x_1,x_2},\bar{P}^K(\Chev{x_1,x_2}))]\]

\[\E_{\mu_{PS}^K}[(\bar{P}^{K}(x) - \bar{f}(x))S^{K}(x,\bar{P}^{K}(x))]=\E_{\mu_1^{K_0} \times \mu_2^{K_0} \times \U_{PS}^K}[(\bar{P}^K(\Chev{x_1,x_2}) - f(x_2))S^K(\Chev{x_1,x_2},\bar{P}^K(\Chev{x_1,x_2}))]\]

\[\E_{\mu_{PS}^K}[(\bar{P}^{K}(x) - \bar{f}(x))S^{K}(x,\bar{P}^{K}(x))]=\E_{\mu_1^{K_0}}[\E_{\mu_2^{K_0} \times \U_{PS}^K}[(\bar{P}^K(\Chev{x_1,x_2}) - f(x_2))S^K(\Chev{x_1,x_2},\bar{P}^K(\Chev{x_1,x_2}))]]\]

Applying Proposition~\ref{prp:smp} (with $Y = \bm{1}$) to the right hand side, we get

\[\E_{\mu_{PS}^K}[(\bar{P}^{K} - \bar{f})S^{K}] \equiv \E_{\UM_L^K}[\E_{\mu_2^{K_0} \times \U_{PS}^K}[(\bar{P}^K(\Chev{L^K,x_2}) - f(x_2))S^K(\Chev{L^K,x_2},\bar{P}^K(\Chev{L^K,x_2}))]] \pmod \Fall\]

\[\E_{\mu_{PS}^K}[(\bar{P}^{K} - \bar{f})S^{K}] \equiv \E_{\UM_L^K}[\E_{\mu_2^{K_0} \times \U_{PS}^K}[(P^K(x_2) - f(x_2))S^K(\Chev{L^K,x_2},P^K(x_2))]] \pmod \Fall\]

\[\E_{\mu_{PS}^K}[(\bar{P}^{K} - \bar{f})S^{K}] \equiv \E_{\mu_2^{K_0} \times \U_{PS}^K \times \UM_L^K}[(P^K(x_2) - f(x_2))S^K(\Chev{L^K,x_2},P^K(x_2))] \pmod \Fall\]

Using the fact $P$ is an $\Fall^\sharp(\Gamma)$-optimal predictor for $(\mu_2,f)$, we conclude
