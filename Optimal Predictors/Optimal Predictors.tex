...
Consider ${p \in \NatPoly}$, ${(\mu,f)}$ a distributional estimation problem and\\ ${P,Q: \Words \Scheme \Rats}$. Denote ${\alpha_{p,i}(J,k):=(J,p(J,k)+i)}$. Suppose that

\begin{align}
%\label{eqn:tbd}
\sup_{i \in \Nats} \E_{ \mu^{\alpha_{p,i}(K)} \times \Un_P^{\alpha_{p,i}(K)}}[(P^{\alpha_{p,i}(K)}(x,y)-f(x))^2] &\leq \E_{ \mu^{K} \times \Un_P^{K}}[(P^{K}(x,y)-f(x))^2] \pmod \FallU \\
\sup_{i \in \Nats} \E_{\mu^{\alpha_{p,i}(K)} \times \Un_P^{\alpha_{p,i}(K)}}[(P^{\alpha_{p,i}(K)}(x,y)-f(x))^2] &\leq \E_{\mu^{K} \times \Un_Q^{K}}[(Q^{K}(x,y)-f(x))^2] \pmod \FallU
\end{align}
