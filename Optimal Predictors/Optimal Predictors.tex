...
It is easy to see $\Fall_W$ is a fall space. Define the word ensemble $\nu$ by ${\nu^k(x):=\frac{\E_{\Un_W^k}[W^k(x)] \mu^k(x)}{\E_{\mu^k \times \Un_W^k}[W^k(x)]}}$. Then, $P$ is an $\Fall_W(\Gamma)$-optimal predictor for $(\nu,f)$.
...
\begin{equation}
\Prb_{(x,y) \sim \mu^k \times \Un_S^k}[S^k(f(x),y)=B^k(x)] \leq \frac{1}{2} + \varepsilon(k)
\end{equation}
