...
Consider any ${Q: \Words \Scheme \Rats}$ bounded. Choose ${q \in \NatPoly}$ s.t. for any ${K \in \Nats^n}$
...
In particular, ${r}$ is steadily growing. Assume ${\GrowR=\Gamma_r}$, ${\GrowA=\Gamma_\textnormal{log}^n}$. Consider ${(\mu,f)}$ an distributional estimation problem of rank ${n-1}$ and ${G}$ an ${\FallU(\Gamma)}$-sampler for ${(\mu^\eta,f)}$. Then, there exists ${P}$ an ${\FallU(\Gamma)}$-optimal estimator for ${(\mu^\eta,f)}$ s.t. ${\A_P=\A_G}$. In particular, if ${G}$ is uniform (i.e. ${\A_G \equiv \Estr}$) then so is ${P}$.
...
Let ${D: \Words \Alg \Rats}$ be s.t. ${D(\En_\Rats(t))=t}$ and ${\T_D(x) \leq p_D(\Abs{x})}$ for some\\ $p_D \in \Nats[k]$. Denote ${l(K):=\Floor{\log (K_{n-1} + 2)}}$. Construct ${L: \Words \Scheme \Rats}$ s.t. for any ${K \in \Nats^n}$,\\ $w \in \Words$, ${a \in \Bool^{l(K)}}$, ${\{y_i \in \Bool^{r_G(K,w)}\}_{i \in [l(K)^2]}}$ and ${\{z_i \in \Bool^{r(K)}\}_{i \in [l(K)^2]}}$
...
For any ${a \in \Bool^{l(K)}}$

\[?\]

\[\E_{\Un_L^K}[L^K(a)] \equiv \E_{(x,z) \sim \mu^{\eta(K)} \times \Un^{r(K)}}[(D(\Ev^{K_{n-1}}(a;x,z))-f(x))^2] \pmod \FallU\]

Consider any ${Q: \Words \Scheme \Rats}$ bounded. Choose ${q \in \NatPoly}$ s.t. ${q(K) \geq K_{n-1}}$ and for all ${K \in \Nats^n}$, \ref{eqn:thm__exists_all__q_rnd} and \ref{eqn:thm__exists_all__q_adv} hold.
