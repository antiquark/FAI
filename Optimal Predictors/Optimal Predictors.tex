...
%\title[Optimal Estimators]{Optimal Estimators: A Bayesian Notion of Approximation Algorithm (Draft)}
\title{Optimal Estimators: A Bayesian Notion of Approximation Algorithm (Draft)}
...
The concept of \enquote{optimal estimator} that arises from the approach turns out to behave much like probabilities, or more generally expected values, in \enquote{classical} probability theory. They display an appropriate form of calibration. The \enquote{expected values} are linear in general and multiplicative for functions that are independent in an appropriate sense. There is a natural parallel of conditional probabilities. For simple examples constructed from one-way functions we get the probability values we expect. Also they are well behaved in the complexity-theoretic sense that a natural class of reductions transforms optimal estimators into optimal estimators, and complete problems for these reductions exist for important complexity classes.

Optimal estimators turn out to be unique up to a certain equivalence relation. The existence of optimal estimators depends on the specific variety you consider. We show that in the non-uniform case (allowing advice) there is a variety of optimal estimators that exist for completely arbitrary problems. Uniform optimal estimators of this kind exist for a certain class of problems we call \enquote{samplable} which can be very roughly regarded as an average-case analogue of $\textsc{NP} \cap \textsc{coNP}$. More generally mapping the class of problems which admit optimal estimators allows for much further research.

Assignment of probabilities to mathematical questions is interesting in the context of artificial general intelligence\cite{Hutter_2013,Christiano_2014}. In this context is useful to consider situations in which an agent reasons about itself or several agents reason about each other \cite{Fallenstein_2015}. We give a formal model of such situations in our setting and use the Kakutani-Glicksberg-Fan theorem to prove they still admit existence of optimal estimators. We then use this to model agents playing a game in normal form and show that such agents converge to the analogue of Nash equilibrium (or even a proper equilibrium, depending on the precise implementation) in the time bounded case. That is, each agent plays a near-optimal response among those it can find within the allotted time. We suggest more potential applications for optimal estimators to decision theory and the theory of self-modifying agents.

The structure of the paper is as follows. Section~\ref{sec:fundamentals} introduces the main definitions and gives a simple example using one-way functions. Section~\ref{sec:probability} shows the parallel between properties of optimal estimators and classical probability theory. Section~\ref{sec:reductions} discusses behavior of optimal estimators under reductions and shows certain natural classes have complete problems under reductions that are appropriate. Section~\ref{sec:e_and_u} discusses existence and uniqueness of optimal estimators. Section~\ref{sec:reflective} discusses applications to AGI. Section~\ref{sec:discussion} discusses possible avenues for further research. The Appendix reviews relevant material about hard-core predicates and one-way functions.
...
$\Words$ is the set of all finite binary strings (words), i.e. $\Words:=\bigsqcup_{n \in \Nats} \Bool^n$. For any ${x \in \Words}$, $\Abs{x}$ is the length of $x$ i.e. $x \in \WordsLen{\Abs{x}}$. ${\lambda \in \Words}$ is the empty string. For any $n \in \Nats$
...
For any $x \in \Words$ and $n \in \Nats$, $x_{\leq n}$ stands for the prefix of $x$ of length $n$ if $\Abs{x} \geq n$ and $x$ otherwise. Given $x,y \in \Words$, $xy$ stands for the concatenation of $x$ and $y$ (in particular $\Abs{xy}=\Abs{x}+\Abs{y}$). Given ${n \in \Nats}$ and ${x_0, x_1 \ldots x_{n-1} \in \Words}$, ${\prod_{i \in [n]} x_i}$ is also concatenation. Given $n \in \Nats$ and $x,y \in \WordsLen{n}$, $x \cdot y$ stands for $\bigoplus_{i \in [n]} x_i y_i$. For any $n \in \Nats$, $\Un^n$ is the uniform probability distribution on $\WordsLen{n}$.
...
\item For any $\gamma \in \Gamma$ there is a $p \in \NatPoly$ s.t. $\gamma \leq p$.
...
\[\GammaPoly^n := \{\gamma: \Nats^n \rightarrow \Nats \mid \exists p \in \NatPoly: \gamma \leq p\}\]
...
\subsubsection{Optimal Estimators}
...
Fix $n \in \Nats$, $\Gamma$ a pair of growth spaces of rank $n$ and $\Fall$ a fall space of rank $n$. Consider $(\mu,f)$ a distributional estimation problem and $P: \Words \Scheme \Rats$ with bounded range. $P$ is called an \emph{$\EG$-optimal estimator for $(\mu,f)$} when for any $Q: \Words \Scheme \Rats$
...
In this subsection we discuss some basic properties of optimal estimators which will be used in the following.
...
Although the word ensemble plays a central role in the definition of an optimal estimator, the dependence on the word ensemble is lax in some sense. To see this, consider the following proposition.
...
Consider $(\mu,f)$ a distributional estimation problem and ${P: \Words \Scheme \Rats}$ with bounded range. Denote $\mu_P^{K}:=\mu^{K} \times \Un_P^{K}$. $P$ is called an \emph{$\ESG$-optimal estimator for $(\mu,f)$} when for any bounded $S: \Words \times \Rats \Scheme \Rats\footnote{The $\Rats$-valued argument of $S$ is only important for non-trivial $\GrowR$, otherwise we can absorb it into the definition of $S$ using $P$ as a subroutine.}$
...
The concept of an optimal estimator is in some sense complementary to the concept of pseudorandom: a pseudorandom process deterministically produces output that appears random to bounded algorithms whereas optimal estimators compute the moments of the perceived random distributions of the outputs of deterministic processes. To demonstrate this complementarity and give an elementary example of an optimal estimator, we use the concept of a hard-core predicate (which may be regarded as en elementary example of pseudorandom).
...
We will now show that optimal estimators satisfy an analogous property.
...
In this subsection and subsection~\ref{subsec:indep_var}, we show that several algebraic identities satisfied by expected values have analogues for optimal estimators.
...
Optimal estimators have an analogous property:
...
For optimal estimators the analogous inequalities don't have to hold strictly (they only hold within an asymptotically small error), but the following proposition shows they can always be enforced.
...
Consider $X$ a measurable space, $\{\mu^K\}_{K \in \Nats^n}$, $\{\nu^K\}_{K \in \Nats^n}$ families of probability measures on $X$ and ${\{f^K: X \rightarrow \Reals\}_{K \in \Nats^n}}$ a uniformly bounded family of measurable functions. Assume ${\mu \equiv \nu \pmod \Fall}$. Then, $\E_{x \sim \mu^K}[f^K(x)] \equiv \E_{x \sim \nu^K}[f^K(x)] \pmod \Fall$.
...
Consider $X$, $Y$ measurable spaces, $\{\mu^K\}_{K \in \Nats^n}$, $\{\nu^K\}_{K \in \Nats^n}$ families of probability measures on $X$ and ${\{f^K: X \Markov Y\}_{K \in \Nats^n}}$ a family of Markov kernels. Then, $\mu \equiv \nu \pmod \Fall$ implies $\mu^K \ltimes f^K \equiv \nu^K \ltimes f^K \pmod \Fall$.
...
In this section we study notions of Karp reduction between distributional estimation problems such that the pull-back of an optimal estimator is an optimal estimator. It is also interesting to study Cook reductions but we avoid it in the present work.
...
We will describe several types of reductions that preserve optimal estimators. After that, we will characterize reductions that can be constructed by composing those types and prove a completeness theorem.
...
The following corollaries are completely analogous to Corollary~\ref{crl:pp_reduce_sharp} and therefore given without proof. We also drop the explicit constructions of the optimal estimators which are obviously modeled on Theorem~\ref{thm:sp_reduce_sharp} and Theorem~\ref{thm:sp_reduce}.
...
We will show that certain classes of functions paired with ${\Fall^1(\Gamma_{\text{rand}}^1)}$-samplable word ensembles have a distributional estimation problem which is complete w.r.t. precise pseudo-invertible ${\Fall^1(\Gamma_{\text{rand}}^1)}$-reductions. The construction is an adaption of the usual construction of a complete problem for ${\mathsf{SampNP}}$.

\begin{samepage}
\begin{theorem}
\label{thm:complete}

Consider an encoded set ${E}$ which is prefix-free, i.e. for all ${\phi,\psi \in E}$ and ${z \in \Bool^{>0}}$, ${\En_E(\phi) \ne \En_E(\psi)z}$. Consider ${\mathfrak{F}: E \times \Words \rightarrow \Reals}$ bounded and ${\{D_\phi: \Nats \Alg \Words\}_{\phi \in E}}$ s.t. for all ${\phi \in E}$, ${\T_{D_\phi} \in \GammaPoly^1}$. Define ${\zeta^k: \Words^2 \rightarrow \Words^2}$ and ${\xi^k: \Words \rightarrow \Words^2}$ by 

\begin{align}
%\label{eqn:tbd}
\zeta^k(a,w)&=(a,\Ev^{k}(a;\En_\Nats(k),w))\\
\xi^k(b)&=\begin{cases}(b,D_\phi(k)) \textnormal{ if } \exists z \in \Words: b=\En_E(\phi)z \\ (b,\lambda) \textnormal{ if } \forall \phi \in E, z \in \Words: b \ne \En_E(\phi)z\end{cases}
\end{align}

Define the distributional estimation problem $({\mu_{\mathfrak{F},D}},f_{\mathfrak{F}})$ by 

\begin{align}
\mu_{\mathfrak{F},D}^k&:=\En_*^4 (\xi_*^k(\Un^k) \times \zeta_*^k(\Un^k \times \Un^k)) \\
...
For any ${\phi \in E}$, define ${f_\phi: \Words \rightarrow \Reals}$ by ${f_\phi(x)=\mathfrak{F}(\phi,x)}$. Then, for any ${\phi \in E}$ and ${\Fall^1(\Gamma_{\textnormal{rand}}^1)}$-samplable world ensemble ${\mu}$, there is a precise pseudo-invertible ${\Fall^1(\Gamma_{\textnormal{rand}}^1)}$-reduction from ${(\mu,f_\phi)}$ to ${(\mu_{\mathfrak{F},D}, f_{\mathfrak{F}})}$ over a polynomial.
...
Let ${\sigma}$ be an ${\EG}$-sampler of ${\mu}$. Choose ${p \in \Nats[k]}$ one-to-one and ${a \in \Words}$ s.t. for any ${k \in \Nats}$, ${z \in \Words}$, ${w_1 \in \Bool^{\R_\sigma(k)}}$ and ${w_2 \in \Words}$, ${\R_\sigma(k) \leq p(k)}$ and ${\Ev^{p(k)}(az;\En_\Nats(p(k)),w_1 w_2)=\sigma^k(w_1)}$. Denote ${b=\En_E(\phi)}$. We construct ${\pi: \Words \xrightarrow{\Gamma_{\textnormal{rand}}^1} \Words}$ s.t. for any ${k \gg 0}$, ${x \in \Supp \sigma_{\bullet}^k}$,  ${z_b \in \Bool^{p(k)-\Abs{b}}}$ and ${z_a \in \Bool^{p(k)-\Abs{a}}}$

\begin{align}
\R_\pi(k) &= 2p(k)-\Abs{a}-\Abs{b} \\
\label{eqn:thm__complete__red}\pi^k(x,z_b z_a)&=\Chev{bz_b,D_\phi(p(k)),a z_a, x} 
\end{align}

We also ensure that for any ${k \gg 0}$, ${x \in \Words}$ and ${z_b,z_a}$ as above, either \ref{eqn:thm__complete__red} holds or 

\[\pi^k(x,z_b z_a)=\lambda\]

To verify condition~\ref{con:def__pp_reduce__dist} of Definition~\ref{def:pp_reduce} (with ${p}$ playing the role of the efficient injection), fix ${q \in \Nats[k]}$ s.t.

\begin{align*}
p(k) &\leq q(k) \\
\Supp \sigma_\bullet^k &\subseteq \Bool^{q(k)}
\end{align*}

Construct ${W: \Words \xrightarrow{\Gamma_{\textnormal{rand}}^1} \Rats^{\geq 0}}$ s.t. 

\[W^k(y)=\begin{cases}2^{\Abs{a}+\Abs{b}} \text{ if } \exists z_b,z_a,x \in \Bool^{\leq q(k)}: y=\Chev{bz_b,D_\phi(p(k)), az_a,x}\\0 \text{ otherwise}\end{cases}\]

${\mu^k \equiv \sigma_\bullet^k \pmod{\Fall^1}}$ since ${\sigma}$ is an ${\Fall^1(\Gamma_{\textnormal{rand}}^1)}$-sampler of ${\mu}$. By Proposition~\ref{prp:prob_cong_push}, ${\pi_*^k\mu^k \equiv \pi_*^k\sigma_\bullet^k \pmod{\Fall^1}}$. It follows that

\[\sum_{y \in \Words} \Abs{\mu_{\mathfrak{F}}^{p(k)}(y)W^k(y)-(\pi_*^k\mu^k)(y)} \equiv \sum_{y \in \Words} \Abs{\mu_{\mathfrak{F}}^{p(k)}(y)W^k(y)-(\pi_*^k\sigma_\bullet^k)(y)} \pmod{\Fall^1}\]

Given ${k \gg 0}$, for any ${y \in \Words}$, if ${W^k(y)=0}$ then ${(\pi_*^k\sigma_\bullet^k)(y) = 0}$. We get

\[\sum_{\Words} \Abs{\mu_{\mathfrak{F}}^{p(k)}W^k-(\pi_*^k\mu^k)} \equiv \sum_{\substack{z_b \in \Bool^{\leq q(k)}\\z_a \in \Bool^{\leq q(k)}\\x \in \Bool^{\leq q(k)}}} \Abs{\mu_{\mathfrak{F}}^{p(k)}(\Chev{bz_b, D_\phi(p(k)), az_a, x})2^{\Abs{a}+\Abs{b}}-(\pi_*^k\sigma_\bullet^k)(\Chev{bz_b,D_\phi(p(k)),az_a,x})} \pmod{\Fall^1}\]

\[\sum_{\Words} \Abs{\mu_{\mathfrak{F}}^{p(k)}W^k-\pi_*^k\mu^k} \equiv \sum_{\substack{z_b \in \Bool^{p(k)-\Abs{b}}\\z_a \in \Bool^{p(k)-\Abs{a}}\\x \in \Bool^{\leq q(k)}}} \Abs{2^{-p(k)}2^{-p(k)}\sigma_\bullet^k(x) 2^{\Abs{a}+\Abs{b}}-(\pi_*^k\sigma_\bullet^k)(\Chev{bz_b, D_\phi(p(k)),az_a,x})} \pmod{\Fall^1}\]

\[\sum_{\Words} \Abs{\mu_{\mathfrak{F}}^{p(k)}W^k-\pi_*^k\mu^k} \equiv \sum_{\substack{z_1 \in \Bool^{p(k)-\Abs{a}}\\z_2 \in \Bool^{p(k)-\Abs{b}}\\x \in \Bool^{\leq q(k)}}} \Abs{2^{-2p(k)+\Abs{a}+\Abs{b}}\sigma_\bullet^k(x) -(\pi_*^k\sigma_\bullet^k)(\Chev{bz_b, D_\phi(p(k)),az_a,x})} \pmod{\Fall^1}\]

\[\sum_{\Words} \Abs{\mu_{\mathfrak{F}}^{p(k)}W^k-\pi_*^k\mu^k} \equiv \sum_{\substack{z_1 \in \Bool^{p(k)-\Abs{a}}\\z_2 \in \Bool^{p(k)-\Abs{b}}\\x \in \Bool^{\leq q(k)}}} \Abs{2^{-2p(k)+\Abs{a}+\Abs{b}}\sigma_\bullet^k(x) -2^{-(p(k)-\Abs{a})}2^{-(p(k)-\Abs{b})}\sigma_\bullet^k(x)} \pmod{\Fall^1}\]

\[\sum_{\Words} \Abs{\mu_{\mathfrak{F}}^{p(k)}W^k-\pi_*^k\mu^k} \equiv 0 \pmod{\Fall^1}\]
...
\[\E_{\mu^k \times \Un_\pi^k}[\Abs{f_\phi(x)-f_{\mathfrak{F}}(\pi^k(x,z))}] \equiv \E_{\sigma_\bullet^k \times \Un_\pi^k}[\Abs{f_\phi(x)-f_{\mathfrak{F}}(\pi^k(x,z))}] \pmod{\Fall^1}\]

\[\E_{\mu^k \times \Un_\pi^k}[\Abs{f_\phi(x)-f_{\mathfrak{F}}(\pi^k(x,z))}] \equiv \E_{\sigma_\bullet^k \times \Un_\pi^k}[\Abs{f_\phi(x)-f_{\mathfrak{F}}(\Chev{bz_b,D_\phi(p(k)),az_a,x})}] \pmod{\Fall^1}\]

\[\E_{\mu^k \times \Un_\pi^k}[\Abs{f_\phi(x)-f_{\mathfrak{F}}(\pi^k(x,z))}] \equiv \E_{\sigma_\bullet^k \times \Un_\pi^k}[\Abs{\mathfrak{F}(\phi,x)-\mathfrak{F}(\phi,x)}] \pmod{\Fall^1}\]

\[\E_{\mu^k \times \Un_\pi^k}[\Abs{f_\phi(x)-f_{\mathfrak{F}}(\pi^k(x,z))}] \equiv 0 \pmod{\Fall^1}\]

To verify condition~\ref{con:def__pp_reduce__smp} of Definition~\ref{def:pp_reduce}, construct ${\tau: \Words \xrightarrow{\Gamma_{\textnormal{rand}}^1} \Words}$ s.t. for any ${z_1,z_2 \in \Bool^{p(k)}}$ and ${x \in \Supp \sigma_\bullet^k}$, ${\tau^k(\Chev{z_1,D_\phi(p(k)),z_2,x})=x}$. By Proposition~\ref{prp:prob_cong_push} and Proposition~\ref{prp:prob_cong_ev}

\[\E_{y \sim \pi_*^k\mu^k}[\Dtv(\mu^k \mid (\pi^k)^{-1}(y),\tau_y^k)] \equiv \E_{y \sim \pi_*^k\sigma_\bullet^k}[\Dtv(\mu^k \mid (\pi^k)^{-1}(y),\tau_y^k)] \pmod{\Fall^1}\]

Denoting ${\Un_{ba}^k:=\Un^{p(k)-\Abs{b}} \times \Un^{p(k)-\Abs{a}}}$

\[\E[\Dtv(\mu^k \mid (\pi^k)^{-1}(y),\tau_y^k)] \equiv \E_{(z_b,z_a,x) \sim \Un_{ba}^k \times \sigma_\bullet^k}[\Dtv(\mu^k \mid (\pi^k)^{-1}(\Chev{bz_b,D_\phi(p(k)),az_a,x}),\tau_{\Chev{bz_b,D_\phi(p(k)),az_a,x}}^k)] \pmod{\Fall^1}\]

\[\E[\Dtv(\mu^k \mid (\pi^k)^{-1}(y),\tau_y^k)] \equiv \E_{(z_b,z_a,x) \sim \Un_{ba}^k \times \sigma_\bullet^k}[\Dtv(\delta_x,\delta_x)] \pmod{\Fall^1}\]

\[\E[\Dtv(\mu^k \mid (\pi^k)^{-1}(y),\tau_y^k)] \equiv 0 \pmod{\Fall^1}\]
%
\end{proof}

Obviously ${\mu_{\mathfrak{F}}}$ is ${\Fall^1(\Gamma_{\text{rand}}^1)}$-samplable. Therefore, if there is ${\phi^* \in E}$ s.t. ${f_{\phi^*}}$ is an extension of ${f_{\mathfrak{F}}}$ then ${(\mu_{\mathfrak{F},D},f_{\phi^*})}$ is complete w.r.t. precise pseudo-invertible ${\Fall^1(\Gamma_{\text{rand}}^1)}$-reductions over polynomials for the class of distributional estimation problems ${(\mu,f_\phi)}$ s.t. ${\mu}$ is ${\Fall^1(\Gamma_{\text{rand}}^1)}$-samplable. 

\begin{samepage}
\begin{example}

${E_{\mathsf{NP}}}$ consists of pairs ${\phi=(M_\phi \in \Words, p_\phi \in \Nats[k])}$. ${\mathfrak{F}_{\mathsf{NP}}}$ is given by 

\begin{equation}
%\label{eqn:tbd}
\mathfrak{F}_{\mathsf{NP}}(\phi,x):=\begin{cases}1 \text{ if } \exists y\in \Bool^{\leq p_\phi(\Abs{x})}: \Ev^{ p_\phi(\Abs{x})}(M_\phi;x,y)=1 \\ 0 \text { otherwise} \end{cases}
\end{equation}

${D_{\mathsf{NP}}}$ is constructed s.t. ${\Abs{D_{\mathsf{NP},\phi}(k)}=p_\phi(k)}$. This padding is necessary for ${\phi^*}$ as above to exist. 
...
${E_{\mathsf{EXP}}}$ consists of pairs ${\phi=(M_\phi \in \Words, p_\phi \in \Nats[k])}$. ${\mathfrak{F}_{\mathsf{EXP}}}$ is given by 

\begin{equation}
%\label{eqn:tbd}
\mathfrak{F}_{\mathsf{EXP}}(\phi,x):=\begin{cases}1 \text{ if } \Ev^{2^{p_\phi(\Abs{x})}}(M_\phi;x)=1 \\ 0 \text { otherwise} \end{cases}
\end{equation}

${D_{\mathsf{EXP}}}$ is constructed s.t. ${\Abs{D_{\mathsf{EXP},\phi}(k)}=p_\phi(k)}$.

\end{example}
\end{samepage}

This completeness property implies that optimal estimators exist for the entire class if an optimal estimator exists for ${(\mu_{\mathfrak{F},D},f_{\mathfrak{F}})}$. More precisely, we have the following corollaries.

\begin{samepage}
\begin{corollary}
\label{crl:complete_sharp}

For any ${p \in \Nats[k]}$, define ${\alpha_p: \Nats^{\bar{n}} \rightarrow \Nats^{\bar{n}}}$ by 

\[\alpha_p(K)=(p(K_0), K_1, K_2 \ldots K_{\bar{n}-1})\]

Define ${\eta: \Nats^{n} \rightarrow \Nats}$ by ${\eta(K)=K_0}$. Assume that ${\Fall^1 \eta \subseteq \Fall}$, ${\GammaPoly^1 \eta \subseteq \GrowR}$ and for any ${p \in \Nats[k]}$ one-to-one, ${\Fall \alpha_p \subseteq \Fall}$, ${\GrowR \alpha_p = \GrowR}$ and ${\GrowA \alpha_p = \GrowA}$. In the setting of Theorem~\ref{thm:complete}, assume there is an ${\ESG}$-optimal estimator for ${(\mu_{\mathfrak{F},D}^\eta,f_{\mathfrak{F}})}$. Then, for any ${\phi \in E}$ and ${\Fall^1(\Gamma_{\textnormal{rand}}^1)}$-samplable word ensemble ${\mu}$, there is an ${\ESG}$-optimal estimator for ${(\mu^\eta,f_\phi)}$.
...
According to Theorem~\ref{thm:complete}, there is ${\pi}$ a precise pseudo-invertible ${\Fall^1(\Gamma_{\textnormal{rand}}^1)}$-reduction of ${(\mu,f_\phi)}$ to ${(\mu_{\mathfrak{F}},f_{\mathfrak{F}})}$ over some ${p \in \Nats[k]}$ one-to-one. This implies ${\pi^\eta}$ is a precise pseudo-invertible ${\EG}$-reduction of ${(\mu^\eta,f_\phi)}$ to ${(\mu_{\mathfrak{F}}^\eta,f_{\mathfrak{F}})}$ over ${\alpha_p}$. Applying Corollary~\ref{crl:pp_reduce_sharp}, we get the desired result. 
%
\end{proof}
...
Assume ${\Fall}$ is ${\GrowA}$-ample. For any ${p \in \Nats[k]}$, define ${\alpha_p: \Nats^{\bar{n}} \rightarrow \Nats^{\bar{n}}}$ by 

\[\alpha_p(K)=(p(K_0), K_1, K_2 \ldots K_{\bar{n}-1})\]

Define ${\eta: \Nats^{n} \rightarrow \Nats}$ by ${\eta(K)=K_0}$. Assume that ${\Fall^1 \eta \subseteq \Fall}$, ${\GammaPoly^1 \eta \subseteq \GrowR}$ and for any ${p \in \Nats[k]}$ one-to-one, ${\Fall \alpha_p \subseteq \Fall}$, ${\GrowR \alpha_p = \GrowR}$ and ${\GrowA \alpha_p = \GrowA}$. In the setting of Theorem~\ref{thm:complete}, assume there is an ${\EG}$-optimal estimator for ${(\mu_{\mathfrak{F},D}^\eta,f_{\mathfrak{F}})}$. Then, for any ${\phi \in E}$ and ${\Fall^1(\Gamma_{\textnormal{rand}}^1)}$-samplable word ensemble ${\mu}$, there is an ${\EG}$-optimal estimator for ${(\mu^\eta,f_\phi)}$.
...
% 
% Add to previous sections analysis of the properties of ${\mathcal{E}_{\text{ll},\phi}}$ and ${\mathcal{E}_{\text{poly}}$ after each fall space property is introduced.
%
