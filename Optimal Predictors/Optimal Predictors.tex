...
\begin{equation}
\label{eqn:plog}
\forall J \in \Nats^{n-1}, k \in \Nats: \gamma(J,k) \leq \frac{1}{2}\gamma(J,q(J,k))
\end{equation}
...
For any ${n \in \Nats^{>0}}$ and ${\gamma^* \in \GammaPoly^n}$ a pseudologarithm, ${\Gamma_{\gamma^*}}$ is a growth space of rank ${n}$. ${\gamma \in \Gamma_{\gamma^*}}$ iff there is ${p \in \NatPoly{1}}$ s.t. ${\gamma(J,k) \leq \gamma^*(J,p(J,k))}$. 

To verify condition~\ref{con:def__grow__add}, consider ${\gamma_1}$, ${\gamma_2}$ s.t. ${\gamma(J,k) \leq \gamma^*(J,p_1(J,k))}$ and ${\gamma_2(J,k) \leq \gamma^*(J,p_2(J,k))}$. Then we can choose ${p,q \in \NatPoly{1}}$ s.t. ${p \geq \max(p_1,p_2)}$ and \ref{eqn:plog} holds and get 

\[\gamma_1(J,k)+\gamma_2(J,k) \leq \gamma^*(J,p_1(J,k))+\gamma^*(J,p_2(J,k))\]

\[\gamma_1(J,k)+\gamma_2(J,k) \leq 2\gamma^*(J,p(J,k))\]

\[\gamma_1(J,k)+\gamma_2(J,k) \leq \gamma^*(J,q(J,p(J,k)))\]

In particular taking ${\gamma^*_{\text{poly}}(J,k):=j}$ and ${\gamma^*_{\text{log}}(J,k):=\Floor{\log (j+1)}}$ we have ${\Gamma_{\text{poly}}^n=\Gamma_{\gamma^*_{\text{poly}}}}$, ${\Gamma_{\text{log}}^n=\Gamma_{\gamma^*_{\text{log}}}}$.
...
For this subsection we assume ${n > 0}$, fix ${r, s \in \GammaPoly^n}$ pseudologarithms s.t. ${r \leq s}$ and denote ${\Gamma_{\textnormal{red}}:=(\Gamma_{r},\Gamma_0^n)}$, ${\Gamma_{\textnormal{smp}}:=(\Gamma_{s},\Gamma_0^n)}$.
...
Consider an encoded set ${E}$ which is prefix-free, i.e. for all ${\phi,\psi \in E}$ and ${z \in \Bool^{>0}}$, ${\En_E(\phi) \ne \En_E(\psi)z}$. Consider ${\mathfrak{F}: E \times \Words \rightarrow \Reals}$ bounded and ${\{D_\phi: \Nats^n \Alg \Words\}_{\phi \in E}}$ s.t. for all ${\phi \in E}$, ${\T_{D_\phi} \in \GammaPoly^n}$. For any ${K \in \Nats^n}$, define ${\zeta^K: \Words^2 \rightarrow \Words^2}$ and ${\xi^K: \Words \rightarrow \Words^2}$ by 
...
For any ${p \in \NatPoly{1}}$, define ${\alpha_p: \Nats^n \rightarrow \Nats^n}$ by
...
Let ${\sigma}$ be an ${\Fall(\Gamma_{\text{smp}})}$-sampler of ${\mu}$. Consider some ${\phi \in E}$ and denote ${b=\En_E(\phi)}$. Choose\\ ${p_\phi \in \NatPoly{1}}$ increasing in the last argument and ${a \in \Words}$ s.t. for any ${K \in \Nats^{n}}$, ${z \in \Words}$, ${w_1 \in \Bool^{\R_\sigma(K)}}$ and ${w_2 \in \Words}$: ${p_\phi(K) \geq \Abs{b}}$, ${p_\phi(K) \geq \Abs{a}}$, ${\R_\sigma(K) \leq s(\alpha_\phi(K))}$ and 

\[\Ev^{p_\phi(K)}(az;\En_{\Nats^n}(\alpha_\phi(K)),w_1 w_2)=\sigma^{K}(w_1)\]

We construct ${\pi: \Words \xrightarrow{\Gamma_{\textnormal{red}}} \Words}$ s.t. for any ${K \in \Nats^{n}}$, ${x \in \Supp \sigma_{\bullet}^{K}}$,  ${z_b \in \Bool^{p(K)-\Abs{b}}}$ and\\ ${z_a \in \Bool^{p(K)-\Abs{a}}}$
...
To verify condition~\ref{con:def__pp_reduce__dist} of Definition~\ref{def:pp_reduce} (with ${\alpha_\phi}$ playing the role of the efficient injection), fix ${q \in \NatPoly{1}}$ s.t. $p \leq q$ and $\Supp \sigma_\bullet^K \subseteq \Bool^{q(K)}$. Construct ${W: \Words \xrightarrow{\Gamma_{\textnormal{red}}} \Rats^{\geq 0}}$ s.t. 
...
\[\sum_{\Words} \Abs{\mu_{\mathfrak{F}}^{\alpha_\phi(K)}W^k-(\pi_*^k\mu^k)} \equiv \sum_{\substack{z_b \in \Bool^{\leq q(k)}\\z_a \in \Bool^{\leq q(k)}\\x \in \Bool^{\leq q(k)}}} \Abs{\mu_{\mathfrak{F}}^{\alpha_\phi(K)}(\Chev{bz_b, D_\phi(p(k)), az_a, x})2^{\Abs{a}+\Abs{b}}-(\pi_*^k\sigma_\bullet^k)(\Chev{bz_b,D_\phi(p(k)),az_a,x})} \pmod{\Fall}\]

\[\sum_{\Words} \Abs{\mu_{\mathfrak{F}}^{\alpha_\phi(K)}W^k-\pi_*^k\mu^k} \equiv \sum_{\substack{z_b \in \Bool^{p(k)-\Abs{b}}\\z_a \in \Bool^{p(k)-\Abs{a}}\\x \in \Bool^{\leq q(k)}}} \Abs{2^{-p(k)}2^{-p(k)}\sigma_\bullet^k(x) 2^{\Abs{a}+\Abs{b}}-(\pi_*^k\sigma_\bullet^k)(\Chev{bz_b, D_\phi(p(k)),az_a,x})} \pmod{\Fall}\]

\[\sum_{\Words} \Abs{\mu_{\mathfrak{F}}^{\alpha_\phi(K)}W^k-\pi_*^k\mu^k} \equiv \sum_{\substack{z_1 \in \Bool^{p(k)-\Abs{a}}\\z_2 \in \Bool^{p(k)-\Abs{b}}\\x \in \Bool^{\leq q(k)}}} \Abs{2^{-2p(k)+\Abs{a}+\Abs{b}}\sigma_\bullet^k(x) -(\pi_*^k\sigma_\bullet^k)(\Chev{bz_b, D_\phi(p(k)),az_a,x})} \pmod{\Fall}\]

\[\sum_{\Words} \Abs{\mu_{\mathfrak{F}}^{\alpha_\phi(K)}W^k-\pi_*^k\mu^k} \equiv \sum_{\substack{z_1 \in \Bool^{p(k)-\Abs{a}}\\z_2 \in \Bool^{p(k)-\Abs{b}}\\x \in \Bool^{\leq q(k)}}} \Abs{2^{-2p(k)+\Abs{a}+\Abs{b}}\sigma_\bullet^k(x) -2^{-(p(k)-\Abs{a})}2^{-(p(k)-\Abs{b})}\sigma_\bullet^k(x)} \pmod{\Fall}\]
...
Obviously ${\mu_{\mathfrak{F}}}$ is ${\Fall(\Gamma_{\text{rand}}^1)}$-samplable. Therefore, if there is ${\phi^* \in E}$ s.t. ${f_{\phi^*}}$ is an extension of ${f_{\mathfrak{F}}}$ then ${(\mu_{\mathfrak{F},D},f_{\phi^*})}$ is complete w.r.t. precise pseudo-invertible ${\Fall(\Gamma_{\text{rand}}^1)}$-reductions over polynomials for the class of distributional estimation problems ${(\mu,f_\phi)}$ s.t. ${\mu}$ is ${\Fall(\Gamma_{\text{rand}}^1)}$-samplable. 
