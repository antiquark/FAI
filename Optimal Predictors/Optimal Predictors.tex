...
Fix ${n \in \Nats^{>0}}$ and ${g \in \GammaPoly^n}$ s.t. ${g}$ is non-decreasing in the last argument and for some ${q \in \NatPoly{1}}$

\begin{equation}
%\label{eqn:tbd}
\forall K \in \Nats^{n-1}, j \in \Nats: g(K,j) \leq \frac{1}{2}g(K,q(K,j))
\end{equation}

${\Gamma_g}$ is a growth space of rank ${n}$. ${\gamma \in \Gamma_g}$ iff there is ${p \in \NatPoly{1}}$ s.t. ${\gamma(K,j) \leq g(K,p(K,j))}$. 

To verify condition~\ref{con:def__grow__add}, consider ${\gamma_1}$, ${\gamma_2}$ s.t. ${\gamma(K,j) \leq g(K,p_1(K,j))}$ and ${\gamma_2(K,j) \leq g(K,p_2(K,j))}$. Then we can choose ${p \in \NatPoly{1}}$ s.t. ${p \geq \max(p_1,p_2)}$ and get 

\[\gamma_1(K,j)+\gamma_2(K,j) \leq g(K,p_1(K,j))+g(K,p_2(K,j))\]

\[\gamma_1(K,j)+\gamma_2(K,j) \leq 2g(K,p(K,j))\]

\[\gamma_1(K,j)+\gamma_2(K,j) \leq g(K,q(K,p(K,j)))\]

In particular taking ${g_{\text{poly}}(K,j):=j}$ and ${g_{\text{log}}(K,j):=\Floor{\log (j+1)}}$ we have ${\Gamma_{\text{poly}}^n=\Gamma_{g_{\text{poly}}}}$, ${\Gamma_{\text{log}}^n=\Gamma_{g_{\text{log}}}}$.
