...
\item ${\forall J \in \Nats^{n-1}, k,l \in \Nats: k < l \implies r(J,k) \leq r(J,l)}$

\item There is ${q \in \NatPoly}$ s.t. ${\forall J \in \Nats^{n-1}, k \in \Nats: \log(k+4)r(J,k) \leq r(J,q(J,k))}$.

\end{enumerate}


Assume ${\GrowR=\Gamma_r}$, ${\GrowA=\Gamma_\textnormal{log}^n}$. Consider ${(\mu,f)}$ an distributional estimation problem of rank ${n-1}$ and ${G}$ an ${\FallU(\Gamma)}$-sampler for ${(\mu^\eta,f)}$. Then, there exists ${P}$ an ${\FallU(\Gamma)}$-optimal estimator for ${(\mu^\eta,f)}$ s.t. ${\A_P=\A_G}$. In particular, if ${G}$ is uniform (i.e. ${\A_G \equiv \Estr}$) then so is ${P}$.
