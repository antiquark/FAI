...
Consider $\zeta: \Nats^2 \rightarrow (0,\frac{1}{2}]$ s.t.  $\zeta \in \mathcal{E}$ and $\Floor{\log \frac{1}{\zeta}} \in \Gamma_{\mathfrak{A}}$. Define $I:=\{(k,j) \in \Nats^2 \mid \frac{\Abs{\delta(k,j)}}{\alpha(k,j)} \geq \zeta(k,j)\}$, $D^{kj}:=\Rats \cap [\frac{\Abs{\delta(k,j)}}{2\alpha(k,j)},\frac{\Abs{\delta(k,j)}}{\alpha(k,j)}]$ and choose ${\epsilon(k,j) \in (\Sgn \delta(k,j)) \cdot \Argmin{t \in D^{kj}} \Abs{\En_\Rats(t)}}$. It is easy to see that ${\Abs{\En_\Rats(\epsilon)} = O(\log \frac{\alpha}{\Abs{\delta}})}$, hence we can construct $Q: \Words \xrightarrow{\Gamma} \Rats$ s.t. $\R_Q=\R_P$ and for any $(k,j) \in I$, and $x,y \in \Words$, $Q^{kj}(x,y)=P^{kj}(x,y)-\epsilon(k,j)$ (we supply $\epsilon$ as advice for $Q$). Applying Proposition~\ref{prp:weight} to $P$, $Q$ and $W$, we conclude there is $\varepsilon \in \mathcal{E}$ s.t.
